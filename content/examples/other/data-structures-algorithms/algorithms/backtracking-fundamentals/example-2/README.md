# Backtracking Fundamentals — Follow-Up: Pruning Heuristics

Discusses constraint ordering and pruning heuristics that reduce branching factor.

## Files
- `EXPLANATION.md`
- `demo.js`
- `README.md`

## Run
`node content/examples/other/data-structures-algorithms/algorithms/backtracking-fundamentals/example-2/demo.js`

## What to Verify
- early pruning cuts off whole subtrees
- ordering variables (MRV) reduces branching
- still exponential in worst case
