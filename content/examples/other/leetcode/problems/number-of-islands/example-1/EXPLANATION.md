# Number OF Islands - example-1 Explanation
...
- `Solution.java`: Provides supporting example content: public class Solution { static int numIslandsMutatingDFS(char[][] grid) { int rows = grid.length; int cols = rows == 0 ? 0 : grid[0].length;.
- `solution.js`: Implements the main logic, including numIslandsMutatingDFS, rows, cols, count, dfs.
