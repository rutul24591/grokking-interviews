# Message Ordering Guarantees - example-1 Explanation

## Article context
This example supports the article `non-functional-requirements/backend-nfr/message-ordering-guarantees`. The article is about Comprehensive guide to message ordering — partition-based ordering, sequence numbers, causal ordering, exactly-once delivery, and ordering testing for staff/principal engineer interviews.. The most relevant article sections for this example are: Definition and Context; Key Distinction: Ordering vs Exactly-Once Delivery; Core Concepts; Partition-Based Ordering; Sequence Numbers and Ordering Detection; Causal Ordering; Architecture and Flow; Partition Assignment and Ordering; Out-of-Order Message Handling; Trade-Offs and Comparisons.
...
- `app/api/stream/publish/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/stream/reset/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/stream/state/route.ts`: Models an API boundary, request handling path, or backend contract.
...
- `lib/orderedStream.ts`: Implements the main logic, including OrderedStreams, existing, s, s, ev.
