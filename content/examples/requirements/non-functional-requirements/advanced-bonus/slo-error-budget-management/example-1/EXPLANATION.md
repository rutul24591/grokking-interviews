# SLO / Error Budget Management - example-1 Explanation
...
- `lib/buckets.ts`: Implements the main logic, including TimeBuckets, start, prev, next, from.
- `lib/random.ts`: Implements the main logic, including stableUniform01, digest, bucket.
- `lib/recent.ts`: Implements the main logic, including RecentLatencies, values, p, idx.
- `lib/slo.ts`: Implements the main logic, including clamp01, allowedBad, budgetFraction, burnRate, allowed.
