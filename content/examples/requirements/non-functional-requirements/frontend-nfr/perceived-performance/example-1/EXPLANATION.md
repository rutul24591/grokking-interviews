# Perceived Performance - example-1 Explanation

## Article context
This example supports the article `non-functional-requirements/frontend-nfr/perceived-performance`. The article is about Comprehensive guide to perceived performance optimization, covering psychological principles, UX patterns, and techniques to make applications feel faster.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Architecture and Flow; Trade-offs and Comparison; Best Practices; Common Pitfalls; Real-World Use Cases; Advanced Perceived Performance Architecture; Common Interview Questions with Detailed Answers.
...
- `app/api/like/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/search/route.ts`: Models an API boundary, request handling path, or backend contract.
...
- `components/SearchPanel.tsx`: Implements the main logic, including sleep, SearchPanel, abortRef, reqSeq, likedCount.
- `lib/http.ts`: Implements the main logic, including jsonOk, jsonError.
- `lib/search.ts`: Implements the main logic, including corpus, search, q, liked, toggleLike.
