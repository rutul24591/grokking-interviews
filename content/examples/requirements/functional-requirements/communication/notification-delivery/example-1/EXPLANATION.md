# Notification Delivery - example-1 Explanation

## Article context
This example supports the article `functional-requirements/communication/notification-delivery`. The article is about Comprehensive guide to implementing notification delivery covering multi-channel routing, user preferences, quiet hours, rate limiting, delivery optimization, and cross-platform notification strategies.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Delivery Channels; Preference Evaluation; Quiet Hours; Rate Limiting; Delivery Optimization; Architecture and Flow; Preference Service; Channel Router.
...
- `app/api/notification-delivery/action/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/notification-delivery/state/route.ts`: Models an API boundary, request handling path, or backend contract.
