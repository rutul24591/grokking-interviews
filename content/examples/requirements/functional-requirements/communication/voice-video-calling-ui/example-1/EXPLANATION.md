# Voice/Video Calling UI - example-1 Explanation

## Article context
This example supports the article `functional-requirements/communication/voice-video-calling-ui`. The article is about Comprehensive guide to implementing voice and video calling UI covering call controls, quality indicators, screen sharing, participant management, and real-time communication features for rich calling experiences.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Call States; Call Controls; Video Layouts; Quality Indicators; Screen Sharing; Architecture and Flow; Call Manager; Media Renderer.
...
- `app/api/voice-video-calling-ui/action/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/voice-video-calling-ui/state/route.ts`: Models an API boundary, request handling path, or backend contract.
