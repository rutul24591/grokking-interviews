...
export default function Page() {
  ...
  async function act(action = "state", itemId?: string) {
    const response = await fetch(action === "state" ? "/api/voice-video-calling-ui/state" : "/api/voice-video-calling-ui/action", action === "state" ? { cache: "no-store" } : { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, itemId }) });
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <h1 className="text-3xl font-bold">Voice Video Calling UI</h1>
      <p className="mt-2 max-w-3xl text-slate-300">Models pre-call readiness, active call recovery, and device-loss fallback in a calling surface.</p>
      <section className="mt-8 grid gap-4 lg:grid-cols-[2fr_1fr]">
        <article className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
          <div className="flex flex-wrap gap-3">
            <button onClick={() => void act("cycle-primary")} className="rounded-lg bg-sky-600 px-4 py-2 text-sm font-semibold">Cycle trust or call stage</button>
            <button onClick={() => void act("cycle-secondary")} className="rounded-lg border border-slate-700 px-4 py-2 text-sm font-semibold">Cycle device or candidate mode</button>
            <button onClick={() => void act("reset")} className="rounded-lg border border-emerald-700 px-4 py-2 text-sm font-semibold">Reset state</button>
          </div>
          <div className="mt-5 grid gap-4 md:grid-cols-3">
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-950/70 p-4"><p className="text-xs uppercase tracking-[0.2em] text-slate-400">Flagged assets</p><p className="mt-2 text-lg font-semibold">{snapshot?.items.filter((item) => item.flagged).length ?? 0}</p></div>
          </div>
        ...
        <article className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
          <h2 className="text-lg font-semibold">Selected asset</h2>
          {selected ? <><p className="mt-4 text-sm text-slate-400">{selected.subtitle}</p><p className="mt-2 text-xl font-semibold">{selected.title}</p><p className="mt-3 text-sm leading-6 text-slate-300">{selected.detail}</p><p className="mt-4 text-sm text-slate-400">Current posture</p><p className="mt-1 font-semibold">{selected.status}</p><button onClick={() => void act("advance-selected", selected.id)} className="mt-5 rounded-lg bg-violet-600 px-4 py-2 text-sm font-semibold">Advance asset</button></> : null}
          <h3 className="mt-8 text-sm font-semibold uppercase tracking-[0.2em] text-slate-400">Security and recovery notes</h3>
