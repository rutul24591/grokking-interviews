...
export default function Page() {
  ...
  async function act(action = "state", itemId?: string) {
    const response = await fetch(action === "state" ? "/api/offline-message-queue/state" : "/api/offline-message-queue/action", action === "state" ? { cache: "no-store" } : { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, itemId }) });
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <h1 className="text-3xl font-bold">Offline Message Queue</h1>
      <p className="mt-2 max-w-3xl text-slate-300">Tracks queued drafts, reconnect flush ordering, and expired local entries in an offline-first messaging flow.</p>
      <section className="mt-8 grid gap-4 lg:grid-cols-[2fr_1fr]">
        <article className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
          <div className="flex flex-wrap gap-3">
            <button onClick={() => void act("cycle-primary")} className="rounded-lg bg-sky-600 px-4 py-2 text-sm font-semibold">Cycle view</button>
            <button onClick={() => void act("cycle-secondary")} className="rounded-lg border border-slate-700 px-4 py-2 text-sm font-semibold">Cycle mode</button>
            <button onClick={() => void act("reset")} className="rounded-lg border border-emerald-700 px-4 py-2 text-sm font-semibold">Reset experience</button>
          </div>
          ...
          <div className="mt-4 rounded-xl border border-slate-800 bg-slate-950/70 px-4 py-3 text-sm text-slate-300">{snapshot?.message}</div>
          <div className="mt-6 space-y-3">{(snapshot?.items ?? []).map((item) => <button key={item.id} onClick={() => setSelectedId(item.id)} className={(selected?.id === item.id ? "border-sky-500 bg-sky-500/10" : "border-slate-800 bg-slate-950/70") + " block w-full rounded-2xl border p-4 text-left"}><div className="flex items-center justify-between gap-3"><div><p className="font-semibold">{item.title}</p><p className="mt-1 text-sm text-slate-400">{item.subtitle}</p></div><span className="rounded-full bg-slate-800 px-2 py-1 text-xs uppercase">{item.status}</span></div><p className="mt-3 text-sm text-slate-300">{item.detail}</p></button>)}</div>
        </article>
        <article className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5">
          <h2 className="text-lg font-semibold">Selected state</h2>
          {selected ? <><p className="mt-4 text-sm text-slate-400">{selected.subtitle}</p><p className="mt-2 text-xl font-semibold">{selected.title}</p><p className="mt-3 text-sm leading-6 text-slate-300">{selected.detail}</p><button onClick={() => void act("advance-selected", selected.id)} className="mt-5 rounded-lg bg-violet-600 px-4 py-2 text-sm font-semibold">Advance state</button></> : null}
          <h3 className="mt-8 text-sm font-semibold uppercase tracking-[0.2em] text-slate-400">User-facing notes</h3>
