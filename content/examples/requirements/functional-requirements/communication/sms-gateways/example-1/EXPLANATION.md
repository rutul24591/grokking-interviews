# SMS Gateways - example-1 Explanation

## Article context
This example supports the article `functional-requirements/communication/sms-gateways`. The article is about Comprehensive guide to implementing SMS gateways covering SMS providers (Twilio, Vonage, AWS SNS), two-way messaging, delivery receipts, short codes vs long codes, international SMS, and rate limiting for reliable text message delivery.. The most relevant article sections for this example are: Definition and Context; Core Concepts; SMS Providers; Phone Number Types; Two-Way Messaging; Delivery Tracking; Compliance; Architecture and Flow; Queue Management; Provider Integration.
...
- `app/api/sms-gateways/action/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/sms-gateways/state/route.ts`: Models an API boundary, request handling path, or backend contract.
