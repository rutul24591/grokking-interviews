...
export default function Page() {
  ...
  async function act(action = "state", itemId?: string) {
    const response = await fetch(action === "state" ? "/api/email-delivery-services/state" : "/api/email-delivery-services/action", action === "state" ? { cache: "no-store" } : { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, itemId }) });
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <h1 className="text-3xl font-bold">Email Delivery Services</h1>
      <p className="mt-2 max-w-3xl text-slate-300">Runs provider routing, suppression review, and retry-budget visibility for transactional and bulk email.</p>
