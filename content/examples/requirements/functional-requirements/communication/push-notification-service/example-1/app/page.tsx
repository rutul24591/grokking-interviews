...
export default function Page() {
  ...
  async function act(action = "state", itemId?: string) {
    const response = await fetch(action === "state" ? "/api/push-notification-service/state" : "/api/push-notification-service/action", action === "state" ? { cache: "no-store" } : { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, itemId }) });
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <h1 className="text-3xl font-bold">Push Notification Service</h1>
      <p className="mt-2 max-w-3xl text-slate-300">Models token fanout, campaign segmentation, and invalid-token cleanup in a push pipeline.</p>
