...
export default function Page() {
  ...
  async function act(action = "state", itemId?: string) {
    const response = await fetch(action === "state" ? "/api/notification-center/state" : "/api/notification-center/action", action === "state" ? { cache: "no-store" } : { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, itemId }) });
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      ...
      <p className="mt-2 max-w-3xl text-slate-300">Runs an inbox-style notification surface with unread drift checks, prioritization, and fallback routing.</p>
