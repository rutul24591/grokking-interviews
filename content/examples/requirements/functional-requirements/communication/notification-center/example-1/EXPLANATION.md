# Notification Center - example-1 Explanation

## Article context
This example supports the article `functional-requirements/communication/notification-center`. The article is about Comprehensive guide to implementing notification centers covering notification aggregation, filtering, preferences, real-time updates, read/unread management, and cross-device synchronization.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Notification Types; Notification Aggregation; Read/Unread Management; Real-time Updates; Architecture and Flow; Client Component Architecture; Notification Storage; Real-time Sync.
...
- `app/api/notification-center/action/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/notification-center/state/route.ts`: Models an API boundary, request handling path, or backend contract.
