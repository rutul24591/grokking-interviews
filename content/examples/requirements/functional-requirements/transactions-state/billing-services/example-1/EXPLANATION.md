# Billing Services - example-1 Explanation

## Article context
This example supports the article `functional-requirements/transactions-state/billing-services`. The article is about Comprehensive guide to implementing billing services covering billing cycles, invoicing, proration calculation, tax calculation and compliance, billing dunning and retries, credit and debit memos, and billing analytics for recurring revenue businesses.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Billing Cycles; Invoicing; Proration Calculation; Tax Calculation; Billing Dunning; Credit and Debit Memos; Architecture and Flow; Billing Engine.
...
- `app/api/billing-services/action/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/billing-services/state/route.ts`: Models an API boundary, request handling path, or backend contract.
