# Financial Logs - example-1 Explanation

## Article context
This example supports the article `functional-requirements/transactions-state/financial-logs`. The article is about Comprehensive guide to implementing financial logs covering audit trails, compliance logging (SOX, PCI, GDPR), financial record keeping, log retention policies, search and retrieval, and security/access control for financial transaction logging.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Audit Trails; Compliance Logging; Financial Record Keeping; Log Retention Policies; Search and Retrieval; Security and Access Control; Architecture and Flow; Log Ingestion.
...
- `app/api/financial-logs/action/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/financial-logs/state/route.ts`: Models an API boundary, request handling path, or backend contract.
