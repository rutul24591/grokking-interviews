# Billing Platforms - example-1 Explanation

## Article context
This example supports the article `functional-requirements/transactions-state/billing-platforms`. The article is about Comprehensive guide to billing platforms covering platform comparison (Stripe Billing, Chargebee, Recurly, Zuora), integration patterns, multi-platform routing, platform selection criteria, migration strategies, and billing infrastructure for recurring revenue businesses.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Platform Comparison; Integration Patterns; Multi-Platform Routing; Platform Selection Criteria; Migration Strategies; Architecture and Flow; Platform Abstraction; Routing Logic.
...
- `app/api/billing-platforms/action/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/billing-platforms/state/route.ts`: Models an API boundary, request handling path, or backend contract.
