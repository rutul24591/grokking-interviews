# Event Sourcing Systems - example-1 Explanation

## Article context
This example supports the article `functional-requirements/transactions-state/event-sourcing-systems`. The article is about Comprehensive guide to implementing event sourcing systems covering event sourcing fundamentals, event store design, event versioning and migration, projections and read models, CQRS integration, and event replay and debugging for transaction systems.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Event Sourcing Fundamentals; Event Store Design; Event Versioning and Migration; Projections and Read Models; CQRS Integration; Event Replay and Debugging; Architecture and Flow; Event Sourcing Fundamentals.
...
- `app/api/event-sourcing-systems/action/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/event-sourcing-systems/state/route.ts`: Models an API boundary, request handling path, or backend contract.
