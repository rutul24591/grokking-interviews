# Subscription Billing - example-1 Explanation

## Article context
This example supports the article `functional-requirements/transactions-state/subscription-billing`. The article is about Comprehensive guide to implementing subscription billing covering recurring charges, proration handling, dunning management, subscription lifecycle, plan changes, and payment retry strategies for SaaS and membership businesses.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Subscription Lifecycle States; Billing Cycle and Invoicing; Proration for Plan Changes; Dunning Management; Subscription Metrics; Architecture and Flow; Subscription Management; Billing Engine.
...
- `app/api/subscription-billing/action/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/subscription-billing/state/route.ts`: Models an API boundary, request handling path, or backend contract.
