...
## Article context
This example supports the article `functional-requirements/transactions-state/payment-processing`. The article is about Comprehensive guide to implementing payment processing covering payment gateway integration, authorization and capture flows, 3D Secure authentication, refund processing, PCI compliance, and handling payment failures at scale.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Payment Lifecycle; Payment Gateway Integration; 3D Secure Authentication; Payment Failures and Retry; Refund Processing; Architecture and Flow; Client-Side Tokenization; Backend Payment Service.
...
- `app/api/payment-processing/action/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/payment-processing/state/route.ts`: Models an API boundary, request handling path, or backend contract.
