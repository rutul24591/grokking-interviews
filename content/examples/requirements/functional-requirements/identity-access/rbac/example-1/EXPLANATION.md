# Role-Based Access Control (RBAC) - example-1 Explanation

## Article context
This example supports the article `functional-requirements/identity-access/rbac`. The article is about Comprehensive guide to implementing RBAC covering role hierarchies, permission models, assignment patterns, caching strategies, and scaling for staff/principal engineer interviews.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Architecture and Flow; Trade-offs and Comparison; RBAC vs ABAC; Centralized vs Decentralized RBAC; Flat vs Hierarchical Roles; Best Practices; Security Implementation; Role Design.
...
- `app/api/rbac/evaluate/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/rbac/state/route.ts`: Models an API boundary, request handling path, or backend contract.
