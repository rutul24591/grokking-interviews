# Conflict Resolution - example-1 Explanation

## Article context
This example supports the article `functional-requirements/other-cross-cutting/conflict-resolution`. The article is about Comprehensive guide to implementing conflict resolution covering conflict detection, conflict strategies, conflict resolution policies, conflict handling, and conflict management for data consistency and system reliability.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Conflict Detection; Conflict Strategies; Resolution Policies; Conflict Handling; Conflict Management; Architecture and Flow; Detection Service; Strategy Service.
...
- `app/api/conflict-resolution/action/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/conflict-resolution/state/route.ts`: Models an API boundary, request handling path, or backend contract.
