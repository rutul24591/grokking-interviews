# Permission Management - example-1 Explanation

## Article context
This example supports the article `functional-requirements/other-cross-cutting/permission-management`. The article is about Comprehensive guide to implementing permission management covering app permissions, feature permissions, user role permissions, permission grants and revocation, and permission auditing for user control over access rights.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Permission Types; Permission Grants; Permission Revocation; Permission Auditing; Permission Enforcement; Architecture and Flow; Permission Service; Grant Management.
...
- `app/api/permission-management/action/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/permission-management/state/route.ts`: Models an API boundary, request handling path, or backend contract.
