# User Blocking - example-1 Explanation

## Article context
This example supports the article `functional-requirements/other-cross-cutting/user-blocking`. The article is about Comprehensive guide to implementing user blocking systems covering block/unblock workflows, block list management, blocked user experience, block evasion prevention, and integration with moderation systems for user safety and harassment prevention.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Block/Unblock Workflows; Block List Management; Blocked User Experience; Block Evasion Prevention; Integration with Moderation Systems; Architecture and Flow; Block Management Layer; Block Enforcement Layer.
...
- `app/api/user-blocking/action/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/user-blocking/state/route.ts`: Models an API boundary, request handling path, or backend contract.
