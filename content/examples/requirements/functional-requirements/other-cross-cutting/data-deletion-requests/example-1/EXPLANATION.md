# Data Deletion Requests - example-1 Explanation

## Article context
This example supports the article `functional-requirements/other-cross-cutting/data-deletion-requests`. The article is about Comprehensive guide to implementing data deletion requests covering deletion request management, request validation, data identification, data deletion, deletion confirmation, and compliance tracking for user data deletion and regulatory compliance.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Request Management; Request Validation; Data Identification; Data Deletion; Deletion Confirmation; Architecture and Flow; Request Service; Identification Service.
...
- `app/api/data-deletion-requests/action/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/data-deletion-requests/state/route.ts`: Models an API boundary, request handling path, or backend contract.
