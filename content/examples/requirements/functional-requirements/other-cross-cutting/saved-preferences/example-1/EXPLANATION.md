# Saved Preferences - example-1 Explanation

## Article context
This example supports the article `functional-requirements/other-cross-cutting/saved-preferences`. The article is about Comprehensive guide to implementing saved preferences covering preference storage, preference sync, preference backup, preference restoration, and preference management for user preference persistence.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Preference Storage; Preference Sync; Preference Backup; Preference Restoration; Preference Management; Architecture and Flow; Preference Service; Storage Service.
...
- `app/api/saved-preferences/action/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/saved-preferences/state/route.ts`: Models an API boundary, request handling path, or backend contract.
