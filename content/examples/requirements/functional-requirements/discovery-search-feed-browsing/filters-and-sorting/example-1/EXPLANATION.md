# Filters and Sorting - example-1 Explanation

## Article context
This example supports the article `functional-requirements/discovery-search-feed-browsing/filters-and-sorting`. The article is about Comprehensive guide to filters and sorting covering filter types, sort options, state management, URL persistence, performance optimization, and UX best practices for search refinement.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Filter Types; Sort Options; Filter Logic; State Management; URL Persistence; Architecture and Flow; Component Structure; Filter Change Flow.
...
- `app/api/filtering/apply/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/filtering/state/route.ts`: Models an API boundary, request handling path, or backend contract.
