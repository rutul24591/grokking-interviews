# Category Navigation - example-1 Explanation

## Article context
This example supports the article `functional-requirements/discovery-search-feed-browsing/category-navigation`. The article is about Comprehensive guide to category navigation covering hierarchical structures, navigation patterns, breadcrumbs, mega menus, mobile navigation, and SEO best practices for content discovery.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Hierarchy Depth; Navigation Patterns; Category Metadata; URL Structure; State Management; Architecture and Flow; Data Loading Strategies; Component Structure.
...
- `app/api/navigation/select/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/navigation/state/route.ts`: Models an API boundary, request handling path, or backend contract.
