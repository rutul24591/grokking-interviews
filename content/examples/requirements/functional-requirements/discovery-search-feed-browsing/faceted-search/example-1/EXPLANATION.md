# Faceted Search - example-1 Explanation

## Article context
This example supports the article `functional-requirements/discovery-search-feed-browsing/faceted-search`. The article is about Comprehensive guide to faceted search covering facet types, computation strategies, multi-select filtering, performance optimization, and UX best practices.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Facet Types; Facet Computation Strategies; Multi-Select Logic; Facet Display Strategies; Architecture and Flow; Component Breakdown; Data Flow: Faceted Search Request; Elasticsearch Faceted Search Example.
...
- `app/api/facets/apply/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/facets/state/route.ts`: Models an API boundary, request handling path, or backend contract.
