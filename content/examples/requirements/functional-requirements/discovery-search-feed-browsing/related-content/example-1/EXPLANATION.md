# Related Content - example-1 Explanation

## Article context
This example supports the article `functional-requirements/discovery-search-feed-browsing/related-content`. The article is about Comprehensive guide to related content recommendations covering similarity algorithms, co-engagement, placement strategies, pre-computation, and real-time updates for increasing engagement.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Similarity Signals; Similarity Computation Approaches; Pre-computation vs Real-time; Placement Strategies; Architecture and Flow; Similarity Computation Pipeline; Co-engagement Computation; Cache Strategy.
...
- `app/api/related/state/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/related/strategy/route.ts`: Models an API boundary, request handling path, or backend contract.
