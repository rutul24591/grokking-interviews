...
## Article context
This example supports the article `functional-requirements/discovery-search-feed-browsing/search-indexing`. The article is about Comprehensive guide to search indexing covering inverted indexes, analyzers, tokenization, incremental updates, and reindexing strategies for production search systems.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Inverted Index; Analyzers; Index Segments; Field Types and Mappings; Architecture and Flow; Indexing Pipeline Components; Indexing Flow; Sharding and Replication.
...
- `app/api/indexing/rebuild/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/indexing/state/route.ts`: Models an API boundary, request handling path, or backend contract.
