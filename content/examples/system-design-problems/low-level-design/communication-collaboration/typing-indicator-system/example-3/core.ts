export type typingIndicatorSystemRuntimeState = {
...
export type typingIndicatorSystemRecoveryPlan = {
...
export function planTypingIndicatorSystemRecovery(
  state: typingIndicatorSystemRuntimeState,
  nowMs: number,
): typingIndicatorSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: typingIndicatorSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runTypingIndicatorSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planTypingIndicatorSystemRecovery(
    {
      topic: "typing-indicator-system",
      mounted: true,
  ...
  const failure = planTypingIndicatorSystemRecovery(
    {
      topic: "typing-indicator-system",
      mounted: true,
      lastSuccessfulVersion: 21,
