export type typingIndicatorSystemSignal = {
  eventLagMs: number;
...
export type typingIndicatorSystemEvent = {
  id: string;
  topic: "typing-indicator-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: typingIndicatorSystemSignal;
};

export type typingIndicatorSystemDecision = {
  accepted: boolean;
...
export function evaluateTypingIndicatorSystemEvent(event: typingIndicatorSystemEvent): typingIndicatorSystemDecision {
  ...
  let action: typingIndicatorSystemDecision["action"] = "commit";
...
export function runTypingIndicatorSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateTypingIndicatorSystemEvent({
    id: "typing-indicator-system-evt-1",
    topic: "typing-indicator-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateTypingIndicatorSystemEvent({
    id: "typing-indicator-system-evt-late",
    topic: "typing-indicator-system",
    actorId: "user-42",
    sequence: 8,
