export type chatMessagingUiRuntimeState = {
...
export type chatMessagingUiRecoveryPlan = {
...
export function planChatMessagingUiRecovery(
  state: chatMessagingUiRuntimeState,
  nowMs: number,
): chatMessagingUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: chatMessagingUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runChatMessagingUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planChatMessagingUiRecovery(
    {
      topic: "chat-messaging-ui",
      mounted: true,
  ...
  const failure = planChatMessagingUiRecovery(
    {
      topic: "chat-messaging-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
