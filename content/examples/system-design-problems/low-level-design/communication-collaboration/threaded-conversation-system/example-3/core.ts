export type threadedConversationSystemRuntimeState = {
...
export type threadedConversationSystemRecoveryPlan = {
...
export function planThreadedConversationSystemRecovery(
  state: threadedConversationSystemRuntimeState,
  nowMs: number,
): threadedConversationSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: threadedConversationSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runThreadedConversationSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planThreadedConversationSystemRecovery(
    {
      topic: "threaded-conversation-system",
      mounted: true,
  ...
  const failure = planThreadedConversationSystemRecovery(
    {
      topic: "threaded-conversation-system",
      mounted: true,
      lastSuccessfulVersion: 21,
