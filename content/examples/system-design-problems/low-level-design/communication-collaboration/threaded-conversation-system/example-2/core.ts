export type threadedConversationSystemSignal = {
  eventLagMs: number;
...
export type threadedConversationSystemEvent = {
  id: string;
  topic: "threaded-conversation-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: threadedConversationSystemSignal;
};

export type threadedConversationSystemDecision = {
  accepted: boolean;
...
export function evaluateThreadedConversationSystemEvent(event: threadedConversationSystemEvent): threadedConversationSystemDecision {
  ...
  let action: threadedConversationSystemDecision["action"] = "commit";
...
export function runThreadedConversationSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateThreadedConversationSystemEvent({
    id: "threaded-conversation-system-evt-1",
    topic: "threaded-conversation-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateThreadedConversationSystemEvent({
    id: "threaded-conversation-system-evt-late",
    topic: "threaded-conversation-system",
    actorId: "user-42",
    sequence: 8,
