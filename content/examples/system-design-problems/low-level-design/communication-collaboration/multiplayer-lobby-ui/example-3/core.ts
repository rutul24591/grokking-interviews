export type multiplayerLobbyUiRuntimeState = {
...
export type multiplayerLobbyUiRecoveryPlan = {
...
export function planMultiplayerLobbyUiRecovery(
  state: multiplayerLobbyUiRuntimeState,
  nowMs: number,
): multiplayerLobbyUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: multiplayerLobbyUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runMultiplayerLobbyUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planMultiplayerLobbyUiRecovery(
    {
      topic: "multiplayer-lobby-ui",
      mounted: true,
  ...
  const failure = planMultiplayerLobbyUiRecovery(
    {
      topic: "multiplayer-lobby-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
