export type conflictResolutionUiSignal = {
  eventLagMs: number;
...
export type conflictResolutionUiEvent = {
  id: string;
  ...
  payloadSize: number;
  signal: conflictResolutionUiSignal;
};

export type conflictResolutionUiDecision = {
  accepted: boolean;
...
export function evaluateConflictResolutionUiEvent(event: conflictResolutionUiEvent): conflictResolutionUiDecision {
  ...
  let action: conflictResolutionUiDecision["action"] = "commit";
...
export function runConflictResolutionUiContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateConflictResolutionUiEvent({
    id: "conflict-resolution-ui-evt-1",
    topic: "conflict-resolution-ui",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateConflictResolutionUiEvent({
    id: "conflict-resolution-ui-evt-late",
    topic: "conflict-resolution-ui",
    actorId: "user-42",
    sequence: 8,
