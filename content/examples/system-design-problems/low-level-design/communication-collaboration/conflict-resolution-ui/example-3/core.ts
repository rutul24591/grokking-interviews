export type conflictResolutionUiRuntimeState = {
...
export type conflictResolutionUiRecoveryPlan = {
...
export function planConflictResolutionUiRecovery(
  state: conflictResolutionUiRuntimeState,
  nowMs: number,
): conflictResolutionUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: conflictResolutionUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runConflictResolutionUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planConflictResolutionUiRecovery(
    {
      topic: "conflict-resolution-ui",
      mounted: true,
  ...
  const failure = planConflictResolutionUiRecovery(
    {
      topic: "conflict-resolution-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
