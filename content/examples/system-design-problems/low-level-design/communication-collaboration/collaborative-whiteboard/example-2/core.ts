export type collaborativeWhiteboardSignal = {
  eventLagMs: number;
...
export type collaborativeWhiteboardEvent = {
  id: string;
  ...
  payloadSize: number;
  signal: collaborativeWhiteboardSignal;
};

export type collaborativeWhiteboardDecision = {
  accepted: boolean;
...
export function evaluateCollaborativeWhiteboardEvent(event: collaborativeWhiteboardEvent): collaborativeWhiteboardDecision {
  ...
  let action: collaborativeWhiteboardDecision["action"] = "commit";
...
export function runCollaborativeWhiteboardContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateCollaborativeWhiteboardEvent({
    id: "collaborative-whiteboard-evt-1",
    topic: "collaborative-whiteboard",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateCollaborativeWhiteboardEvent({
    id: "collaborative-whiteboard-evt-late",
    topic: "collaborative-whiteboard",
    actorId: "user-42",
    sequence: 8,
