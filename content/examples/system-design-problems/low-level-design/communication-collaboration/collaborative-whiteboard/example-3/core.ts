export type collaborativeWhiteboardRuntimeState = {
...
export type collaborativeWhiteboardRecoveryPlan = {
...
export function planCollaborativeWhiteboardRecovery(
  state: collaborativeWhiteboardRuntimeState,
  nowMs: number,
): collaborativeWhiteboardRecoveryPlan {
  const evidence: string[] = [];
  const actions: collaborativeWhiteboardRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runCollaborativeWhiteboardEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planCollaborativeWhiteboardRecovery(
    {
      topic: "collaborative-whiteboard",
      mounted: true,
  ...
  const failure = planCollaborativeWhiteboardRecovery(
    {
      topic: "collaborative-whiteboard",
      mounted: true,
      lastSuccessfulVersion: 21,
