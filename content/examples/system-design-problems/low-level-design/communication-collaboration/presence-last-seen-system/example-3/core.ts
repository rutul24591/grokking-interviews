export type presenceLastSeenSystemRuntimeState = {
...
export type presenceLastSeenSystemRecoveryPlan = {
...
export function planPresenceLastSeenSystemRecovery(
  state: presenceLastSeenSystemRuntimeState,
  nowMs: number,
): presenceLastSeenSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: presenceLastSeenSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runPresenceLastSeenSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planPresenceLastSeenSystemRecovery(
    {
      topic: "presence-last-seen-system",
      mounted: true,
  ...
  const failure = planPresenceLastSeenSystemRecovery(
    {
      topic: "presence-last-seen-system",
      mounted: true,
      lastSuccessfulVersion: 21,
