export type liveCursorPresenceSystemRuntimeState = {
...
export type liveCursorPresenceSystemRecoveryPlan = {
...
export function planLiveCursorPresenceSystemRecovery(
  state: liveCursorPresenceSystemRuntimeState,
  nowMs: number,
): liveCursorPresenceSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: liveCursorPresenceSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runLiveCursorPresenceSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planLiveCursorPresenceSystemRecovery(
    {
      topic: "live-cursor-presence-system",
      mounted: true,
  ...
  const failure = planLiveCursorPresenceSystemRecovery(
    {
      topic: "live-cursor-presence-system",
      mounted: true,
      lastSuccessfulVersion: 21,
