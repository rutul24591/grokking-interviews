# Design a Live Cursor / Presence System - example-1 Explanation

## Article context
This example supports the article `low-level-design/communication-collaboration/live-cursor-presence-system`. The article is about LLD for live cursors and presence: throttled cursor broadcasts, smooth interpolation, presence list with colors, idle/away detection, and cleanup on disconnect.. The most relevant article sections for this example are: 🎯 Problem Context and Scope Definition; Problem Statement; User Context; Assumptions; Non-Goals; ⚙️ Functional Requirements; Core (Must-have); Secondary (Nice-to-have); Out of Scope; 📊 Non-Functional Requirements.
...
- `core.ts`: Implements the main logic, including upsert, prev, prune, byUser, throttle.
