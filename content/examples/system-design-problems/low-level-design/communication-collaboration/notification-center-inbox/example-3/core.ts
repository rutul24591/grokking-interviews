export type notificationCenterInboxRuntimeState = {
...
export type notificationCenterInboxRecoveryPlan = {
...
export function planNotificationCenterInboxRecovery(
  state: notificationCenterInboxRuntimeState,
  nowMs: number,
): notificationCenterInboxRecoveryPlan {
  const evidence: string[] = [];
  const actions: notificationCenterInboxRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runNotificationCenterInboxEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planNotificationCenterInboxRecovery(
    {
      topic: "notification-center-inbox",
      mounted: true,
  ...
  const failure = planNotificationCenterInboxRecovery(
    {
      topic: "notification-center-inbox",
      mounted: true,
      lastSuccessfulVersion: 21,
