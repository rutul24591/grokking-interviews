export type realTimeNotificationDeliverySystemSignal = {
  eventLagMs: number;
...
export type realTimeNotificationDeliverySystemEvent = {
  id: string;
  topic: "real-time-notification-delivery-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: realTimeNotificationDeliverySystemSignal;
};

export type realTimeNotificationDeliverySystemDecision = {
  accepted: boolean;
...
export function evaluateRealTimeNotificationDeliverySystemEvent(event: realTimeNotificationDeliverySystemEvent): realTimeNotificationDeliverySystemDecision {
  ...
  let action: realTimeNotificationDeliverySystemDecision["action"] = "commit";
...
export function runRealTimeNotificationDeliverySystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateRealTimeNotificationDeliverySystemEvent({
    id: "real-time-notification-delivery-system-evt-1",
    topic: "real-time-notification-delivery-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateRealTimeNotificationDeliverySystemEvent({
    id: "real-time-notification-delivery-system-evt-late",
    topic: "real-time-notification-delivery-system",
    actorId: "user-42",
    sequence: 8,
