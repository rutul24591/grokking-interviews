export type messageDeliveryStateSystemSignal = {
  eventLagMs: number;
...
export type messageDeliveryStateSystemEvent = {
  id: string;
  topic: "message-delivery-state-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: messageDeliveryStateSystemSignal;
};

export type messageDeliveryStateSystemDecision = {
  accepted: boolean;
...
export function evaluateMessageDeliveryStateSystemEvent(event: messageDeliveryStateSystemEvent): messageDeliveryStateSystemDecision {
  ...
  let action: messageDeliveryStateSystemDecision["action"] = "commit";
...
export function runMessageDeliveryStateSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateMessageDeliveryStateSystemEvent({
    id: "message-delivery-state-system-evt-1",
    topic: "message-delivery-state-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateMessageDeliveryStateSystemEvent({
    id: "message-delivery-state-system-evt-late",
    topic: "message-delivery-state-system",
    actorId: "user-42",
    sequence: 8,
