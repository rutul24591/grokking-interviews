export type messageDeliveryStateSystemRuntimeState = {
...
export type messageDeliveryStateSystemRecoveryPlan = {
...
export function planMessageDeliveryStateSystemRecovery(
  state: messageDeliveryStateSystemRuntimeState,
  nowMs: number,
): messageDeliveryStateSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: messageDeliveryStateSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runMessageDeliveryStateSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planMessageDeliveryStateSystemRecovery(
    {
      topic: "message-delivery-state-system",
      mounted: true,
  ...
  const failure = planMessageDeliveryStateSystemRecovery(
    {
      topic: "message-delivery-state-system",
      mounted: true,
      lastSuccessfulVersion: 21,
