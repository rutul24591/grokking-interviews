export type passwordResetRecoveryFlowSignal = {
  sessionAgeMs: number;
...
export type passwordResetRecoveryFlowEvent = {
  id: string;
  topic: "password-reset-recovery-flow";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: passwordResetRecoveryFlowSignal;
};

export type passwordResetRecoveryFlowDecision = {
  accepted: boolean;
...
export function evaluatePasswordResetRecoveryFlowEvent(event: passwordResetRecoveryFlowEvent): passwordResetRecoveryFlowDecision {
  ...
  let action: passwordResetRecoveryFlowDecision["action"] = "commit";
...
export function runPasswordResetRecoveryFlowContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluatePasswordResetRecoveryFlowEvent({
    id: "password-reset-recovery-flow-evt-1",
    topic: "password-reset-recovery-flow",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluatePasswordResetRecoveryFlowEvent({
    id: "password-reset-recovery-flow-evt-late",
    topic: "password-reset-recovery-flow",
    actorId: "user-42",
    sequence: 8,
