export type passwordResetRecoveryFlowRuntimeState = {
...
export type passwordResetRecoveryFlowRecoveryPlan = {
...
export function planPasswordResetRecoveryFlowRecovery(
  state: passwordResetRecoveryFlowRuntimeState,
  nowMs: number,
): passwordResetRecoveryFlowRecoveryPlan {
  const evidence: string[] = [];
  const actions: passwordResetRecoveryFlowRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runPasswordResetRecoveryFlowEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planPasswordResetRecoveryFlowRecovery(
    {
      topic: "password-reset-recovery-flow",
      mounted: true,
  ...
  const failure = planPasswordResetRecoveryFlowRecovery(
    {
      topic: "password-reset-recovery-flow",
      mounted: true,
      lastSuccessfulVersion: 21,
