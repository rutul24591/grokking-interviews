export type userOnboardingGuidedTourRuntimeState = {
...
export type userOnboardingGuidedTourRecoveryPlan = {
...
export function planUserOnboardingGuidedTourRecovery(
  state: userOnboardingGuidedTourRuntimeState,
  nowMs: number,
): userOnboardingGuidedTourRecoveryPlan {
  const evidence: string[] = [];
  const actions: userOnboardingGuidedTourRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runUserOnboardingGuidedTourEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planUserOnboardingGuidedTourRecovery(
    {
      topic: "user-onboarding-guided-tour",
      mounted: true,
  ...
  const failure = planUserOnboardingGuidedTourRecovery(
    {
      topic: "user-onboarding-guided-tour",
      mounted: true,
      lastSuccessfulVersion: 21,
