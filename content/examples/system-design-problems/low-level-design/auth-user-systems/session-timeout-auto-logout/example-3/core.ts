export type sessionTimeoutAutoLogoutRuntimeState = {
...
export type sessionTimeoutAutoLogoutRecoveryPlan = {
...
export function planSessionTimeoutAutoLogoutRecovery(
  state: sessionTimeoutAutoLogoutRuntimeState,
  nowMs: number,
): sessionTimeoutAutoLogoutRecoveryPlan {
  const evidence: string[] = [];
  const actions: sessionTimeoutAutoLogoutRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runSessionTimeoutAutoLogoutEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planSessionTimeoutAutoLogoutRecovery(
    {
      topic: "session-timeout-auto-logout",
      mounted: true,
  ...
  const failure = planSessionTimeoutAutoLogoutRecovery(
    {
      topic: "session-timeout-auto-logout",
      mounted: true,
      lastSuccessfulVersion: 21,
