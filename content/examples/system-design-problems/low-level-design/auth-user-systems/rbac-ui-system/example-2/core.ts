export type rbacUiSystemSignal = {
  sessionAgeMs: number;
...
export type rbacUiSystemEvent = {
  id: string;
  topic: "rbac-ui-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: rbacUiSystemSignal;
};

export type rbacUiSystemDecision = {
  accepted: boolean;
...
export function evaluateRbacUiSystemEvent(event: rbacUiSystemEvent): rbacUiSystemDecision {
  ...
  let action: rbacUiSystemDecision["action"] = "commit";
...
export function runRbacUiSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateRbacUiSystemEvent({
    id: "rbac-ui-system-evt-1",
    topic: "rbac-ui-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateRbacUiSystemEvent({
    id: "rbac-ui-system-evt-late",
    topic: "rbac-ui-system",
    actorId: "user-42",
    sequence: 8,
