export type rbacUiSystemRuntimeState = {
...
export type rbacUiSystemRecoveryPlan = {
...
export function planRbacUiSystemRecovery(
  state: rbacUiSystemRuntimeState,
  nowMs: number,
): rbacUiSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: rbacUiSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runRbacUiSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planRbacUiSystemRecovery(
    {
      topic: "rbac-ui-system",
      mounted: true,
  ...
  const failure = planRbacUiSystemRecovery(
    {
      topic: "rbac-ui-system",
      mounted: true,
      lastSuccessfulVersion: 21,
