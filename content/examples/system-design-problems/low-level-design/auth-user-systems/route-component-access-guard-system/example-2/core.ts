export type routeComponentAccessGuardSystemSignal = {
  sessionAgeMs: number;
...
export type routeComponentAccessGuardSystemEvent = {
  id: string;
  topic: "route-component-access-guard-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: routeComponentAccessGuardSystemSignal;
};

export type routeComponentAccessGuardSystemDecision = {
  accepted: boolean;
...
export function evaluateRouteComponentAccessGuardSystemEvent(event: routeComponentAccessGuardSystemEvent): routeComponentAccessGuardSystemDecision {
  ...
  let action: routeComponentAccessGuardSystemDecision["action"] = "commit";
...
export function runRouteComponentAccessGuardSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateRouteComponentAccessGuardSystemEvent({
    id: "route-component-access-guard-system-evt-1",
    topic: "route-component-access-guard-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateRouteComponentAccessGuardSystemEvent({
    id: "route-component-access-guard-system-evt-late",
    topic: "route-component-access-guard-system",
    actorId: "user-42",
    sequence: 8,
