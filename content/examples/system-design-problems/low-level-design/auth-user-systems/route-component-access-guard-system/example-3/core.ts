export type routeComponentAccessGuardSystemRuntimeState = {
...
export type routeComponentAccessGuardSystemRecoveryPlan = {
...
export function planRouteComponentAccessGuardSystemRecovery(
  state: routeComponentAccessGuardSystemRuntimeState,
  nowMs: number,
): routeComponentAccessGuardSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: routeComponentAccessGuardSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runRouteComponentAccessGuardSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planRouteComponentAccessGuardSystemRecovery(
    {
      topic: "route-component-access-guard-system",
      mounted: true,
  ...
  const failure = planRouteComponentAccessGuardSystemRecovery(
    {
      topic: "route-component-access-guard-system",
      mounted: true,
      lastSuccessfulVersion: 21,
