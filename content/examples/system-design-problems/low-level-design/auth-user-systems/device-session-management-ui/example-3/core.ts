export type deviceSessionManagementUiRuntimeState = {
...
export type deviceSessionManagementUiRecoveryPlan = {
...
export function planDeviceSessionManagementUiRecovery(
  state: deviceSessionManagementUiRuntimeState,
  nowMs: number,
): deviceSessionManagementUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: deviceSessionManagementUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runDeviceSessionManagementUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planDeviceSessionManagementUiRecovery(
    {
      topic: "device-session-management-ui",
      mounted: true,
  ...
  const failure = planDeviceSessionManagementUiRecovery(
    {
      topic: "device-session-management-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
