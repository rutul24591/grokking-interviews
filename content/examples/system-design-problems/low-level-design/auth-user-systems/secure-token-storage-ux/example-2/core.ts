export type secureTokenStorageUxSignal = {
  sessionAgeMs: number;
...
export type secureTokenStorageUxEvent = {
  id: string;
  ...
  payloadSize: number;
  signal: secureTokenStorageUxSignal;
};

export type secureTokenStorageUxDecision = {
  accepted: boolean;
...
export function evaluateSecureTokenStorageUxEvent(event: secureTokenStorageUxEvent): secureTokenStorageUxDecision {
  ...
  let action: secureTokenStorageUxDecision["action"] = "commit";
...
export function runSecureTokenStorageUxContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateSecureTokenStorageUxEvent({
    id: "secure-token-storage-ux-evt-1",
    topic: "secure-token-storage-ux",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateSecureTokenStorageUxEvent({
    id: "secure-token-storage-ux-evt-late",
    topic: "secure-token-storage-ux",
    actorId: "user-42",
    sequence: 8,
