export type adminImpersonationUiSignal = {
  sessionAgeMs: number;
...
export type adminImpersonationUiEvent = {
  id: string;
  topic: "admin-impersonation-ui";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: adminImpersonationUiSignal;
};

export type adminImpersonationUiDecision = {
  accepted: boolean;
...
export function evaluateAdminImpersonationUiEvent(event: adminImpersonationUiEvent): adminImpersonationUiDecision {
  ...
  let action: adminImpersonationUiDecision["action"] = "commit";
...
export function runAdminImpersonationUiContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateAdminImpersonationUiEvent({
    id: "admin-impersonation-ui-evt-1",
    topic: "admin-impersonation-ui",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateAdminImpersonationUiEvent({
    id: "admin-impersonation-ui-evt-late",
    topic: "admin-impersonation-ui",
    actorId: "user-42",
    sequence: 8,
