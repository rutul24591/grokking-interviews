export type adminImpersonationUiRuntimeState = {
...
export type adminImpersonationUiRecoveryPlan = {
...
export function planAdminImpersonationUiRecovery(
  state: adminImpersonationUiRuntimeState,
  nowMs: number,
): adminImpersonationUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: adminImpersonationUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runAdminImpersonationUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planAdminImpersonationUiRecovery(
    {
      topic: "admin-impersonation-ui",
      mounted: true,
  ...
  const failure = planAdminImpersonationUiRecovery(
    {
      topic: "admin-impersonation-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
