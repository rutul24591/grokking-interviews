export type loginAuthFlowRuntimeState = {
...
export type loginAuthFlowRecoveryPlan = {
...
export function planLoginAuthFlowRecovery(
  state: loginAuthFlowRuntimeState,
  nowMs: number,
): loginAuthFlowRecoveryPlan {
  const evidence: string[] = [];
  const actions: loginAuthFlowRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runLoginAuthFlowEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planLoginAuthFlowRecovery(
    {
      topic: "login-auth-flow",
      mounted: true,
  ...
  const failure = planLoginAuthFlowRecovery(
    {
      topic: "login-auth-flow",
      mounted: true,
      lastSuccessfulVersion: 21,
