export type loginAuthFlowSignal = {
  sessionAgeMs: number;
...
export type loginAuthFlowEvent = {
  id: string;
  topic: "login-auth-flow";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: loginAuthFlowSignal;
};

export type loginAuthFlowDecision = {
  accepted: boolean;
...
export function evaluateLoginAuthFlowEvent(event: loginAuthFlowEvent): loginAuthFlowDecision {
  ...
  let action: loginAuthFlowDecision["action"] = "commit";
...
export function runLoginAuthFlowContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateLoginAuthFlowEvent({
    id: "login-auth-flow-evt-1",
    topic: "login-auth-flow",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateLoginAuthFlowEvent({
    id: "login-auth-flow-evt-late",
    topic: "login-auth-flow",
    actorId: "user-42",
    sequence: 8,
