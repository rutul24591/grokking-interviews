export type permissionEditorUiSignal = {
  sessionAgeMs: number;
...
export type permissionEditorUiEvent = {
  id: string;
  topic: "permission-editor-ui";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: permissionEditorUiSignal;
};

export type permissionEditorUiDecision = {
  accepted: boolean;
...
export function evaluatePermissionEditorUiEvent(event: permissionEditorUiEvent): permissionEditorUiDecision {
  ...
  let action: permissionEditorUiDecision["action"] = "commit";
...
export function runPermissionEditorUiContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluatePermissionEditorUiEvent({
    id: "permission-editor-ui-evt-1",
    topic: "permission-editor-ui",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluatePermissionEditorUiEvent({
    id: "permission-editor-ui-evt-late",
    topic: "permission-editor-ui",
    actorId: "user-42",
    sequence: 8,
