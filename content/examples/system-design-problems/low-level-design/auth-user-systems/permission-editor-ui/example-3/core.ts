export type permissionEditorUiRuntimeState = {
...
export type permissionEditorUiRecoveryPlan = {
...
export function planPermissionEditorUiRecovery(
  state: permissionEditorUiRuntimeState,
  nowMs: number,
): permissionEditorUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: permissionEditorUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runPermissionEditorUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planPermissionEditorUiRecovery(
    {
      topic: "permission-editor-ui",
      mounted: true,
  ...
  const failure = planPermissionEditorUiRecovery(
    {
      topic: "permission-editor-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
