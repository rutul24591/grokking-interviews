export interface CheckoutFlowMetric{operationId:string;
phase:"accepted"|"rejected"|"settled"|"rolled-back";
durationMs:number;
reason?:string;
}
export class CheckoutFlowDiagnostics{private events:CheckoutFlowMetric[]=[];
record(e:CheckoutFlowMetric){this.events.push(e);
...
export function planCheckoutFlowRecovery(input:{mounted:boolean;
authorized:boolean;
versionGap:number;
...
return{mode:reasons.length?"degrade-and-reconcile":"continue",reasons,edge:"payment succeeds but the order response times out before the UI receives confirmation",structures:"checkout session, step state, validation map, price quote version, payment attempt, idempotency key"};
}
export function runCheckoutFlowRecovery(){return planCheckoutFlowRecovery({mounted:true,authorized:false,versionGap:2,pendingAgeMs:16000});
}
