export type checkoutFlowRuntimeState = {
  topic: "checkout-flow";
  mounted: boolean;
...
export type checkoutFlowRecoveryPlan = {
...
export function planCheckoutFlowRecovery(
  state: checkoutFlowRuntimeState,
  nowMs: number,
): checkoutFlowRecoveryPlan {
  const evidence: string[] = [];
  const actions: checkoutFlowRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runCheckoutFlowEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planCheckoutFlowRecovery(
    {
      topic: "checkout-flow",
      mounted: true,
  ...
  const failure = planCheckoutFlowRecovery(
    {
      topic: "checkout-flow",
      mounted: true,
      lastSuccessfulVersion: 21,
