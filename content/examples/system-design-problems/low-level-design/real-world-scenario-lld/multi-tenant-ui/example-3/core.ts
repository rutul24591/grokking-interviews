export type multiTenantUiRuntimeState = {
...
export type multiTenantUiRecoveryPlan = {
...
export function planMultiTenantUiRecovery(
  state: multiTenantUiRuntimeState,
  nowMs: number,
): multiTenantUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: multiTenantUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runMultiTenantUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planMultiTenantUiRecovery(
    {
      topic: "multi-tenant-ui",
      mounted: true,
  ...
  const failure = planMultiTenantUiRecovery(
    {
      topic: "multi-tenant-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
