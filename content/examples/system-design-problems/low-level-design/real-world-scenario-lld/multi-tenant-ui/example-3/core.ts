export interface MultiTenantUiMetric{operationId:string;
phase:"accepted"|"rejected"|"settled"|"rolled-back";
durationMs:number;
reason?:string;
}
export class MultiTenantUiDiagnostics{private events:MultiTenantUiMetric[]=[];
record(e:MultiTenantUiMetric){this.events.push(e);
...
export function planMultiTenantUiRecovery(input:{mounted:boolean;
authorized:boolean;
versionGap:number;
...
return{mode:reasons.length?"degrade-and-reconcile":"continue",reasons,edge:"a user switches tenants while requests and optimistic updates are still in flight",structures:"tenant id, permission snapshot, cache namespace, route scope, feature snapshot, audit context"};
}
export function runMultiTenantUiRecovery(){return planMultiTenantUiRecovery({mounted:true,authorized:false,versionGap:2,pendingAgeMs:16000});
}
