export type multiTenantUiSignal = {
  latencyMs: number;
  staleAgeMs: number;
...
export type multiTenantUiEvent = {
  id: string;
  topic: "multi-tenant-ui";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: multiTenantUiSignal;
};

export type multiTenantUiDecision = {
  accepted: boolean;
...
export function evaluateMultiTenantUiEvent(event: multiTenantUiEvent): multiTenantUiDecision {
  ...
  let action: multiTenantUiDecision["action"] = "commit";
...
export function runMultiTenantUiContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateMultiTenantUiEvent({
    id: "multi-tenant-ui-evt-1",
    topic: "multi-tenant-ui",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateMultiTenantUiEvent({
    id: "multi-tenant-ui-evt-late",
    topic: "multi-tenant-ui",
    actorId: "user-42",
    sequence: 8,
