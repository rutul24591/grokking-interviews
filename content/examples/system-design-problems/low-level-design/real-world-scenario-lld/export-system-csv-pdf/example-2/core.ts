export interface ExportSystemCsvPdfMutation{id:string;
scope:string;
baseVersion:number;
payloadSize:number;
authorized:boolean;
...
export interface ExportSystemCsvPdfDecision{accepted:boolean;
action:"commit"|"refresh"|"block"|"dedupe"|"review";
reasons:string[];
nextVersion:number;
}
export class ExportSystemCsvPdfPolicy{private seen=new Set<string>();
constructor(private version:number){}evaluate(m:ExportSystemCsvPdfMutation):ExportSystemCsvPdfDecision{const reasons:string[]=[];
...
export function runExportSystemCsvPdfPolicy(){const p=new ExportSystemCsvPdfPolicy(7);
