export type paymentUiSystemSignal = {
  latencyMs: number;
  staleAgeMs: number;
...
export type paymentUiSystemEvent = {
  id: string;
  topic: "payment-ui-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: paymentUiSystemSignal;
};

export type paymentUiSystemDecision = {
  accepted: boolean;
...
export function evaluatePaymentUiSystemEvent(event: paymentUiSystemEvent): paymentUiSystemDecision {
  ...
  let action: paymentUiSystemDecision["action"] = "commit";
...
export function runPaymentUiSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluatePaymentUiSystemEvent({
    id: "payment-ui-system-evt-1",
    topic: "payment-ui-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluatePaymentUiSystemEvent({
    id: "payment-ui-system-evt-late",
    topic: "payment-ui-system",
    actorId: "user-42",
    sequence: 8,
