export interface PaymentUiSystemMutation{id:string;
scope:string;
baseVersion:number;
payloadSize:number;
authorized:boolean;
...
export interface PaymentUiSystemDecision{accepted:boolean;
action:"commit"|"refresh"|"block"|"dedupe"|"review";
reasons:string[];
nextVersion:number;
}
export class PaymentUiSystemPolicy{private seen=new Set<string>();
constructor(private version:number){}evaluate(m:PaymentUiSystemMutation):PaymentUiSystemDecision{const reasons:string[]=[];
...
export function runPaymentUiSystemPolicy(){const p=new PaymentUiSystemPolicy(7);
