export type paymentUiSystemRuntimeState = {
...
export type paymentUiSystemRecoveryPlan = {
...
export function planPaymentUiSystemRecovery(
  state: paymentUiSystemRuntimeState,
  nowMs: number,
): paymentUiSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: paymentUiSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runPaymentUiSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planPaymentUiSystemRecovery(
    {
      topic: "payment-ui-system",
      mounted: true,
  ...
  const failure = planPaymentUiSystemRecovery(
    {
      topic: "payment-ui-system",
      mounted: true,
      lastSuccessfulVersion: 21,
