export interface OfflineFormSyncSystemMutation{id:string;
scope:string;
baseVersion:number;
payloadSize:number;
authorized:boolean;
...
export interface OfflineFormSyncSystemDecision{accepted:boolean;
action:"commit"|"refresh"|"block"|"dedupe"|"review";
reasons:string[];
nextVersion:number;
}
export class OfflineFormSyncSystemPolicy{private seen=new Set<string>();
constructor(private version:number){}evaluate(m:OfflineFormSyncSystemMutation):OfflineFormSyncSystemDecision{const reasons:string[]=[];
...
export function runOfflineFormSyncSystemPolicy(){const p=new OfflineFormSyncSystemPolicy(7);
