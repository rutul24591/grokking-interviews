export type offlineFormSyncSystemSignal = {
  latencyMs: number;
  staleAgeMs: number;
...
export type offlineFormSyncSystemEvent = {
  id: string;
  topic: "offline-form-sync-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: offlineFormSyncSystemSignal;
};

export type offlineFormSyncSystemDecision = {
  accepted: boolean;
...
export function evaluateOfflineFormSyncSystemEvent(event: offlineFormSyncSystemEvent): offlineFormSyncSystemDecision {
  ...
  let action: offlineFormSyncSystemDecision["action"] = "commit";
...
export function runOfflineFormSyncSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateOfflineFormSyncSystemEvent({
    id: "offline-form-sync-system-evt-1",
    topic: "offline-form-sync-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateOfflineFormSyncSystemEvent({
    id: "offline-form-sync-system-evt-late",
    topic: "offline-form-sync-system",
    actorId: "user-42",
    sequence: 8,
