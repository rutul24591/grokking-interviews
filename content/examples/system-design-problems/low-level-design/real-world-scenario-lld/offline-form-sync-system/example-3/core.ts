export type offlineFormSyncSystemRuntimeState = {
...
export type offlineFormSyncSystemRecoveryPlan = {
...
export function planOfflineFormSyncSystemRecovery(
  state: offlineFormSyncSystemRuntimeState,
  nowMs: number,
): offlineFormSyncSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: offlineFormSyncSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runOfflineFormSyncSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planOfflineFormSyncSystemRecovery(
    {
      topic: "offline-form-sync-system",
      mounted: true,
  ...
  const failure = planOfflineFormSyncSystemRecovery(
    {
      topic: "offline-form-sync-system",
      mounted: true,
      lastSuccessfulVersion: 21,
