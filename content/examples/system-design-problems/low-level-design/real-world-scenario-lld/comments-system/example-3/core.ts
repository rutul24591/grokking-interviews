export type commentsSystemRuntimeState = {
...
export type commentsSystemRecoveryPlan = {
...
export function planCommentsSystemRecovery(
  state: commentsSystemRuntimeState,
  nowMs: number,
): commentsSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: commentsSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runCommentsSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planCommentsSystemRecovery(
    {
      topic: "comments-system",
      mounted: true,
  ...
  const failure = planCommentsSystemRecovery(
    {
      topic: "comments-system",
      mounted: true,
      lastSuccessfulVersion: 21,
