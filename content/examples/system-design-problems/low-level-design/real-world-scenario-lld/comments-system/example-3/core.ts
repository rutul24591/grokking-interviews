export interface CommentsSystemMetric{operationId:string;
phase:"accepted"|"rejected"|"settled"|"rolled-back";
durationMs:number;
reason?:string;
}
export class CommentsSystemDiagnostics{private events:CommentsSystemMetric[]=[];
record(e:CommentsSystemMetric){this.events.push(e);
...
export function planCommentsSystemRecovery(input:{mounted:boolean;
authorized:boolean;
versionGap:number;
...
return{mode:reasons.length?"degrade-and-reconcile":"continue",reasons,edge:"an optimistic reply is retried after moderation removes its parent comment",structures:"comment entity map, thread index, optimistic id map, revision, reaction ledger, moderation state"};
}
export function runCommentsSystemRecovery(){return planCommentsSystemRecovery({mounted:true,authorized:false,versionGap:2,pendingAgeMs:16000});
}
