export interface CommentsSystemMutation{id:string;
scope:string;
baseVersion:number;
payloadSize:number;
authorized:boolean;
...
export interface CommentsSystemDecision{accepted:boolean;
action:"commit"|"refresh"|"block"|"dedupe"|"review";
reasons:string[];
nextVersion:number;
}
export class CommentsSystemPolicy{private seen=new Set<string>();
constructor(private version:number){}evaluate(m:CommentsSystemMutation):CommentsSystemDecision{const reasons:string[]=[];
...
export function runCommentsSystemPolicy(){const p=new CommentsSystemPolicy(7);
