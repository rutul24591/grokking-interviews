export type shoppingCartSystemSignal = {
  latencyMs: number;
  staleAgeMs: number;
...
export type shoppingCartSystemEvent = {
  id: string;
  topic: "shopping-cart-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: shoppingCartSystemSignal;
};

export type shoppingCartSystemDecision = {
  accepted: boolean;
...
export function evaluateShoppingCartSystemEvent(event: shoppingCartSystemEvent): shoppingCartSystemDecision {
  ...
  let action: shoppingCartSystemDecision["action"] = "commit";
...
export function runShoppingCartSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateShoppingCartSystemEvent({
    id: "shopping-cart-system-evt-1",
    topic: "shopping-cart-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateShoppingCartSystemEvent({
    id: "shopping-cart-system-evt-late",
    topic: "shopping-cart-system",
    actorId: "user-42",
    sequence: 8,
