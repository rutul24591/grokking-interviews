export type shoppingCartSystemRuntimeState = {
...
export type shoppingCartSystemRecoveryPlan = {
...
export function planShoppingCartSystemRecovery(
  state: shoppingCartSystemRuntimeState,
  nowMs: number,
): shoppingCartSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: shoppingCartSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runShoppingCartSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planShoppingCartSystemRecovery(
    {
      topic: "shopping-cart-system",
      mounted: true,
  ...
  const failure = planShoppingCartSystemRecovery(
    {
      topic: "shopping-cart-system",
      mounted: true,
      lastSuccessfulVersion: 21,
