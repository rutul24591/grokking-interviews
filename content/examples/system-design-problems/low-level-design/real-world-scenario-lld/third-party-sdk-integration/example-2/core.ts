export interface ThirdPartySdkIntegrationMutation{id:string;
scope:string;
baseVersion:number;
payloadSize:number;
authorized:boolean;
...
export interface ThirdPartySdkIntegrationDecision{accepted:boolean;
action:"commit"|"refresh"|"block"|"dedupe"|"review";
reasons:string[];
nextVersion:number;
}
export class ThirdPartySdkIntegrationPolicy{private seen=new Set<string>();
constructor(private version:number){}evaluate(m:ThirdPartySdkIntegrationMutation):ThirdPartySdkIntegrationDecision{const reasons:string[]=[];
...
export function runThirdPartySdkIntegrationPolicy(){const p=new ThirdPartySdkIntegrationPolicy(7);
