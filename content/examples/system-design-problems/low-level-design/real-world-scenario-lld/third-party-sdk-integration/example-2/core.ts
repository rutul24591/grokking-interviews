export type thirdPartySdkIntegrationSignal = {
  latencyMs: number;
  staleAgeMs: number;
...
export type thirdPartySdkIntegrationEvent = {
  id: string;
  topic: "third-party-sdk-integration";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: thirdPartySdkIntegrationSignal;
};

export type thirdPartySdkIntegrationDecision = {
  accepted: boolean;
...
export function evaluateThirdPartySdkIntegrationEvent(event: thirdPartySdkIntegrationEvent): thirdPartySdkIntegrationDecision {
  ...
  let action: thirdPartySdkIntegrationDecision["action"] = "commit";
...
export function runThirdPartySdkIntegrationContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateThirdPartySdkIntegrationEvent({
    id: "third-party-sdk-integration-evt-1",
    topic: "third-party-sdk-integration",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateThirdPartySdkIntegrationEvent({
    id: "third-party-sdk-integration-evt-late",
    topic: "third-party-sdk-integration",
    actorId: "user-42",
    sequence: 8,
