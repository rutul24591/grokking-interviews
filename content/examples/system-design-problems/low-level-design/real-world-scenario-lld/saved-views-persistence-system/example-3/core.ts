export type savedViewsPersistenceSystemRuntimeState = {
...
export type savedViewsPersistenceSystemRecoveryPlan = {
...
export function planSavedViewsPersistenceSystemRecovery(
  state: savedViewsPersistenceSystemRuntimeState,
  nowMs: number,
): savedViewsPersistenceSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: savedViewsPersistenceSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runSavedViewsPersistenceSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planSavedViewsPersistenceSystemRecovery(
    {
      topic: "saved-views-persistence-system",
      mounted: true,
  ...
  const failure = planSavedViewsPersistenceSystemRecovery(
    {
      topic: "saved-views-persistence-system",
      mounted: true,
      lastSuccessfulVersion: 21,
