export interface SavedViewsPersistenceSystemMutation{id:string;
scope:string;
baseVersion:number;
payloadSize:number;
authorized:boolean;
...
export interface SavedViewsPersistenceSystemDecision{accepted:boolean;
action:"commit"|"refresh"|"block"|"dedupe"|"review";
reasons:string[];
nextVersion:number;
}
export class SavedViewsPersistenceSystemPolicy{private seen=new Set<string>();
constructor(private version:number){}evaluate(m:SavedViewsPersistenceSystemMutation):SavedViewsPersistenceSystemDecision{const reasons:string[]=[];
...
export function runSavedViewsPersistenceSystemPolicy(){const p=new SavedViewsPersistenceSystemPolicy(7);
