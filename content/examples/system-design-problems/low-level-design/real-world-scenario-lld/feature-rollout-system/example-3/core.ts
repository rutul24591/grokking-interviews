export type featureRolloutSystemRuntimeState = {
...
export type featureRolloutSystemRecoveryPlan = {
...
export function planFeatureRolloutSystemRecovery(
  state: featureRolloutSystemRuntimeState,
  nowMs: number,
): featureRolloutSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: featureRolloutSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runFeatureRolloutSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planFeatureRolloutSystemRecovery(
    {
      topic: "feature-rollout-system",
      mounted: true,
  ...
  const failure = planFeatureRolloutSystemRecovery(
    {
      topic: "feature-rollout-system",
      mounted: true,
      lastSuccessfulVersion: 21,
