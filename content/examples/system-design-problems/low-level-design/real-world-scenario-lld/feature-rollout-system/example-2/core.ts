export interface FeatureRolloutSystemMutation{id:string;
scope:string;
baseVersion:number;
payloadSize:number;
authorized:boolean;
...
export interface FeatureRolloutSystemDecision{accepted:boolean;
action:"commit"|"refresh"|"block"|"dedupe"|"review";
reasons:string[];
nextVersion:number;
}
export class FeatureRolloutSystemPolicy{private seen=new Set<string>();
constructor(private version:number){}evaluate(m:FeatureRolloutSystemMutation):FeatureRolloutSystemDecision{const reasons:string[]=[];
...
export function runFeatureRolloutSystemPolicy(){const p=new FeatureRolloutSystemPolicy(7);
