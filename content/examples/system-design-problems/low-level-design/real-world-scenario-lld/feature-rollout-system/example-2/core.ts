export type featureRolloutSystemSignal = {
  latencyMs: number;
  staleAgeMs: number;
...
export type featureRolloutSystemEvent = {
  id: string;
  topic: "feature-rollout-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: featureRolloutSystemSignal;
};

export type featureRolloutSystemDecision = {
  accepted: boolean;
...
export function evaluateFeatureRolloutSystemEvent(event: featureRolloutSystemEvent): featureRolloutSystemDecision {
  ...
  let action: featureRolloutSystemDecision["action"] = "commit";
...
export function runFeatureRolloutSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateFeatureRolloutSystemEvent({
    id: "feature-rollout-system-evt-1",
    topic: "feature-rollout-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateFeatureRolloutSystemEvent({
    id: "feature-rollout-system-evt-late",
    topic: "feature-rollout-system",
    actorId: "user-42",
    sequence: 8,
