export type versionHistorySystemRuntimeState = {
...
export type versionHistorySystemRecoveryPlan = {
...
export function planVersionHistorySystemRecovery(
  state: versionHistorySystemRuntimeState,
  nowMs: number,
): versionHistorySystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: versionHistorySystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runVersionHistorySystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planVersionHistorySystemRecovery(
    {
      topic: "version-history-system",
      mounted: true,
  ...
  const failure = planVersionHistorySystemRecovery(
    {
      topic: "version-history-system",
      mounted: true,
      lastSuccessfulVersion: 21,
