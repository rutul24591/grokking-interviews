export type telemetryAnalyticsPipelineRuntimeState = {
...
export type telemetryAnalyticsPipelineRecoveryPlan = {
...
export function planTelemetryAnalyticsPipelineRecovery(
  state: telemetryAnalyticsPipelineRuntimeState,
  nowMs: number,
): telemetryAnalyticsPipelineRecoveryPlan {
  const evidence: string[] = [];
  const actions: telemetryAnalyticsPipelineRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runTelemetryAnalyticsPipelineEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planTelemetryAnalyticsPipelineRecovery(
    {
      topic: "telemetry-analytics-pipeline",
      mounted: true,
  ...
  const failure = planTelemetryAnalyticsPipelineRecovery(
    {
      topic: "telemetry-analytics-pipeline",
      mounted: true,
      lastSuccessfulVersion: 21,
