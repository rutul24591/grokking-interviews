export interface MapBasedUiSystemMutation{id:string;
scope:string;
baseVersion:number;
payloadSize:number;
authorized:boolean;
...
export interface MapBasedUiSystemDecision{accepted:boolean;
action:"commit"|"refresh"|"block"|"dedupe"|"review";
reasons:string[];
nextVersion:number;
}
export class MapBasedUiSystemPolicy{private seen=new Set<string>();
constructor(private version:number){}evaluate(m:MapBasedUiSystemMutation):MapBasedUiSystemDecision{const reasons:string[]=[];
...
export function runMapBasedUiSystemPolicy(){const p=new MapBasedUiSystemPolicy(7);
