export type mapBasedUiSystemSignal = {
  latencyMs: number;
  staleAgeMs: number;
...
export type mapBasedUiSystemEvent = {
  id: string;
  topic: "map-based-ui-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: mapBasedUiSystemSignal;
};

export type mapBasedUiSystemDecision = {
  accepted: boolean;
...
export function evaluateMapBasedUiSystemEvent(event: mapBasedUiSystemEvent): mapBasedUiSystemDecision {
  ...
  let action: mapBasedUiSystemDecision["action"] = "commit";
...
export function runMapBasedUiSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateMapBasedUiSystemEvent({
    id: "map-based-ui-system-evt-1",
    topic: "map-based-ui-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateMapBasedUiSystemEvent({
    id: "map-based-ui-system-evt-late",
    topic: "map-based-ui-system",
    actorId: "user-42",
    sequence: 8,
