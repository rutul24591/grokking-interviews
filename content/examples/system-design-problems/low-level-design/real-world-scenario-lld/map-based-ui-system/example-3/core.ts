export type mapBasedUiSystemRuntimeState = {
...
export type mapBasedUiSystemRecoveryPlan = {
...
export function planMapBasedUiSystemRecovery(
  state: mapBasedUiSystemRuntimeState,
  nowMs: number,
): mapBasedUiSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: mapBasedUiSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runMapBasedUiSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planMapBasedUiSystemRecovery(
    {
      topic: "map-based-ui-system",
      mounted: true,
  ...
  const failure = planMapBasedUiSystemRecovery(
    {
      topic: "map-based-ui-system",
      mounted: true,
      lastSuccessfulVersion: 21,
