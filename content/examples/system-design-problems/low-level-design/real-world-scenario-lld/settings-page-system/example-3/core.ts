export type settingsPageSystemRuntimeState = {
...
export type settingsPageSystemRecoveryPlan = {
...
export function planSettingsPageSystemRecovery(
  state: settingsPageSystemRuntimeState,
  nowMs: number,
): settingsPageSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: settingsPageSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runSettingsPageSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planSettingsPageSystemRecovery(
    {
      topic: "settings-page-system",
      mounted: true,
  ...
  const failure = planSettingsPageSystemRecovery(
    {
      topic: "settings-page-system",
      mounted: true,
      lastSuccessfulVersion: 21,
