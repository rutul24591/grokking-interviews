export type settingsPageSystemSignal = {
  latencyMs: number;
  staleAgeMs: number;
...
export type settingsPageSystemEvent = {
  id: string;
  topic: "settings-page-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: settingsPageSystemSignal;
};

export type settingsPageSystemDecision = {
  accepted: boolean;
...
export function evaluateSettingsPageSystemEvent(event: settingsPageSystemEvent): settingsPageSystemDecision {
  ...
  let action: settingsPageSystemDecision["action"] = "commit";
...
export function runSettingsPageSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateSettingsPageSystemEvent({
    id: "settings-page-system-evt-1",
    topic: "settings-page-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateSettingsPageSystemEvent({
    id: "settings-page-system-evt-late",
    topic: "settings-page-system",
    actorId: "user-42",
    sequence: 8,
