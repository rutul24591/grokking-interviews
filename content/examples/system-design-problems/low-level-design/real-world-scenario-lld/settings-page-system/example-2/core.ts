export interface SettingsPageSystemMutation{id:string;
scope:string;
baseVersion:number;
payloadSize:number;
authorized:boolean;
...
export interface SettingsPageSystemDecision{accepted:boolean;
action:"commit"|"refresh"|"block"|"dedupe"|"review";
reasons:string[];
nextVersion:number;
}
export class SettingsPageSystemPolicy{private seen=new Set<string>();
constructor(private version:number){}evaluate(m:SettingsPageSystemMutation):SettingsPageSystemDecision{const reasons:string[]=[];
...
export function runSettingsPageSystemPolicy(){const p=new SettingsPageSystemPolicy(7);
