export type auditLogViewerUiRuntimeState = {
...
export type auditLogViewerUiRecoveryPlan = {
...
export function planAuditLogViewerUiRecovery(
  state: auditLogViewerUiRuntimeState,
  nowMs: number,
): auditLogViewerUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: auditLogViewerUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runAuditLogViewerUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planAuditLogViewerUiRecovery(
    {
      topic: "audit-log-viewer-ui",
      mounted: true,
  ...
  const failure = planAuditLogViewerUiRecovery(
    {
      topic: "audit-log-viewer-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
