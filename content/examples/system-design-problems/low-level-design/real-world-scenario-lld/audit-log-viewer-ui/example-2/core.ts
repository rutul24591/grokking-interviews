export interface AuditLogViewerUiMutation{id:string;
scope:string;
baseVersion:number;
payloadSize:number;
authorized:boolean;
...
export interface AuditLogViewerUiDecision{accepted:boolean;
action:"commit"|"refresh"|"block"|"dedupe"|"review";
reasons:string[];
nextVersion:number;
}
export class AuditLogViewerUiPolicy{private seen=new Set<string>();
constructor(private version:number){}evaluate(m:AuditLogViewerUiMutation):AuditLogViewerUiDecision{const reasons:string[]=[];
...
export function runAuditLogViewerUiPolicy(){const p=new AuditLogViewerUiPolicy(7);
