export type auditLogViewerUiSignal = {
  latencyMs: number;
  staleAgeMs: number;
...
export type auditLogViewerUiEvent = {
  id: string;
  topic: "audit-log-viewer-ui";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: auditLogViewerUiSignal;
};

export type auditLogViewerUiDecision = {
  accepted: boolean;
...
export function evaluateAuditLogViewerUiEvent(event: auditLogViewerUiEvent): auditLogViewerUiDecision {
  ...
  let action: auditLogViewerUiDecision["action"] = "commit";
...
export function runAuditLogViewerUiContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateAuditLogViewerUiEvent({
    id: "audit-log-viewer-ui-evt-1",
    topic: "audit-log-viewer-ui",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateAuditLogViewerUiEvent({
    id: "audit-log-viewer-ui-evt-late",
    topic: "audit-log-viewer-ui",
    actorId: "user-42",
    sequence: 8,
