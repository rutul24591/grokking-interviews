export interface ActivityFeedSystemMutation{id:string;
scope:string;
baseVersion:number;
payloadSize:number;
authorized:boolean;
...
export interface ActivityFeedSystemDecision{accepted:boolean;
action:"commit"|"refresh"|"block"|"dedupe"|"review";
reasons:string[];
nextVersion:number;
}
export class ActivityFeedSystemPolicy{private seen=new Set<string>();
constructor(private version:number){}evaluate(m:ActivityFeedSystemMutation):ActivityFeedSystemDecision{const reasons:string[]=[];
...
export function runActivityFeedSystemPolicy(){const p=new ActivityFeedSystemPolicy(7);
