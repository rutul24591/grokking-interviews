export type activityFeedSystemSignal = {
  latencyMs: number;
  staleAgeMs: number;
...
export type activityFeedSystemEvent = {
  id: string;
  topic: "activity-feed-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: activityFeedSystemSignal;
};

export type activityFeedSystemDecision = {
  accepted: boolean;
...
export function evaluateActivityFeedSystemEvent(event: activityFeedSystemEvent): activityFeedSystemDecision {
  ...
  let action: activityFeedSystemDecision["action"] = "commit";
...
export function runActivityFeedSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateActivityFeedSystemEvent({
    id: "activity-feed-system-evt-1",
    topic: "activity-feed-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateActivityFeedSystemEvent({
    id: "activity-feed-system-evt-late",
    topic: "activity-feed-system",
    actorId: "user-42",
    sequence: 8,
