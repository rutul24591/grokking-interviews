export type activityFeedSystemRuntimeState = {
...
export type activityFeedSystemRecoveryPlan = {
...
export function planActivityFeedSystemRecovery(
  state: activityFeedSystemRuntimeState,
  nowMs: number,
): activityFeedSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: activityFeedSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runActivityFeedSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planActivityFeedSystemRecovery(
    {
      topic: "activity-feed-system",
      mounted: true,
  ...
  const failure = planActivityFeedSystemRecovery(
    {
      topic: "activity-feed-system",
      mounted: true,
      lastSuccessfulVersion: 21,
