export type bulkEditingUiRuntimeState = {
...
export type bulkEditingUiRecoveryPlan = {
...
export function planBulkEditingUiRecovery(
  state: bulkEditingUiRuntimeState,
  nowMs: number,
): bulkEditingUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: bulkEditingUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runBulkEditingUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planBulkEditingUiRecovery(
    {
      topic: "bulk-editing-ui",
      mounted: true,
  ...
  const failure = planBulkEditingUiRecovery(
    {
      topic: "bulk-editing-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
