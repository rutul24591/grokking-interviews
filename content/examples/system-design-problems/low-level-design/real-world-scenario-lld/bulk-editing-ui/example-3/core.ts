export interface BulkEditingUiMetric{operationId:string;
phase:"accepted"|"rejected"|"settled"|"rolled-back";
durationMs:number;
reason?:string;
}
export class BulkEditingUiDiagnostics{private events:BulkEditingUiMetric[]=[];
record(e:BulkEditingUiMetric){this.events.push(e);
...
export function planBulkEditingUiRecovery(input:{mounted:boolean;
authorized:boolean;
versionGap:number;
...
return{mode:reasons.length?"degrade-and-reconcile":"continue",reasons,edge:"ten thousand selected rows include stale, unauthorized, and validation-failing records",structures:"selection model, edit patch, validation results, batch id, per-row outcome, rollback plan"};
}
export function runBulkEditingUiRecovery(){return planBulkEditingUiRecovery({mounted:true,authorized:false,versionGap:2,pendingAgeMs:16000});
}
