export interface BulkEditingUiMutation{id:string;
scope:string;
baseVersion:number;
payloadSize:number;
authorized:boolean;
...
export interface BulkEditingUiDecision{accepted:boolean;
action:"commit"|"refresh"|"block"|"dedupe"|"review";
reasons:string[];
nextVersion:number;
}
export class BulkEditingUiPolicy{private seen=new Set<string>();
constructor(private version:number){}evaluate(m:BulkEditingUiMutation):BulkEditingUiDecision{const reasons:string[]=[];
...
export function runBulkEditingUiPolicy(){const p=new BulkEditingUiPolicy(7);
