export type bulkEditingUiSignal = {
  latencyMs: number;
  staleAgeMs: number;
...
export type bulkEditingUiEvent = {
  id: string;
  topic: "bulk-editing-ui";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: bulkEditingUiSignal;
};

export type bulkEditingUiDecision = {
  accepted: boolean;
...
export function evaluateBulkEditingUiEvent(event: bulkEditingUiEvent): bulkEditingUiDecision {
  ...
  let action: bulkEditingUiDecision["action"] = "commit";
...
export function runBulkEditingUiContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateBulkEditingUiEvent({
    id: "bulk-editing-ui-evt-1",
    topic: "bulk-editing-ui",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateBulkEditingUiEvent({
    id: "bulk-editing-ui-evt-late",
    topic: "bulk-editing-ui",
    actorId: "user-42",
    sequence: 8,
