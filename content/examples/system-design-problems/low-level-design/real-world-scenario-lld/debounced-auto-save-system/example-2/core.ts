export interface DebouncedAutoSaveSystemMutation{id:string;
scope:string;
baseVersion:number;
payloadSize:number;
authorized:boolean;
...
export interface DebouncedAutoSaveSystemDecision{accepted:boolean;
action:"commit"|"refresh"|"block"|"dedupe"|"review";
reasons:string[];
nextVersion:number;
}
export class DebouncedAutoSaveSystemPolicy{private seen=new Set<string>();
constructor(private version:number){}evaluate(m:DebouncedAutoSaveSystemMutation):DebouncedAutoSaveSystemDecision{const reasons:string[]=[];
...
export function runDebouncedAutoSaveSystemPolicy(){const p=new DebouncedAutoSaveSystemPolicy(7);
