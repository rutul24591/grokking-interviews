export type debouncedAutoSaveSystemSignal = {
  latencyMs: number;
  staleAgeMs: number;
...
export type debouncedAutoSaveSystemEvent = {
  id: string;
  topic: "debounced-auto-save-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: debouncedAutoSaveSystemSignal;
};

export type debouncedAutoSaveSystemDecision = {
  accepted: boolean;
...
export function evaluateDebouncedAutoSaveSystemEvent(event: debouncedAutoSaveSystemEvent): debouncedAutoSaveSystemDecision {
  ...
  let action: debouncedAutoSaveSystemDecision["action"] = "commit";
...
export function runDebouncedAutoSaveSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateDebouncedAutoSaveSystemEvent({
    id: "debounced-auto-save-system-evt-1",
    topic: "debounced-auto-save-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateDebouncedAutoSaveSystemEvent({
    id: "debounced-auto-save-system-evt-late",
    topic: "debounced-auto-save-system",
    actorId: "user-42",
    sequence: 8,
