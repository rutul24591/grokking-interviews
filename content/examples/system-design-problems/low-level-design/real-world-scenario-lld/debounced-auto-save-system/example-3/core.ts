export type debouncedAutoSaveSystemRuntimeState = {
...
export type debouncedAutoSaveSystemRecoveryPlan = {
...
export function planDebouncedAutoSaveSystemRecovery(
  state: debouncedAutoSaveSystemRuntimeState,
  nowMs: number,
): debouncedAutoSaveSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: debouncedAutoSaveSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runDebouncedAutoSaveSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planDebouncedAutoSaveSystemRecovery(
    {
      topic: "debounced-auto-save-system",
      mounted: true,
  ...
  const failure = planDebouncedAutoSaveSystemRecovery(
    {
      topic: "debounced-auto-save-system",
      mounted: true,
      lastSuccessfulVersion: 21,
