export type featureFlagSystemRuntimeState = {
...
export type featureFlagSystemRecoveryPlan = {
...
export function planFeatureFlagSystemRecovery(
  state: featureFlagSystemRuntimeState,
  nowMs: number,
): featureFlagSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: featureFlagSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runFeatureFlagSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planFeatureFlagSystemRecovery(
    {
      topic: "feature-flag-system",
      mounted: true,
  ...
  const failure = planFeatureFlagSystemRecovery(
    {
      topic: "feature-flag-system",
      mounted: true,
      lastSuccessfulVersion: 21,
