export type notificationsBadgeSystemRuntimeState = {
...
export type notificationsBadgeSystemRecoveryPlan = {
...
export function planNotificationsBadgeSystemRecovery(
  state: notificationsBadgeSystemRuntimeState,
  nowMs: number,
): notificationsBadgeSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: notificationsBadgeSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runNotificationsBadgeSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planNotificationsBadgeSystemRecovery(
    {
      topic: "notifications-badge-system",
      mounted: true,
  ...
  const failure = planNotificationsBadgeSystemRecovery(
    {
      topic: "notifications-badge-system",
      mounted: true,
      lastSuccessfulVersion: 21,
