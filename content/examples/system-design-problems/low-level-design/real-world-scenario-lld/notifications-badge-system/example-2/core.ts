export interface NotificationsBadgeSystemMutation{id:string;
scope:string;
baseVersion:number;
payloadSize:number;
authorized:boolean;
...
export interface NotificationsBadgeSystemDecision{accepted:boolean;
action:"commit"|"refresh"|"block"|"dedupe"|"review";
reasons:string[];
nextVersion:number;
}
export class NotificationsBadgeSystemPolicy{private seen=new Set<string>();
constructor(private version:number){}evaluate(m:NotificationsBadgeSystemMutation):NotificationsBadgeSystemDecision{const reasons:string[]=[];
...
export function runNotificationsBadgeSystemPolicy(){const p=new NotificationsBadgeSystemPolicy(7);
