export type notificationsBadgeSystemSignal = {
  latencyMs: number;
  staleAgeMs: number;
...
export type notificationsBadgeSystemEvent = {
  id: string;
  topic: "notifications-badge-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: notificationsBadgeSystemSignal;
};

export type notificationsBadgeSystemDecision = {
  accepted: boolean;
...
export function evaluateNotificationsBadgeSystemEvent(event: notificationsBadgeSystemEvent): notificationsBadgeSystemDecision {
  ...
  let action: notificationsBadgeSystemDecision["action"] = "commit";
...
export function runNotificationsBadgeSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateNotificationsBadgeSystemEvent({
    id: "notifications-badge-system-evt-1",
    topic: "notifications-badge-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateNotificationsBadgeSystemEvent({
    id: "notifications-badge-system-evt-late",
    topic: "notifications-badge-system",
    actorId: "user-42",
    sequence: 8,
