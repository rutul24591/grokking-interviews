export type pdfViewerRuntimeState = {
  topic: "pdf-viewer";
  mounted: boolean;
...
export function planPdfViewerRecovery(
  state: pdfViewerRuntimeState,
  nowMs: number,
): pdfViewerRecoveryPlan {
  const evidence: string[] = [];
  const actions: pdfViewerRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runPdfViewerEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planPdfViewerRecovery(
    {
      topic: "pdf-viewer",
      mounted: true,
  ...
  const failure = planPdfViewerRecovery(
    {
      topic: "pdf-viewer",
      mounted: true,
      lastSuccessfulVersion: 21,
