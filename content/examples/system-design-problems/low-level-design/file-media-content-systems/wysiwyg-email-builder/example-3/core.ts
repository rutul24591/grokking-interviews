export type wysiwygEmailBuilderRuntimeState = {
...
export function planWysiwygEmailBuilderRecovery(
  state: wysiwygEmailBuilderRuntimeState,
  nowMs: number,
): wysiwygEmailBuilderRecoveryPlan {
  const evidence: string[] = [];
  const actions: wysiwygEmailBuilderRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runWysiwygEmailBuilderEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planWysiwygEmailBuilderRecovery(
    {
      topic: "wysiwyg-email-builder",
      mounted: true,
  ...
  const failure = planWysiwygEmailBuilderRecovery(
    {
      topic: "wysiwyg-email-builder",
      mounted: true,
      lastSuccessfulVersion: 21,
