export type fileUploadSystemRuntimeState = {
...
export function planFileUploadSystemRecovery(
  state: fileUploadSystemRuntimeState,
  nowMs: number,
): fileUploadSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: fileUploadSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runFileUploadSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planFileUploadSystemRecovery(
    {
      topic: "file-upload-system",
      mounted: true,
  ...
  const failure = planFileUploadSystemRecovery(
    {
      topic: "file-upload-system",
      mounted: true,
      lastSuccessfulVersion: 21,
