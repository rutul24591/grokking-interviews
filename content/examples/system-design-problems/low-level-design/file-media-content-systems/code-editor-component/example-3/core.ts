export type codeEditorComponentRuntimeState = {
...
export function planCodeEditorComponentRecovery(
  state: codeEditorComponentRuntimeState,
  nowMs: number,
): codeEditorComponentRecoveryPlan {
  const evidence: string[] = [];
  const actions: codeEditorComponentRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runCodeEditorComponentEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planCodeEditorComponentRecovery(
    {
      topic: "code-editor-component",
      mounted: true,
  ...
  const failure = planCodeEditorComponentRecovery(
    {
      topic: "code-editor-component",
      mounted: true,
      lastSuccessfulVersion: 21,
