export type audioVideoPlayerRuntimeState = {
...
export function planAudioVideoPlayerRecovery(
  state: audioVideoPlayerRuntimeState,
  nowMs: number,
): audioVideoPlayerRecoveryPlan {
  const evidence: string[] = [];
  const actions: audioVideoPlayerRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runAudioVideoPlayerEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planAudioVideoPlayerRecovery(
    {
      topic: "audio-video-player",
      mounted: true,
  ...
  const failure = planAudioVideoPlayerRecovery(
    {
      topic: "audio-video-player",
      mounted: true,
      lastSuccessfulVersion: 21,
