export type searchAutocompleteRuntimeState = {
...
export type searchAutocompleteRecoveryPlan = {
...
export function planSearchAutocompleteRecovery(
  state: searchAutocompleteRuntimeState,
  nowMs: number,
): searchAutocompleteRecoveryPlan {
  const evidence: string[] = [];
  const actions: searchAutocompleteRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runSearchAutocompleteEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planSearchAutocompleteRecovery(
    {
      topic: "search-autocomplete",
      mounted: true,
  ...
  const failure = planSearchAutocompleteRecovery(
    {
      topic: "search-autocomplete",
      mounted: true,
      lastSuccessfulVersion: 21,
