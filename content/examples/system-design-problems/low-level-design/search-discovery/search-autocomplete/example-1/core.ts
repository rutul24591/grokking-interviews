export type SearchAutocompleteStatus = "closed" | "debouncing" | "loading" | "open" | "empty" | "failed";
export interface SearchAutocompleteSnapshot { status: SearchAutocompleteStatus; query: string; focused: boolean; requestToken: number; suggestions: string[]; highlightedIndex: number; }
export class SearchAutocompleteCoordinator {
  private snapshot: SearchAutocompleteSnapshot = { status: "closed", query: "", focused: false, requestToken: 0, suggestions: [], highlightedIndex: -1 };
...
export function runSearchAutocompleteFocusScenario(){const c=new SearchAutocompleteCoordinator();c.focus();const pending=c.query("rea");c.blur();const ignored=c.settle(pending.requestToken,["react","reason"]);return{invariant:"Suggestions must open only for an active search interaction and stale network results must never replace newer intent.",ignored};}
