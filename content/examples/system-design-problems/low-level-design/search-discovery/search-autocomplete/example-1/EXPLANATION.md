# Design a Search Autocomplete - example-1 Explanation

## Article context
This example supports the article `low-level-design/search-discovery/search-autocomplete`. The article is about LLD for a search autocomplete: debounced fetching, abortable requests, keyboard navigation, highlighted suggestions, result caching, and accessibility.. The most relevant article sections for this example are: 🎯 Problem Context and Scope Definition; Problem Statement; User Context; Assumptions; Non-Goals; Functional Requirements; Core (Must-have); Secondary (Nice-to-have); Out of Scope; Non-Functional Requirements.
...
- `core.ts`: Implements the main logic, including createInitialState, queryKey, highlightMatch, q, idx.
