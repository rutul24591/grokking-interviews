export type ragBasedSearchUiSignal = {
...
export type ragBasedSearchUiEvent = {
  id: string;
  topic: "rag-based-search-ui";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: ragBasedSearchUiSignal;
};

export type ragBasedSearchUiDecision = {
  accepted: boolean;
...
export function evaluateRagBasedSearchUiEvent(event: ragBasedSearchUiEvent): ragBasedSearchUiDecision {
  ...
  let action: ragBasedSearchUiDecision["action"] = "commit";
...
export function runRagBasedSearchUiContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateRagBasedSearchUiEvent({
    id: "rag-based-search-ui-evt-1",
    topic: "rag-based-search-ui",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateRagBasedSearchUiEvent({
    id: "rag-based-search-ui-evt-late",
    topic: "rag-based-search-ui",
    actorId: "user-42",
    sequence: 8,
