export type ragBasedSearchUiRuntimeState = {
...
export type ragBasedSearchUiRecoveryPlan = {
...
export function planRagBasedSearchUiRecovery(
  state: ragBasedSearchUiRuntimeState,
  nowMs: number,
): ragBasedSearchUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: ragBasedSearchUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runRagBasedSearchUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planRagBasedSearchUiRecovery(
    {
      topic: "rag-based-search-ui",
      mounted: true,
  ...
  const failure = planRagBasedSearchUiRecovery(
    {
      topic: "rag-based-search-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
