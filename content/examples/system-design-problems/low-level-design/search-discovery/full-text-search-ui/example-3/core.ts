export type fullTextSearchUiRuntimeState = {
...
export type fullTextSearchUiRecoveryPlan = {
...
export function planFullTextSearchUiRecovery(
  state: fullTextSearchUiRuntimeState,
  nowMs: number,
): fullTextSearchUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: fullTextSearchUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runFullTextSearchUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planFullTextSearchUiRecovery(
    {
      topic: "full-text-search-ui",
      mounted: true,
  ...
  const failure = planFullTextSearchUiRecovery(
    {
      topic: "full-text-search-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
