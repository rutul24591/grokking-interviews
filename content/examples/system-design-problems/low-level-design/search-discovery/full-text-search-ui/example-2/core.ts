export type fullTextSearchUiSignal = {
...
export type fullTextSearchUiEvent = {
  id: string;
  topic: "full-text-search-ui";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: fullTextSearchUiSignal;
};

export type fullTextSearchUiDecision = {
  accepted: boolean;
...
export function evaluateFullTextSearchUiEvent(event: fullTextSearchUiEvent): fullTextSearchUiDecision {
  ...
  let action: fullTextSearchUiDecision["action"] = "commit";
...
export function runFullTextSearchUiContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateFullTextSearchUiEvent({
    id: "full-text-search-ui-evt-1",
    topic: "full-text-search-ui",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateFullTextSearchUiEvent({
    id: "full-text-search-ui-evt-late",
    topic: "full-text-search-ui",
    actorId: "user-42",
    sequence: 8,
