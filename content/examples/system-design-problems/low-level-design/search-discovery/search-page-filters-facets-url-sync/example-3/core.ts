export type searchPageFiltersFacetsUrlSyncRuntimeState = {
...
export type searchPageFiltersFacetsUrlSyncRecoveryPlan = {
...
export function planSearchPageFiltersFacetsUrlSyncRecovery(
  state: searchPageFiltersFacetsUrlSyncRuntimeState,
  nowMs: number,
): searchPageFiltersFacetsUrlSyncRecoveryPlan {
  const evidence: string[] = [];
  const actions: searchPageFiltersFacetsUrlSyncRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runSearchPageFiltersFacetsUrlSyncEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planSearchPageFiltersFacetsUrlSyncRecovery(
    {
      topic: "search-page-filters-facets-url-sync",
      mounted: true,
  ...
  const failure = planSearchPageFiltersFacetsUrlSyncRecovery(
    {
      topic: "search-page-filters-facets-url-sync",
      mounted: true,
      lastSuccessfulVersion: 21,
