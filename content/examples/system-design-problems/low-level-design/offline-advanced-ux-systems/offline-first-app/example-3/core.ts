export type offlineFirstAppRuntimeState = {
...
export type offlineFirstAppRecoveryPlan = {
...
export function planOfflineFirstAppRecovery(
  state: offlineFirstAppRuntimeState,
  nowMs: number,
): offlineFirstAppRecoveryPlan {
  const evidence: string[] = [];
  const actions: offlineFirstAppRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runOfflineFirstAppEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planOfflineFirstAppRecovery(
    {
      topic: "offline-first-app",
      mounted: true,
  ...
  const failure = planOfflineFirstAppRecovery(
    {
      topic: "offline-first-app",
      mounted: true,
      lastSuccessfulVersion: 21,
