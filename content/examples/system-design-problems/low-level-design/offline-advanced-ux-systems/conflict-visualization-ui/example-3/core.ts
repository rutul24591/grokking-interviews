export type conflictVisualizationUiRuntimeState = {
...
export type conflictVisualizationUiRecoveryPlan = {
...
export function planConflictVisualizationUiRecovery(
  state: conflictVisualizationUiRuntimeState,
  nowMs: number,
): conflictVisualizationUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: conflictVisualizationUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runConflictVisualizationUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planConflictVisualizationUiRecovery(
    {
      topic: "conflict-visualization-ui",
      mounted: true,
  ...
  const failure = planConflictVisualizationUiRecovery(
    {
      topic: "conflict-visualization-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
