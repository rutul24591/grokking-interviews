export interface ConflictResolutionOfflineEditsVersionedValue {
  resourceId: string;
  version: number;
  fields: Record<string, string>;
}

export interface ConflictResolutionOfflineEditsConflict {
  field: string;
  base?: string;
  local?: string;
  remote?: string;
...
export interface ConflictResolutionOfflineEditsResolutionPlan {
  merged: Record<string, string>;
  conflicts: ConflictResolutionOfflineEditsConflict[];
  audit: string[];
  canCommitSilently: boolean;
}

export function buildConflictResolutionOfflineEditsResolutionPlan(
  base: ConflictResolutionOfflineEditsVersionedValue,
  local: ConflictResolutionOfflineEditsVersionedValue,
  remote: ConflictResolutionOfflineEditsVersionedValue,
): ConflictResolutionOfflineEditsResolutionPlan {
  const fields = new Set([...Object.keys(base.fields), ...Object.keys(local.fields), ...Object.keys(remote.fields)]);
  const merged: Record<string, string> = {};
  const conflicts: ConflictResolutionOfflineEditsConflict[] = [];
  ...
  return {
    ...
    audit: ["topic:conflict-resolution-offline-edits", "policy:field-level merge when safe, explicit conflict UI when semantic intent differs", `base:${base.version}`, `local:${local.version}`, `remote:${remote.version}`],
  };
}

export function runConflictResolutionOfflineEditsConflictScenario() {
  ...
  return buildConflictResolutionOfflineEditsResolutionPlan(base, local, remote);
}
