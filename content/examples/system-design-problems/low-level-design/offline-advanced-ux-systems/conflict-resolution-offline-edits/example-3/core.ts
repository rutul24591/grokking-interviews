export type conflictResolutionOfflineEditsRuntimeState = {
...
export type conflictResolutionOfflineEditsRecoveryPlan = {
...
export function planConflictResolutionOfflineEditsRecovery(
  state: conflictResolutionOfflineEditsRuntimeState,
  nowMs: number,
): conflictResolutionOfflineEditsRecoveryPlan {
  const evidence: string[] = [];
  const actions: conflictResolutionOfflineEditsRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runConflictResolutionOfflineEditsEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planConflictResolutionOfflineEditsRecovery(
    {
      topic: "conflict-resolution-offline-edits",
      mounted: true,
  ...
  const failure = planConflictResolutionOfflineEditsRecovery(
    {
      topic: "conflict-resolution-offline-edits",
      mounted: true,
      lastSuccessfulVersion: 21,
