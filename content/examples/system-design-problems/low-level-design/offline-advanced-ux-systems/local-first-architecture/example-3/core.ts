export type localFirstArchitectureRuntimeState = {
...
export type localFirstArchitectureRecoveryPlan = {
...
export function planLocalFirstArchitectureRecovery(
  state: localFirstArchitectureRuntimeState,
  nowMs: number,
): localFirstArchitectureRecoveryPlan {
  const evidence: string[] = [];
  const actions: localFirstArchitectureRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runLocalFirstArchitectureEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planLocalFirstArchitectureRecovery(
    {
      topic: "local-first-architecture",
      mounted: true,
  ...
  const failure = planLocalFirstArchitectureRecovery(
    {
      topic: "local-first-architecture",
      mounted: true,
      lastSuccessfulVersion: 21,
