export type localFirstArchitectureSignal = {
  offlineAgeMs: number;
...
export type localFirstArchitectureEvent = {
  id: string;
  ...
  payloadSize: number;
  signal: localFirstArchitectureSignal;
};

export type localFirstArchitectureDecision = {
  accepted: boolean;
...
export function evaluateLocalFirstArchitectureEvent(event: localFirstArchitectureEvent): localFirstArchitectureDecision {
  ...
  let action: localFirstArchitectureDecision["action"] = "commit";
...
export function runLocalFirstArchitectureContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateLocalFirstArchitectureEvent({
    id: "local-first-architecture-evt-1",
    topic: "local-first-architecture",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateLocalFirstArchitectureEvent({
    id: "local-first-architecture-evt-late",
    topic: "local-first-architecture",
    actorId: "user-42",
    sequence: 8,
