export type LocalFirstArchitectureHealth = "healthy" | "degraded" | "offline" | "blocked";

export interface LocalFirstArchitectureSignal {
  onlineHint: boolean;
  apiProbeOk: boolean;
...
export interface LocalFirstArchitectureDecision {
  health: LocalFirstArchitectureHealth;
...
export function classifyLocalFirstArchitectureHealth(signal: LocalFirstArchitectureSignal): LocalFirstArchitectureDecision {
...
export function runLocalFirstArchitectureHealthScenario() {
  const healthy = classifyLocalFirstArchitectureHealth({
    onlineHint: true,
    apiProbeOk: true,
  ...
  const blocked = classifyLocalFirstArchitectureHealth({
    onlineHint: true,
    apiProbeOk: false,
    storageWritable: true,
    permissionOk: false,
  ...
  return { invariant: "Local reads and writes remain available while the system eventually converges with trusted peers or server state.", healthy, blocked };
}
