export interface BackgroundSyncQueueSystemVersionedValue {
  resourceId: string;
  version: number;
  fields: Record<string, string>;
}

export interface BackgroundSyncQueueSystemConflict {
  field: string;
  base?: string;
  local?: string;
  remote?: string;
...
export interface BackgroundSyncQueueSystemResolutionPlan {
  merged: Record<string, string>;
  conflicts: BackgroundSyncQueueSystemConflict[];
  audit: string[];
  canCommitSilently: boolean;
}

export function buildBackgroundSyncQueueSystemResolutionPlan(
  base: BackgroundSyncQueueSystemVersionedValue,
  local: BackgroundSyncQueueSystemVersionedValue,
  remote: BackgroundSyncQueueSystemVersionedValue,
): BackgroundSyncQueueSystemResolutionPlan {
  const fields = new Set([...Object.keys(base.fields), ...Object.keys(local.fields), ...Object.keys(remote.fields)]);
  const merged: Record<string, string> = {};
  const conflicts: BackgroundSyncQueueSystemConflict[] = [];
  ...
  return {
    ...
    audit: ["topic:background-sync-queue-system", "policy:at-least-once local replay with server-side idempotency and client-side ack compaction", `base:${base.version}`, `local:${local.version}`, `remote:${remote.version}`],
  };
}

export function runBackgroundSyncQueueSystemConflictScenario() {
  ...
  return buildBackgroundSyncQueueSystemResolutionPlan(base, local, remote);
}
