export type backgroundSyncQueueSystemRuntimeState = {
...
export type backgroundSyncQueueSystemRecoveryPlan = {
...
export function planBackgroundSyncQueueSystemRecovery(
  state: backgroundSyncQueueSystemRuntimeState,
  nowMs: number,
): backgroundSyncQueueSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: backgroundSyncQueueSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runBackgroundSyncQueueSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planBackgroundSyncQueueSystemRecovery(
    {
      topic: "background-sync-queue-system",
      mounted: true,
  ...
  const failure = planBackgroundSyncQueueSystemRecovery(
    {
      topic: "background-sync-queue-system",
      mounted: true,
      lastSuccessfulVersion: 21,
