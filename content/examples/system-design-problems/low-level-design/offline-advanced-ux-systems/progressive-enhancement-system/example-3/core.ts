export type progressiveEnhancementSystemRuntimeState = {
...
export type progressiveEnhancementSystemRecoveryPlan = {
...
export function planProgressiveEnhancementSystemRecovery(
  state: progressiveEnhancementSystemRuntimeState,
  nowMs: number,
): progressiveEnhancementSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: progressiveEnhancementSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runProgressiveEnhancementSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planProgressiveEnhancementSystemRecovery(
    {
      topic: "progressive-enhancement-system",
      mounted: true,
  ...
  const failure = planProgressiveEnhancementSystemRecovery(
    {
      topic: "progressive-enhancement-system",
      mounted: true,
      lastSuccessfulVersion: 21,
