export type permissionsUxSignal = {
...
export type permissionsUxEvent = {
  id: string;
  ...
  payloadSize: number;
  signal: permissionsUxSignal;
};

export type permissionsUxDecision = {
  accepted: boolean;
...
export function evaluatePermissionsUxEvent(event: permissionsUxEvent): permissionsUxDecision {
  ...
  let action: permissionsUxDecision["action"] = "commit";
...
export function runPermissionsUxContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluatePermissionsUxEvent({
    id: "permissions-ux-evt-1",
    topic: "permissions-ux",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluatePermissionsUxEvent({
    id: "permissions-ux-evt-late",
    topic: "permissions-ux",
    actorId: "user-42",
    sequence: 8,
