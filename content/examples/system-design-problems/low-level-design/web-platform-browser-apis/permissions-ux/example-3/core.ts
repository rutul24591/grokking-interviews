export type permissionsUxRuntimeState = {
...
export type permissionsUxRecoveryPlan = {
...
export function planPermissionsUxRecovery(
  state: permissionsUxRuntimeState,
  nowMs: number,
): permissionsUxRecoveryPlan {
  const evidence: string[] = [];
  const actions: permissionsUxRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runPermissionsUxEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planPermissionsUxRecovery(
    {
      topic: "permissions-ux",
      mounted: true,
  ...
  const failure = planPermissionsUxRecovery(
    {
      topic: "permissions-ux",
      mounted: true,
      lastSuccessfulVersion: 21,
