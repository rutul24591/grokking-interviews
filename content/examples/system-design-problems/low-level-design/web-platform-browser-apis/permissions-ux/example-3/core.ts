export interface PermissionsUxTelemetryEvent {
  operationId: string;
  api: "permissions-ux";
...
export class PermissionsUxTelemetry {
  private events: PermissionsUxTelemetryEvent[] = [];

  record(event: PermissionsUxTelemetryEvent): void {
...
export function runPermissionsUxTelemetryScenario() {
  const telemetry = new PermissionsUxTelemetry();
  telemetry.record({ operationId: "op-1", api: "permissions-ux", outcome: "accepted", durationMs: 18, sensitivePayloadCaptured: false });
  telemetry.record({ operationId: "op-2", api: "permissions-ux", outcome: "fallback", reason: "permission-denied", durationMs: 4, sensitivePayloadCaptured: false });
  telemetry.record({ operationId: "op-3", api: "permissions-ux", outcome: "accepted", durationMs: 90, sensitivePayloadCaptured: true });
  return { summary: telemetry.summary(), slow: telemetry.slowOperations(50), fallbackRate: telemetry.fallbackRate() };
}

export function buildPermissionsUxIncidentReport(events: PermissionsUxTelemetryEvent[]): string[] {
...
export function choosePermissionsUxAlertLevel(fallbackRate: number, slowCount: number): "normal" | "watch" | "page" {
