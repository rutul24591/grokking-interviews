export type NativeDragDropFileSystemOutcome = "accepted" | "unsupported" | "permission-denied" | "fallback" | "cancelled";

export interface NativeDragDropFileSystemCapability {
  apiPresent: boolean;
...
export interface NativeDragDropFileSystemDecision {
  outcome: NativeDragDropFileSystemOutcome;
  allowed: boolean;
  fallback: "none" | "manual-control" | "read-only" | "defer-until-visible";
  reasons: string[];
}

export function decideNativeDragDropFileSystemCapability(capability: NativeDragDropFileSystemCapability): NativeDragDropFileSystemDecision {
...
export function runNativeDragDropFileSystemCapabilityScenario() {
  const accepted = decideNativeDragDropFileSystemCapability({ apiPresent: true, secureContext: true, permission: "granted", documentVisible: true, userActivation: true });
  const fallback = decideNativeDragDropFileSystemCapability({ apiPresent: true, secureContext: true, permission: "prompt", documentVisible: false, userActivation: false });
  return { invariant: "The drop zone must accept only safe files, keep focus and pointer state coherent, and never trust client MIME alone.", accepted, fallback };
}

export function explainNativeDragDropFileSystemOutcome(decision: NativeDragDropFileSystemDecision): string {
...
export function shouldRetryNativeDragDropFileSystemLater(decision: NativeDragDropFileSystemDecision): boolean {
  return decision.reasons.includes("document-hidden") || decision.reasons.includes("missing-user-activation");
}

export function buildNativeDragDropFileSystemUserMessage(decision: NativeDragDropFileSystemDecision): string {
...
export const NativeDragDropFileSystemExampleInvariant = "The drop zone must accept only safe files, keep focus and pointer state coherent, and never trust client MIME alone.";
