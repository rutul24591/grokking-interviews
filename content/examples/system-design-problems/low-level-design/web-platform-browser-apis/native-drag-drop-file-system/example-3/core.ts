export type nativeDragDropFileSystemRuntimeState = {
...
export type nativeDragDropFileSystemRecoveryPlan = {
...
export function planNativeDragDropFileSystemRecovery(
  state: nativeDragDropFileSystemRuntimeState,
  nowMs: number,
): nativeDragDropFileSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: nativeDragDropFileSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runNativeDragDropFileSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planNativeDragDropFileSystemRecovery(
    {
      topic: "native-drag-drop-file-system",
      mounted: true,
  ...
  const failure = planNativeDragDropFileSystemRecovery(
    {
      topic: "native-drag-drop-file-system",
      mounted: true,
      lastSuccessfulVersion: 21,
