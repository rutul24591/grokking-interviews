export type idleTaskSchedulingSystemRuntimeState = {
...
export type idleTaskSchedulingSystemRecoveryPlan = {
...
export function planIdleTaskSchedulingSystemRecovery(
  state: idleTaskSchedulingSystemRuntimeState,
  nowMs: number,
): idleTaskSchedulingSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: idleTaskSchedulingSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runIdleTaskSchedulingSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planIdleTaskSchedulingSystemRecovery(
    {
      topic: "idle-task-scheduling-system",
      mounted: true,
  ...
  const failure = planIdleTaskSchedulingSystemRecovery(
    {
      topic: "idle-task-scheduling-system",
      mounted: true,
      lastSuccessfulVersion: 21,
