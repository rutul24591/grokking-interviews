export type idleTaskSchedulingSystemSignal = {
...
export type idleTaskSchedulingSystemEvent = {
  id: string;
  topic: "idle-task-scheduling-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: idleTaskSchedulingSystemSignal;
};

export type idleTaskSchedulingSystemDecision = {
  accepted: boolean;
...
export function evaluateIdleTaskSchedulingSystemEvent(event: idleTaskSchedulingSystemEvent): idleTaskSchedulingSystemDecision {
  ...
  let action: idleTaskSchedulingSystemDecision["action"] = "commit";
...
export function runIdleTaskSchedulingSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateIdleTaskSchedulingSystemEvent({
    id: "idle-task-scheduling-system-evt-1",
    topic: "idle-task-scheduling-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateIdleTaskSchedulingSystemEvent({
    id: "idle-task-scheduling-system-evt-late",
    topic: "idle-task-scheduling-system",
    actorId: "user-42",
    sequence: 8,
