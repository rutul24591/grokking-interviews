export type webWorkerComputationSystemSignal = {
...
export type webWorkerComputationSystemEvent = {
  id: string;
  topic: "web-worker-computation-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: webWorkerComputationSystemSignal;
};

export type webWorkerComputationSystemDecision = {
  accepted: boolean;
...
export function evaluateWebWorkerComputationSystemEvent(event: webWorkerComputationSystemEvent): webWorkerComputationSystemDecision {
  ...
  let action: webWorkerComputationSystemDecision["action"] = "commit";
...
export function runWebWorkerComputationSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateWebWorkerComputationSystemEvent({
    id: "web-worker-computation-system-evt-1",
    topic: "web-worker-computation-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateWebWorkerComputationSystemEvent({
    id: "web-worker-computation-system-evt-late",
    topic: "web-worker-computation-system",
    actorId: "user-42",
    sequence: 8,
