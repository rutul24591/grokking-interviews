export type ResizeobserverLayoutSystemOutcome = "accepted" | "unsupported" | "permission-denied" | "fallback" | "cancelled";

export interface ResizeobserverLayoutSystemCapability {
  apiPresent: boolean;
...
export interface ResizeobserverLayoutSystemDecision {
  outcome: ResizeobserverLayoutSystemOutcome;
  allowed: boolean;
  fallback: "none" | "manual-control" | "read-only" | "defer-until-visible";
  reasons: string[];
}

export function decideResizeobserverLayoutSystemCapability(capability: ResizeobserverLayoutSystemCapability): ResizeobserverLayoutSystemDecision {
...
export function runResizeobserverLayoutSystemCapabilityScenario() {
  const accepted = decideResizeobserverLayoutSystemCapability({ apiPresent: true, secureContext: true, permission: "granted", documentVisible: true, userActivation: true });
  const fallback = decideResizeobserverLayoutSystemCapability({ apiPresent: true, secureContext: true, permission: "prompt", documentVisible: false, userActivation: false });
...
export function explainResizeobserverLayoutSystemOutcome(decision: ResizeobserverLayoutSystemDecision): string {
...
export function shouldRetryResizeobserverLayoutSystemLater(decision: ResizeobserverLayoutSystemDecision): boolean {
  return decision.reasons.includes("document-hidden") || decision.reasons.includes("missing-user-activation");
}

export function buildResizeobserverLayoutSystemUserMessage(decision: ResizeobserverLayoutSystemDecision): string {
...
export const ResizeobserverLayoutSystemExampleInvariant = "Visibility and layout reactions must be batched so browser observer callbacks do not create render loops.";
