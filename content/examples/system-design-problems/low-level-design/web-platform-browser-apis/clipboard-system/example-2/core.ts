export type clipboardSystemSignal = {
...
export type clipboardSystemEvent = {
  id: string;
  topic: "clipboard-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: clipboardSystemSignal;
};

export type clipboardSystemDecision = {
  accepted: boolean;
...
export function evaluateClipboardSystemEvent(event: clipboardSystemEvent): clipboardSystemDecision {
  ...
  let action: clipboardSystemDecision["action"] = "commit";
...
export function runClipboardSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateClipboardSystemEvent({
    id: "clipboard-system-evt-1",
    topic: "clipboard-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateClipboardSystemEvent({
    id: "clipboard-system-evt-late",
    topic: "clipboard-system",
    actorId: "user-42",
    sequence: 8,
