export type ClipboardSystemOutcome = "accepted" | "unsupported" | "permission-denied" | "fallback" | "cancelled";

export interface ClipboardSystemCapability {
  apiPresent: boolean;
...
export interface ClipboardSystemDecision {
  outcome: ClipboardSystemOutcome;
  allowed: boolean;
  fallback: "none" | "manual-control" | "read-only" | "defer-until-visible";
  reasons: string[];
}

export function decideClipboardSystemCapability(capability: ClipboardSystemCapability): ClipboardSystemDecision {
...
export function runClipboardSystemCapabilityScenario() {
  const accepted = decideClipboardSystemCapability({ apiPresent: true, secureContext: true, permission: "granted", documentVisible: true, userActivation: true });
  const fallback = decideClipboardSystemCapability({ apiPresent: true, secureContext: true, permission: "prompt", documentVisible: false, userActivation: false });
  return { invariant: "Clipboard access must be user-initiated, sanitized, and explainable without leaking sensitive data.", accepted, fallback };
}

export function explainClipboardSystemOutcome(decision: ClipboardSystemDecision): string {
...
export function shouldRetryClipboardSystemLater(decision: ClipboardSystemDecision): boolean {
  return decision.reasons.includes("document-hidden") || decision.reasons.includes("missing-user-activation");
}

export function buildClipboardSystemUserMessage(decision: ClipboardSystemDecision): string {
...
export const ClipboardSystemExampleInvariant = "Clipboard access must be user-initiated, sanitized, and explainable without leaking sensitive data.";
