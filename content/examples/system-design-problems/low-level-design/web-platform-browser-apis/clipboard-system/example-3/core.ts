export type clipboardSystemRuntimeState = {
...
export type clipboardSystemRecoveryPlan = {
...
export function planClipboardSystemRecovery(
  state: clipboardSystemRuntimeState,
  nowMs: number,
): clipboardSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: clipboardSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runClipboardSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planClipboardSystemRecovery(
    {
      topic: "clipboard-system",
      mounted: true,
  ...
  const failure = planClipboardSystemRecovery(
    {
      topic: "clipboard-system",
      mounted: true,
      lastSuccessfulVersion: 21,
