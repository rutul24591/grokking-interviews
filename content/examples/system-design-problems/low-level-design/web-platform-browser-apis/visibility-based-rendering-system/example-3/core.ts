export type visibilityBasedRenderingSystemRuntimeState = {
...
export type visibilityBasedRenderingSystemRecoveryPlan = {
...
export function planVisibilityBasedRenderingSystemRecovery(
  state: visibilityBasedRenderingSystemRuntimeState,
  nowMs: number,
): visibilityBasedRenderingSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: visibilityBasedRenderingSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runVisibilityBasedRenderingSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planVisibilityBasedRenderingSystemRecovery(
    {
      topic: "visibility-based-rendering-system",
      mounted: true,
  ...
  const failure = planVisibilityBasedRenderingSystemRecovery(
    {
      topic: "visibility-based-rendering-system",
      mounted: true,
      lastSuccessfulVersion: 21,
