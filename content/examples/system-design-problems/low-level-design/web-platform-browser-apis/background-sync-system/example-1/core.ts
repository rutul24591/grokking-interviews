export type BackgroundSyncSystemOutcome = "accepted" | "unsupported" | "permission-denied" | "fallback" | "cancelled";

export interface BackgroundSyncSystemCapability {
  apiPresent: boolean;
...
export interface BackgroundSyncSystemDecision {
  outcome: BackgroundSyncSystemOutcome;
  allowed: boolean;
  fallback: "none" | "manual-control" | "read-only" | "defer-until-visible";
  reasons: string[];
}

export function decideBackgroundSyncSystemCapability(capability: BackgroundSyncSystemCapability): BackgroundSyncSystemDecision {
...
export function runBackgroundSyncSystemCapabilityScenario() {
  const accepted = decideBackgroundSyncSystemCapability({ apiPresent: true, secureContext: true, permission: "granted", documentVisible: true, userActivation: true });
  const fallback = decideBackgroundSyncSystemCapability({ apiPresent: true, secureContext: true, permission: "prompt", documentVisible: false, userActivation: false });
  return { invariant: "Failed user intent must survive page close and replay without duplicate server side effects.", accepted, fallback };
}

export function explainBackgroundSyncSystemOutcome(decision: BackgroundSyncSystemDecision): string {
...
export function shouldRetryBackgroundSyncSystemLater(decision: BackgroundSyncSystemDecision): boolean {
  return decision.reasons.includes("document-hidden") || decision.reasons.includes("missing-user-activation");
}

export function buildBackgroundSyncSystemUserMessage(decision: BackgroundSyncSystemDecision): string {
...
export const BackgroundSyncSystemExampleInvariant = "Failed user intent must survive page close and replay without duplicate server side effects.";
