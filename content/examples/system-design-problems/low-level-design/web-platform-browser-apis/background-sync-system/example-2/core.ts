export type backgroundSyncSystemSignal = {
...
export type backgroundSyncSystemEvent = {
  id: string;
  topic: "background-sync-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: backgroundSyncSystemSignal;
};

export type backgroundSyncSystemDecision = {
  accepted: boolean;
...
export function evaluateBackgroundSyncSystemEvent(event: backgroundSyncSystemEvent): backgroundSyncSystemDecision {
  ...
  let action: backgroundSyncSystemDecision["action"] = "commit";
...
export function runBackgroundSyncSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateBackgroundSyncSystemEvent({
    id: "background-sync-system-evt-1",
    topic: "background-sync-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateBackgroundSyncSystemEvent({
    id: "background-sync-system-evt-late",
    topic: "background-sync-system",
    actorId: "user-42",
    sequence: 8,
