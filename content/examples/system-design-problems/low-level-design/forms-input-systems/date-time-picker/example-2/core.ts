export type dateTimePickerSignal = {
...
export type dateTimePickerEvent = {
  ...
  payloadSize: number;
  signal: dateTimePickerSignal;
};

export type dateTimePickerDecision = {
  accepted: boolean;
...
export function evaluateDateTimePickerEvent(event: dateTimePickerEvent): dateTimePickerDecision {
  ...
  let action: dateTimePickerDecision["action"] = "commit";
...
export function runDateTimePickerContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateDateTimePickerEvent({
    id: "date-time-picker-evt-1",
    topic: "date-time-picker",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateDateTimePickerEvent({
    id: "date-time-picker-evt-late",
    topic: "date-time-picker",
    actorId: "user-42",
    sequence: 8,
