export type dateTimePickerRuntimeState = {
...
export type dateTimePickerRecoveryPlan = {
...
export function planDateTimePickerRecovery(
  state: dateTimePickerRuntimeState,
  nowMs: number,
): dateTimePickerRecoveryPlan {
  const evidence: string[] = [];
  const actions: dateTimePickerRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runDateTimePickerEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planDateTimePickerRecovery(
    {
      topic: "date-time-picker",
      mounted: true,
  ...
  const failure = planDateTimePickerRecovery(
    {
      topic: "date-time-picker",
      mounted: true,
      lastSuccessfulVersion: 21,
