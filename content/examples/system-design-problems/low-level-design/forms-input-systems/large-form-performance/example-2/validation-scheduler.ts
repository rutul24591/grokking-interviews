export type largeFormPerformanceSignal = {
...
export type largeFormPerformanceEvent = {
  id: string;
  topic: "large-form-performance";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: largeFormPerformanceSignal;
};

export type largeFormPerformanceDecision = {
  accepted: boolean;
...
export function evaluateLargeFormPerformanceEvent(event: largeFormPerformanceEvent): largeFormPerformanceDecision {
  ...
  let action: largeFormPerformanceDecision["action"] = "commit";
...
export function runLargeFormPerformanceContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateLargeFormPerformanceEvent({
    id: "large-form-performance-evt-1",
    topic: "large-form-performance",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateLargeFormPerformanceEvent({
    id: "large-form-performance-evt-late",
    topic: "large-form-performance",
    actorId: "user-42",
    sequence: 8,
