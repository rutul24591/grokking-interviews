export type largeFormPerformanceRuntimeState = {
...
export type largeFormPerformanceRecoveryPlan = {
...
export function planLargeFormPerformanceRecovery(
  state: largeFormPerformanceRuntimeState,
  nowMs: number,
): largeFormPerformanceRecoveryPlan {
  const evidence: string[] = [];
  const actions: largeFormPerformanceRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runLargeFormPerformanceEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planLargeFormPerformanceRecovery(
    {
      topic: "large-form-performance",
      mounted: true,
  ...
  const failure = planLargeFormPerformanceRecovery(
    {
      topic: "large-form-performance",
      mounted: true,
      lastSuccessfulVersion: 21,
