export type fileInputSystemRuntimeState = {
...
export type fileInputSystemRecoveryPlan = {
...
export function planFileInputSystemRecovery(
  state: fileInputSystemRuntimeState,
  nowMs: number,
): fileInputSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: fileInputSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runFileInputSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planFileInputSystemRecovery(
    {
      topic: "file-input-system",
      mounted: true,
  ...
  const failure = planFileInputSystemRecovery(
    {
      topic: "file-input-system",
      mounted: true,
      lastSuccessfulVersion: 21,
