export type fileInputSystemSignal = {
...
export type fileInputSystemEvent = {
  id: string;
  topic: "file-input-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: fileInputSystemSignal;
};

export type fileInputSystemDecision = {
  accepted: boolean;
...
export function evaluateFileInputSystemEvent(event: fileInputSystemEvent): fileInputSystemDecision {
  ...
  let action: fileInputSystemDecision["action"] = "commit";
...
export function runFileInputSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateFileInputSystemEvent({
    id: "file-input-system-evt-1",
    topic: "file-input-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateFileInputSystemEvent({
    id: "file-input-system-evt-late",
    topic: "file-input-system",
    actorId: "user-42",
    sequence: 8,
