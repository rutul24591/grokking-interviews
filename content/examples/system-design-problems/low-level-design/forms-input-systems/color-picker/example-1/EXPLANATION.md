# Design a Color Picker - example-1 Explanation

## Article context
This example supports the article `low-level-design/forms-input-systems/color-picker`. The article is about LLD for an accessible Color Picker with HSV/HSL/RGB/HEX, alpha, eyedropper, saved palettes, and contrast preview against a reference background.. The most relevant article sections for this example are: 🎯 Problem Context and Scope Definition; Problem Statement; User Context; Assumptions; Non-Goals; Functional Requirements; Core (Must-have); Secondary (Nice-to-have); Out of Scope; Non-Functional Requirements.
...
- `color-math.ts`: Implements the main logic, including clamp01, srgbToLinear, x, relativeLuminance, R.
- `palette-store.ts`: Models client or service state transitions and update behavior.
