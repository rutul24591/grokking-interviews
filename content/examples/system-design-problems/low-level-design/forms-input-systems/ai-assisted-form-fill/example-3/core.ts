export type aiAssistedFormFillRuntimeState = {
...
export type aiAssistedFormFillRecoveryPlan = {
...
export function planAiAssistedFormFillRecovery(
  state: aiAssistedFormFillRuntimeState,
  nowMs: number,
): aiAssistedFormFillRecoveryPlan {
  const evidence: string[] = [];
  const actions: aiAssistedFormFillRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runAiAssistedFormFillEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planAiAssistedFormFillRecovery(
    {
      topic: "ai-assisted-form-fill",
      mounted: true,
  ...
  const failure = planAiAssistedFormFillRecovery(
    {
      topic: "ai-assisted-form-fill",
      mounted: true,
      lastSuccessfulVersion: 21,
