export type aiAssistedFormFillSignal = {
...
export type aiAssistedFormFillEvent = {
  id: string;
  topic: "ai-assisted-form-fill";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: aiAssistedFormFillSignal;
};

export type aiAssistedFormFillDecision = {
  accepted: boolean;
...
export function evaluateAiAssistedFormFillEvent(event: aiAssistedFormFillEvent): aiAssistedFormFillDecision {
  ...
  let action: aiAssistedFormFillDecision["action"] = "commit";
...
export function runAiAssistedFormFillContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateAiAssistedFormFillEvent({
    id: "ai-assisted-form-fill-evt-1",
    topic: "ai-assisted-form-fill",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateAiAssistedFormFillEvent({
    id: "ai-assisted-form-fill-evt-late",
    topic: "ai-assisted-form-fill",
    actorId: "user-42",
    sequence: 8,
