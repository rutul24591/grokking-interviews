export type formBuilderRuntimeState = {
  topic: "form-builder";
  mounted: boolean;
...
export type formBuilderRecoveryPlan = {
...
export function planFormBuilderRecovery(
  state: formBuilderRuntimeState,
  nowMs: number,
): formBuilderRecoveryPlan {
  const evidence: string[] = [];
  const actions: formBuilderRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runFormBuilderEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planFormBuilderRecovery(
    {
      topic: "form-builder",
      mounted: true,
  ...
  const failure = planFormBuilderRecovery(
    {
      topic: "form-builder",
      mounted: true,
      lastSuccessfulVersion: 21,
