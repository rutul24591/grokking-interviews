export type formValidationEngineRuntimeState = {
...
export type formValidationEngineRecoveryPlan = {
...
export function planFormValidationEngineRecovery(
  state: formValidationEngineRuntimeState,
  nowMs: number,
): formValidationEngineRecoveryPlan {
  const evidence: string[] = [];
  const actions: formValidationEngineRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runFormValidationEngineEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planFormValidationEngineRecovery(
    {
      topic: "form-validation-engine",
      mounted: true,
  ...
  const failure = planFormValidationEngineRecovery(
    {
      topic: "form-validation-engine",
      mounted: true,
      lastSuccessfulVersion: 21,
