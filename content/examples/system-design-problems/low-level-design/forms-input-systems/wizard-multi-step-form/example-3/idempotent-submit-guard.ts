export type wizardMultiStepFormRuntimeState = {
...
export type wizardMultiStepFormRecoveryPlan = {
...
export function planWizardMultiStepFormRecovery(
  state: wizardMultiStepFormRuntimeState,
  nowMs: number,
): wizardMultiStepFormRecoveryPlan {
  const evidence: string[] = [];
  const actions: wizardMultiStepFormRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runWizardMultiStepFormEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planWizardMultiStepFormRecovery(
    {
      topic: "wizard-multi-step-form",
      mounted: true,
  ...
  const failure = planWizardMultiStepFormRecovery(
    {
      topic: "wizard-multi-step-form",
      mounted: true,
      lastSuccessfulVersion: 21,
