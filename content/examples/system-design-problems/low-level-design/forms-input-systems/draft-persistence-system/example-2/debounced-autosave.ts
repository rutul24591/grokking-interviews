export type draftPersistenceSystemSignal = {
...
export type draftPersistenceSystemEvent = {
  id: string;
  topic: "draft-persistence-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: draftPersistenceSystemSignal;
};

export type draftPersistenceSystemDecision = {
  accepted: boolean;
...
export function evaluateDraftPersistenceSystemEvent(event: draftPersistenceSystemEvent): draftPersistenceSystemDecision {
  ...
  let action: draftPersistenceSystemDecision["action"] = "commit";
...
export function runDraftPersistenceSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateDraftPersistenceSystemEvent({
    id: "draft-persistence-system-evt-1",
    topic: "draft-persistence-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateDraftPersistenceSystemEvent({
    id: "draft-persistence-system-evt-late",
    topic: "draft-persistence-system",
    actorId: "user-42",
    sequence: 8,
