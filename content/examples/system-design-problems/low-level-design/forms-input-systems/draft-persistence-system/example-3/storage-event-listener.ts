export type draftPersistenceSystemRuntimeState = {
...
export type draftPersistenceSystemRecoveryPlan = {
...
export function planDraftPersistenceSystemRecovery(
  state: draftPersistenceSystemRuntimeState,
  nowMs: number,
): draftPersistenceSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: draftPersistenceSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runDraftPersistenceSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planDraftPersistenceSystemRecovery(
    {
      topic: "draft-persistence-system",
      mounted: true,
  ...
  const failure = planDraftPersistenceSystemRecovery(
    {
      topic: "draft-persistence-system",
      mounted: true,
      lastSuccessfulVersion: 21,
