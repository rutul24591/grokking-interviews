export type dynamicConditionalFormEngineRuntimeState = {
...
export type dynamicConditionalFormEngineRecoveryPlan = {
...
export function planDynamicConditionalFormEngineRecovery(
  state: dynamicConditionalFormEngineRuntimeState,
  nowMs: number,
): dynamicConditionalFormEngineRecoveryPlan {
  const evidence: string[] = [];
  const actions: dynamicConditionalFormEngineRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runDynamicConditionalFormEngineEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planDynamicConditionalFormEngineRecovery(
    {
      topic: "dynamic-conditional-form-engine",
      mounted: true,
  ...
  const failure = planDynamicConditionalFormEngineRecovery(
    {
      topic: "dynamic-conditional-form-engine",
      mounted: true,
      lastSuccessfulVersion: 21,
