export type dynamicConditionalFormEngineSignal = {
...
export type dynamicConditionalFormEngineEvent = {
  id: string;
  topic: "dynamic-conditional-form-engine";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: dynamicConditionalFormEngineSignal;
};

export type dynamicConditionalFormEngineDecision = {
  accepted: boolean;
...
export function evaluateDynamicConditionalFormEngineEvent(event: dynamicConditionalFormEngineEvent): dynamicConditionalFormEngineDecision {
  ...
  let action: dynamicConditionalFormEngineDecision["action"] = "commit";
...
export function runDynamicConditionalFormEngineContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateDynamicConditionalFormEngineEvent({
    id: "dynamic-conditional-form-engine-evt-1",
    topic: "dynamic-conditional-form-engine",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateDynamicConditionalFormEngineEvent({
    id: "dynamic-conditional-form-engine-evt-late",
    topic: "dynamic-conditional-form-engine",
    actorId: "user-42",
    sequence: 8,
