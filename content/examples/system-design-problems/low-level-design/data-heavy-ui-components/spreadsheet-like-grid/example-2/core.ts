export type spreadsheetLikeGridSignal = {
  rowCount: number;
  cursorDrift: number;
  cacheAgeMs: number;
  renderCostMs: number;
...
export type spreadsheetLikeGridEvent = {
  id: string;
  topic: "spreadsheet-like-grid";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: spreadsheetLikeGridSignal;
};

export type spreadsheetLikeGridDecision = {
  accepted: boolean;
...
export function evaluateSpreadsheetLikeGridEvent(event: spreadsheetLikeGridEvent): spreadsheetLikeGridDecision {
  ...
  let action: spreadsheetLikeGridDecision["action"] = "commit";
...
export function runSpreadsheetLikeGridContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateSpreadsheetLikeGridEvent({
    id: "spreadsheet-like-grid-evt-1",
    topic: "spreadsheet-like-grid",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateSpreadsheetLikeGridEvent({
    id: "spreadsheet-like-grid-evt-late",
    topic: "spreadsheet-like-grid",
    actorId: "user-42",
    sequence: 8,
