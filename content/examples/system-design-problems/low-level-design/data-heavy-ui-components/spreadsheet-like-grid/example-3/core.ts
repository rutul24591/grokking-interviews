export type spreadsheetLikeGridRuntimeState = {
...
export type spreadsheetLikeGridRecoveryPlan = {
...
export function planSpreadsheetLikeGridRecovery(
  state: spreadsheetLikeGridRuntimeState,
  nowMs: number,
): spreadsheetLikeGridRecoveryPlan {
  const evidence: string[] = [];
  const actions: spreadsheetLikeGridRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runSpreadsheetLikeGridEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planSpreadsheetLikeGridRecovery(
    {
      topic: "spreadsheet-like-grid",
      mounted: true,
  ...
  const failure = planSpreadsheetLikeGridRecovery(
    {
      topic: "spreadsheet-like-grid",
      mounted: true,
      lastSuccessfulVersion: 21,
