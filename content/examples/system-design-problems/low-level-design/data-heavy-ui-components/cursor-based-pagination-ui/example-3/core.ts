export type cursorBasedPaginationUiRuntimeState = {
...
export type cursorBasedPaginationUiRecoveryPlan = {
...
export function planCursorBasedPaginationUiRecovery(
  state: cursorBasedPaginationUiRuntimeState,
  nowMs: number,
): cursorBasedPaginationUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: cursorBasedPaginationUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runCursorBasedPaginationUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planCursorBasedPaginationUiRecovery(
    {
      topic: "cursor-based-pagination-ui",
      mounted: true,
  ...
  const failure = planCursorBasedPaginationUiRecovery(
    {
      topic: "cursor-based-pagination-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
