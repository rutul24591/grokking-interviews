export type clientSideDataNormalizationSignal = {
  rowCount: number;
  cursorDrift: number;
  cacheAgeMs: number;
  renderCostMs: number;
...
export type clientSideDataNormalizationEvent = {
  id: string;
  topic: "client-side-data-normalization";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: clientSideDataNormalizationSignal;
};

export type clientSideDataNormalizationDecision = {
  accepted: boolean;
...
export function evaluateClientSideDataNormalizationEvent(event: clientSideDataNormalizationEvent): clientSideDataNormalizationDecision {
  ...
  let action: clientSideDataNormalizationDecision["action"] = "commit";
...
export function runClientSideDataNormalizationContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateClientSideDataNormalizationEvent({
    id: "client-side-data-normalization-evt-1",
    topic: "client-side-data-normalization",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateClientSideDataNormalizationEvent({
    id: "client-side-data-normalization-evt-late",
    topic: "client-side-data-normalization",
    actorId: "user-42",
    sequence: 8,
