export type clientSideDataNormalizationRuntimeState = {
...
export type clientSideDataNormalizationRecoveryPlan = {
...
export function planClientSideDataNormalizationRecovery(
  state: clientSideDataNormalizationRuntimeState,
  nowMs: number,
): clientSideDataNormalizationRecoveryPlan {
  const evidence: string[] = [];
  const actions: clientSideDataNormalizationRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runClientSideDataNormalizationEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planClientSideDataNormalizationRecovery(
    {
      topic: "client-side-data-normalization",
      mounted: true,
  ...
  const failure = planClientSideDataNormalizationRecovery(
    {
      topic: "client-side-data-normalization",
      mounted: true,
      lastSuccessfulVersion: 21,
