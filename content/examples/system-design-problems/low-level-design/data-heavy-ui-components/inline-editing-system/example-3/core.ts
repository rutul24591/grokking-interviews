export type inlineEditingSystemRuntimeState = {
...
export type inlineEditingSystemRecoveryPlan = {
...
export function planInlineEditingSystemRecovery(
  state: inlineEditingSystemRuntimeState,
  nowMs: number,
): inlineEditingSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: inlineEditingSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runInlineEditingSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planInlineEditingSystemRecovery(
    {
      topic: "inline-editing-system",
      mounted: true,
  ...
  const failure = planInlineEditingSystemRecovery(
    {
      topic: "inline-editing-system",
      mounted: true,
      lastSuccessfulVersion: 21,
