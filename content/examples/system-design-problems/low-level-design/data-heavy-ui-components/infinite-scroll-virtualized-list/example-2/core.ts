export type infiniteScrollVirtualizedListSignal = {
  rowCount: number;
  cursorDrift: number;
  cacheAgeMs: number;
  renderCostMs: number;
...
export type infiniteScrollVirtualizedListEvent = {
  id: string;
  topic: "infinite-scroll-virtualized-list";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: infiniteScrollVirtualizedListSignal;
};

export type infiniteScrollVirtualizedListDecision = {
  accepted: boolean;
...
export function evaluateInfiniteScrollVirtualizedListEvent(event: infiniteScrollVirtualizedListEvent): infiniteScrollVirtualizedListDecision {
  ...
  let action: infiniteScrollVirtualizedListDecision["action"] = "commit";
...
export function runInfiniteScrollVirtualizedListContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateInfiniteScrollVirtualizedListEvent({
    id: "infinite-scroll-virtualized-list-evt-1",
    topic: "infinite-scroll-virtualized-list",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateInfiniteScrollVirtualizedListEvent({
    id: "infinite-scroll-virtualized-list-evt-late",
    topic: "infinite-scroll-virtualized-list",
    actorId: "user-42",
    sequence: 8,
