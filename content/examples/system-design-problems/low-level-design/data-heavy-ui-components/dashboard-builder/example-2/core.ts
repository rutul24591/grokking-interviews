export type dashboardBuilderSignal = {
  rowCount: number;
  cursorDrift: number;
  cacheAgeMs: number;
  renderCostMs: number;
...
export type dashboardBuilderEvent = {
  id: string;
  topic: "dashboard-builder";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: dashboardBuilderSignal;
};

export type dashboardBuilderDecision = {
  accepted: boolean;
...
export function evaluateDashboardBuilderEvent(event: dashboardBuilderEvent): dashboardBuilderDecision {
  ...
  let action: dashboardBuilderDecision["action"] = "commit";
...
export function runDashboardBuilderContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateDashboardBuilderEvent({
    id: "dashboard-builder-evt-1",
    topic: "dashboard-builder",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateDashboardBuilderEvent({
    id: "dashboard-builder-evt-late",
    topic: "dashboard-builder",
    actorId: "user-42",
    sequence: 8,
