export type dashboardBuilderRuntimeState = {
...
export type dashboardBuilderRecoveryPlan = {
...
export function planDashboardBuilderRecovery(
  state: dashboardBuilderRuntimeState,
  nowMs: number,
): dashboardBuilderRecoveryPlan {
  const evidence: string[] = [];
  const actions: dashboardBuilderRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runDashboardBuilderEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planDashboardBuilderRecovery(
    {
      topic: "dashboard-builder",
      mounted: true,
  ...
  const failure = planDashboardBuilderRecovery(
    {
      topic: "dashboard-builder",
      mounted: true,
      lastSuccessfulVersion: 21,
