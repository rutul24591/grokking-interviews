export type savedViewsFiltersSystemSignal = {
  rowCount: number;
  cursorDrift: number;
  cacheAgeMs: number;
  renderCostMs: number;
...
export type savedViewsFiltersSystemEvent = {
  id: string;
  ...
  payloadSize: number;
  signal: savedViewsFiltersSystemSignal;
};

export type savedViewsFiltersSystemDecision = {
  accepted: boolean;
...
export function evaluateSavedViewsFiltersSystemEvent(event: savedViewsFiltersSystemEvent): savedViewsFiltersSystemDecision {
  ...
  let action: savedViewsFiltersSystemDecision["action"] = "commit";
...
export function runSavedViewsFiltersSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateSavedViewsFiltersSystemEvent({
    id: "saved-views-filters-system-evt-1",
    topic: "saved-views-filters-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateSavedViewsFiltersSystemEvent({
    id: "saved-views-filters-system-evt-late",
    topic: "saved-views-filters-system",
    actorId: "user-42",
    sequence: 8,
