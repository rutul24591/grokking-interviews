export type realTimeDataDashboardRuntimeState = {
...
export type realTimeDataDashboardRecoveryPlan = {
...
export function planRealTimeDataDashboardRecovery(
  state: realTimeDataDashboardRuntimeState,
  nowMs: number,
): realTimeDataDashboardRecoveryPlan {
  const evidence: string[] = [];
  const actions: realTimeDataDashboardRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runRealTimeDataDashboardEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planRealTimeDataDashboardRecovery(
    {
      topic: "real-time-data-dashboard",
      mounted: true,
  ...
  const failure = planRealTimeDataDashboardRecovery(
    {
      topic: "real-time-data-dashboard",
      mounted: true,
      lastSuccessfulVersion: 21,
