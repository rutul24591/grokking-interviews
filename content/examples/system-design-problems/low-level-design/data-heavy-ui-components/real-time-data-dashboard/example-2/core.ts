export type realTimeDataDashboardSignal = {
  rowCount: number;
  cursorDrift: number;
  cacheAgeMs: number;
  renderCostMs: number;
...
export type realTimeDataDashboardEvent = {
  id: string;
  ...
  payloadSize: number;
  signal: realTimeDataDashboardSignal;
};

export type realTimeDataDashboardDecision = {
  accepted: boolean;
...
export function evaluateRealTimeDataDashboardEvent(event: realTimeDataDashboardEvent): realTimeDataDashboardDecision {
  ...
  let action: realTimeDataDashboardDecision["action"] = "commit";
...
export function runRealTimeDataDashboardContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateRealTimeDataDashboardEvent({
    id: "real-time-data-dashboard-evt-1",
    topic: "real-time-data-dashboard",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateRealTimeDataDashboardEvent({
    id: "real-time-data-dashboard-evt-late",
    topic: "real-time-data-dashboard",
    actorId: "user-42",
    sequence: 8,
