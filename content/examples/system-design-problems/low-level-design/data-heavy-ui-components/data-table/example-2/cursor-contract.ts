...
export type dataTableEvent = {
  ...
  payloadSize: number;
  signal: dataTableSignal;
};

export type dataTableDecision = {
  accepted: boolean;
...
export function evaluateDataTableEvent(event: dataTableEvent): dataTableDecision {
  ...
  let action: dataTableDecision["action"] = "commit";
...
export function runDataTableContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateDataTableEvent({
    id: "data-table-evt-1",
    topic: "data-table",
    actorId: "user-42",
    sequence: 7,
  ...
  const guarded = evaluateDataTableEvent({
    id: "data-table-evt-late",
    topic: "data-table",
    actorId: "user-42",
    sequence: 8,
