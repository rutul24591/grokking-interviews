export type dataTableRuntimeState = {
  topic: "data-table";
  mounted: boolean;
...
export type dataTableRecoveryPlan = {
...
export function planDataTableRecovery(
  state: dataTableRuntimeState,
  nowMs: number,
): dataTableRecoveryPlan {
  const evidence: string[] = [];
  const actions: dataTableRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runDataTableEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planDataTableRecovery(
    {
      topic: "data-table",
      mounted: true,
  ...
  const failure = planDataTableRecovery(
    {
      topic: "data-table",
      mounted: true,
      lastSuccessfulVersion: 21,
