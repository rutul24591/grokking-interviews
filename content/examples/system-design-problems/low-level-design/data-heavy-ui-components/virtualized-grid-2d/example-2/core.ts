export type virtualizedGrid2dSignal = {
  rowCount: number;
  cursorDrift: number;
  cacheAgeMs: number;
  renderCostMs: number;
...
export type virtualizedGrid2dEvent = {
  id: string;
  topic: "virtualized-grid-2d";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: virtualizedGrid2dSignal;
};

export type virtualizedGrid2dDecision = {
  accepted: boolean;
...
export function evaluateVirtualizedGrid2dEvent(event: virtualizedGrid2dEvent): virtualizedGrid2dDecision {
  ...
  let action: virtualizedGrid2dDecision["action"] = "commit";
...
export function runVirtualizedGrid2dContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateVirtualizedGrid2dEvent({
    id: "virtualized-grid-2d-evt-1",
    topic: "virtualized-grid-2d",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateVirtualizedGrid2dEvent({
    id: "virtualized-grid-2d-evt-late",
    topic: "virtualized-grid-2d",
    actorId: "user-42",
    sequence: 8,
