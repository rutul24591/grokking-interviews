export type virtualizedGrid2dRuntimeState = {
...
export type virtualizedGrid2dRecoveryPlan = {
...
export function planVirtualizedGrid2dRecovery(
  state: virtualizedGrid2dRuntimeState,
  nowMs: number,
): virtualizedGrid2dRecoveryPlan {
  const evidence: string[] = [];
  const actions: virtualizedGrid2dRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runVirtualizedGrid2dEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planVirtualizedGrid2dRecovery(
    {
      topic: "virtualized-grid-2d",
      mounted: true,
  ...
  const failure = planVirtualizedGrid2dRecovery(
    {
      topic: "virtualized-grid-2d",
      mounted: true,
      lastSuccessfulVersion: 21,
