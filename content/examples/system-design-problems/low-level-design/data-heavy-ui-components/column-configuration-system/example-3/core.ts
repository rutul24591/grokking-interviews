export type columnConfigurationSystemRuntimeState = {
...
export type columnConfigurationSystemRecoveryPlan = {
...
export function planColumnConfigurationSystemRecovery(
  state: columnConfigurationSystemRuntimeState,
  nowMs: number,
): columnConfigurationSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: columnConfigurationSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runColumnConfigurationSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planColumnConfigurationSystemRecovery(
    {
      topic: "column-configuration-system",
      mounted: true,
  ...
  const failure = planColumnConfigurationSystemRecovery(
    {
      topic: "column-configuration-system",
      mounted: true,
      lastSuccessfulVersion: 21,
