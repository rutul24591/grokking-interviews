export type columnConfigurationSystemSignal = {
  rowCount: number;
  cursorDrift: number;
  cacheAgeMs: number;
  renderCostMs: number;
...
export type columnConfigurationSystemEvent = {
  id: string;
  topic: "column-configuration-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: columnConfigurationSystemSignal;
};

export type columnConfigurationSystemDecision = {
  accepted: boolean;
...
export function evaluateColumnConfigurationSystemEvent(event: columnConfigurationSystemEvent): columnConfigurationSystemDecision {
  ...
  let action: columnConfigurationSystemDecision["action"] = "commit";
...
export function runColumnConfigurationSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateColumnConfigurationSystemEvent({
    id: "column-configuration-system-evt-1",
    topic: "column-configuration-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateColumnConfigurationSystemEvent({
    id: "column-configuration-system-evt-late",
    topic: "column-configuration-system",
    actorId: "user-42",
    sequence: 8,
