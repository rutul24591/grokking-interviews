export type timeTravelDebuggingSystemRuntimeState = {
...
export type timeTravelDebuggingSystemRecoveryPlan = {
...
export function planTimeTravelDebuggingSystemRecovery(
  state: timeTravelDebuggingSystemRuntimeState,
  nowMs: number,
): timeTravelDebuggingSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: timeTravelDebuggingSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runTimeTravelDebuggingSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planTimeTravelDebuggingSystemRecovery(
    {
      topic: "time-travel-debugging-system",
      mounted: true,
  ...
  const failure = planTimeTravelDebuggingSystemRecovery(
    {
      topic: "time-travel-debugging-system",
      mounted: true,
      lastSuccessfulVersion: 21,
