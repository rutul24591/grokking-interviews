export type stateMachineForWorkflowsSignal = {
  offlineAgeMs: number;
...
export type stateMachineForWorkflowsEvent = {
  id: string;
  topic: "state-machine-for-workflows";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: stateMachineForWorkflowsSignal;
};

export type stateMachineForWorkflowsDecision = {
  accepted: boolean;
...
export function evaluateStateMachineForWorkflowsEvent(event: stateMachineForWorkflowsEvent): stateMachineForWorkflowsDecision {
  ...
  let action: stateMachineForWorkflowsDecision["action"] = "commit";
...
export function runStateMachineForWorkflowsContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateStateMachineForWorkflowsEvent({
    id: "state-machine-for-workflows-evt-1",
    topic: "state-machine-for-workflows",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateStateMachineForWorkflowsEvent({
    id: "state-machine-for-workflows-evt-late",
    topic: "state-machine-for-workflows",
    actorId: "user-42",
    sequence: 8,
