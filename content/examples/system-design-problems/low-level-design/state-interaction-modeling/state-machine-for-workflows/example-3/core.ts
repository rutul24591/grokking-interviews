export type stateMachineForWorkflowsRuntimeState = {
...
export type stateMachineForWorkflowsRecoveryPlan = {
...
export function planStateMachineForWorkflowsRecovery(
  state: stateMachineForWorkflowsRuntimeState,
  nowMs: number,
): stateMachineForWorkflowsRecoveryPlan {
  const evidence: string[] = [];
  const actions: stateMachineForWorkflowsRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runStateMachineForWorkflowsEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planStateMachineForWorkflowsRecovery(
    {
      topic: "state-machine-for-workflows",
      mounted: true,
  ...
  const failure = planStateMachineForWorkflowsRecovery(
    {
      topic: "state-machine-for-workflows",
      mounted: true,
      lastSuccessfulVersion: 21,
