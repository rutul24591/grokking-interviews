export type globalEventBusSystemRuntimeState = {
...
export type globalEventBusSystemRecoveryPlan = {
...
export function planGlobalEventBusSystemRecovery(
  state: globalEventBusSystemRuntimeState,
  nowMs: number,
): globalEventBusSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: globalEventBusSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runGlobalEventBusSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planGlobalEventBusSystemRecovery(
    {
      topic: "global-event-bus-system",
      mounted: true,
  ...
  const failure = planGlobalEventBusSystemRecovery(
    {
      topic: "global-event-bus-system",
      mounted: true,
      lastSuccessfulVersion: 21,
