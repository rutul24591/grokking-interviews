export type globalEventBusSystemSignal = {
  offlineAgeMs: number;
...
export type globalEventBusSystemEvent = {
  id: string;
  topic: "global-event-bus-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: globalEventBusSystemSignal;
};

export type globalEventBusSystemDecision = {
  accepted: boolean;
...
export function evaluateGlobalEventBusSystemEvent(event: globalEventBusSystemEvent): globalEventBusSystemDecision {
  ...
  let action: globalEventBusSystemDecision["action"] = "commit";
...
export function runGlobalEventBusSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateGlobalEventBusSystemEvent({
    id: "global-event-bus-system-evt-1",
    topic: "global-event-bus-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateGlobalEventBusSystemEvent({
    id: "global-event-bus-system-evt-late",
    topic: "global-event-bus-system",
    actorId: "user-42",
    sequence: 8,
