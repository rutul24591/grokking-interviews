export type multiStepAsyncWorkflowRuntimeState = {
...
export type multiStepAsyncWorkflowRecoveryPlan = {
...
export function planMultiStepAsyncWorkflowRecovery(
  state: multiStepAsyncWorkflowRuntimeState,
  nowMs: number,
): multiStepAsyncWorkflowRecoveryPlan {
  const evidence: string[] = [];
  const actions: multiStepAsyncWorkflowRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runMultiStepAsyncWorkflowEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planMultiStepAsyncWorkflowRecovery(
    {
      topic: "multi-step-async-workflow",
      mounted: true,
  ...
  const failure = planMultiStepAsyncWorkflowRecovery(
    {
      topic: "multi-step-async-workflow",
      mounted: true,
      lastSuccessfulVersion: 21,
