export type undoRedoSystemRuntimeState = {
...
export type undoRedoSystemRecoveryPlan = {
...
export function planUndoRedoSystemRecovery(
  state: undoRedoSystemRuntimeState,
  nowMs: number,
): undoRedoSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: undoRedoSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runUndoRedoSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planUndoRedoSystemRecovery(
    {
      topic: "undo-redo-system",
      mounted: true,
  ...
  const failure = planUndoRedoSystemRecovery(
    {
      topic: "undo-redo-system",
      mounted: true,
      lastSuccessfulVersion: 21,
