export type undoRedoSystemSignal = {
  offlineAgeMs: number;
...
export type undoRedoSystemEvent = {
  ...
  payloadSize: number;
  signal: undoRedoSystemSignal;
};

export type undoRedoSystemDecision = {
  accepted: boolean;
...
export function evaluateUndoRedoSystemEvent(event: undoRedoSystemEvent): undoRedoSystemDecision {
  ...
  let action: undoRedoSystemDecision["action"] = "commit";
...
export function runUndoRedoSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateUndoRedoSystemEvent({
    id: "undo-redo-system-evt-1",
    topic: "undo-redo-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateUndoRedoSystemEvent({
    id: "undo-redo-system-evt-late",
    topic: "undo-redo-system",
    actorId: "user-42",
    sequence: 8,
