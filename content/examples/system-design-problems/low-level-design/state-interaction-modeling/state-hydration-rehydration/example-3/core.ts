...
export function recordStateHydrationRehydrationFailure(diagnostics: RuntimeDiagnostics, operationId: string, reason: string): void {
  diagnostics.record({
    topic: "state-hydration-rehydration",
    operationId,
    phase: "rejected",
    reason,
    at: Date.now(),
