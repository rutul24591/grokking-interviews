export type stateHydrationRehydrationSystemRuntimeState = {
  topic: "state-hydration-rehydration-system";
  mounted: boolean;
...
export type stateHydrationRehydrationSystemRecoveryPlan = {
...
export function planStateHydrationRehydrationSystemRecovery(
  state: stateHydrationRehydrationSystemRuntimeState,
  nowMs: number,
): stateHydrationRehydrationSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: stateHydrationRehydrationSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runStateHydrationRehydrationSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planStateHydrationRehydrationSystemRecovery(
    {
      topic: "state-hydration-rehydration-system",
      mounted: true,
  ...
  const failure = planStateHydrationRehydrationSystemRecovery(
    {
      topic: "state-hydration-rehydration-system",
      mounted: true,
      lastSuccessfulVersion: 21,
