export type crossTabStateSyncSystemRuntimeState = {
...
export type crossTabStateSyncSystemRecoveryPlan = {
...
export function planCrossTabStateSyncSystemRecovery(
  state: crossTabStateSyncSystemRuntimeState,
  nowMs: number,
): crossTabStateSyncSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: crossTabStateSyncSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runCrossTabStateSyncSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planCrossTabStateSyncSystemRecovery(
    {
      topic: "cross-tab-state-sync-system",
      mounted: true,
  ...
  const failure = planCrossTabStateSyncSystemRecovery(
    {
      topic: "cross-tab-state-sync-system",
      mounted: true,
      lastSuccessfulVersion: 21,
