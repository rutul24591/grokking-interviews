export type derivedStateSystemSignal = {
  offlineAgeMs: number;
...
export type derivedStateSystemEvent = {
  id: string;
  topic: "derived-state-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: derivedStateSystemSignal;
};

export type derivedStateSystemDecision = {
  accepted: boolean;
...
export function evaluateDerivedStateSystemEvent(event: derivedStateSystemEvent): derivedStateSystemDecision {
  ...
  let action: derivedStateSystemDecision["action"] = "commit";
...
export function runDerivedStateSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateDerivedStateSystemEvent({
    id: "derived-state-system-evt-1",
    topic: "derived-state-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateDerivedStateSystemEvent({
    id: "derived-state-system-evt-late",
    topic: "derived-state-system",
    actorId: "user-42",
    sequence: 8,
