export type derivedStateSystemRuntimeState = {
...
export type derivedStateSystemRecoveryPlan = {
...
export function planDerivedStateSystemRecovery(
  state: derivedStateSystemRuntimeState,
  nowMs: number,
): derivedStateSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: derivedStateSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runDerivedStateSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planDerivedStateSystemRecovery(
    {
      topic: "derived-state-system",
      mounted: true,
  ...
  const failure = planDerivedStateSystemRecovery(
    {
      topic: "derived-state-system",
      mounted: true,
      lastSuccessfulVersion: 21,
