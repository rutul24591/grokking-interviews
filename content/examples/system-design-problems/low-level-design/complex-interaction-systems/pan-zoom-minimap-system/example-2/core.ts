export type panZoomMinimapSystemSignal = {
  frameCostMs: number;
  focusDrift: number;
...
export type panZoomMinimapSystemEvent = {
  id: string;
  topic: "pan-zoom-minimap-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: panZoomMinimapSystemSignal;
};

export type panZoomMinimapSystemDecision = {
  accepted: boolean;
...
export function evaluatePanZoomMinimapSystemEvent(event: panZoomMinimapSystemEvent): panZoomMinimapSystemDecision {
  ...
  let action: panZoomMinimapSystemDecision["action"] = "commit";
...
export function runPanZoomMinimapSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluatePanZoomMinimapSystemEvent({
    id: "pan-zoom-minimap-system-evt-1",
    topic: "pan-zoom-minimap-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluatePanZoomMinimapSystemEvent({
    id: "pan-zoom-minimap-system-evt-late",
    topic: "pan-zoom-minimap-system",
    actorId: "user-42",
    sequence: 8,
