export type panZoomMinimapSystemRuntimeState = {
...
export type panZoomMinimapSystemRecoveryPlan = {
...
export function planPanZoomMinimapSystemRecovery(
  state: panZoomMinimapSystemRuntimeState,
  nowMs: number,
): panZoomMinimapSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: panZoomMinimapSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runPanZoomMinimapSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planPanZoomMinimapSystemRecovery(
    {
      topic: "pan-zoom-minimap-system",
      mounted: true,
  ...
  const failure = planPanZoomMinimapSystemRecovery(
    {
      topic: "pan-zoom-minimap-system",
      mounted: true,
      lastSuccessfulVersion: 21,
