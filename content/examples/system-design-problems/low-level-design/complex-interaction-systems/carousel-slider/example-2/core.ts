export type carouselSliderSignal = {
  frameCostMs: number;
  focusDrift: number;
...
export type carouselSliderEvent = {
  id: string;
  ...
  payloadSize: number;
  signal: carouselSliderSignal;
};

export type carouselSliderDecision = {
  accepted: boolean;
...
export function evaluateCarouselSliderEvent(event: carouselSliderEvent): carouselSliderDecision {
  ...
  let action: carouselSliderDecision["action"] = "commit";
...
export function runCarouselSliderContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateCarouselSliderEvent({
    id: "carousel-slider-evt-1",
    topic: "carousel-slider",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateCarouselSliderEvent({
    id: "carousel-slider-evt-late",
    topic: "carousel-slider",
    actorId: "user-42",
    sequence: 8,
