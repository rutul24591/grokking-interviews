export type gestureSystemRuntimeState = {
...
export type gestureSystemRecoveryPlan = {
...
export function planGestureSystemRecovery(
  state: gestureSystemRuntimeState,
  nowMs: number,
): gestureSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: gestureSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runGestureSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planGestureSystemRecovery(
    {
      topic: "gesture-system",
      mounted: true,
  ...
  const failure = planGestureSystemRecovery(
    {
      topic: "gesture-system",
      mounted: true,
      lastSuccessfulVersion: 21,
