export type calendarSchedulerSignal = {
  frameCostMs: number;
  focusDrift: number;
...
export type calendarSchedulerEvent = {
  ...
  payloadSize: number;
  signal: calendarSchedulerSignal;
};

export type calendarSchedulerDecision = {
  accepted: boolean;
...
export function evaluateCalendarSchedulerEvent(event: calendarSchedulerEvent): calendarSchedulerDecision {
  ...
  let action: calendarSchedulerDecision["action"] = "commit";
...
export function runCalendarSchedulerContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateCalendarSchedulerEvent({
    id: "calendar-scheduler-evt-1",
    topic: "calendar-scheduler",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateCalendarSchedulerEvent({
    id: "calendar-scheduler-evt-late",
    topic: "calendar-scheduler",
    actorId: "user-42",
    sequence: 8,
