export type dragDropListSignal = {
  frameCostMs: number;
  focusDrift: number;
...
export type dragDropListEvent = {
  ...
  payloadSize: number;
  signal: dragDropListSignal;
};

export type dragDropListDecision = {
  accepted: boolean;
...
export function evaluateDragDropListEvent(event: dragDropListEvent): dragDropListDecision {
  ...
  let action: dragDropListDecision["action"] = "commit";
...
export function runDragDropListContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateDragDropListEvent({
    id: "drag-drop-list-evt-1",
    topic: "drag-drop-list",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateDragDropListEvent({
    id: "drag-drop-list-evt-late",
    topic: "drag-drop-list",
    actorId: "user-42",
    sequence: 8,
