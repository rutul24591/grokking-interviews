export type historyTimelineUiRuntimeState = {
...
export type historyTimelineUiRecoveryPlan = {
...
export function planHistoryTimelineUiRecovery(
  state: historyTimelineUiRuntimeState,
  nowMs: number,
): historyTimelineUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: historyTimelineUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runHistoryTimelineUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planHistoryTimelineUiRecovery(
    {
      topic: "history-timeline-ui",
      mounted: true,
  ...
  const failure = planHistoryTimelineUiRecovery(
    {
      topic: "history-timeline-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
