export type historyTimelineUiSignal = {
  frameCostMs: number;
  focusDrift: number;
...
export type historyTimelineUiEvent = {
  id: string;
  topic: "history-timeline-ui";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: historyTimelineUiSignal;
};

export type historyTimelineUiDecision = {
  accepted: boolean;
...
export function evaluateHistoryTimelineUiEvent(event: historyTimelineUiEvent): historyTimelineUiDecision {
  ...
  let action: historyTimelineUiDecision["action"] = "commit";
...
export function runHistoryTimelineUiContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateHistoryTimelineUiEvent({
    id: "history-timeline-ui-evt-1",
    topic: "history-timeline-ui",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateHistoryTimelineUiEvent({
    id: "history-timeline-ui-evt-late",
    topic: "history-timeline-ui",
    actorId: "user-42",
    sequence: 8,
