export type kanbanBoardRuntimeState = {
  topic: "kanban-board";
  mounted: boolean;
...
export type kanbanBoardRecoveryPlan = {
...
export function planKanbanBoardRecovery(
  state: kanbanBoardRuntimeState,
  nowMs: number,
): kanbanBoardRecoveryPlan {
  const evidence: string[] = [];
  const actions: kanbanBoardRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runKanbanBoardEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planKanbanBoardRecovery(
    {
      topic: "kanban-board",
      mounted: true,
  ...
  const failure = planKanbanBoardRecovery(
    {
      topic: "kanban-board",
      mounted: true,
      lastSuccessfulVersion: 21,
