export type zoomableCanvasSystemRuntimeState = {
...
export type zoomableCanvasSystemRecoveryPlan = {
...
export function planZoomableCanvasSystemRecovery(
  state: zoomableCanvasSystemRuntimeState,
  nowMs: number,
): zoomableCanvasSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: zoomableCanvasSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runZoomableCanvasSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planZoomableCanvasSystemRecovery(
    {
      topic: "zoomable-canvas-system",
      mounted: true,
  ...
  const failure = planZoomableCanvasSystemRecovery(
    {
      topic: "zoomable-canvas-system",
      mounted: true,
      lastSuccessfulVersion: 21,
