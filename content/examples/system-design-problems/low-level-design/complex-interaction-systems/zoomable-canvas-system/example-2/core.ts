export type zoomableCanvasSystemSignal = {
  frameCostMs: number;
  focusDrift: number;
...
export type zoomableCanvasSystemEvent = {
  id: string;
  topic: "zoomable-canvas-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: zoomableCanvasSystemSignal;
};

export type zoomableCanvasSystemDecision = {
  accepted: boolean;
...
export function evaluateZoomableCanvasSystemEvent(event: zoomableCanvasSystemEvent): zoomableCanvasSystemDecision {
  ...
  let action: zoomableCanvasSystemDecision["action"] = "commit";
...
export function runZoomableCanvasSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateZoomableCanvasSystemEvent({
    id: "zoomable-canvas-system-evt-1",
    topic: "zoomable-canvas-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateZoomableCanvasSystemEvent({
    id: "zoomable-canvas-system-evt-late",
    topic: "zoomable-canvas-system",
    actorId: "user-42",
    sequence: 8,
