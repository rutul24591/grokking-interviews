export type snapToGridAlignmentSystemRuntimeState = {
...
export type snapToGridAlignmentSystemRecoveryPlan = {
...
export function planSnapToGridAlignmentSystemRecovery(
  state: snapToGridAlignmentSystemRuntimeState,
  nowMs: number,
): snapToGridAlignmentSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: snapToGridAlignmentSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runSnapToGridAlignmentSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planSnapToGridAlignmentSystemRecovery(
    {
      topic: "snap-to-grid-alignment-system",
      mounted: true,
  ...
  const failure = planSnapToGridAlignmentSystemRecovery(
    {
      topic: "snap-to-grid-alignment-system",
      mounted: true,
      lastSuccessfulVersion: 21,
