export type bottomSheetDrawerRuntimeState = {
...
export type bottomSheetDrawerRecoveryPlan = {
...
export function planBottomSheetDrawerRecovery(
  state: bottomSheetDrawerRuntimeState,
  nowMs: number,
): bottomSheetDrawerRecoveryPlan {
  const evidence: string[] = [];
  const actions: bottomSheetDrawerRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runBottomSheetDrawerEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planBottomSheetDrawerRecovery(
    {
      topic: "bottom-sheet-drawer",
      mounted: true,
  ...
  const failure = planBottomSheetDrawerRecovery(
    {
      topic: "bottom-sheet-drawer",
      mounted: true,
      lastSuccessfulVersion: 21,
