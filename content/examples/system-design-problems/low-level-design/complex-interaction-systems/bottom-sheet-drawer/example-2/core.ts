export type bottomSheetDrawerSignal = {
  frameCostMs: number;
  focusDrift: number;
...
export type bottomSheetDrawerEvent = {
  id: string;
  topic: "bottom-sheet-drawer";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: bottomSheetDrawerSignal;
};

export type bottomSheetDrawerDecision = {
  accepted: boolean;
...
export function evaluateBottomSheetDrawerEvent(event: bottomSheetDrawerEvent): bottomSheetDrawerDecision {
  ...
  let action: bottomSheetDrawerDecision["action"] = "commit";
...
export function runBottomSheetDrawerContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateBottomSheetDrawerEvent({
    id: "bottom-sheet-drawer-evt-1",
    topic: "bottom-sheet-drawer",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateBottomSheetDrawerEvent({
    id: "bottom-sheet-drawer-evt-late",
    topic: "bottom-sheet-drawer",
    actorId: "user-42",
    sequence: 8,
