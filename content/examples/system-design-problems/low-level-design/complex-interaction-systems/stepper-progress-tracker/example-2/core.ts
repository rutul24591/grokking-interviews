export type stepperProgressTrackerSignal = {
  frameCostMs: number;
  focusDrift: number;
...
export type stepperProgressTrackerEvent = {
  id: string;
  topic: "stepper-progress-tracker";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: stepperProgressTrackerSignal;
};

export type stepperProgressTrackerDecision = {
  accepted: boolean;
...
export function evaluateStepperProgressTrackerEvent(event: stepperProgressTrackerEvent): stepperProgressTrackerDecision {
  ...
  let action: stepperProgressTrackerDecision["action"] = "commit";
...
export function runStepperProgressTrackerContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateStepperProgressTrackerEvent({
    id: "stepper-progress-tracker-evt-1",
    topic: "stepper-progress-tracker",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateStepperProgressTrackerEvent({
    id: "stepper-progress-tracker-evt-late",
    topic: "stepper-progress-tracker",
    actorId: "user-42",
    sequence: 8,
