export type stepperProgressTrackerRuntimeState = {
...
export type stepperProgressTrackerRecoveryPlan = {
...
export function planStepperProgressTrackerRecovery(
  state: stepperProgressTrackerRuntimeState,
  nowMs: number,
): stepperProgressTrackerRecoveryPlan {
  const evidence: string[] = [];
  const actions: stepperProgressTrackerRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runStepperProgressTrackerEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planStepperProgressTrackerRecovery(
    {
      topic: "stepper-progress-tracker",
      mounted: true,
  ...
  const failure = planStepperProgressTrackerRecovery(
    {
      topic: "stepper-progress-tracker",
      mounted: true,
      lastSuccessfulVersion: 21,
