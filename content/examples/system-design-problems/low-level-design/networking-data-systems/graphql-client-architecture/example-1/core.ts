export type GraphqlClientArchitectureExample1Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type GraphqlClientArchitectureExample1RuntimeState = {
...
export type GraphqlClientArchitectureExample1Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: GraphqlClientArchitectureExample1RuntimeState;
...
export function evaluateGraphqlClientArchitectureExample1(
  operation: GraphqlClientArchitectureExample1Operation,
  state: GraphqlClientArchitectureExample1RuntimeState,
  nowMs: number,
): GraphqlClientArchitectureExample1Decision {
  ...
  let action: GraphqlClientArchitectureExample1Decision["action"] = "start-transport";
...
export function runGraphqlClientArchitectureExample1Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: GraphqlClientArchitectureExample1RuntimeState = {
    currentSequence: 8,
    status: "idle",
    cacheAgeMs: 60_000,
    subscribers: 1,
  ...
  const operation: GraphqlClientArchitectureExample1Operation = {
  ...
  return evaluateGraphqlClientArchitectureExample1(operation, baseState, nowMs);
}
