export type GraphqlClientArchitectureExample3Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type GraphqlClientArchitectureExample3RuntimeState = {
...
export type GraphqlClientArchitectureExample3Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: GraphqlClientArchitectureExample3RuntimeState;
...
export function evaluateGraphqlClientArchitectureExample3(
  operation: GraphqlClientArchitectureExample3Operation,
  state: GraphqlClientArchitectureExample3RuntimeState,
  nowMs: number,
): GraphqlClientArchitectureExample3Decision {
  ...
  let action: GraphqlClientArchitectureExample3Decision["action"] = "start-transport";
...
export function runGraphqlClientArchitectureExample3Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: GraphqlClientArchitectureExample3RuntimeState = {
    currentSequence: 8,
    status: "success",
    cacheAgeMs: 2_000,
    subscribers: 3,
  ...
  const operation: GraphqlClientArchitectureExample3Operation = {
  ...
  return evaluateGraphqlClientArchitectureExample3(operation, baseState, nowMs);
}
