export type GraphqlClientArchitectureExample2Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type GraphqlClientArchitectureExample2RuntimeState = {
...
export type GraphqlClientArchitectureExample2Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: GraphqlClientArchitectureExample2RuntimeState;
  telemetry: Record<string, string | number | boolean | undefined>;
};

const topic = "Design a GraphQL Client Architecture";
const invariant = "GraphQL errors can coexist with partial data and must not always be treated as total failure";

export function evaluateGraphqlClientArchitectureExample2(
  operation: GraphqlClientArchitectureExample2Operation,
  state: GraphqlClientArchitectureExample2RuntimeState,
  nowMs: number,
): GraphqlClientArchitectureExample2Decision {
  ...
  let action: GraphqlClientArchitectureExample2Decision["action"] = "start-transport";
...
export function runGraphqlClientArchitectureExample2Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: GraphqlClientArchitectureExample2RuntimeState = {
    currentSequence: 8,
    status: "loading",
    cacheAgeMs: 120_000,
    subscribers: 2,
    retryBudget: 2,
    lastCommittedKey: "graphql-client-architecture:previous",
  };

  const operation: GraphqlClientArchitectureExample2Operation = {
    key: "graphql-client-architecture:tenant-acme:resource-42",
    tenantId: "tenant-acme",
    sequence: 7,
    startedAtMs: nowMs - 800,
    deadlineMs: 15_000,
    idempotencyKey: "idem-graphql-client-architecture-42",
    payloadBytes: 18_000,
  };

  return evaluateGraphqlClientArchitectureExample2(operation, baseState, nowMs);
}
