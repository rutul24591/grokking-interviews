export type RaceConditionHandlingExample1Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type RaceConditionHandlingExample1RuntimeState = {
...
export type RaceConditionHandlingExample1Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: RaceConditionHandlingExample1RuntimeState;
...
export function evaluateRaceConditionHandlingExample1(
  operation: RaceConditionHandlingExample1Operation,
  state: RaceConditionHandlingExample1RuntimeState,
  nowMs: number,
): RaceConditionHandlingExample1Decision {
  ...
  let action: RaceConditionHandlingExample1Decision["action"] = "start-transport";
...
export function runRaceConditionHandlingExample1Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: RaceConditionHandlingExample1RuntimeState = {
    currentSequence: 8,
    status: "idle",
    cacheAgeMs: 60_000,
    subscribers: 1,
  ...
  const operation: RaceConditionHandlingExample1Operation = {
  ...
  return evaluateRaceConditionHandlingExample1(operation, baseState, nowMs);
}
