export type RaceConditionHandlingExample3Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type RaceConditionHandlingExample3RuntimeState = {
...
export type RaceConditionHandlingExample3Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: RaceConditionHandlingExample3RuntimeState;
...
export function evaluateRaceConditionHandlingExample3(
  operation: RaceConditionHandlingExample3Operation,
  state: RaceConditionHandlingExample3RuntimeState,
  nowMs: number,
): RaceConditionHandlingExample3Decision {
  ...
  let action: RaceConditionHandlingExample3Decision["action"] = "start-transport";
...
export function runRaceConditionHandlingExample3Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: RaceConditionHandlingExample3RuntimeState = {
    currentSequence: 8,
    status: "success",
    cacheAgeMs: 2_000,
    subscribers: 3,
  ...
  const operation: RaceConditionHandlingExample3Operation = {
  ...
  return evaluateRaceConditionHandlingExample3(operation, baseState, nowMs);
}
