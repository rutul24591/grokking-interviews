export type dataFetchingHookRuntimeState = {
...
export type dataFetchingHookRecoveryPlan = {
...
export function planDataFetchingHookRecovery(
  state: dataFetchingHookRuntimeState,
  nowMs: number,
): dataFetchingHookRecoveryPlan {
  const evidence: string[] = [];
  const actions: dataFetchingHookRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runDataFetchingHookEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planDataFetchingHookRecovery(
    {
      topic: "data-fetching-hook",
      mounted: true,
  ...
  const failure = planDataFetchingHookRecovery(
    {
      topic: "data-fetching-hook",
      mounted: true,
      lastSuccessfulVersion: 21,
