export type RaceConditionHandlingSystemExample3Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type RaceConditionHandlingSystemExample3RuntimeState = {
...
export type RaceConditionHandlingSystemExample3Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: RaceConditionHandlingSystemExample3RuntimeState;
...
export function evaluateRaceConditionHandlingSystemExample3(
  operation: RaceConditionHandlingSystemExample3Operation,
  state: RaceConditionHandlingSystemExample3RuntimeState,
  nowMs: number,
): RaceConditionHandlingSystemExample3Decision {
  ...
  let action: RaceConditionHandlingSystemExample3Decision["action"] = "start-transport";
...
export function runRaceConditionHandlingSystemExample3Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: RaceConditionHandlingSystemExample3RuntimeState = {
    currentSequence: 8,
    status: "success",
    cacheAgeMs: 2_000,
    subscribers: 3,
  ...
  const operation: RaceConditionHandlingSystemExample3Operation = {
  ...
  return evaluateRaceConditionHandlingSystemExample3(operation, baseState, nowMs);
}
