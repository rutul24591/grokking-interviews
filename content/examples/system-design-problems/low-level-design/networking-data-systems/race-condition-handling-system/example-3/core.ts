export type raceConditionHandlingSystemRuntimeState = {
...
export type raceConditionHandlingSystemRecoveryPlan = {
...
export function planRaceConditionHandlingSystemRecovery(
  state: raceConditionHandlingSystemRuntimeState,
  nowMs: number,
): raceConditionHandlingSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: raceConditionHandlingSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runRaceConditionHandlingSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planRaceConditionHandlingSystemRecovery(
    {
      topic: "race-condition-handling-system",
      mounted: true,
  ...
  const failure = planRaceConditionHandlingSystemRecovery(
    {
      topic: "race-condition-handling-system",
      mounted: true,
      lastSuccessfulVersion: 21,
