export type RateLimitedAutocompleteSystemExample1Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type RateLimitedAutocompleteSystemExample1RuntimeState = {
...
export type RateLimitedAutocompleteSystemExample1Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: RateLimitedAutocompleteSystemExample1RuntimeState;
...
export function evaluateRateLimitedAutocompleteSystemExample1(
  operation: RateLimitedAutocompleteSystemExample1Operation,
  state: RateLimitedAutocompleteSystemExample1RuntimeState,
  nowMs: number,
): RateLimitedAutocompleteSystemExample1Decision {
  ...
  let action: RateLimitedAutocompleteSystemExample1Decision["action"] = "start-transport";
...
export function runRateLimitedAutocompleteSystemExample1Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: RateLimitedAutocompleteSystemExample1RuntimeState = {
    currentSequence: 8,
    status: "idle",
    cacheAgeMs: 60_000,
    subscribers: 1,
  ...
  const operation: RateLimitedAutocompleteSystemExample1Operation = {
  ...
  return evaluateRateLimitedAutocompleteSystemExample1(operation, baseState, nowMs);
}
