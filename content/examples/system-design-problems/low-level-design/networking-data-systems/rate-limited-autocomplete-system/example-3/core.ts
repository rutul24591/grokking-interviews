export type rateLimitedAutocompleteSystemRuntimeState = {
...
export type rateLimitedAutocompleteSystemRecoveryPlan = {
...
export function planRateLimitedAutocompleteSystemRecovery(
  state: rateLimitedAutocompleteSystemRuntimeState,
  nowMs: number,
): rateLimitedAutocompleteSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: rateLimitedAutocompleteSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runRateLimitedAutocompleteSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planRateLimitedAutocompleteSystemRecovery(
    {
      topic: "rate-limited-autocomplete-system",
      mounted: true,
  ...
  const failure = planRateLimitedAutocompleteSystemRecovery(
    {
      topic: "rate-limited-autocomplete-system",
      mounted: true,
      lastSuccessfulVersion: 21,
