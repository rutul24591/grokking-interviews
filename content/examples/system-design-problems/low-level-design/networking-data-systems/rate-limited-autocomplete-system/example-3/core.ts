export type RateLimitedAutocompleteSystemExample3Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type RateLimitedAutocompleteSystemExample3RuntimeState = {
...
export type RateLimitedAutocompleteSystemExample3Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: RateLimitedAutocompleteSystemExample3RuntimeState;
...
export function evaluateRateLimitedAutocompleteSystemExample3(
  operation: RateLimitedAutocompleteSystemExample3Operation,
  state: RateLimitedAutocompleteSystemExample3RuntimeState,
  nowMs: number,
): RateLimitedAutocompleteSystemExample3Decision {
  ...
  let action: RateLimitedAutocompleteSystemExample3Decision["action"] = "start-transport";
...
export function runRateLimitedAutocompleteSystemExample3Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: RateLimitedAutocompleteSystemExample3RuntimeState = {
    currentSequence: 8,
    status: "success",
    cacheAgeMs: 2_000,
    subscribers: 3,
  ...
  const operation: RateLimitedAutocompleteSystemExample3Operation = {
  ...
  return evaluateRateLimitedAutocompleteSystemExample3(operation, baseState, nowMs);
}
