export type rateLimitedAutocompleteSystemSignal = {
  attempt: number;
  deadlineMs: number;
  cacheAgeMs: number;
...
export type rateLimitedAutocompleteSystemEvent = {
  id: string;
  topic: "rate-limited-autocomplete-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: rateLimitedAutocompleteSystemSignal;
};

export type rateLimitedAutocompleteSystemDecision = {
  accepted: boolean;
...
export function evaluateRateLimitedAutocompleteSystemEvent(event: rateLimitedAutocompleteSystemEvent): rateLimitedAutocompleteSystemDecision {
  ...
  let action: rateLimitedAutocompleteSystemDecision["action"] = "commit";
...
export function runRateLimitedAutocompleteSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateRateLimitedAutocompleteSystemEvent({
    id: "rate-limited-autocomplete-system-evt-1",
    topic: "rate-limited-autocomplete-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateRateLimitedAutocompleteSystemEvent({
    id: "rate-limited-autocomplete-system-evt-late",
    topic: "rate-limited-autocomplete-system",
    actorId: "user-42",
    sequence: 8,
