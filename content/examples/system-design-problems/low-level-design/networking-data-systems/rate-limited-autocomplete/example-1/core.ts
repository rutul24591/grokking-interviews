export type RateLimitedAutocompleteExample1Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type RateLimitedAutocompleteExample1RuntimeState = {
...
export type RateLimitedAutocompleteExample1Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: RateLimitedAutocompleteExample1RuntimeState;
...
export function evaluateRateLimitedAutocompleteExample1(
  operation: RateLimitedAutocompleteExample1Operation,
  state: RateLimitedAutocompleteExample1RuntimeState,
  nowMs: number,
): RateLimitedAutocompleteExample1Decision {
  ...
  let action: RateLimitedAutocompleteExample1Decision["action"] = "start-transport";
...
export function runRateLimitedAutocompleteExample1Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: RateLimitedAutocompleteExample1RuntimeState = {
    currentSequence: 8,
    status: "idle",
    cacheAgeMs: 60_000,
    subscribers: 1,
  ...
  const operation: RateLimitedAutocompleteExample1Operation = {
  ...
  return evaluateRateLimitedAutocompleteExample1(operation, baseState, nowMs);
}
