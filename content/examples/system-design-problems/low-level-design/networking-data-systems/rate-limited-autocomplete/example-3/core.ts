export type RateLimitedAutocompleteExample3Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type RateLimitedAutocompleteExample3RuntimeState = {
...
export type RateLimitedAutocompleteExample3Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: RateLimitedAutocompleteExample3RuntimeState;
...
export function evaluateRateLimitedAutocompleteExample3(
  operation: RateLimitedAutocompleteExample3Operation,
  state: RateLimitedAutocompleteExample3RuntimeState,
  nowMs: number,
): RateLimitedAutocompleteExample3Decision {
  ...
  let action: RateLimitedAutocompleteExample3Decision["action"] = "start-transport";
...
export function runRateLimitedAutocompleteExample3Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: RateLimitedAutocompleteExample3RuntimeState = {
    currentSequence: 8,
    status: "success",
    cacheAgeMs: 2_000,
    subscribers: 3,
    retryBudget: 0,
    lastCommittedKey: "rate-limited-autocomplete:previous",
  };

  const operation: RateLimitedAutocompleteExample3Operation = {
    key: "rate-limited-autocomplete:tenant-acme:resource-42",
    tenantId: "tenant-acme",
    sequence: 9,
    startedAtMs: nowMs - 40_000,
    deadlineMs: 15_000,
    idempotencyKey: "idem-rate-limited-autocomplete-42",
    payloadBytes: 4_200,
  };

  return evaluateRateLimitedAutocompleteExample3(operation, baseState, nowMs);
}
