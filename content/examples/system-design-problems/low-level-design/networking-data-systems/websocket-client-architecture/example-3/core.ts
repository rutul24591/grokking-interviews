export type WebsocketClientArchitectureExample3Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type WebsocketClientArchitectureExample3RuntimeState = {
...
export type WebsocketClientArchitectureExample3Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: WebsocketClientArchitectureExample3RuntimeState;
...
export function evaluateWebsocketClientArchitectureExample3(
  operation: WebsocketClientArchitectureExample3Operation,
  state: WebsocketClientArchitectureExample3RuntimeState,
  nowMs: number,
): WebsocketClientArchitectureExample3Decision {
  ...
  let action: WebsocketClientArchitectureExample3Decision["action"] = "start-transport";
...
export function runWebsocketClientArchitectureExample3Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: WebsocketClientArchitectureExample3RuntimeState = {
    currentSequence: 8,
    status: "success",
    cacheAgeMs: 2_000,
    subscribers: 3,
  ...
  const operation: WebsocketClientArchitectureExample3Operation = {
  ...
  return evaluateWebsocketClientArchitectureExample3(operation, baseState, nowMs);
}
