export type cacheInvalidationStrategySignal = {
  attempt: number;
  deadlineMs: number;
  cacheAgeMs: number;
...
export type cacheInvalidationStrategyEvent = {
  id: string;
  ...
  payloadSize: number;
  signal: cacheInvalidationStrategySignal;
};

export type cacheInvalidationStrategyDecision = {
  accepted: boolean;
...
export function evaluateCacheInvalidationStrategyEvent(event: cacheInvalidationStrategyEvent): cacheInvalidationStrategyDecision {
  ...
  let action: cacheInvalidationStrategyDecision["action"] = "commit";
...
export function runCacheInvalidationStrategyContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateCacheInvalidationStrategyEvent({
    id: "cache-invalidation-strategy-evt-1",
    topic: "cache-invalidation-strategy",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateCacheInvalidationStrategyEvent({
    id: "cache-invalidation-strategy-evt-late",
    topic: "cache-invalidation-strategy",
    actorId: "user-42",
    sequence: 8,
