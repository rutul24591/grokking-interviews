export type GlobalApiErrorHandlingLayerExample1Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type GlobalApiErrorHandlingLayerExample1RuntimeState = {
...
export type GlobalApiErrorHandlingLayerExample1Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: GlobalApiErrorHandlingLayerExample1RuntimeState;
...
export function evaluateGlobalApiErrorHandlingLayerExample1(
  operation: GlobalApiErrorHandlingLayerExample1Operation,
  state: GlobalApiErrorHandlingLayerExample1RuntimeState,
  nowMs: number,
): GlobalApiErrorHandlingLayerExample1Decision {
  ...
  let action: GlobalApiErrorHandlingLayerExample1Decision["action"] = "start-transport";
...
export function runGlobalApiErrorHandlingLayerExample1Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: GlobalApiErrorHandlingLayerExample1RuntimeState = {
    currentSequence: 8,
    status: "idle",
    cacheAgeMs: 60_000,
    subscribers: 1,
  ...
  const operation: GlobalApiErrorHandlingLayerExample1Operation = {
  ...
  return evaluateGlobalApiErrorHandlingLayerExample1(operation, baseState, nowMs);
}
