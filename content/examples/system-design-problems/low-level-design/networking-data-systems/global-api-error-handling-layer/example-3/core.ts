export type globalApiErrorHandlingLayerRuntimeState = {
...
export type globalApiErrorHandlingLayerRecoveryPlan = {
...
export function planGlobalApiErrorHandlingLayerRecovery(
  state: globalApiErrorHandlingLayerRuntimeState,
  nowMs: number,
): globalApiErrorHandlingLayerRecoveryPlan {
  const evidence: string[] = [];
  const actions: globalApiErrorHandlingLayerRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runGlobalApiErrorHandlingLayerEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planGlobalApiErrorHandlingLayerRecovery(
    {
      topic: "global-api-error-handling-layer",
      mounted: true,
  ...
  const failure = planGlobalApiErrorHandlingLayerRecovery(
    {
      topic: "global-api-error-handling-layer",
      mounted: true,
      lastSuccessfulVersion: 21,
