export type GlobalApiErrorHandlingLayerExample3Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type GlobalApiErrorHandlingLayerExample3RuntimeState = {
...
export type GlobalApiErrorHandlingLayerExample3Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: GlobalApiErrorHandlingLayerExample3RuntimeState;
  telemetry: Record<string, string | number | boolean | undefined>;
};

const topic = "Design a Global API Error Handling Layer";
const invariant = "global handlers must not hide domain errors that a local component can recover from";

export function evaluateGlobalApiErrorHandlingLayerExample3(
  operation: GlobalApiErrorHandlingLayerExample3Operation,
  state: GlobalApiErrorHandlingLayerExample3RuntimeState,
  nowMs: number,
): GlobalApiErrorHandlingLayerExample3Decision {
  ...
  let action: GlobalApiErrorHandlingLayerExample3Decision["action"] = "start-transport";
...
export function runGlobalApiErrorHandlingLayerExample3Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: GlobalApiErrorHandlingLayerExample3RuntimeState = {
    currentSequence: 8,
    status: "success",
    cacheAgeMs: 2_000,
    subscribers: 3,
    retryBudget: 0,
    lastCommittedKey: "global-api-error-handling-layer:previous",
  };

  const operation: GlobalApiErrorHandlingLayerExample3Operation = {
    key: "global-api-error-handling-layer:tenant-acme:resource-42",
    tenantId: "tenant-acme",
    sequence: 9,
    startedAtMs: nowMs - 40_000,
    deadlineMs: 15_000,
    idempotencyKey: "idem-global-api-error-handling-layer-42",
    payloadBytes: 4_200,
  };

  return evaluateGlobalApiErrorHandlingLayerExample3(operation, baseState, nowMs);
}
