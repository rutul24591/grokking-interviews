export type GlobalApiErrorHandlingExample2Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type GlobalApiErrorHandlingExample2RuntimeState = {
...
export type GlobalApiErrorHandlingExample2Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: GlobalApiErrorHandlingExample2RuntimeState;
...
export function evaluateGlobalApiErrorHandlingExample2(
  operation: GlobalApiErrorHandlingExample2Operation,
  state: GlobalApiErrorHandlingExample2RuntimeState,
  nowMs: number,
): GlobalApiErrorHandlingExample2Decision {
  ...
  let action: GlobalApiErrorHandlingExample2Decision["action"] = "start-transport";
...
export function runGlobalApiErrorHandlingExample2Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: GlobalApiErrorHandlingExample2RuntimeState = {
    currentSequence: 8,
    status: "loading",
    cacheAgeMs: 120_000,
    subscribers: 2,
    retryBudget: 2,
    lastCommittedKey: "global-api-error-handling:previous",
  };

  const operation: GlobalApiErrorHandlingExample2Operation = {
    key: "global-api-error-handling:tenant-acme:resource-42",
    tenantId: "tenant-acme",
    sequence: 7,
    startedAtMs: nowMs - 800,
    deadlineMs: 15_000,
    idempotencyKey: "idem-global-api-error-handling-42",
    payloadBytes: 18_000,
  };

  return evaluateGlobalApiErrorHandlingExample2(operation, baseState, nowMs);
}
