export type GlobalApiErrorHandlingExample3Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type GlobalApiErrorHandlingExample3RuntimeState = {
...
export type GlobalApiErrorHandlingExample3Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: GlobalApiErrorHandlingExample3RuntimeState;
...
export function evaluateGlobalApiErrorHandlingExample3(
  operation: GlobalApiErrorHandlingExample3Operation,
  state: GlobalApiErrorHandlingExample3RuntimeState,
  nowMs: number,
): GlobalApiErrorHandlingExample3Decision {
  ...
  let action: GlobalApiErrorHandlingExample3Decision["action"] = "start-transport";
...
export function runGlobalApiErrorHandlingExample3Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: GlobalApiErrorHandlingExample3RuntimeState = {
    currentSequence: 8,
    status: "success",
    cacheAgeMs: 2_000,
    subscribers: 3,
  ...
  const operation: GlobalApiErrorHandlingExample3Operation = {
  ...
  return evaluateGlobalApiErrorHandlingExample3(operation, baseState, nowMs);
}
