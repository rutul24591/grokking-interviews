export type GlobalApiErrorHandlingExample1Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type GlobalApiErrorHandlingExample1RuntimeState = {
...
export type GlobalApiErrorHandlingExample1Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: GlobalApiErrorHandlingExample1RuntimeState;
...
export function evaluateGlobalApiErrorHandlingExample1(
  operation: GlobalApiErrorHandlingExample1Operation,
  state: GlobalApiErrorHandlingExample1RuntimeState,
  nowMs: number,
): GlobalApiErrorHandlingExample1Decision {
  ...
  let action: GlobalApiErrorHandlingExample1Decision["action"] = "start-transport";
...
export function runGlobalApiErrorHandlingExample1Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: GlobalApiErrorHandlingExample1RuntimeState = {
    currentSequence: 8,
    status: "idle",
    cacheAgeMs: 60_000,
    subscribers: 1,
  ...
  const operation: GlobalApiErrorHandlingExample1Operation = {
  ...
  return evaluateGlobalApiErrorHandlingExample1(operation, baseState, nowMs);
}
