export type frontendCachingLayerRuntimeState = {
...
export type frontendCachingLayerRecoveryPlan = {
...
export function planFrontendCachingLayerRecovery(
  state: frontendCachingLayerRuntimeState,
  nowMs: number,
): frontendCachingLayerRecoveryPlan {
  const evidence: string[] = [];
  const actions: frontendCachingLayerRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runFrontendCachingLayerEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planFrontendCachingLayerRecovery(
    {
      topic: "frontend-caching-layer",
      mounted: true,
  ...
  const failure = planFrontendCachingLayerRecovery(
    {
      topic: "frontend-caching-layer",
      mounted: true,
      lastSuccessfulVersion: 21,
