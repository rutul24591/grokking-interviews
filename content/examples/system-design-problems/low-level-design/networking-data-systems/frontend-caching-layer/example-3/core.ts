export type FrontendCachingLayerExample3Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type FrontendCachingLayerExample3RuntimeState = {
...
export type FrontendCachingLayerExample3Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: FrontendCachingLayerExample3RuntimeState;
...
export function evaluateFrontendCachingLayerExample3(
  operation: FrontendCachingLayerExample3Operation,
  state: FrontendCachingLayerExample3RuntimeState,
  nowMs: number,
): FrontendCachingLayerExample3Decision {
  ...
  let action: FrontendCachingLayerExample3Decision["action"] = "start-transport";
...
export function runFrontendCachingLayerExample3Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: FrontendCachingLayerExample3RuntimeState = {
    currentSequence: 8,
    status: "success",
    cacheAgeMs: 2_000,
    subscribers: 3,
  ...
  const operation: FrontendCachingLayerExample3Operation = {
  ...
  return evaluateFrontendCachingLayerExample3(operation, baseState, nowMs);
}
