export type frontendCachingLayerSignal = {
  attempt: number;
  deadlineMs: number;
  cacheAgeMs: number;
...
export type frontendCachingLayerEvent = {
  id: string;
  topic: "frontend-caching-layer";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: frontendCachingLayerSignal;
};

export type frontendCachingLayerDecision = {
  accepted: boolean;
...
export function evaluateFrontendCachingLayerEvent(event: frontendCachingLayerEvent): frontendCachingLayerDecision {
  ...
  let action: frontendCachingLayerDecision["action"] = "commit";
...
export function runFrontendCachingLayerContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateFrontendCachingLayerEvent({
    id: "frontend-caching-layer-evt-1",
    topic: "frontend-caching-layer",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateFrontendCachingLayerEvent({
    id: "frontend-caching-layer-evt-late",
    topic: "frontend-caching-layer",
    actorId: "user-42",
    sequence: 8,
