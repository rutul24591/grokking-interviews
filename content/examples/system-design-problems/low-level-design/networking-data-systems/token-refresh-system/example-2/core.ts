export type TokenRefreshSystemExample2Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type TokenRefreshSystemExample2RuntimeState = {
...
export type TokenRefreshSystemExample2Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: TokenRefreshSystemExample2RuntimeState;
...
export function evaluateTokenRefreshSystemExample2(
  operation: TokenRefreshSystemExample2Operation,
  state: TokenRefreshSystemExample2RuntimeState,
  nowMs: number,
): TokenRefreshSystemExample2Decision {
  ...
  let action: TokenRefreshSystemExample2Decision["action"] = "start-transport";
...
export function runTokenRefreshSystemExample2Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: TokenRefreshSystemExample2RuntimeState = {
    currentSequence: 8,
    status: "loading",
    cacheAgeMs: 120_000,
    subscribers: 2,
  ...
  const operation: TokenRefreshSystemExample2Operation = {
  ...
  return evaluateTokenRefreshSystemExample2(operation, baseState, nowMs);
}
