export type pollingVsWebsocketSystemRuntimeState = {
...
export type pollingVsWebsocketSystemRecoveryPlan = {
...
export function planPollingVsWebsocketSystemRecovery(
  state: pollingVsWebsocketSystemRuntimeState,
  nowMs: number,
): pollingVsWebsocketSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: pollingVsWebsocketSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runPollingVsWebsocketSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planPollingVsWebsocketSystemRecovery(
    {
      topic: "polling-vs-websocket-system",
      mounted: true,
  ...
  const failure = planPollingVsWebsocketSystemRecovery(
    {
      topic: "polling-vs-websocket-system",
      mounted: true,
      lastSuccessfulVersion: 21,
