export type pollingVsWebsocketSystemSignal = {
  attempt: number;
  deadlineMs: number;
  cacheAgeMs: number;
...
export type pollingVsWebsocketSystemEvent = {
  id: string;
  topic: "polling-vs-websocket-system";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: pollingVsWebsocketSystemSignal;
};

export type pollingVsWebsocketSystemDecision = {
  accepted: boolean;
...
export function evaluatePollingVsWebsocketSystemEvent(event: pollingVsWebsocketSystemEvent): pollingVsWebsocketSystemDecision {
  ...
  let action: pollingVsWebsocketSystemDecision["action"] = "commit";
...
export function runPollingVsWebsocketSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluatePollingVsWebsocketSystemEvent({
    id: "polling-vs-websocket-system-evt-1",
    topic: "polling-vs-websocket-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluatePollingVsWebsocketSystemEvent({
    id: "polling-vs-websocket-system-evt-late",
    topic: "polling-vs-websocket-system",
    actorId: "user-42",
    sequence: 8,
