export type RetryMechanismExample1Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type RetryMechanismExample1RuntimeState = {
...
export type RetryMechanismExample1Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: RetryMechanismExample1RuntimeState;
...
export function evaluateRetryMechanismExample1(
  operation: RetryMechanismExample1Operation,
  state: RetryMechanismExample1RuntimeState,
  nowMs: number,
): RetryMechanismExample1Decision {
  ...
  let action: RetryMechanismExample1Decision["action"] = "start-transport";
...
export function runRetryMechanismExample1Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: RetryMechanismExample1RuntimeState = {
    currentSequence: 8,
    status: "idle",
    cacheAgeMs: 60_000,
    subscribers: 1,
  ...
  const operation: RetryMechanismExample1Operation = {
  ...
  return evaluateRetryMechanismExample1(operation, baseState, nowMs);
}
