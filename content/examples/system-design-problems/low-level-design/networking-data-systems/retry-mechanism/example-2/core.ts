export type RetryMechanismExample2Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type RetryMechanismExample2RuntimeState = {
...
export type RetryMechanismExample2Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: RetryMechanismExample2RuntimeState;
...
export function evaluateRetryMechanismExample2(
  operation: RetryMechanismExample2Operation,
  state: RetryMechanismExample2RuntimeState,
  nowMs: number,
): RetryMechanismExample2Decision {
  ...
  let action: RetryMechanismExample2Decision["action"] = "start-transport";
...
export function runRetryMechanismExample2Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: RetryMechanismExample2RuntimeState = {
    currentSequence: 8,
    status: "loading",
    cacheAgeMs: 120_000,
    subscribers: 2,
  ...
  const operation: RetryMechanismExample2Operation = {
  ...
  return evaluateRetryMechanismExample2(operation, baseState, nowMs);
}
