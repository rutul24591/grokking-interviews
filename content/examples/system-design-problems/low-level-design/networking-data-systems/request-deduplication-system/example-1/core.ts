export type RequestDeduplicationSystemExample1Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type RequestDeduplicationSystemExample1RuntimeState = {
...
export type RequestDeduplicationSystemExample1Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: RequestDeduplicationSystemExample1RuntimeState;
...
export function evaluateRequestDeduplicationSystemExample1(
  operation: RequestDeduplicationSystemExample1Operation,
  state: RequestDeduplicationSystemExample1RuntimeState,
  nowMs: number,
): RequestDeduplicationSystemExample1Decision {
  ...
  let action: RequestDeduplicationSystemExample1Decision["action"] = "start-transport";
...
export function runRequestDeduplicationSystemExample1Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: RequestDeduplicationSystemExample1RuntimeState = {
    currentSequence: 8,
    status: "idle",
    cacheAgeMs: 60_000,
    subscribers: 1,
  ...
  const operation: RequestDeduplicationSystemExample1Operation = {
  ...
  return evaluateRequestDeduplicationSystemExample1(operation, baseState, nowMs);
}
