export type requestDeduplicationSystemRuntimeState = {
...
export type requestDeduplicationSystemRecoveryPlan = {
...
export function planRequestDeduplicationSystemRecovery(
  state: requestDeduplicationSystemRuntimeState,
  nowMs: number,
): requestDeduplicationSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: requestDeduplicationSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runRequestDeduplicationSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planRequestDeduplicationSystemRecovery(
    {
      topic: "request-deduplication-system",
      mounted: true,
  ...
  const failure = planRequestDeduplicationSystemRecovery(
    {
      topic: "request-deduplication-system",
      mounted: true,
      lastSuccessfulVersion: 21,
