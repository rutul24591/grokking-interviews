export type RequestDeduplicationSystemExample3Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type RequestDeduplicationSystemExample3RuntimeState = {
...
export type RequestDeduplicationSystemExample3Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: RequestDeduplicationSystemExample3RuntimeState;
...
export function evaluateRequestDeduplicationSystemExample3(
  operation: RequestDeduplicationSystemExample3Operation,
  state: RequestDeduplicationSystemExample3RuntimeState,
  nowMs: number,
): RequestDeduplicationSystemExample3Decision {
  ...
  let action: RequestDeduplicationSystemExample3Decision["action"] = "start-transport";
...
export function runRequestDeduplicationSystemExample3Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: RequestDeduplicationSystemExample3RuntimeState = {
    currentSequence: 8,
    status: "success",
    cacheAgeMs: 2_000,
    subscribers: 3,
  ...
  const operation: RequestDeduplicationSystemExample3Operation = {
  ...
  return evaluateRequestDeduplicationSystemExample3(operation, baseState, nowMs);
}
