export type OptimisticUiSystemExample1Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type OptimisticUiSystemExample1RuntimeState = {
...
export type OptimisticUiSystemExample1Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: OptimisticUiSystemExample1RuntimeState;
...
export function evaluateOptimisticUiSystemExample1(
  operation: OptimisticUiSystemExample1Operation,
  state: OptimisticUiSystemExample1RuntimeState,
  nowMs: number,
): OptimisticUiSystemExample1Decision {
  ...
  let action: OptimisticUiSystemExample1Decision["action"] = "start-transport";
...
export function runOptimisticUiSystemExample1Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: OptimisticUiSystemExample1RuntimeState = {
    currentSequence: 8,
    status: "idle",
    cacheAgeMs: 60_000,
    subscribers: 1,
  ...
  const operation: OptimisticUiSystemExample1Operation = {
  ...
  return evaluateOptimisticUiSystemExample1(operation, baseState, nowMs);
}
