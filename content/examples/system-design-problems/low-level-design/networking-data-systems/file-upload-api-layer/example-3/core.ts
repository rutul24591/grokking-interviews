export type fileUploadApiLayerRuntimeState = {
...
export type fileUploadApiLayerRecoveryPlan = {
...
export function planFileUploadApiLayerRecovery(
  state: fileUploadApiLayerRuntimeState,
  nowMs: number,
): fileUploadApiLayerRecoveryPlan {
  const evidence: string[] = [];
  const actions: fileUploadApiLayerRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runFileUploadApiLayerEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planFileUploadApiLayerRecovery(
    {
      topic: "file-upload-api-layer",
      mounted: true,
  ...
  const failure = planFileUploadApiLayerRecovery(
    {
      topic: "file-upload-api-layer",
      mounted: true,
      lastSuccessfulVersion: 21,
