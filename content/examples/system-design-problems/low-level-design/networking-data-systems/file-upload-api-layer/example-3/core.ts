export type FileUploadApiLayerExample3Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type FileUploadApiLayerExample3RuntimeState = {
...
export type FileUploadApiLayerExample3Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: FileUploadApiLayerExample3RuntimeState;
...
export function evaluateFileUploadApiLayerExample3(
  operation: FileUploadApiLayerExample3Operation,
  state: FileUploadApiLayerExample3RuntimeState,
  nowMs: number,
): FileUploadApiLayerExample3Decision {
  ...
  let action: FileUploadApiLayerExample3Decision["action"] = "start-transport";
...
export function runFileUploadApiLayerExample3Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: FileUploadApiLayerExample3RuntimeState = {
    currentSequence: 8,
    status: "success",
    cacheAgeMs: 2_000,
    subscribers: 3,
  ...
  const operation: FileUploadApiLayerExample3Operation = {
  ...
  return evaluateFileUploadApiLayerExample3(operation, baseState, nowMs);
}
