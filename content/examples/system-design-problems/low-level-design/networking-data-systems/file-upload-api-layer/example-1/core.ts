export type FileUploadApiLayerExample1Operation = {
  key: string;
  tenantId: string;
  sequence: number;
  startedAtMs: number;
...
export type FileUploadApiLayerExample1RuntimeState = {
...
export type FileUploadApiLayerExample1Decision = {
  action:
    | "start-transport"
    | "reuse-inflight"
    ...
    | "commit";
  reasons: string[];
  nextState: FileUploadApiLayerExample1RuntimeState;
...
export function evaluateFileUploadApiLayerExample1(
  operation: FileUploadApiLayerExample1Operation,
  state: FileUploadApiLayerExample1RuntimeState,
  nowMs: number,
): FileUploadApiLayerExample1Decision {
  ...
  let action: FileUploadApiLayerExample1Decision["action"] = "start-transport";
...
export function runFileUploadApiLayerExample1Scenario() {
  const nowMs = Date.parse("2026-05-29T10:00:00.000Z");
  const baseState: FileUploadApiLayerExample1RuntimeState = {
    currentSequence: 8,
    status: "idle",
    cacheAgeMs: 60_000,
    subscribers: 1,
  ...
  const operation: FileUploadApiLayerExample1Operation = {
  ...
  return evaluateFileUploadApiLayerExample1(operation, baseState, nowMs);
}
