export type requestBatchingSystemSignal = {
  attempt: number;
  deadlineMs: number;
  cacheAgeMs: number;
...
export type requestBatchingSystemEvent = {
  id: string;
  ...
  payloadSize: number;
  signal: requestBatchingSystemSignal;
};

export type requestBatchingSystemDecision = {
  accepted: boolean;
...
export function evaluateRequestBatchingSystemEvent(event: requestBatchingSystemEvent): requestBatchingSystemDecision {
  ...
  let action: requestBatchingSystemDecision["action"] = "commit";
...
export function runRequestBatchingSystemContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateRequestBatchingSystemEvent({
    id: "request-batching-system-evt-1",
    topic: "request-batching-system",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateRequestBatchingSystemEvent({
    id: "request-batching-system-evt-late",
    topic: "request-batching-system",
    actorId: "user-42",
    sequence: 8,
