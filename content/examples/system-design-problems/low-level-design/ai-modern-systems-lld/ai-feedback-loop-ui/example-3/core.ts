export type aiFeedbackLoopUiRuntimeState = {
...
export type aiFeedbackLoopUiRecoveryPlan = {
...
export function planAiFeedbackLoopUiRecovery(
  state: aiFeedbackLoopUiRuntimeState,
  nowMs: number,
): aiFeedbackLoopUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: aiFeedbackLoopUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runAiFeedbackLoopUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planAiFeedbackLoopUiRecovery(
    {
      topic: "ai-feedback-loop-ui",
      mounted: true,
  ...
  const failure = planAiFeedbackLoopUiRecovery(
    {
      topic: "ai-feedback-loop-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
