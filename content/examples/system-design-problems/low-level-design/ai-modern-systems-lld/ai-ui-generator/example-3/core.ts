export type aiUiGeneratorRuntimeState = {
...
export type aiUiGeneratorRecoveryPlan = {
...
export function planAiUiGeneratorRecovery(
  state: aiUiGeneratorRuntimeState,
  nowMs: number,
): aiUiGeneratorRecoveryPlan {
  const evidence: string[] = [];
  const actions: aiUiGeneratorRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runAiUiGeneratorEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planAiUiGeneratorRecovery(
    {
      topic: "ai-ui-generator",
      mounted: true,
  ...
  const failure = planAiUiGeneratorRecovery(
    {
      topic: "ai-ui-generator",
      mounted: true,
      lastSuccessfulVersion: 21,
