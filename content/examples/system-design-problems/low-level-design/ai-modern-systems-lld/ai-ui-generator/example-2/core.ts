export type aiUiGeneratorSignal = {
...
export type aiUiGeneratorEvent = {
  id: string;
  topic: "ai-ui-generator";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: aiUiGeneratorSignal;
};

export type aiUiGeneratorDecision = {
  accepted: boolean;
...
export function evaluateAiUiGeneratorEvent(event: aiUiGeneratorEvent): aiUiGeneratorDecision {
  ...
  let action: aiUiGeneratorDecision["action"] = "commit";
...
export function runAiUiGeneratorContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateAiUiGeneratorEvent({
    id: "ai-ui-generator-evt-1",
    topic: "ai-ui-generator",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateAiUiGeneratorEvent({
    id: "ai-ui-generator-evt-late",
    topic: "ai-ui-generator",
    actorId: "user-42",
    sequence: 8,
