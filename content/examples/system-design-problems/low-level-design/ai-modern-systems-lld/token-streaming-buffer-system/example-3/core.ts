export type tokenStreamingBufferSystemRuntimeState = {
...
export type tokenStreamingBufferSystemRecoveryPlan = {
...
export function planTokenStreamingBufferSystemRecovery(
  state: tokenStreamingBufferSystemRuntimeState,
  nowMs: number,
): tokenStreamingBufferSystemRecoveryPlan {
  const evidence: string[] = [];
  const actions: tokenStreamingBufferSystemRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runTokenStreamingBufferSystemEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planTokenStreamingBufferSystemRecovery(
    {
      topic: "token-streaming-buffer-system",
      mounted: true,
  ...
  const failure = planTokenStreamingBufferSystemRecovery(
    {
      topic: "token-streaming-buffer-system",
      mounted: true,
      lastSuccessfulVersion: 21,
