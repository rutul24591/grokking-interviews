export type streamingChatUiRuntimeState = {
...
export type streamingChatUiRecoveryPlan = {
...
export function planStreamingChatUiRecovery(
  state: streamingChatUiRuntimeState,
  nowMs: number,
): streamingChatUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: streamingChatUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runStreamingChatUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planStreamingChatUiRecovery(
    {
      topic: "streaming-chat-ui",
      mounted: true,
  ...
  const failure = planStreamingChatUiRecovery(
    {
      topic: "streaming-chat-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
