export type aiAssistedSearchQnaUiRuntimeState = {
...
export type aiAssistedSearchQnaUiRecoveryPlan = {
...
export function planAiAssistedSearchQnaUiRecovery(
  state: aiAssistedSearchQnaUiRuntimeState,
  nowMs: number,
): aiAssistedSearchQnaUiRecoveryPlan {
  const evidence: string[] = [];
  const actions: aiAssistedSearchQnaUiRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runAiAssistedSearchQnaUiEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planAiAssistedSearchQnaUiRecovery(
    {
      topic: "ai-assisted-search-qna-ui",
      mounted: true,
  ...
  const failure = planAiAssistedSearchQnaUiRecovery(
    {
      topic: "ai-assisted-search-qna-ui",
      mounted: true,
      lastSuccessfulVersion: 21,
