export type aiAssistedSearchQnaUiSignal = {
...
export type aiAssistedSearchQnaUiEvent = {
  id: string;
  topic: "ai-assisted-search-qna-ui";
  actorId: string;
  sequence: number;
  receivedAtMs: number;
  ...
  payloadSize: number;
  signal: aiAssistedSearchQnaUiSignal;
};

export type aiAssistedSearchQnaUiDecision = {
  accepted: boolean;
...
export function evaluateAiAssistedSearchQnaUiEvent(event: aiAssistedSearchQnaUiEvent): aiAssistedSearchQnaUiDecision {
  ...
  let action: aiAssistedSearchQnaUiDecision["action"] = "commit";
  ...
  return {
    ...
    reasons,
    audit: [
      "topic:Ai Assisted Search Q&A UI",
...
export function runAiAssistedSearchQnaUiContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateAiAssistedSearchQnaUiEvent({
    id: "ai-assisted-search-qna-ui-evt-1",
    topic: "ai-assisted-search-qna-ui",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateAiAssistedSearchQnaUiEvent({
    id: "ai-assisted-search-qna-ui-evt-late",
    topic: "ai-assisted-search-qna-ui",
    actorId: "user-42",
    sequence: 8,
