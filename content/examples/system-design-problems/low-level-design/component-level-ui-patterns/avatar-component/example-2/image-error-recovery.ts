export type avatarComponentSignal = {
  frameCostMs: number;
  focusDrift: number;
...
export type avatarComponentEvent = {
  id: string;
  ...
  payloadSize: number;
  signal: avatarComponentSignal;
};

export type avatarComponentDecision = {
  accepted: boolean;
...
export function evaluateAvatarComponentEvent(event: avatarComponentEvent): avatarComponentDecision {
  ...
  let action: avatarComponentDecision["action"] = "commit";
...
export function runAvatarComponentContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateAvatarComponentEvent({
    id: "avatar-component-evt-1",
    topic: "avatar-component",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateAvatarComponentEvent({
    id: "avatar-component-evt-late",
    topic: "avatar-component",
    actorId: "user-42",
    sequence: 8,
