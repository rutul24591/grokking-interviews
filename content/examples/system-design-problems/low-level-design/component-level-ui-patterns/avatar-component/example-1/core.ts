export type AvatarComponentState="closed"|"open"|"disabled"|"busy";
export interface AvatarComponentSnapshot{state:AvatarComponentState;
activeIndex:number;
focused:boolean;
items:string[];
evidence:string[];
}export class AvatarComponentController{private s:AvatarComponentSnapshot={state:"closed",activeIndex:-1,focused:false,items:[],evidence:[]};
...
}}export function runAvatarComponentKeyboardScenario(){const c=new AvatarComponentController();
c.open(["one","two","three"]);
const moved=c.move(1,new Set([1]));
const closed=c.close("escape");
return{invariant:"Identity remains understandable when images are slow, broken, private, or absent.",moved,closed};
}
