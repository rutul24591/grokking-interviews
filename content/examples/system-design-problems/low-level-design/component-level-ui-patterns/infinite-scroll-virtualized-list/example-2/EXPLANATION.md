# Design an Infinite Scroll / Virtualized List - example-2 Explanation
...
- `dynamic-height-items.ts`: Implements the main logic, including createSizeCache, getItemSizeFromCache, updateItemSize, previousHeight, newHeights.
- `scroll-position-loss.ts`: Implements the main logic, including STORAGE_PREFIX, getStorageKey, saveToStorage, serialized, loadFromStorage.
- `scroll-restoration.ts`: Implements the main logic, including useScrollRestoration, savedStateRef, saveScrollPosition, container, state.
- `variable-height-items.ts`: Implements the main logic, including useVariableHeightVirtualization, scrollOffsetRef, getVisibleItems, items, currentOffset.
