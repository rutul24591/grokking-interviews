export type RatingStarsComponentState="closed"|"open"|"disabled"|"busy";
export interface RatingStarsComponentSnapshot{state:RatingStarsComponentState;
activeIndex:number;
focused:boolean;
items:string[];
evidence:string[];
}export class RatingStarsComponentController{private s:RatingStarsComponentSnapshot={state:"closed",activeIndex:-1,focused:false,items:[],evidence:[]};
...
}}export function runRatingStarsComponentKeyboardScenario(){const c=new RatingStarsComponentController();
c.open(["one","two","three"]);
const moved=c.move(1,new Set([1]));
const closed=c.close("escape");
return{invariant:"Preview must never overwrite the committed value until the user explicitly commits.",moved,closed};
}
