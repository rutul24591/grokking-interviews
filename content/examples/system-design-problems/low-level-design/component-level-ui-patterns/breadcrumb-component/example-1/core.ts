export type BreadcrumbComponentState="closed"|"open"|"disabled"|"busy";
export interface BreadcrumbComponentSnapshot{state:BreadcrumbComponentState;
activeIndex:number;
focused:boolean;
items:string[];
evidence:string[];
}export class BreadcrumbComponentController{private s:BreadcrumbComponentSnapshot={state:"closed",activeIndex:-1,focused:false,items:[],evidence:[]};
...
}}export function runBreadcrumbComponentKeyboardScenario(){const c=new BreadcrumbComponentController();
c.open(["one","two","three"]);
const moved=c.move(1,new Set([1]));
const closed=c.close("escape");
return{invariant:"Breadcrumbs must reflect navigable hierarchy without making the current page an ambiguous link.",moved,closed};
}
