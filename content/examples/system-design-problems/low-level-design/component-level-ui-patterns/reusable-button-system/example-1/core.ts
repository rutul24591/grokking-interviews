export type ReusableButtonSystemState="closed"|"open"|"disabled"|"busy";
export interface ReusableButtonSystemSnapshot{state:ReusableButtonSystemState;
activeIndex:number;
focused:boolean;
items:string[];
evidence:string[];
}export class ReusableButtonSystemController{private s:ReusableButtonSystemSnapshot={state:"closed",activeIndex:-1,focused:false,items:[],evidence:[]};
...
}}export function runReusableButtonSystemKeyboardScenario(){const c=new ReusableButtonSystemController();
c.open(["one","two","three"]);
const moved=c.move(1,new Set([1]));
const closed=c.close("escape");
return{invariant:"Buttons must preserve native semantics and prevent duplicate activation while loading.",moved,closed};
}
