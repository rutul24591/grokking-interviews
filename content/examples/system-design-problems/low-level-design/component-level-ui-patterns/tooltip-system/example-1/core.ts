export type TooltipSystemState="closed"|"open"|"disabled"|"busy";
export interface TooltipSystemSnapshot{state:TooltipSystemState;
activeIndex:number;
focused:boolean;
items:string[];
evidence:string[];
}export class TooltipSystemController{private s:TooltipSystemSnapshot={state:"closed",activeIndex:-1,focused:false,items:[],evidence:[]};
...
}}export function runTooltipSystemKeyboardScenario(){const c=new TooltipSystemController();
c.open(["one","two","three"]);
const moved=c.move(1,new Set([1]));
const closed=c.close("escape");
return{invariant:"Tooltips must supplement labels without trapping focus or flashing during pointer movement.",moved,closed};
}
