export interface TooltipSystemLayoutInput{anchor:{x:number;
y:number;
width:number;
height:number};
...
height:number};
gap:number;
}export function positionTooltipSystem(i:TooltipSystemLayoutInput){const below=i.anchor.y+i.anchor.height+i.gap;
...
}export function runTooltipSystemCollisionScenario(){return positionTooltipSystem({anchor:{x:360,y:560,width:80,height:32},overlay:{width:240,height:180},viewport:{width:420,height:700},gap:8});
}
