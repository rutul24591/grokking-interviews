export type multiSelectTagInputRuntimeState = {
  topic: "multi-select-tag-input";
  mounted: boolean;
...
export type multiSelectTagInputRecoveryPlan = {
...
export function planMultiSelectTagInputRecovery(
  state: multiSelectTagInputRuntimeState,
  nowMs: number,
): multiSelectTagInputRecoveryPlan {
  const evidence: string[] = [];
  const actions: multiSelectTagInputRecoveryPlan["actions"] = ["emit-telemetry"];
...
export function runMultiSelectTagInputEdgeCaseScenario() {
  const nowMs = Date.parse("2026-05-29T09:15:00.000Z");
  const normal = planMultiSelectTagInputRecovery(
    {
      topic: "multi-select-tag-input",
      mounted: true,
  ...
  const failure = planMultiSelectTagInputRecovery(
    {
      topic: "multi-select-tag-input",
      mounted: true,
      lastSuccessfulVersion: 21,
