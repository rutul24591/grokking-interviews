export interface ContextMenuLayoutInput{anchor:{x:number;
y:number;
width:number;
height:number};
...
height:number};
gap:number;
}export function positionContextMenu(i:ContextMenuLayoutInput){const below=i.anchor.y+i.anchor.height+i.gap;
...
}export function runContextMenuCollisionScenario(){return positionContextMenu({anchor:{x:360,y:560,width:80,height:32},overlay:{width:240,height:180},viewport:{width:420,height:700},gap:8});
}
