export type paginationComponentSignal = {
  frameCostMs: number;
  focusDrift: number;
...
export type paginationComponentEvent = {
  id: string;
  ...
  payloadSize: number;
  signal: paginationComponentSignal;
};

export type paginationComponentDecision = {
  accepted: boolean;
...
export function evaluatePaginationComponentEvent(event: paginationComponentEvent): paginationComponentDecision {
  ...
  let action: paginationComponentDecision["action"] = "commit";
...
export function runPaginationComponentContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluatePaginationComponentEvent({
    id: "pagination-component-evt-1",
    topic: "pagination-component",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluatePaginationComponentEvent({
    id: "pagination-component-evt-late",
    topic: "pagination-component",
    actorId: "user-42",
    sequence: 8,
