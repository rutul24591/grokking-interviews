export type modalComponentSignal = {
  frameCostMs: number;
  focusDrift: number;
...
export type modalComponentEvent = {
  id: string;
  ...
  payloadSize: number;
  signal: modalComponentSignal;
};

export type modalComponentDecision = {
  accepted: boolean;
...
export function evaluateModalComponentEvent(event: modalComponentEvent): modalComponentDecision {
  ...
  let action: modalComponentDecision["action"] = "commit";
...
export function runModalComponentContractScenario() {
  const base = Date.parse("2026-05-29T09:00:00.000Z");
  const accepted = evaluateModalComponentEvent({
    id: "modal-component-evt-1",
    topic: "modal-component",
    actorId: "user-42",
    sequence: 7,
    receivedAtMs: base,
  ...
  const guarded = evaluateModalComponentEvent({
    id: "modal-component-evt-late",
    topic: "modal-component",
    actorId: "user-42",
    sequence: 8,
