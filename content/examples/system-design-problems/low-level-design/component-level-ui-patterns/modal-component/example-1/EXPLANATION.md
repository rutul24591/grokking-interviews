# Design a Modal / Dialog Component - example-1 Explanation
...
- `components/modal-backdrop.tsx`: Implements the main logic, including ModalBackdrop, timer, handleClick, handleKeyDown.
- `components/modal-component.tsx`: Implements the main logic, including ModalPanel, containerRef, timer, handleDismiss, handleConfirm.
- `components/modal-container.tsx`: Implements the main logic, including ModalContainer, currentModal, isOpen, zIndex, close.
- `hooks/use-focus-trap.ts`: Implements the main logic, including FOCUSABLE_SELECTOR, useFocusTrap, previousFocusRef, handleKeyDown, focusableElements.
- `lib/modal-store.ts`: Models client or service state transitions and update behavior.
- `lib/modal-types.ts`: Implements the main logic, including BASE_Z_INDEX, DEFAULT_CONFIRM_LABELS, DEFAULT_ALERT_LABEL.
