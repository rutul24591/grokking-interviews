...
export type ProductionContext = {
  ...
  errorRate: number;
  approvals: string[];
  metrics: Partial<Record<["p95LatencyMs","memoryMb","cacheHitRate","hydrationDelayMs","errorBudgetBurn"][number], number>>;
};
...
export type ExecutionPlan = {
  ...
  nextState: ScaleScenario["state"];
  requiredActions: string[];
  telemetry: string[];
  auditTrail: string[];
};

const topic = "Frontend FOR 1M Concurrent Users";
...
export function validateScaleScenario(entity: ScaleScenario, ctx: ProductionContext) {
  const errors: string[] = [];
...
export function buildExecutionPlan(entity: ScaleScenario, ctx: ProductionContext): ExecutionPlan {
  const validation = validateScaleScenario(entity, ctx);
...
export function simulateSafeRollout(entity: ScaleScenario, ctx: ProductionContext) {
  const plan = buildExecutionPlan(entity, ctx);
  const ramp = plan.decision === "publish" ? [5, 25, 50, 100] : plan.decision === "canary" ? [1, 5] : [];
  return {
    slug: "frontend-for-1m-concurrent-users",
    plan,
    ramp,
