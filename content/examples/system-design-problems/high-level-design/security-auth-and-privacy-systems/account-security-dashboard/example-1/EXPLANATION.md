# Design an Account Security Dashboard (Sessions, Devices) - example-1 Explanation

## Article context
This example supports the article `high-level-design/security-auth-and-privacy-systems/account-security-dashboard`. The article is about Architecture for an account security dashboard: active session listing with device fingerprint, IP geolocation, and last-active timestamp; session revocation with real-time WebSocket push to invalidate the revoked session; device trust management with remembered and untrusted device lists; login history with anomaly highlighting (new location, unusual time); two-factor authentication enrollment and backup codes; password change with breach check via HaveIBeenPwned k-anonymity API; security event timeline; and account recovery options management.. The most relevant article sections for this example are: Problem Clarification; Requirements; Functional Requirements; Non-Functional Requirements; High-Level Architecture; Detailed Design; Session Data Model and Fingerprinting; Login History Anomaly Detection UI; TOTP Enrollment Flow; Device Trust and Remembered Devices.
