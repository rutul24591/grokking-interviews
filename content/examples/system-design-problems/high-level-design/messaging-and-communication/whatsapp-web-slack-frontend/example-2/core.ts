...
export type ProductionContext = {
  ...
  errorRate: number;
  approvals: string[];
  metrics: Partial<Record<["deliveryLagMs","fanoutSize","retryCount","unreadDrift","threadDepth"][number], number>>;
};
...
export type ExecutionPlan = {
  ...
  nextState: MessageFlow["state"];
  requiredActions: string[];
  telemetry: string[];
  auditTrail: string[];
};

const topic = "Whatsapp WEB Slack Frontend";
...
export function validateMessageFlow(entity: MessageFlow, ctx: ProductionContext) {
  const errors: string[] = [];
...
export function buildExecutionPlan(entity: MessageFlow, ctx: ProductionContext): ExecutionPlan {
  const validation = validateMessageFlow(entity, ctx);
