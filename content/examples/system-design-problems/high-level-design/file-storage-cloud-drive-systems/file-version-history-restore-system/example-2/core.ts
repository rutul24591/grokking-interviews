...
export type ProductionContext = {
  ...
  errorRate: number;
  approvals: string[];
  metrics: Partial<Record<["orphanedObjectCount","scanBacklog","quotaDriftBytes","aclCacheAgeMs","restoreChainDepth"][number], number>>;
};
...
export type ExecutionPlan = {
  ...
  nextState: FileOperation["state"];
  requiredActions: string[];
  telemetry: string[];
  auditTrail: string[];
};

const topic = "File Version History Restore System";
const domain = "cloud file storage";
...
export function validateFileOperation(entity: FileOperation, ctx: ProductionContext) {
  const errors: string[] = [];
...
export function buildExecutionPlan(entity: FileOperation, ctx: ProductionContext): ExecutionPlan {
  const validation = validateFileOperation(entity, ctx);
...
export function simulateSafeRollout(entity: FileOperation, ctx: ProductionContext) {
  const plan = buildExecutionPlan(entity, ctx);
  const ramp = plan.decision === "publish" ? [5, 25, 50, 100] : plan.decision === "canary" ? [1, 5] : [];
  return {
    slug: "file-version-history-restore-system",
    plan,
    ramp,
