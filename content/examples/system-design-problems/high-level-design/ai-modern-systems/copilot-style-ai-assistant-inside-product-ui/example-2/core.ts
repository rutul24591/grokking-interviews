...
export type ProductionContext = {
  ...
  errorRate: number;
  approvals: string[];
  metrics: Partial<Record<["tokenLatencyMs","retrievalConfidence","citationCoverage","policySeverity","fallbackRate"][number], number>>;
};
...
export type ExecutionPlan = {
  ...
  nextState: AiInteraction["state"];
  requiredActions: string[];
  telemetry: string[];
  auditTrail: string[];
};

const topic = "Copilot Style AI Assistant Inside Product UI";
const domain = "AI product";
...
export function validateAiInteraction(entity: AiInteraction, ctx: ProductionContext) {
  const errors: string[] = [];
...
export function buildExecutionPlan(entity: AiInteraction, ctx: ProductionContext): ExecutionPlan {
  const validation = validateAiInteraction(entity, ctx);
...
export function simulateSafeRollout(entity: AiInteraction, ctx: ProductionContext) {
  const plan = buildExecutionPlan(entity, ctx);
  const ramp = plan.decision === "publish" ? [5, 25, 50, 100] : plan.decision === "canary" ? [1, 5] : [];
  return {
    slug: "copilot-style-ai-assistant-inside-product-ui",
    plan,
    ramp,
