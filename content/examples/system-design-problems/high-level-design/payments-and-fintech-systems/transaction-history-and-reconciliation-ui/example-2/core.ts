...
export type ProductionContext = {
  ...
  errorRate: number;
  approvals: string[];
  metrics: Partial<Record<["authRate","ledgerDriftCents","riskScore","providerLatencyMs","reconcileAgeMs"][number], number>>;
};
...
export type ExecutionPlan = {
  ...
  nextState: PaymentFlow["state"];
  requiredActions: string[];
  telemetry: string[];
  auditTrail: string[];
};

const topic = "Transaction History AND Reconciliation UI";
...
export function validatePaymentFlow(entity: PaymentFlow, ctx: ProductionContext) {
  const errors: string[] = [];
...
export function buildExecutionPlan(entity: PaymentFlow, ctx: ProductionContext): ExecutionPlan {
  const validation = validatePaymentFlow(entity, ctx);
...
export function simulateSafeRollout(entity: PaymentFlow, ctx: ProductionContext) {
  const plan = buildExecutionPlan(entity, ctx);
  const ramp = plan.decision === "publish" ? [5, 25, 50, 100] : plan.decision === "canary" ? [1, 5] : [];
  return {
    slug: "transaction-history-and-reconciliation-ui",
    plan,
    ramp,
