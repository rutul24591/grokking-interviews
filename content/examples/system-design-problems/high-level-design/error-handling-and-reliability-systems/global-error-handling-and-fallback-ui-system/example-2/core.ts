...
export type ProductionContext = {
  ...
  errorRate: number;
  approvals: string[];
  metrics: Partial<Record<["errorBudgetBurn","retryAttempt","fallbackHitRate","incidentAgeMs","recoveryConfidence"][number], number>>;
};
...
export type ExecutionPlan = {
  ...
  nextState: ReliabilityEvent["state"];
  requiredActions: string[];
  telemetry: string[];
  auditTrail: string[];
};

const topic = "Global Error Handling AND Fallback UI System";
...
export function validateReliabilityEvent(entity: ReliabilityEvent, ctx: ProductionContext) {
  const errors: string[] = [];
...
export function buildExecutionPlan(entity: ReliabilityEvent, ctx: ProductionContext): ExecutionPlan {
  const validation = validateReliabilityEvent(entity, ctx);
...
export function simulateSafeRollout(entity: ReliabilityEvent, ctx: ProductionContext) {
  const plan = buildExecutionPlan(entity, ctx);
  const ramp = plan.decision === "publish" ? [5, 25, 50, 100] : plan.decision === "canary" ? [1, 5] : [];
  return {
    slug: "global-error-handling-and-fallback-ui-system",
    plan,
    ramp,
