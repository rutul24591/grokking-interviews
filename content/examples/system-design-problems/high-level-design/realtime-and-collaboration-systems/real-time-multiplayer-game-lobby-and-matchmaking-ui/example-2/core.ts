...
export type ProductionContext = {
  ...
  errorRate: number;
  approvals: string[];
  metrics: Partial<Record<["operationLagMs","conflictCount","presenceAgeMs","reconnectAttempt","fanoutDepth"][number], number>>;
};
...
export type ExecutionPlan = {
  ...
  nextState: RealtimeSession["state"];
  requiredActions: string[];
  telemetry: string[];
  auditTrail: string[];
};

const topic = "Real Time Multiplayer Game Lobby AND Matchmaking UI";
...
export function validateRealtimeSession(entity: RealtimeSession, ctx: ProductionContext) {
  const errors: string[] = [];
...
export function buildExecutionPlan(entity: RealtimeSession, ctx: ProductionContext): ExecutionPlan {
  const validation = validateRealtimeSession(entity, ctx);
...
export function simulateSafeRollout(entity: RealtimeSession, ctx: ProductionContext) {
