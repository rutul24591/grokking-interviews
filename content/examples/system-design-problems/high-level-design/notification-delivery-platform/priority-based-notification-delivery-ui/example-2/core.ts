...
export type ProductionContext = {
  ...
  errorRate: number;
  approvals: string[];
  metrics: Partial<Record<["providerLatencyMs","quietHourSuppression","dedupeHit","priorityScore","deliveryAttempt"][number], number>>;
};
...
export type ExecutionPlan = {
  ...
  nextState: NotificationFlow["state"];
  requiredActions: string[];
  telemetry: string[];
  auditTrail: string[];
};

const topic = "Priority Based Notification Delivery UI";
const domain = "notification delivery";
...
export function validateNotificationFlow(entity: NotificationFlow, ctx: ProductionContext) {
  const errors: string[] = [];
...
export function buildExecutionPlan(entity: NotificationFlow, ctx: ProductionContext): ExecutionPlan {
  const validation = validateNotificationFlow(entity, ctx);
...
export function simulateSafeRollout(entity: NotificationFlow, ctx: ProductionContext) {
  const plan = buildExecutionPlan(entity, ctx);
  const ramp = plan.decision === "publish" ? [5, 25, 50, 100] : plan.decision === "canary" ? [1, 5] : [];
  return {
    slug: "priority-based-notification-delivery-ui",
    plan,
    ramp,
