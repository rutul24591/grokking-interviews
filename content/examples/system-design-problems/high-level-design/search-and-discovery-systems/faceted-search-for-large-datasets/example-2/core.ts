...
export type ProductionContext = {
  ...
  errorRate: number;
  approvals: string[];
  metrics: Partial<Record<["zeroResultRate","indexLagMs","facetCardinality","rankScore","clickThroughRate"][number], number>>;
};
...
export type ExecutionPlan = {
  ...
  nextState: SearchDecision["state"];
  requiredActions: string[];
  telemetry: string[];
  auditTrail: string[];
};

const topic = "Faceted Search FOR Large Datasets";
...
export function validateSearchDecision(entity: SearchDecision, ctx: ProductionContext) {
  const errors: string[] = [];
...
export function buildExecutionPlan(entity: SearchDecision, ctx: ProductionContext): ExecutionPlan {
  const validation = validateSearchDecision(entity, ctx);
...
export function simulateSafeRollout(entity: SearchDecision, ctx: ProductionContext) {
  const plan = buildExecutionPlan(entity, ctx);
  const ramp = plan.decision === "publish" ? [5, 25, 50, 100] : plan.decision === "canary" ? [1, 5] : [];
  return {
    slug: "faceted-search-for-large-datasets",
    plan,
    ramp,
