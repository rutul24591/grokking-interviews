# CDN Strategy - example-3 Explanation
...
- `.env.example`: Provides supporting example content: NEXT_PUBLIC_PRIMARY_CDN=http://localhost:4001 NEXT_PUBLIC_SECONDARY_CDN=http://localhost:4002 PRIMARY_FAIL_PCT=0.
...
- `lib/circuitBreaker.ts`: Implements the main logic, including KEY, readBreakerState, raw, parsed, openBreaker.
- `lib/env.ts`: Implements the main logic, including EnvSchema, getPublicEnv.
- `lib/fetchWithFailover.ts`: Implements the main logic, including tryFetch, res, buf, fetchWithCdnFailover, breaker.
