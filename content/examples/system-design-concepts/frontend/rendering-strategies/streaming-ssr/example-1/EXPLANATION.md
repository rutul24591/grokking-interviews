# Streaming SSR - example-1 Explanation

## Article context
This example supports the article `frontend/rendering-strategies/streaming-ssr`. The article is about Understand streaming server-side rendering for faster time-to-first-byte and improved perceived performance with progressive content delivery.. The most relevant article sections for this example are: Definition & Context; Core Concepts; Architecture & Flow; Streaming SSR Request Flow; Streaming vs. Traditional SSR; Trade-offs & Limitations; Best Practices; Real-World Use Cases.
...
- `.env.example`: Provides supporting example content: # Express origin API ORIGIN_API=http://localhost:4010.
- `api/server.ts`: Models an API boundary, request handling path, or backend contract.
- `app/_components/RecommendationsPanel.tsx`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
- `app/_components/SidebarPanel.tsx`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
- `app/_components/Skeleton.tsx`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
...
- `lib/fetchJson.ts`: Implements the main logic, including fetchJson, abortController, timeout, res.
