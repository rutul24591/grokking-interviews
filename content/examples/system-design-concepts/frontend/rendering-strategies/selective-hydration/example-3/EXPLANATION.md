# Selective Hydration - example-3 Explanation

## Article context
This example supports the article `frontend/rendering-strategies/selective-hydration`. The article is about Comprehensive guide to Selective Hydration covering concepts, implementation patterns, and best practices for minimizing JavaScript overhead.. The most relevant article sections for this example are: Definition & Context; Core Concepts; Architecture & Flow; Selective Hydration Flow; Trade-offs & Comparisons; Selective vs Progressive vs Partial Hydration; Best Practices; Common Pitfalls; Real-World Use Cases; Case Study: Astro Documentation.
...
- `app/_components/Comments.tsx`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
- `app/_components/Demo.tsx`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
- `app/_components/SearchBox.tsx`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
...
- `lib/clientSuspend.ts`: Implements the main logic, including gates, suspendOnClient, gate.
