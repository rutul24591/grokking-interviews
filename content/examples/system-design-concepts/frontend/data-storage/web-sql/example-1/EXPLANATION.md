# Web SQL (Deprecated) - example-1 Explanation

## Article context
This example supports the article `frontend/data-storage/web-sql`. The article is about Guide to the deprecated Web SQL Database API covering its history, SQL-based transactions, why it was abandoned, and migration to IndexedDB.. The most relevant article sections for this example are: Definition & Context; Core Concepts; openDatabase(); transaction() and readTransaction(); executeSql(); SQL Syntax in the Browser; Version Management; Architecture & Flow; Storage API Comparison; Best Practices: Migration & Detection.
...
- `app/websql-client.tsx`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
