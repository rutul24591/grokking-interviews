# SessionStorage - example-1 Explanation

## Article context
This example supports the article `frontend/data-storage/sessionstorage`. The article is about Deep dive into sessionStorage covering tab-scoped persistence, session lifetime, security properties, and differences from localStorage.. The most relevant article sections for this example are: Definition & Context; Core Concepts; API Surface; Tab Isolation Model; Crash Recovery; Storage Quota; Navigation Persistence; Iframe Scoping Details; Architecture & Flow; Read/Write Path.
...
- `app/wizard-client.tsx`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
