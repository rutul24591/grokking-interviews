# Server-Sent Events (SSE) - example-1 Explanation

## Article context
This example supports the article `frontend/networking-api-communication/server-sent-events`. The article is about Deep dive into Server-Sent Events covering EventSource API, auto-reconnection, event types, Last-Event-ID, streaming over HTTP/2, and comparison with WebSockets.. The most relevant article sections for this example are: Definition & Context; Core Concepts; Architecture & Flow; SSE Connection Lifecycle; Trade-offs & Comparisons; Best Practices; Common Pitfalls; Real-World Use Cases; When NOT to Use SSE; Security Considerations.
...
- `app/sse-board.tsx`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
