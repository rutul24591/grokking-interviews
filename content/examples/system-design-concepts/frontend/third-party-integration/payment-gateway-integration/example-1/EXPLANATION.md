# Payment Gateway Integration - example-1 Explanation

## Article context
This example supports the article `frontend/third-party-integration/payment-gateway-integration`. The article is about End-to-end payment gateway integration design: security boundaries, PCI scope reduction, 3DS/SCA flows, webhook reliability, idempotency, and operational readiness for high-scale frontends.. The most relevant article sections for this example are: Definition & Context; Core Concepts; PCI DSS Compliance; 3DS/SCA Authentication; Webhooks for Async Confirmation; Idempotency; Payment States; Architecture & Flow; Payment Flow Architecture; Error Handling.
...
- `app/payment-workbench.tsx`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
