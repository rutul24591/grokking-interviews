# Script Loading Strategies - example-1 Explanation

## Article context
This example supports the article `frontend/third-party-integration/script-loading-strategies`. The article is about Staff-level guide to third-party script loading: critical path control, security hardening with CSP and SRI, performance budgets, failure isolation, and operational guardrails for large SPAs.. The most relevant article sections for this example are: Definition & Context; Core Concepts; Script Loading Attributes; Critical Rendering Path; Security: Content Security Policy (CSP); Security: Subresource Integrity (SRI); Performance Impact Measurement; Architecture & Flow; Decision Framework; Loading Patterns.
...
- `app/script-loading-lab.tsx`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
