# Error Reporting (Sentry, LogRocket) - example-1 Explanation

## Article context
This example supports the article `frontend/error-handling-monitoring/error-reporting`. The article is about Comprehensive guide to frontend error reporting services including Sentry, LogRocket, and Bugsnag — covering SDK integration, source map uploading, issue grouping, alerting strategies, and production debugging workflows.. The most relevant article sections for this example are: Definition and Context; Core Concepts; SDK Integration Patterns; Error Capture Mechanisms; Issue Grouping and Deduplication; Source Map Integration; Alerting and Triage; Architecture and Flow; Trade-offs and Comparisons; Best Practices.
...
- `app/api/reporting/capture/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/reporting/state/route.ts`: Models an API boundary, request handling path, or backend contract.
