# Controlled vs Uncontrolled Components - example-1 Explanation

## Article context
This example supports the article `frontend/component-design/controlled-vs-uncontrolled-components`. The article is about Staff-level guide to controlled and uncontrolled components: state ownership, form handling, performance trade-offs, and when to use each pattern.. The most relevant article sections for this example are: Definition & Context; Core Concepts; Controlled Components; Uncontrolled Components; Hybrid Approach; When to Use Controlled; When to Use Uncontrolled; Architecture & Flow; Controlled Implementation; Uncontrolled Implementation.
...
- `app/form-strategy-lab.tsx`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
