# Offline-First Architecture - example-1 Explanation

## Article context
This example supports the article `frontend/offline-support/offline-first-architecture`. The article is about Deep dive into offline-first design covering local-first data, sync protocols, conflict resolution, optimistic UI, and architectural patterns for applications that treat offline as the default state.. The most relevant article sections for this example are: Definition & Context; Core Concepts; Architecture & Flow; Layered Architecture; Write Path; Read Path; Trade-offs & Comparisons; Best Practices; Common Pitfalls; Real-World Use Cases.
...
- `app/api/sync/route.ts`: Models an API boundary, request handling path, or backend contract.
...
- `app/ui-client.tsx`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
- `lib/idb.ts`: Implements the main logic, including openDb, req, db, oldVersion, newVersion.
- `lib/store.ts`: Models client or service state transitions and update behavior.
- `lib/sync.ts`: Implements the main logic, including syncOutbox, apiPath, applied, conflicts, items.
