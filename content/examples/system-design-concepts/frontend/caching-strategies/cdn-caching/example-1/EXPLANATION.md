...
## Article context
This example supports the article `frontend/caching-strategies/cdn-caching`. The article is about Comprehensive guide to CDN caching covering edge caching, cache invalidation, cache keys, Vary header, and strategies for global content delivery.. The most relevant article sections for this example are: Definition & Context; Core Concepts; Cache Keys; The Vary Header; Surrogate Keys & Cache Tags; Edge Compute; Cache Hierarchies (L1/L2); Architecture & Flow; Trade-offs & Comparisons; Best Practices.
...
- `app/edge-cache-console.tsx`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
