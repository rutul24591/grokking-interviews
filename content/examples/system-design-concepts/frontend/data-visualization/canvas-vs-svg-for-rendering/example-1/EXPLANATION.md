# Canvas vs SVG for Rendering - example-1 Explanation

## Article context
This example supports the article `frontend/data-visualization/canvas-vs-svg-for-rendering`. The article is about Staff-level guide to Canvas vs SVG: performance characteristics, use case selection, accessibility considerations, and hybrid approaches for data visualization at scale.. The most relevant article sections for this example are: Definition & Context; Core Concepts; Canvas: Raster-Based Rendering; SVG: Vector-Based Rendering; Performance Comparison; Accessibility Considerations; Interactivity Patterns; Architecture & Flow; Canvas Rendering Architecture; SVG Rendering Architecture.
...
- `app/api/rendering/mode/route.ts`: Models an API boundary, request handling path, or backend contract.
- `app/api/rendering/state/route.ts`: Models an API boundary, request handling path, or backend contract.
