# Nested Routes - example-1 Explanation

## Article context
This example supports the article `frontend/routing/nested-routes`. The article is about Understanding nested routing patterns where parent layouts persist while child routes swap, including outlet patterns, layout composition, and parallel route segments.. The most relevant article sections for this example are: Definition and Context; Core Concepts; The Outlet Pattern; Layout Routes; Index Routes; Parallel Routes (Next.js); Architecture and Flow; Route Matching with Nesting; Data Loading per Segment; Error Boundaries per Segment.
...
- `app/nested-routes-lab.tsx`: Models an API boundary, request handling path, or backend contract.
