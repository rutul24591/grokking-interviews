# Hash-Based Routing - example-1 Explanation

## Article context
This example supports the article `frontend/routing/hash-based-routing`. The article is about Understanding hash-based routing using the URL fragment identifier, hashchange events, and why it was the original SPA routing mechanism before the History API.. The most relevant article sections for this example are: Definition and Context; Core Concepts; The Fragment Identifier; The hashchange Event; Hash Routing Patterns; Architecture and Flow; Minimal Hash Router Implementation; Server Configuration; Migration from Hash to History Routing; Trade-offs and Comparisons.
...
- `app/hash-router-lab.tsx`: Models an API boundary, request handling path, or backend contract.
