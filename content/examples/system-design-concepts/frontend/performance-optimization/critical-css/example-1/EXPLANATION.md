# Critical CSS - example-1 Explanation

## Article context
This example supports the article `frontend/performance-optimization/critical-css`. The article is about Comprehensive guide to critical CSS extraction, inlining strategies, and eliminating render-blocking stylesheets for faster first paint.. The most relevant article sections for this example are: Definition & Context; Core Concepts; Render-Blocking CSS; Above-the-Fold Content; CSS Critical Path; Font Loading Considerations; Architecture & Flow; Critical CSS Implementation Patterns; Async Loading Techniques; Trade-offs & Comparison.
...
- `.env.example`: Provides supporting example content: ORIGIN_API=http://localhost:4140.
...
- `app/non-critical-styles.tsx`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
...
- `public/non-critical.css`: Provides supporting example content: body { background: #f8f2e8; color: #1f2937; } .detail-shell { max-width: 72rem; margin: 0 auto; padding: 0 1.5rem 3rem; } .detail-panel { bo.
