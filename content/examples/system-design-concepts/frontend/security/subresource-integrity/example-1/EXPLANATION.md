# Subresource Integrity (SRI) - example-1 Explanation

## Article context
This example supports the article `frontend/security/subresource-integrity`. The article is about Comprehensive guide to Subresource Integrity (SRI), hash-based resource verification, CDN security, and implementation strategies for staff/principal engineer interviews.. The most relevant article sections for this example are: Definition & Context; Key Insight: SRI Is Supply Chain Security; SRI Implementation; Basic SRI Syntax; SRI Attribute Format; Crossorigin Attribute; Generating SRI Hashes; Online Tools; Command Line (OpenSSL); Build Tool Integration.
...
- `server/app.js`: Runs the main scenario and connects the supporting modules into an end-to-end flow.
- `server/package.json`: Provides structured configuration, sample data, schema, or expected output used by the example.
- `server/server.js`: Implements the main logic, including PORT, appJsPath, appJs, sriSha384, hash.
