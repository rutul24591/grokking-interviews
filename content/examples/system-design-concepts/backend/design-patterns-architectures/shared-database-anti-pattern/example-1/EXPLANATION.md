# Shared Database Anti-Pattern - example-1 Explanation

## Article context
This example supports the article `backend/design-patterns-architectures/shared-database-anti-pattern`. The article is about Why letting multiple services share a database creates hidden coupling, the data coupling problems and schema contention it causes, migration strategies including CDC and dual-write, and when sharing is actually acceptable.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Data Coupling Through Shared Persistence; Schema Contention and Ownership Ambiguity; Performance Contention on Shared Resources; Security and Compliance Implications; Architecture and Flow; How Shared Databases Emerge in Practice; Migration Strategies: Escaping the Shared Database; When Sharing Is Acceptable.
