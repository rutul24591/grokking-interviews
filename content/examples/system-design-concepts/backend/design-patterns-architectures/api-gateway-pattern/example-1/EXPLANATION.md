...
## Article context
This example supports the article `backend/design-patterns-architectures/api-gateway-pattern`. The article is about Design an API gateway that centralizes edge concerns (auth, routing, rate limits) without becoming a bottleneck or an accidental monolith.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Routing Strategies; Authentication and Authorization; Rate Limiting and Throttling; Request and Response Transformation; API Versioning; Architecture and Flow; High-Availability Deployment; Gateway and Reverse Proxy Distinction.
