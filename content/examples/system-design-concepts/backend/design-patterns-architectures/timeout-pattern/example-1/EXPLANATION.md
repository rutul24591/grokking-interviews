# Timeout Pattern - example-1 Explanation

## Article context
This example supports the article `backend/design-patterns-architectures/timeout-pattern`. The article is about Bound waiting time with explicit deadlines so slow dependencies cannot consume unbounded resources or collapse tail latency.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Timeout Budgeting Across Call Chains; Adaptive Timeouts; Deadline Propagation in gRPC and Multi-Hop Systems; Tail Latency and Percentile-Based Timeout Selection; Cancellation Propagation; Architecture and Flow; Timeout Layer Design; Deadline Propagation Flow.
