# Horizontal vs Vertical Scaling - example-1 Explanation

## Article context
This example supports the article `backend/fundamentals-building-blocks/horizontal-vs-vertical-scaling`. The article is about Comprehensive guide to scaling strategies covering vertical scaling limits, horizontal scaling patterns, database scaling, autoscaling, and production trade-offs for backend systems.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Architecture and Flow; Scaling Decision Framework; Trade-offs and Comparisons; When to Use Each Strategy; Best Practices; Common Pitfalls; Production Case Studies; Case Study 1: Session Loss During Horizontal Scaling.
...
- `autoscaler.js`: Implements the main logic, including decideScaling, currentCapacity, largest, scaledUp, newInstance.
- `capacity-model.js`: Defines the domain entities and typed structures used across the example.
- `client.js`: Implements the main logic, including instances, scenarios, scenario, decision.
