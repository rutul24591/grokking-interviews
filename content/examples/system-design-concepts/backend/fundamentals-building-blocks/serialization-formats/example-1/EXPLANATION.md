# Serialization Formats - example-1 Explanation

## Article context
This example supports the article `backend/fundamentals-building-blocks/serialization-formats`. The article is about Comprehensive guide to serialization formats covering JSON, Protobuf, Avro, Thrift, MessagePack, and trade-offs for distributed systems.. The most relevant article sections for this example are: Definition & Context; Key Insight: There Is No "Best" Format; Text-Based Formats; JSON (JavaScript Object Notation); XML (Extensible Markup Language); YAML (YAML Ain&apos;t Markup Language); Binary Formats; Protocol Buffers (Protobuf); Apache Avro; Apache Thrift.
...
- `benchmark.js`: Implements the main logic, including sizeReport, json, binary.
- `client.js`: Implements the main logic, including payload.
- `formats.js`: Implements the main logic, including jsonEncode, jsonDecode, binaryEncode, nameBuffer, buffer.
