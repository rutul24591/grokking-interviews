# TCP vs UDP - example-1 Explanation

## Article context
This example supports the article `backend/fundamentals-building-blocks/tcp-vs-udp`. The article is about Comprehensive guide to TCP and UDP protocols covering connection management, reliability mechanisms, congestion control, and production trade-offs for distributed systems.. The most relevant article sections for this example are: Definition & Context; Key Insight: Modern Protocols Blur the Line; Core Concepts; TCP: Connection-Oriented Reliability; UDP: Connectionless Simplicity; Detailed Comparison; When to Use TCP; When to Use UDP; Key Insight: Protocol Choice Is About Failure Modes; Architecture & Flow.
...
- `client.js`: Implements the main logic, including net, dgram, sendTcp, client, sendUdp.
...
- `tcp-server.js`: Implements the main logic, including net, server.
- `udp-server.js`: Implements the main logic, including dgram, server, address.
