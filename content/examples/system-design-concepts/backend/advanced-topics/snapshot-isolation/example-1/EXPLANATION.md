# Snapshot Isolation - example-1 Explanation

## Article context
This example supports the article `backend/advanced-topics/snapshot-isolation`. The article is about Staff-level deep dive into snapshot isolation: MVCC, read consistency, write skew anomaly, comparison with serializability, and production-scale transaction isolation patterns.. The most relevant article sections for this example are: Definition and Context; Core Concepts; Multi-Version Concurrency Control; First-Committer-Wins Rule; Write Skew Anomaly; Architecture and Flow; Snapshot Isolation in PostgreSQL; Serializable Snapshot Isolation; Trade-offs and Comparison; Best Practices.
