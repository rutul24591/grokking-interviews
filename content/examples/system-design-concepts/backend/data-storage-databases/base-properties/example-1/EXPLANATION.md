# BASE Properties - example-1 Explanation

## Article context
This example supports the article `backend/data-storage-databases/base-properties`. The article is about Comprehensive guide to BASE properties: Basically Available, Soft State, Eventually Consistent. Learn when to choose eventual consistency over ACID transactions.. The most relevant article sections for this example are: BASE Properties; Core Concepts: The Three Pillars of BASE; Basically Available: Always On; Soft State: State Can Change Without Input; Eventually Consistent: Convergence Over Time; Architecture and Implementation Flow; Replication Patterns in BASE Systems; Conflict Resolution Mechanisms; Read Repair and Anti-Entropy; Trade-offs and Comparison: BASE vs ACID.
...
- `Example-Eventual-Consistency-Workflow.txt`: Provides supporting example content: // Write locally, replicate asynchronously await db.write({ consistency: "local", value: profile }); // Later: replicas converge and conflic.
- `notes.txt`: Provides supporting example content: Basically Available, Soft state, Eventual consistency..
