# CAP Theorem - example-1 Explanation

## Article context
This example supports the article `backend/data-storage-databases/cap-theorem`. The article is about Comprehensive guide to the CAP theorem: Consistency, Availability, Partition Tolerance. Learn when to choose CP vs AP with real-world examples and trade-off analysis.. The most relevant article sections for this example are: CAP Theorem; Core Concepts: The Three Properties; Consistency: Linearizability; Availability: Every Request Gets a Response; Partition Tolerance: Networks Will Fail; Architecture and Implementation: CP vs AP Systems; CP Systems: Consistency Over Availability; AP Systems: Availability Over Consistency; Tunable Consistency: The Middle Ground; Trade-offs and Comparison: Real-World CAP Decisions.
...
- `Example-CP-vs-AP-Behavior.txt`: Provides supporting example content: // CP-style write: require majority await db.write({ consistency: "quorum", value: data }); // AP-style write: accept local write await db.w.
