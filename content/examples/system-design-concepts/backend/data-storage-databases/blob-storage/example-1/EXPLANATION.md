# Blob Storage - example-1 Explanation

## Article context
This example supports the article `backend/data-storage-databases/blob-storage`. The article is about Comprehensive guide to blob storage: block/page/append blobs, access tiers, snapshots, and when to use Azure Blob Storage or similar services for unstructured data.. The most relevant article sections for this example are: Blob Storage; Core Concepts: Blob Types and Containers; Blob Types; Containers and Organization; Access Tiers; Architecture and Implementation: Specialized Features; Snapshots; Leases; SAS Tokens; CDN Integration.
...
- `Example-Versioning-and-Lifecycle.txt`: Provides supporting example content: await blob.enableVersioning("media"); await blob.setLifecycle({ rule: "archive after 90 days" });.
