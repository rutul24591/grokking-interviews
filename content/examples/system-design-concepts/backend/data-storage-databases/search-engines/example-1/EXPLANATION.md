# Search Engines - example-1 Explanation

## Article context
This example supports the article `backend/data-storage-databases/search-engines`. The article is about Comprehensive guide to search engines: inverted indexes, tokenization, TF-IDF/BM25 ranking, Elasticsearch/Solr, and when to use search engines vs database LIKE queries.. The most relevant article sections for this example are: Search Engines; Core Concepts: Inverted Index and Ranking; Inverted Index; Tokenization and Analysis; Ranking: TF-IDF and BM25; Architecture and Implementation: Elasticsearch and Alternatives; Elasticsearch; Solr; PostgreSQL Full-Text Search; When to Use Search Engine vs Database.
...
- `index.js`: Implements the main logic, including tokenize, buildIndex, index, doc, counts.
...
- `search.js`: Implements the main logic, including search, tokens, scores, token, postings.
