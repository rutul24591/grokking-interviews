# File Systems - example-1 Explanation

## Article context
This example supports the article `backend/data-storage-databases/file-systems`. The article is about Comprehensive guide to file systems: hierarchical namespace, POSIX semantics, NFS/SMB protocols, and when to use EFS or similar services for shared file access.. The most relevant article sections for this example are: File Systems; Core Concepts: Hierarchy and POSIX Semantics; Hierarchical Namespace; POSIX Semantics; File System Types; Architecture and Implementation: Protocols and Cloud Services; NFS (Network File System); SMB/CIFS (Server Message Block); Cloud File Services; Trade-offs and Comparison: File vs Block vs Object.
...
- `Example-File-Access.txt`: Provides supporting example content: const fs = require("fs"); const fd = fs.openSync("/data/report.csv", "r"); const buffer = Buffer.alloc(1024); fs.readSync(fd, buffer, 0, 102.
