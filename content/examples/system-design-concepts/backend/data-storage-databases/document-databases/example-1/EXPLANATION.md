# Document Databases - example-1 Explanation

## Article context
This example supports the article `backend/data-storage-databases/document-databases`. The article is about Comprehensive guide to document databases: data modeling, indexing strategies, sharding patterns, and when to use document stores over relational databases.. The most relevant article sections for this example are: Document Databases; Core Concepts: Document Data Modeling; Embedding vs Referencing; Schema Flexibility; Indexes and Query Patterns; Architecture and Implementation: Sharding and Scaling; Shard Key Selection; Sharding Strategies; Scaling Approaches; Trade-offs and Comparison: Document DB vs Relational.
...
- `documents.json`: Provides structured configuration, sample data, schema, or expected output used by the example.
...
- `queries.js`: Implements the main logic, including data, readEmbeddedOrders, readReferencedOrders, customer.
