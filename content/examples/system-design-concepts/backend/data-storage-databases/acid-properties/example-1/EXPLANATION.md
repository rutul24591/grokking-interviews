# ACID Properties - example-1 Explanation

## Article context
This example supports the article `backend/data-storage-databases/acid-properties`. The article is about Comprehensive guide to ACID transaction properties: Atomicity, Consistency, Isolation, and Durability with real-world trade-offs and implementation patterns.. The most relevant article sections for this example are: ACID Properties; Core Concepts: The Four Pillars of ACID; Atomicity: All or Nothing; Consistency: Preserving Invariants; Isolation: Concurrent Safety; Durability: Surviving Failures; Architecture and Implementation Flow; Write-Ahead Logging: The Foundation of Atomicity and Durability; Lock Management: Enforcing Isolation; MVCC: Isolation Without Locking.
...
- `ledger.js`: Implements the main logic, including accounts, getBalance, applyTransfer, fromBalance, snapshot.
...
- `transaction-manager.js`: Implements the main logic, including locks, wal, acquire, release, record.
