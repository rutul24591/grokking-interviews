# Query Patterns - example-1 Explanation

## Article context
This example supports the article `backend/data-storage-databases/query-patterns`. The article is about Comprehensive guide to database query patterns: master-detail, hierarchical, many-to-many, polymorphic associations, and avoiding anti-patterns like EAV and N+1 queries.. The most relevant article sections for this example are: Query Patterns; Core Concepts: Common Patterns and Anti-Patterns; Master-Detail (Header-Line) Pattern; Hierarchical (Tree) Pattern; Many-to-Many Pattern; Polymorphic Associations; Entity-Attribute-Value (EAV) Anti-Pattern; N+1 Query Anti-Pattern; Architecture and Implementation: Hierarchical Queries and Solutions; Recursive CTEs for Hierarchical Data.
...
- `Example-Recent-Activity-Feed.txt`: Provides supporting example content: SELECT * FROM activities WHERE user_id = $1 ORDER BY created_at DESC LIMIT 50;.
