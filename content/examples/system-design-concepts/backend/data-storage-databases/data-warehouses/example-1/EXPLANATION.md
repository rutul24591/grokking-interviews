# Data Warehouses - example-1 Explanation

## Article context
This example supports the article `backend/data-storage-databases/data-warehouses`. The article is about Comprehensive guide to data warehouses: dimensional modeling (star schema), ETL/ELT pipelines, and when to use data warehouses vs data lakes for BI and analytics.. The most relevant article sections for this example are: Data Warehouses; Core Concepts: Dimensional Modeling and ETL; Dimensional Modeling (Star Schema); ETL vs ELT; Incremental Loading; Architecture and Implementation: Modern Cloud Warehouses; Storage/Compute Separation; Data Sharing; Time Travel; Trade-offs and Comparison: Warehouse vs Lake vs Lakehouse.
...
- `Example-Aggregation-Query.txt`: Provides supporting example content: SELECT region, SUM(revenue) AS total_revenue FROM sales_fact JOIN region_dim USING (region_id) GROUP BY region;.
