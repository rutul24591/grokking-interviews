# Stream Processing - example-1 Explanation

## Article context
This example supports the article `backend/data-processing-analytics/stream-processing`. The article is about Process events continuously with explicit semantics for time, state, and failures so results remain correct under late data and retries.. The most relevant article sections for this example are: Definition and Context; Stream Processing vs Batch Processing; Core Concepts; Architecture and Flow; Trade-offs and Comparison; Best Practices; Common Pitfalls; Real-world Use Cases; Interview Questions; Question 1: What is the difference between event-time and processing-time, and when would you use each?.
