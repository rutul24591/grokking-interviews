...
export const metadata: ArticleMetadata = {
  id: "leetcode-problem-word-search",
  title: "Word Search",
  ...
  tags: ["leetcode","problems","backtracking","dfs"],
  relatedTopics: ["backtracking","dfs"],
};
  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <HighlightBlock as="p" tier="crucial" className="mb-4">
        Word Search is a representative Leetcode interview problem that tests whether the candidate can recognize the right pattern, state the invariant, handle edge cases, and explain the complexity trade-off without drifting into brute force.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
        The interview value is not memorizing a final answer. The value is recognizing that this problem belongs to the backtracking search family and then defending the invariant that makes the optimized solution safe.
      </HighlightBlock>
      <p className="mb-4">
      ...
      <HighlightBlock as="p" tier="crucial" className="mb-4">
        Recognition signal: the problem explores choices under constraints and must undo state after trying each branch.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
        Core invariant: the current path and visited markers represent exactly one partial candidate, and every recursive return restores the previous state.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
        Expected complexity: exponential in branching depth, often O(rows * cols * 4^wordLength) for grid word search. Interviewers usually expect you to state this before optimization details, because it proves you understand the cost envelope.
      </HighlightBlock>
      <p className="mb-4">
      ...
      <HighlightBlock as="p" tier="important" className="mb-4">
        For Word Search, the optimized flow should be framed around the backtracking search decision loop. Each iteration must either consume new input, shrink the search space, settle a subproblem, or advance a traversal frontier.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
      ...
      <HighlightBlock as="p" tier="crucial" className="mb-4">
        Trade-off to defend: backtracking is exponential in the worst case, so pruning and early rejection are the real interview signal.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
      ...
      <HighlightBlock as="p" tier="important" className="mb-4">
        Compared with a more general-purpose approach, the backtracking search solution is more specialized. It is faster or smaller only because the problem constraints provide structure; if those constraints change, the correct technique may change as well.
      </HighlightBlock>
      <p className="mb-4">
      ...
      <HighlightBlock as="p" tier="crucial" className="mb-4">
        Primary failure mode: forgetting to unmark visited cells, exploring invalid branches too late, and sharing mutable path state incorrectly.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
        Edge cases to call out: empty word, repeated letters, reused cells, boundary cells, and boards with many equivalent paths.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
      ...
      <HighlightBlock as="p" tier="important" className="mb-4">
        Word Search maps to production work whenever services must process streams, collections, dependencies, or user-generated structures with predictable memory and latency.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
        The backtracking search pattern appears in ranking pipelines, validation services, deduplication jobs, scheduling systems, compilers, graph-based dependency planners, text processing, and telemetry aggregation.
      </HighlightBlock>
      <p className="mb-4">
      ...
      <h3 className="text-xl font-semibold mt-6 mb-3">How would you solve Word Search, and how do you prove the optimized solution is correct?</h3>
      <HighlightBlock as="p" tier="crucial" className="mb-4">
        A strong answer names the backtracking search pattern, states the invariant, explains the update rule, and then gives complexity plus edge cases.
      </HighlightBlock>
      <p className="mb-4">
        I would first describe the brute-force solution and identify the repeated work. Then I would introduce the optimized state: the current path and visited markers represent exactly one partial candidate, and every recursive return restores the previous state. That invariant is the proof anchor. At every step the algorithm either records information needed later, discards a candidate that cannot contribute, or combines already-solved subproblems.
      </p>
      <HighlightBlock as="p" tier="important" className="mb-4">
      ...
      <p className="mb-4">
        The complexity is exponential in branching depth, often O(rows * cols * 4^wordLength) for grid word search. I would test empty word, repeated letters, reused cells, boundary cells, and boards with many equivalent paths. because those cases usually expose incorrect initialization or update order. If the interviewer changes constraints, I would revisit the trade-off rather than forcing the same pattern.
      </p>
      ...
      <ul className="list-disc pl-6 space-y-2">
        <li>
          <a href="https://leetcode.com/problems/word-search/" target="_blank" rel="noreferrer" className="text-primary underline">
