...
export const metadata: ArticleMetadata = {
  id: "leetcode-problem-lowest-common-ancestor-of-bst",
  ...
  tags: ["leetcode","problems","bst","binary-search-tree","trees"],
  relatedTopics: ["binary-search-tree","trees"],
};

export default function LowestCommonAncestorOfBstArticle() {
  return (
    <ArticleLayout metadata={metadata}>
      <h2 className="text-2xl font-bold mt-8 mb-4">Definition &amp; Context</h2>
      <HighlightBlock as="p" tier="crucial" className="mb-4">
        Lowest Common Ancestor of BST is a representative Leetcode interview problem that tests whether the candidate can recognize the right pattern, state the invariant, handle edge cases, and explain the complexity trade-off without drifting into brute force.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
        The interview value is not memorizing a final answer. The value is recognizing that this problem belongs to the binary search tree invariants family and then defending the invariant that makes the optimized solution safe.
      </HighlightBlock>
      <p className="mb-4">
      ...
      <HighlightBlock as="p" tier="crucial" className="mb-4">
        Recognition signal: the problem uses ordered-tree semantics, ancestor bounds, or in-order sorted traversal.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
        Core invariant: every node must satisfy lower and upper bounds inherited from ancestors, not only direct parent comparisons.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
        Expected complexity: O(n) traversal for validation or kth search, O(h) auxiliary space. Interviewers usually expect you to state this before optimization details, because it proves you understand the cost envelope.
      </HighlightBlock>
      <p className="mb-4">
      ...
      <HighlightBlock as="p" tier="important" className="mb-4">
        For Lowest Common Ancestor of BST, the optimized flow should be framed around the binary search tree invariants decision loop. Each iteration must either consume new input, shrink the search space, settle a subproblem, or advance a traversal frontier.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
      ...
      <HighlightBlock as="p" tier="crucial" className="mb-4">
        Trade-off to defend: recursive bounds are direct and O(h) stack; iterative inorder avoids bound objects but needs previous value tracking.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
      ...
      <HighlightBlock as="p" tier="important" className="mb-4">
        Compared with a more general-purpose approach, the binary search tree invariants solution is more specialized. It is faster or smaller only because the problem constraints provide structure; if those constraints change, the correct technique may change as well.
      </HighlightBlock>
      <p className="mb-4">
      ...
      <HighlightBlock as="p" tier="crucial" className="mb-4">
        Primary failure mode: checking only parent-child relationships, mishandling duplicate policies, and not using long-range numeric bounds.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
        Edge cases to call out: minimum and maximum integer values, duplicates, one-sided trees, and kth values near the ends.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
      ...
      <HighlightBlock as="p" tier="important" className="mb-4">
        Lowest Common Ancestor of BST maps to production work whenever services must process streams, collections, dependencies, or user-generated structures with predictable memory and latency.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
        The binary search tree invariants pattern appears in ranking pipelines, validation services, deduplication jobs, scheduling systems, compilers, graph-based dependency planners, text processing, and telemetry aggregation.
      </HighlightBlock>
      <p className="mb-4">
      ...
      <h3 className="text-xl font-semibold mt-6 mb-3">How would you solve Lowest Common Ancestor of BST, and how do you prove the optimized solution is correct?</h3>
      <HighlightBlock as="p" tier="crucial" className="mb-4">
        A strong answer names the binary search tree invariants pattern, states the invariant, explains the update rule, and then gives complexity plus edge cases.
      </HighlightBlock>
      <p className="mb-4">
        I would first describe the brute-force solution and identify the repeated work. Then I would introduce the optimized state: every node must satisfy lower and upper bounds inherited from ancestors, not only direct parent comparisons. That invariant is the proof anchor. At every step the algorithm either records information needed later, discards a candidate that cannot contribute, or combines already-solved subproblems.
      </p>
      <HighlightBlock as="p" tier="important" className="mb-4">
      ...
      <p className="mb-4">
        The complexity is O(n) traversal for validation or kth search, O(h) auxiliary space. I would test minimum and maximum integer values, duplicates, one-sided trees, and kth values near the ends. because those cases usually expose incorrect initialization or update order. If the interviewer changes constraints, I would revisit the trade-off rather than forcing the same pattern.
      </p>
      ...
      <ul className="list-disc pl-6 space-y-2">
        <li>
          <a href="https://leetcode.com/problems/lowest-common-ancestor-of-a-binary-search-tree/" target="_blank" rel="noreferrer" className="text-primary underline">
