...
export const metadata: ArticleMetadata = {
  id: "leetcode-problem-maximum-subarray",
  ...
  tags: ["leetcode","problems","kadane","dynamic-programming","arrays"],
  relatedTopics: ["dynamic-programming","arrays"],
};

export default function MaximumSubarrayArticle() {
  return (
    <ArticleLayout metadata={metadata}>
      <h2 className="text-2xl font-bold mt-8 mb-4">Definition &amp; Context</h2>
      <HighlightBlock as="p" tier="crucial" className="mb-4">
        Maximum Subarray is a representative Leetcode interview problem that tests whether the candidate can recognize the right pattern, state the invariant, handle edge cases, and explain the complexity trade-off without drifting into brute force.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
        The interview value is not memorizing a final answer. The value is recognizing that this problem belongs to the Kadane dynamic programming family and then defending the invariant that makes the optimized solution safe.
      </HighlightBlock>
      <p className="mb-4">
      ...
      <HighlightBlock as="p" tier="crucial" className="mb-4">
        Recognition signal: the problem asks for the best contiguous subarray where every step chooses between extending the previous run or starting fresh.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
        Core invariant: current best ending here and global best so far are updated in that order; the local state must represent a subarray that actually ends at the current index.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
        Expected complexity: O(n) time and O(1) space. Interviewers usually expect you to state this before optimization details, because it proves you understand the cost envelope.
      </HighlightBlock>
      <p className="mb-4">
      ...
      <HighlightBlock as="p" tier="important" className="mb-4">
        For Maximum Subarray, the optimized flow should be framed around the Kadane dynamic programming decision loop. Each iteration must either consume new input, shrink the search space, settle a subproblem, or advance a traversal frontier.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
      ...
      <HighlightBlock as="p" tier="crucial" className="mb-4">
        Trade-off to defend: Kadane is constant-space and linear, but it depends on contiguity; non-contiguous variants require different DP.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
      ...
      <HighlightBlock as="p" tier="important" className="mb-4">
        Compared with a more general-purpose approach, the Kadane dynamic programming solution is more specialized. It is faster or smaller only because the problem constraints provide structure; if those constraints change, the correct technique may change as well.
      </HighlightBlock>
      <p className="mb-4">
      ...
      <HighlightBlock as="p" tier="crucial" className="mb-4">
        Primary failure mode: initializing to zero and failing all-negative inputs, confusing subsequence with subarray, or not explaining why negative prefixes can be dropped.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
        Edge cases to call out: all-negative arrays, single-element arrays, zeros, and large positive-negative swings.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
      ...
      <HighlightBlock as="p" tier="important" className="mb-4">
        Maximum Subarray maps to production work whenever services must process streams, collections, dependencies, or user-generated structures with predictable memory and latency.
      </HighlightBlock>
      <HighlightBlock as="p" tier="important" className="mb-4">
        The Kadane dynamic programming pattern appears in ranking pipelines, validation services, deduplication jobs, scheduling systems, compilers, graph-based dependency planners, text processing, and telemetry aggregation.
      </HighlightBlock>
      <p className="mb-4">
      ...
      <h3 className="text-xl font-semibold mt-6 mb-3">How would you solve Maximum Subarray, and how do you prove the optimized solution is correct?</h3>
      <HighlightBlock as="p" tier="crucial" className="mb-4">
        A strong answer names the Kadane dynamic programming pattern, states the invariant, explains the update rule, and then gives complexity plus edge cases.
      </HighlightBlock>
      <p className="mb-4">
        I would first describe the brute-force solution and identify the repeated work. Then I would introduce the optimized state: current best ending here and global best so far are updated in that order; the local state must represent a subarray that actually ends at the current index. That invariant is the proof anchor. At every step the algorithm either records information needed later, discards a candidate that cannot contribute, or combines already-solved subproblems.
      </p>
      <HighlightBlock as="p" tier="important" className="mb-4">
      ...
      <p className="mb-4">
        The complexity is O(n) time and O(1) space. I would test all-negative arrays, single-element arrays, zeros, and large positive-negative swings. because those cases usually expose incorrect initialization or update order. If the interviewer changes constraints, I would revisit the trade-off rather than forcing the same pattern.
      </p>
      ...
      <ul className="list-disc pl-6 space-y-2">
        <li>
          <a href="https://leetcode.com/problems/maximum-subarray/" target="_blank" rel="noreferrer" className="text-primary underline">
