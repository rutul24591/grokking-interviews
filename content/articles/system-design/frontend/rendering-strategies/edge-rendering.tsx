    ...
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <HighlightBlock as="p" tier="crucial">
          <strong>Edge Rendering</strong> is a rendering strategy that executes
          server-side rendering (SSR) logic at edge locations—CDN nodes
          distributed globally—rather than on a centralized origin server.
          Instead of routing all requests to a single data center (e.g.,
          us-east-1), edge rendering processes requests at the CDN location
        ...
        <p>
          ...
          server even starts rendering.{" "}
          <strong>Edge Rendering solves this</strong> by running SSR code in
        ...
        <p>
          ...
          (2017), <strong>AWS Lambda@Edge</strong> (2017),{" "}
          <strong>Fastly Compute@Edge</strong> (2019),
          <strong>Vercel Edge Functions</strong> (2021), and{" "}
          <strong>Deno Deploy</strong> (2021). These platforms moved beyond
          simple CDN caching to enable arbitrary code execution at the edge.
          React frameworks like Next.js, Remix, and SvelteKit now support edge
          runtimes, making edge rendering accessible to mainstream developers.
      ...
      <section>
        <h2>Core Concepts</h2>
        <p>
          Understanding Edge Rendering requires grasping several fundamental
          concepts:
        </p>
        <ul>
          <li>
            <strong>Edge Locations:</strong> CDN providers operate 200-300
            globally distributed Points of Presence (PoPs). Cloudflare has 310+
            locations, AWS has 450+ edge locations, Vercel uses Cloudflare{"'"}s
            network. Edge functions deploy to all locations simultaneously via
            one deployment, ensuring every user gets low-latency access.
          </li>
          <li>
            <strong>Request Routing:</strong> When a user requests a page, DNS
            and Anycast routing direct them to the nearest edge location based
            on network proximity (not just geography). The edge node executes
          ...
          <HighlightBlock as="li" tier="important">
            ...
            while maintaining CDN-level latency. This was impossible with static
            CDN caching—you couldn{"'"}t cache per-user pages. Edge rendering
            makes per-user SSR fast.
          </HighlightBlock>
          <li>
            <strong>Hybrid Architecture:</strong> Not all rendering happens at
            the edge. Heavy computation, complex database queries, or legacy
            APIs may still run on origin servers. Edge functions handle fast,
            lightweight rendering (shell, above-the-fold content) and proxy slow
            operations to origin.
          </li>
        </ul>
      </section>
      ...
      <section>
        ...
        <p>
          The Edge Rendering architecture follows this distributed
        ...
        <div className="my-6 rounded-lg bg-panel-soft p-6">
          ...
          <ol className="space-y-3">
            <li>
              <strong>1. User Request:</strong> User in Tokyo requests{" "}
              <code>example.com/products</code>
            </li>
            <li>
              <strong>2. DNS + Anycast Routing:</strong> Request routes to
              nearest edge location (Tokyo or Singapore, 10-20ms latency)
            </li>
            <li>
              <strong>3. Edge Function Execution:</strong> Edge worker (V8
              isolate) starts executing rendering code (cold start: 0-5ms)
            </li>
            <li>
              <strong>4. Check Edge Cache:</strong> Query KV store for cached
              data (products list, user preferences)
            </li>
            <li>
              <strong>5. Fetch Data (if needed):</strong> Call nearby database
              (Cloudflare D1, Supabase in ap-southeast-1) or origin API
            </li>
            <li>
              <strong>6. Render HTML:</strong> Execute React SSR at the edge,
              generate HTML with user-specific data
            </li>
            <li>
              <strong>7. Stream to User:</strong> Stream HTML chunks
              progressively (using Web Streams API)
            </li>
            <li>
              <strong>8. TTFB:</strong> User receives first byte in 30-80ms (vs.
              200-400ms origin SSR)
            </li>
            <li>
              <strong>9. Cache Response:</strong> Store rendered HTML in edge
              cache (if cacheable) with short TTL
            </li>
            <li>
              <strong>10. Browser Hydration:</strong> JavaScript downloads and
        ...
        <HighlightBlock as="p" tier="crucial">
          This architecture eliminates the geography tax of centralized servers.
          Whether users are in New York, London, Tokyo, or Sydney, they all
        ...
        <ArticleImage
          src="/diagrams/system-design-concepts/frontend/rendering-strategies/edge-rendering-flow.svg"
      ...
      <section>
        ...
        <ArticleImage
          src="/diagrams/system-design-concepts/frontend/rendering-strategies/edge-rendering-performance.svg"
      ...
      <section>
        ...
        <ul>
          ...
          <li>
            ...
            auto-scaling groups, or capacity planning. Built-in DDoS protection
            and high availability.
          </li>
          <li>
            <strong>Fast Cold Starts:</strong> V8 isolates start in 0-5ms (vs.
          ...
          <li>
            ...
            Cloudflare Workers: $0.50/million requests. Vercel Edge: $2/million
            requests. No idle server costs.
          </li>
        </ul>
        ...
        <ul>
          <li>
            <strong>Runtime Restrictions:</strong> No Node.js standard library
            (fs, child_process, etc.). Limited to Web APIs (fetch, Response,
            Request). Many npm packages don{"'"}t work. Must use edge-compatible
            libraries.
          </li>
          <li>
            <strong>Small Bundle Size Limits:</strong> Cloudflare Workers: 1-5MB
            compressed. Vercel Edge: 1MB. Large frameworks or heavy dependencies
            may not fit. Requires aggressive tree-shaking and code-splitting.
          </li>
          <li>
            <strong>Limited Execution Time:</strong> Cloudflare: 50ms CPU time.
            Vercel: 25s wall time. Heavy computations, large database queries,
          ...
          <li>
            ...
            in production. Testing edge-specific behavior locally is
            challenging.
          </li>
          <li>
            <strong>Vendor Lock-In:</strong> Edge platforms have different APIs
            (Cloudflare KV, Vercel Edge Config, Deno KV). Migrating between
            platforms requires code changes. Less portability than containerized
            SSR.
          </li>
        </ul>
      </section>
      ...
      <section>
        ...
        <p>
          To build performant applications with Edge Rendering, follow these
          practices:
        </p>
        ...
        <p>
          ...
          and minimal dependencies. Avoid large libraries (moment.js, lodash—use
          date-fns, lodash-es instead). Measure bundle size with{" "}
          <code>@vercel/ncc</code> or Cloudflare wrangler CLI. Stay under 500KB
          compressed.
        </p>
        ...
        <p>
          ...
          cache hit rates, and error rates per edge location. Use Real User
          Monitoring (RUM) to see actual user experiences. Identify slow regions
          or cache misses.
        </p>

        <p>
          <strong>Hybrid Architecture:</strong> Not everything needs edge
          rendering. Use edge for above-the-fold content (shell, hero,
          navigation) and origin SSR for below-the-fold or slow operations
          (comments, recommendations). Combine edge with streaming SSR for best
          results.
        </p>

        <p>
          <strong>Test Edge-Specific Behaviors:</strong> Use{" "}
          <code>miniflare</code> (Cloudflare Workers local simulator) or Vercel
          Edge runtime locally. Test timeouts, memory limits, and API
          restrictions before deploying. Edge environments differ from Node.js.
        </p>
      </section>
      ...
      <section>
        ...
        <ul>
          ...
          <li>
            ...
            hero images, CTAs, or pricing based on user attributes (location,
          ...
          <li>
            ...
            load by handling requests at the edge. Used for GraphQL gateways,
            REST API aggregation.
          </li>
        </ul>

        <p>
          Edge Rendering is <strong>not ideal</strong> for computation-heavy
          tasks (video encoding, data processing), applications requiring large
          dependencies (full Next.js, heavyweight ORMs), or pages with complex
          backend integrations (legacy monoliths, slow databases without
          replication).
        </p>
      </section>
      ...
      <section>
        ...
        <ul>
          ...
          <li>
            ...
            Advanced caching and VCL scripting. Used by Vimeo, Stripe.
          </li>
          <li>
            <strong>Netlify Edge Functions:</strong> Built on Deno, global edge
            network. Seamless Netlify integration. $20/month for 2 million
            requests. Used by Netlify customers, Jamstack sites.
          </li>
        </ul>
      </section>
      ...
      <section>
        ...
        <ul>
          ...
          <li>
            ...
            - Educational resource on edge computing concepts
          </li>
          <li>
            <a
            ...
            - When to use edge vs Node.js runtime in Next.js
          </li>
        </ul>
      </section>
    </ArticleLayout>
