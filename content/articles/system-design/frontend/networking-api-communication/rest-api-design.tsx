    ...
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>
          ...
          or standard but a set of constraints that, when applied to the design
          of a system, create a scalable, performant, and maintainable
        ...
        <p>
          ...
          each to evolve independently. Second,
          <strong>statelessness</strong> requires that each request from client
          to server must contain all the information necessary to understand and
          process the request; the server holds no session state between calls.
          Third,
          <strong>cacheability</strong> demands that responses explicitly label
          themselves as cacheable or non-cacheable, enabling clients and
          intermediaries to reuse responses and reduce latency. Fourth, a{" "}
          <strong>uniform interface</strong> simplifies the architecture by
          applying four sub-constraints: identification of resources via URIs,
          manipulation of resources through representations, self-descriptive
          messages, and hypermedia as the engine of application state (HATEOAS).
          Fifth, a <strong>layered system</strong> allows intermediaries
          (proxies, gateways, CDNs) to be inserted between client and server
          without the client needing to know. Sixth,{" "}
        ...
        <p>
          ...
          single URI and POST method for everything (RPC-style). Level 1
          introduces individual resources with distinct URIs but still uses a
          single HTTP method. Level 2 adds correct use of HTTP verbs (GET, POST,
          PUT, DELETE) with proper status codes, and this is where the vast
          majority of production APIs land. Level 3 adds HATEOAS, where
          responses include hyperlinks that guide the client through valid state
          transitions. Virtually no public API reaches true Level 3. The
          persistent debate around whether an API is truly "RESTful" versus
          merely "REST-like" or "HTTP-based" stems from the fact that most APIs
          claiming to be RESTful only satisfy Levels 1 and 2. Fielding himself
          has argued that an API without HATEOAS should not be called REST, but
      ...
      <section>
        ...
        <ul>
          <li>
            ...
            /createUser. Collections use plural nouns (/users), individual
            resources use identifiers (/users/42), and nested resources express
            relationships (/users/42/orders). This noun-based approach means the
            same URI supports multiple operations differentiated by HTTP method,
            keeping the API surface predictable and discoverable. At scale,
          ...
          <li>
            ...
            it twice creates two resources. PUT replaces a resource entirely and
            is idempotent; calling it multiple times with the same payload
            produces the same result. PATCH partially updates a resource and,
            depending on implementation, may or may not be idempotent. DELETE
            removes a resource and is idempotent; deleting the same resource
            twice results in the same final state. Understanding idempotency is
          ...
          <li>
            ...
            creation (with a Location header pointing to the new resource), 204
            No Content for successful deletion with no response body. The 3xx
            family handles redirection: 301 Moved Permanently, 304 Not Modified
            for conditional requests. The 4xx family indicates client errors:
            400 Bad Request for malformed input, 401 Unauthorized for missing
            authentication, 403 Forbidden for insufficient permissions, 404 Not
            Found, 409 Conflict for state conflicts, 422 Unprocessable Entity
            for validation failures, and 429 Too Many Requests for rate
            limiting. The 5xx family signals server errors: 500 Internal Server
          ...
          <li>
            ...
            format of the request body. JSON has become the de facto standard
            for REST APIs, but content negotiation allows APIs to support
            multiple formats without changing URIs. Advanced negotiation
            includes Accept-Language for internationalization and
            Accept-Encoding for compression (gzip, br). From the frontend,
          ...
          <li>
            ...
            In a truly RESTful API, responses include links that describe what
            actions the client can take next. For example, a response for an
            order might include links to cancel, pay, or view the associated
            user. The client does not hardcode URI patterns but follows links
            dynamically. In practice, very few APIs implement full HATEOAS.
            Notable exceptions include the GitHub API, which includes pagination
            links and resource relationship URLs. For frontend developers, even
          ...
          <li>
            ...
            desired representations. The server maintains no session between
            requests. This constraint is what makes REST APIs horizontally
            scalable: any server instance can handle any request because there
            is no session affinity requirement. For the frontend, statelessness
            means the client is responsible for managing its own state, tracking
            auth tokens, maintaining pagination cursors, and reconstructing full
      ...
      <section>
        ...
        <p>
          ...
          filtering, sorting, or pagination. Next, the client sets request
          headers: Authorization for authentication (typically a Bearer token),
          Content-Type to describe the request body format, Accept to specify
          the desired response format, and optionally If-None-Match or
          If-Modified-Since for conditional requests that leverage caching. The
        ...
        <p>
          ...
          extracts the response body (typically JSON), parses it, and normalizes
          the data into the client-side state shape. For error responses, it
          parses the error body to extract machine-readable error codes and
          human-readable messages, then routes to appropriate error handling:
          retry for 429 (respecting Retry-After header), re-authenticate for
          401, show validation errors for 422, or display generic error UI for
          500. Response headers are also important: ETag and Last-Modified for
        ...
        <p>
          ...
          is the simplest: the server skips offset rows and returns limit rows.
          It supports random access (jump to page 5) but suffers from data drift
          (items shifting between pages as new data is inserted) and degrades at
          high offsets because the database must scan and discard all preceding
          rows. <strong>Cursor-based pagination</strong> (after=abc123) uses an
          opaque cursor (often a base64-encoded identifier) to mark the position
          in the dataset. It is consistent under concurrent writes, performant
          at any depth, and is the standard for social feeds and real-time data.
          However, it does not support random page access.{" "}
          <strong>Keyset pagination</strong> (where id &gt; 100 order by id
          limit 20) is similar to cursor-based but uses actual column values
          rather than opaque tokens. It is the most performant approach when the
          sort key is indexed but requires the client to track the last-seen
          sort value. For frontend engineers, cursor-based pagination is
        ...
        <p>
          ...
          route, and obvious in documentation, but it duplicates endpoints.
          Header versioning (Accept: application/vnd.api+json;version=2) keeps
          URLs clean but is harder to test and less visible. Query parameter
          versioning (?version=2) is a middle ground. Stripe popularized
          date-based versioning where clients pin to an API version date and the
          server handles compatibility transformations internally; this is
        ...
        <table className="w-full border-collapse">
          ...
          <tbody className="divide-y divide-theme">
            <tr>
              ...
              <td className="p-3">
                • Uses existing HTTP infrastructure natively
                <br />
                • Well-understood by all developers
                <br />• Minimal tooling required to get started
              </td>
              <td className="p-3">
                • No formal specification (unlike GraphQL SDL)
                <br />
            ...
            <tr>
              ...
              <td className="p-3">
                • Leverages HTTP caching natively (ETags, Cache-Control)
                <br />
                • CDN-friendly with deterministic URLs
                <br />• Browser cache works out of the box for GET
              </td>
              <td className="p-3">
                • POST/PUT/PATCH bypass caching entirely
                <br />
            ...
            <tr>
              ...
              <td className="p-3">
                • Sparse fieldsets (?fields=id,name) reduce payload
                <br />
                • Resource embedding (?include=author) reduces round trips
                <br />• Predictable response shapes per endpoint
              </td>
              <td className="p-3">
                • Fixed response shapes cause over-fetching by default
                <br />
            ...
            <tr>
              ...
              <td className="p-3">
                • Simple polling with conditional GET (304 responses)
                <br />
                • Long-polling works within HTTP semantics
                <br />• Server-Sent Events for one-way streaming
              </td>
              <td className="p-3">
                • No built-in subscription model
                <br />
                • Polling is wasteful for high-frequency updates
                <br />• WebSockets needed for true bidirectional real-time
              </td>
            </tr>
            <tr>
              <td className="p-3">
              ...
              <td className="p-3">
                • Massive ecosystem (Postman, Swagger, OpenAPI)
                <br />
                • Auto-generated client SDKs from OpenAPI specs
                <br />• Universal debugging with browser DevTools
              </td>
              <td className="p-3">
                • No intrinsic type system (unlike GraphQL schema)
                <br />
                • Documentation can drift from implementation
                <br />• No built-in query language for complex filtering
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <section>
        ...
        <ol className="space-y-3">
          <li>
            ...
            OPTIONS for CORS preflight understanding. The HTTP method
          ...
          <li>
            ...
            Default to cursor-based pagination for any dataset that grows over
            time. Use offset pagination only for small, static datasets where
            users need random page access (paginated admin tables). Always
          ...
          <li>
            ...
            (/v1/users) is easier to test, document, route, and cache. It makes
            API version visible in logs and debugging tools. Reserve header or
          ...
          <li>
            ...
            This prevents data duplication across different views, simplifies
            cache updates when a single entity changes, and enables efficient
          ...
          <li>
            ...
            mutex to prevent thundering herd, logging request timing,
          ...
          <li>
            ...
            and backend agree on shapes. Use tools like openapi-typescript or
          ...
          <li>
            ...
            per-item error details. The frontend must process each item
      ...
      <section>
        ...
        <ul className="space-y-3">
          ...
          <li>
            ...
            generate a unique idempotency key (UUID) per user action and include
          ...
          <li>
            ...
            profiles). Solve this by requesting embedded resources
          ...
          <li>
            ...
            &quot;errors&quot;: [...]{"}"}, others with
            {"{"}&quot;detail&quot;: &quot;...&quot;{"}"}), the frontend cannot
            write generic error handling. Advocate for a standard error envelope
          ...
          <li>
            ...
            Modified responses that carry no body. For mobile-first
      ...
      <section>
        ...
        <ul className="space-y-3">
          ...
          <li>
            ...
            override per-request. The server internally transforms between
          ...
          <li>
            ...
            Their cursor-based pagination with next_token handles the immense
          ...
          <li>
            ...
            use case. Their REST API uses cursor-based pagination (page_info
            parameter) after deprecating offset-based pagination at scale,
            illustrating the practical limits of offset pagination for large
            merchant catalogs.
          </li>
        </ul>
        ...
        <div className="mt-6 rounded-lg border border-theme bg-panel-soft p-6">
          ...
          <ul className="mt-2 space-y-2">
            ...
            <li>
              ...
              not display. GraphQL or sparse fieldset REST extensions are
              better.
            </li>
            <li>
              • <strong>Server-to-server streaming pipelines:</strong> gRPC with
              Protocol Buffers offers binary encoding, streaming, and code
              generation that REST/JSON cannot match for internal microservice
              communication.
            </li>
          </ul>
        </div>
        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            ...
            <p className="mt-2 text-sm">
              ...
              hypermedia links that guide the client through valid state
              transitions, so the client discovers available actions dynamically
              rather than hardcoding URL patterns. In practice, almost no
              production API reaches Level 3. Most APIs described as
              &quot;RESTful&quot; are actually Level 2: they use
              resource-oriented URLs, proper HTTP methods, and meaningful status
              codes, but responses are static data without navigational links.
              The distinction matters architecturally because true HATEOAS
              decouples clients from server URL structures, allowing the server
              to evolve without breaking clients. However, the industry has
          ...
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            ...
            <p className="mt-2 text-sm">
              ...
              making deep pages increasingly expensive. With cursor-based
              pagination, each response includes an opaque cursor (typically the
              encoded ID or timestamp of the last item) and the client passes it
              as an &quot;after&quot; parameter for the next page. The server
              uses the cursor as a WHERE clause (WHERE created_at &lt;
              cursor_timestamp) with an indexed column, making every page
              equally fast regardless of depth. For real-time feeds where new
              items are inserted continuously, cursor-based pagination also
              prevents the &quot;shifting window&quot; problem where items
              appear on multiple pages. The response should include hasNextPage,
              endCursor, and optionally totalCount (only if computable without a
              full table scan). On the frontend, implement infinite scroll by
          ...
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            ...
            <p className="mt-2 text-sm">
              ...
              layer, browser cache), when the team values simplicity and
              convention over flexibility, or when the API is public-facing and
              must be universally accessible. Choose GraphQL when the frontend
              needs to aggregate data from multiple resources in a single
              request, when different clients (web, mobile, TV) need different
              fields from the same data, when the data model is highly
              relational with deep nesting, or when rapid frontend iteration
              matters more than caching simplicity. In practice, many
              organizations use both: REST for simple CRUD microservices and
              public APIs, GraphQL as a BFF (Backend for Frontend) that
              aggregates multiple REST services into a single flexible endpoint.
              The worst choice is picking GraphQL for a simple CRUD app
