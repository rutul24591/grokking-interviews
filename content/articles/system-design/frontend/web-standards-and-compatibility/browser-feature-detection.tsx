    ...
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <p>
          ...
          capabilities based on the user agent string (browser sniffing), feature
          detection directly probes the runtime environment — checking whether an
          object exists, a method is callable, a CSS property is recognized, or a
          media query evaluates correctly. This approach yields reliable,
          future-proof compatibility decisions because it tests actual
        ...
        <p>
          ...
          and they provide no information about which specific features a browser
          version supports. Modernizr, released in 2009, popularized systematic
          feature detection by providing a comprehensive library of tests that
          probed for specific HTML5 and CSS3 features and exposed results as CSS
          classes and JavaScript properties.
        </p>
        <p>
          ...
          infrastructure (rather than ad hoc checks sprinkled through components)
          build more maintainable, adaptable frontend architectures.
        </p>
        <p>
          ...
          platform capabilities (service worker support, Web Workers, WebAssembly
          support). This expanded scope makes centralized, well-architected
          detection services essential — the alternative is a proliferation of
          inconsistent, duplicated checks that create maintenance burden and
          testing complexity.
        </p>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-3">
          ...
          <li>
            ...
            grid) applies styles only in browsers that support CSS Grid.
            In JavaScript, CSS.supports() provides the same detection
            capability programmatically. Feature queries are the CSS equivalent
            of JavaScript API probing and handle visual degradation without
          ...
          <li>
            ...
            prefers-contrast detect user accessibility and aesthetic
            preferences set at the operating system level. These are not browser
            capabilities in the traditional sense — they are user-declared
            preferences that applications should respect. Detection via
            matchMedia in JavaScript or @media in CSS allows adapting the
            experience to the user&apos;s stated needs.
          </li>
          <li>
            <strong>Asynchronous Detection:</strong> Some capabilities cannot
            be detected synchronously. Permission-gated APIs (camera,
            microphone, geolocation), storage quotas (IndexedDB available
            space), and codec support (via MediaSource.isTypeSupported or
            canPlayType) require asynchronous probing. The detection
            architecture must accommodate async detection without blocking
            initial render, typically by rendering a loading state or default
            variant while detection completes and then updating if needed.
          </li>
          <li>
      ...
      <section>
        ...
        <p>
          ...
          into a support tier (full, standard, basic, minimal). The profile
          is exposed to the component tree via a React context provider (or
          equivalent framework mechanism), making detection results available
          to any component without prop drilling. Components query the
          capability context to decide which variant to render. The service
          also emits the capability profile to analytics, creating visibility
          into the capability distribution of the actual user population.
        </p>
        ...
        <p>
          ...
          hardware acceleration. If not, does it support Canvas 2D? If yes,
          use the canvas-based renderer with software rendering. If not, does
          it support inline SVG? If yes, use the SVG renderer with DOM-based
          ...
          component implementation, and the tree is traversed once at mount
          time with results cached for the component&apos;s lifecycle.
        </p>
        ...
        <p>
          ...
          preference changes, viewport-dependent capabilities), JavaScript&apos;s
          matchMedia API provides both initial state and change listeners via
          the addEventListener method on MediaQueryList objects. This enables
          responsive detection — when a user changes their system color scheme
          preference, the matchMedia listener fires and the application adapts
          in real time without a page reload. The combination of CSS @supports
          for static capabilities and matchMedia for dynamic preferences
          provides comprehensive detection coverage across both visual and
          ...
          <tbody>
            ...
            <tr>
              ...
              <td className="border border-theme p-2">
                ...
                rendering logic. Asynchronous detection handles
                permission-gated and resource-dependent capabilities.
              </td>
              <td className="border border-theme p-2">
                Synchronous detection cannot cover all capabilities. Asynchronous
                detection introduces loading states and potential layout shifts
                as results arrive after initial render.
              </td>
            </tr>
            <tr>
              <td className="border border-theme p-2">
            ...
            <tr>
              ...
              <td className="border border-theme p-2">
                ...
                detection suites. They handle edge cases and browser quirks
                that native checks might miss.
              </td>
              <td className="border border-theme p-2">
      ...
      <section>
        ...
        <ol className="space-y-3">
          <li>
            <strong>Always detect the specific feature you intend to use:</strong>{" "}
          ...
          <li>
            <strong>Centralize detection in a capability service for applications with multiple detection needs:</strong>{" "}
          ...
          <li>
            <strong>Use CSS @supports for visual-only detection:</strong>{" "}
            When the detection decision only affects visual presentation (layout
          ...
          <li>
            ...
            detection results in module-level variables or a detection store
            to avoid repeatedly creating test elements, calling CSS.supports(),
            or checking for API existence. For expensive detection tests
            (WebGL context creation, canvas performance benchmarks), consider
            caching results in sessionStorage for faster detection on
            subsequent page loads.
          </li>
          <li>
            <strong>Report detection results to analytics:</strong>{" "}
            Instrument the capability service to emit detection results as
            analytics events. This creates a real-time view of the capability
            distribution of your actual user population — far more actionable
            than caniuse.com statistics. Use this data to drive browserslist
            updates, compatibility contract reviews, and polyfill investment
            decisions.
          </li>
          <li>
            <strong>Test detection logic with capability mocking:</strong>{" "}
            Unit tests for detection-dependent components should mock the
            capability service to simulate different environments. Create test
            fixtures for each supported capability tier (full, standard, basic,
            minimal) and verify that components render the correct variant at
            each tier. Integration tests should verify the detection service
            itself by selectively disabling APIs in the test environment.
          </li>
          <li>
            <strong>Handle detection failure gracefully:</strong>{" "}
            Detection probes themselves can fail — trying to create a WebGL
            context might throw an exception on devices with exhausted GPU
            memory. Wrap detection probes in try-catch blocks and treat
            detection failure as feature absence. Never let a detection error
            crash the application or prevent rendering entirely.
          </li>
        </ol>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-3">
          <li>
            ...
            A browser may expose an API (the property exists, the constructor
            is defined) but implement it incorrectly or incompletely. Early
            ...
            critical, detection tests must go beyond existence checks and
            verify behavior — calling the API with known inputs and checking
            the output. This is more expensive but necessary for APIs with a
            history of inconsistent implementations.
          </li>
          <li>
            <strong>Running expensive detection synchronously in the critical path:</strong>{" "}
          ...
          <li>
            <strong>Feature inference chains:</strong>{" "}
            Assuming that because a browser supports feature A, it must also
            support features B, C, and D. Browser vendors implement features on
            their own schedules — Safari might support CSS Grid but not
            Container Queries, while Firefox supports both. Each capability
            must be independently detected. Feature inference was the root cause
            of many Modernizr-era bugs where libraries made chain assumptions
            that broke on specific browser-version combinations.
          </li>
          <li>
            <strong>Not accounting for feature being present but disabled:</strong>{" "}
            ...
            firewalls. Detection must account for these scenarios — typically
            by trying to use the feature in a safe way and catching the
            resulting error, rather than just checking for the API&apos;s
            existence.
          </li>
          <li>
            <strong>Duplicating detection logic across components:</strong>{" "}
            When multiple components independently check for the same feature,
          ...
          <li>
            ...
            Server-rendered markup has no browser environment — feature detection
            APIs are unavailable. Components that depend on detection must either
            render a universal default during SSR (to be hydrated with the
            detected variant on the client) or defer rendering entirely until
            client-side detection completes. Mismatched SSR output and client
            hydration output causes hydration errors and visual flicker.
          </li>
        </ul>
      </section>
      ...
      <section>
        ...
        <p>
          ...
          detects WebGL support and GPU capabilities to determine whether to
          use its WebGL-based rendering engine (for hardware-accelerated vector
        ...
        <p>
          ...
          initial resources and uses those measurements as a proxy for connection
          quality. This adaptive approach reduces data usage and improves
          perceived performance for users on constrained networks.
        </p>
        <p>
          <strong>Notion&apos;s editor input detection:</strong> Notion
          detects input method capabilities to adapt its editor behavior.
          Touch input detection determines whether to show touch-optimized
          drag handles and mobile-friendly block menus. Keyboard layout
          detection (via the Keyboard API where available) influences shortcut
          hints and modifier key labels. Pointer precision detection (via
          pointer media queries) adjusts hit target sizes for stylus, finger,
          and mouse input. These detection results create an editor experience
          that feels native across laptops, tablets, and phones without
        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            ...
            <p className="mt-2 text-sm">
              ...
              identifiers), increasingly frozen by browser vendors via User-Agent
              Client Hints, and they provide no granularity about which specific
              features a version supports. Feature detection works with future
              unknown browsers, handles incremental feature adoption across
              browser versions, and produces correct results regardless of UA
              string manipulation. The only scenario where UA analysis adds value
              is detecting known browser bugs where the feature exists but
              behaves incorrectly — and even then, behavioral feature testing is
          ...
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            ...
            <p className="mt-2 text-sm">
              ...
              service caches results in memory for the session and optionally
              in sessionStorage for faster initialization on subsequent loads.
          ...
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            ...
            <p className="mt-2 text-sm">
              ...
              is impossible. The SSR pass should render a universal default — the
              most broadly compatible variant of each detection-dependent
          ...
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            ...
            <p className="mt-2 text-sm">
              ...
              support including prefixed variants, or checking for nuanced
              HTML5 feature availability. For modern APIs (IntersectionObserver,
              ResizeObserver, CSS Grid, Custom Properties), simple existence
              checks are sufficient and do not justify a library dependency.
              If you need Modernizr, use its custom build tool to include only
              the specific tests you require — the full library includes
              hundreds of tests, most of which are irrelevant to any single
              application. In most modern applications, native detection covers
              all needs, and Modernizr is reserved for edge cases involving
              legacy features or complex detection logic that would be error-prone
              to reimplement.
            </p>
          </div>
        </div>
