  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <p>
          It is also important to know <em>who</em> consumes the signal. Kubernetes uses probes to restart or remove pods.
          Load balancers use checks to route traffic. Service discovery systems use checks to publish or withdraw endpoints.
          Synthetic monitors validate the user journey. These consumers have different failure consequences, so a single
          health endpoint should not automatically drive every decision without clear intent.
        </p>
        <ArticleImage
          src="/diagrams/system-design-concepts/backend/reliability-fault-tolerance/heartbeat-table-health-check.png"
      ...
      <section>
        ...
        <p>
          Make timeouts explicit. A probe that hangs can be worse than a probe that fails quickly, because it can exhaust
          threads or connections. Keep probe logic isolated from the main request path where possible, and avoid probes
          that depend on optional dependencies. If a dependency check is required, keep it minimal and cached, and define
          what happens when the check cannot be performed due to local resource exhaustion.
        </p>
      </section>
      ...
      <section>
        ...
        <p>
          Probe storms are another failure mode. When probes are too frequent or too expensive, the health check traffic
          becomes a meaningful share of load and can push a marginal system into failure. Under incident conditions,
          orchestrators and load balancers may increase churn, which increases probe rates further. This feedback loop is
          why probes must be cheap and why hysteresis and cooldown windows matter.
        </p>
        <ArticleImage
          src="/diagrams/system-design-concepts/backend/reliability-fault-tolerance/failover-route53-health-check-weighted.png"
      ...
      <section>
        ...
        <ArticleImage
          src="/diagrams/system-design-concepts/backend/reliability-fault-tolerance/failover-route53-latency-alias-weighted.png"
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Ejection rate:</strong> how often instances are removed from rotation and why.</li>
          <li><strong>Probe latency and error rate:</strong> slow or failing probes indicate the probe itself may be too expensive.</li>
          <li><strong>Flapping:</strong> repeated ejection and re-admission that signals thresholds are too aggressive.</li>
          <li><strong>Churn during deploys:</strong> increased probe failures during rolling updates often indicates readiness is wrong.</li>
          <li><strong>User impact correlation:</strong> whether probe failures correlate with real error budget burn and user-visible errors.</li>
        </ul>
      </section>

      <section>
        <h2>Runbook: Flapping and Probe Storms</h2>
        <p>
          When health checks behave badly, they can turn a small issue into a capacity collapse. Two common incident
          signatures are <strong>flapping</strong> (instances repeatedly ejected and re-admitted) and <strong>probe storms</strong>{" "}
          (probe traffic becomes large enough to matter). The safest response is to stabilize routing first, then fix the
          underlying signal.
        </p>
        <ol className="space-y-2">
          <li>Confirm whether ejections correlate with real user errors or are false positives.</li>
          <li>Reduce sensitivity: increase failure thresholds, add cooldowns, and enforce hysteresis.</li>
          <li>Protect capacity: slow down restarts and limit how many instances can be ejected at once.</li>
          <li>Make probes cheaper: remove heavy dependency calls or cache dependency status briefly.</li>
          <li>Validate recovery: ensure the system re-stabilizes before restoring aggressive checks.</li>
        </ol>
      </section>
      ...
      <section>
        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            <p className="font-semibold">Q: What is the difference between liveness and readiness?</p>
            <p className="mt-2 text-sm text-muted">
              A: Liveness answers “is the process alive?” and is usually shallow. Readiness answers “can this instance
              safely serve traffic?” and can include critical dependency checks and initialization gates.
            </p>
          </div>
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            <p className="font-semibold">Q: How can health checks cause outages?</p>
            <p className="mt-2 text-sm text-muted">
              A: By ejecting healthy nodes (false negatives) and collapsing capacity, or by creating probe storms that
              add load during incidents. Misaligned checks can turn partial dependency issues into full outages.
            </p>
          </div>
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            <p className="font-semibold">Q: How do you design checks for many dependencies?</p>
            <p className="mt-2 text-sm text-muted">
              A: Keep liveness independent, and make readiness depend only on the minimal set of dependencies required
              for safe operation. Avoid optional dependency checks that can eject the whole fleet.
            </p>
          </div>
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            <p className="font-semibold">Q: When should you disable a health check during an incident?</p>
            <p className="mt-2 text-sm text-muted">
              A: Rarely. Prefer reducing sensitivity or switching to a simpler check. If you disable it, compensate
              with manual routing or traffic shaping so you do not send traffic to known-bad instances.
            </p>
          </div>
        </div>
      </section>
