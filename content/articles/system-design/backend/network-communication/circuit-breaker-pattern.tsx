...
export default function CircuitBreakerPatternConciseArticle() {
  ...
  <section>
    ...
    <ArticleImage
      src="/diagrams/system-design-concepts/backend/network-communication/circuit-breaker-state-diagram.svg"
  ...
  <section>
    ...
    <ArticleImage
      src="/diagrams/system-design-concepts/backend/network-communication/circuit-breaker-pattern.svg"
...
<section>
  ...
  <section>
    ...
    <ul className="space-y-2">
      <li>Walk the breaker state machine (closed, open, half-open) and what signals trigger each transition.</li>
      <li>Explain how you choose thresholds and windows to avoid flapping during intermittent failures.</li>
      <li>Describe safe fallbacks and the fail-open vs fail-closed choice for different operations.</li>
      <li>Discuss retry interaction: breakers should reduce amplification, not mask real outages.</li>
      <li>Show the observability: open events, probe success, downstream latency, and fallback rate.</li>
    </ul>
  </section>
...
<section>
  ...
  <ul className="space-y-2">
    <li>What failures count (timeouts, 5xx, slow responses), and how do you distinguish dependency slowness from client timeouts?</li>
    <li>What is the fallback behavior, and what correctness guarantees remain in the degraded mode?</li>
    <li>How are probes scheduled in half-open, and how do you keep probe traffic from harming recovery?</li>
    <li>Is the breaker scoped per dependency, per route, or per tenant, and why?</li>
    <li>How do you avoid hiding incidents by turning every failure into a fallback with no alert?</li>
  </ul>
</section>
