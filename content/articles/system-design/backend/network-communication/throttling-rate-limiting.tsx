...
<section>
  ...
  <section>
    ...
    <ul className="space-y-2">
      <li>Explain rate limiting goals: fairness, abuse prevention, and protecting downstream capacity.</li>
      <li>Compare algorithms (token bucket, leaky bucket, fixed window) and what behaviors each produces under bursts.</li>
      <li>Discuss keying and identity: per API key, per user, per IP, and how NAT or shared IPs affect fairness.</li>
      <li>Cover distributed enforcement: consistency trade-offs, multi-region coordination, and preventing bypass.</li>
      <li>Show what clients need: response codes, Retry-After guidance, and metrics that confirm limits are working.</li>
    </ul>
  </section>
...
<section>
  ...
  <ul className="space-y-2">
    <li>What is the limiting dimension (user, key, tenant, IP) and how do you prevent attackers from rotating identities?</li>
    <li>Where is limiting enforced (edge, gateway, service) and how do you keep behavior consistent across layers?</li>
    <li>What burst behavior is allowed, and which algorithm matches that policy?</li>
    <li>How do you handle privileged traffic and internal callers without creating an unsafe bypass?</li>
    <li>What telemetry proves limits are helping (reduced saturation, fewer incidents) rather than just blocking users?</li>
  </ul>
</section>
