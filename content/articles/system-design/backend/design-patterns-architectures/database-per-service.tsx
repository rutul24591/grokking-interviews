...
export default function DatabasePerServiceArticle() {
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        <h2>Contract Evolution: Keeping Consumers Off Your Tables</h2>
        <p>
          The pattern works only if the service truly owns its storage. That means other teams cannot treat tables as an
          integration shortcut, even for read access. Instead, you need a deliberate strategy for how data is shared and
          how contracts evolve.
        </p>
        <ul className="mt-4 space-y-2">
          <li>
            <strong>API and event versioning:</strong> evolve contracts additively when possible, and provide clear
            deprecation windows so consumers can migrate without emergency coordination.
          </li>
          <li>
            <strong>Derived views for query needs:</strong> when consumers need joins, build read models or warehouse
            pipelines rather than granting direct access to the write store.
          </li>
          <li>
            <strong>Controlled break-glass access:</strong> if you must allow direct reads in an incident, make it
            explicit, audited, and temporary so it does not become the normal path.
          </li>
          <li>
            <strong>Backfill and replay readiness:</strong> contract changes often imply reprocessing historical events
            to rebuild views, so rebuild workflows must be safe and well understood.
          </li>
        </ul>
        <p className="mt-4">
          In practice, &quot;database per service&quot; is as much a governance rule as it is a technical choice. Without
          enforcement, the system drifts back toward shared-schema coupling.
        </p>
      </section>
