    ...
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ul className="space-y-2">
          ...
          <HighlightBlock as="li" tier="important"><strong>MFA challenges:</strong> After primary credential verification, the server returns a 200 with &#123;mfaRequired: true, mfaToken: "challenge-token", mfaMethods: ["totp", "webauthn"]&#125;. The frontend shows the MFA screen. For TOTP: the user enters a 6-digit code from their authenticator app; the client POSTs &#123;mfaToken, totpCode&#125; to the verification endpoint. For WebAuthn: the client calls navigator.credentials.get(&#123;publicKey: serverChallenge&#125;) and sends the assertion back to the server.</HighlightBlock>
        ...
        <ul className="space-y-2">
          ...
          <HighlightBlock as="li" tier="important"><strong>Login form security:</strong> The password field uses type="password" with autocomplete="current-password" for password manager compatibility. The username field uses autocomplete="email". The submit button uses type="submit" (not type="button") so the native form submit behavior (Enter key) works. Passkey support: the login form shows a "Sign in with passkey" button if navigator.credentials.get is available and the user has registered a passkey.</HighlightBlock>
        </ul>
      </section>

      <section>
        <h2>High-Level Architecture</h2>
        <HighlightBlock as="p" tier="important">The authentication flow is modeled as a state machine: Unauthenticated → LoginForm → (MFAChallenge | OAuthRedirect) → Authenticated → (TokenRefresh loop) → Unauthenticated (on logout or token revocation). The React authentication context (AuthContext) holds the current auth state and provides login/logout/refresh methods. Components subscribe to the context to get the current user and token. The axios (or fetch) interceptor automatically attaches the in-memory access token to every API request and triggers a silent refresh on 401 responses, transparent to the calling component.</HighlightBlock>
      </section>
