...
const definition = [
  "Design an Authentication System is a high-stakes product system because mistakes create account takeover, data exposure, compliance violations, or permanent loss of user trust. A principal-ready design treats an authentication system as a trust boundary, not as a form or settings page.",
...
const concepts = [
  "The first concept is explicit trust boundary modeling. identity provider, credential verifier, and MFA service must define who can make a decision, what evidence they use, and how that decision is propagated to downstream systems.",
...
const pitfalls = [
  "credential stuffing is usually a sign that the design treats security as a single endpoint instead of a control loop with detection, throttling, challenge, and recovery.",
  "MFA fatigue often comes from UX that optimizes completion over risk explanation. Users need enough context to make safe decisions without exposing sensitive signals to attackers.",
  "account enumeration can happen when error messages, policy caches, or derived surfaces reveal information that the primary API intended to hide.",
  "session fixation requires explicit audit and rollback controls. Emergency access and recovery paths are necessary, but they are also high-risk and must be observable.",
...
const useCases = [
  "consumer login requires the system to balance usability, security, auditability, and recovery rather than applying one static rule to every user.",
  "enterprise SSO requires the system to balance usability, security, auditability, and recovery rather than applying one static rule to every user.",
  "step-up authentication requires the system to balance usability, security, auditability, and recovery rather than applying one static rule to every user.",
...
const questions = [
  {
    "question": "How would you design an authentication system end to end?",
    "answer": "I would start with assets, actors, trust boundaries, and attacker capabilities. Then I would design the decision path around identity provider, credential verifier, MFA service, risk engine, session issuer. The frontend collects intent and shows safe recovery state, but backend enforcement owns policy. Durable state includes decision evidence, policy version, token or consent lineage, and audit logs. Operations require revocation, rollback, support visibility, anomaly detection, and incident playbooks."
  },
  {
  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/security-auth-privacy-systems/authentication-system.svg" alt="Design an Authentication System architecture" caption="Architecture view: trust boundary, policy decision, enforcement point, state store, and audit/control plane." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/security-auth-privacy-systems/authentication-system-flow.svg" alt="Design an Authentication System flow" caption="Flow view: user intent, risk decision, policy enforcement, user recovery, and incident response." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/security-auth-privacy-systems/authentication-system-operations.svg" alt="Design an Authentication System operations" caption="Operations view: revocation, rollback, suspicious activity, privacy propagation, and forensic auditability." />
      </section>
