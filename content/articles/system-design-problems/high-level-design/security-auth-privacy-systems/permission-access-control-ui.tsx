  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>A permission and access control system governs what actions each user can perform in a multi-tenant or multi-role application. GitHub's organization permissions, Linear's workspace roles, and Notion's page sharing all implement variations of this pattern. The frontend has two distinct responsibilities: enforcing permissions in the UI (hide/disable features the user cannot access) and providing an admin interface for managing roles and permissions. Both require a clear mental model of the permission data structure and a consistent enforcement mechanism.</p>
        <p>The critical insight for frontend permission enforcement: UI enforcement is cosmetic, not security. Hiding a button does not prevent an API call — a user can always call the API directly. All permission enforcement must happen server-side. The UI hides or disables features for UX reasons (reducing confusion), not for security. The server must check permissions on every request regardless of whether the UI hides the triggering button. The frontend permission system answers: "Should I show this button?" The server permission system answers: "Should I execute this action?"</p>
      ...
      <section>
        ...
        <ul className="space-y-2">
          ...
          <li><strong>Permissions management UI:</strong> The admin panel shows a matrix of (roles × permissions) with checkboxes. Admins can create custom roles, assign them to users, and configure which permissions each role has. Changes take effect immediately for new sessions; existing sessions pick up permission changes on their next token refresh (within 15 minutes). Critical permission revocations (removing admin access) are propagated immediately via WebSocket to all active sessions for the affected user.</li>
        ...
        <ul className="space-y-2">
          <li><strong>Permission evaluation performance:</strong> Permission checks in the UI must be synchronous and under 1ms (no async, no server round trips). The permission set is loaded once from the JWT claims on authentication and cached in the auth context (React context or Zustand). The usePermission hook is a simple Set.has() lookup — O(1).</li>
          <li><strong>Permission propagation:</strong> When an admin changes a user's role, the affected user's permissions must update within 15 minutes (next token refresh) for non-critical changes, or immediately (WebSocket push) for critical changes (role downgrade, access revocation). The WebSocket PERMISSION_UPDATED event contains the new permission set, which the client stores in the auth context, triggering re-renders of permission-guarded components.</li>
          <li><strong>Audit logging:</strong> Every permission change (role assignment, custom permission override) is logged with: actorId, targetUserId, action, oldValue, newValue, timestamp, ipAddress. The audit log is shown in the admin panel as an append-only list, filterable by actor, target, and date range. Permission audit logs are immutable — they cannot be deleted or edited (enforced server-side).</li>
        </ul>
      </section>

      <section>
        <h2>High-Level Architecture</h2>
        <p>The permission system has three layers. The Data Layer: the permission set is embedded in the JWT as claims (for role-level permissions) and fetched from the API on demand (for resource-level permissions). The Evaluation Layer: the usePermission hook and a PermissionGate component evaluate permissions synchronously from the in-memory permission set. The Management Layer: the admin permissions UI reads from and writes to the permissions API, which immediately updates the database and queues a permission propagation event for affected users' active sessions. React context provides the current user's permission set to all components, with a stable reference (Zustand with shallow equality) to prevent unnecessary re-renders when unrelated state changes.</p>
      </section>
