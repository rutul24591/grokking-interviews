    ...
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Active session listing:</strong> Each active session is shown with: device type (desktop/mobile, inferred from user-agent), browser name and version, IP address with geolocation (city, country, flag emoji), last active timestamp (relative: "2 hours ago"), and a "This device" badge on the current session. Sessions are sorted by last-active descending. The current session cannot be revoked from this panel (to prevent accidental self-lockout); it shows a "Sign out" button that performs a normal logout.</li>
          <li><strong>Session revocation:</strong> Clicking "Revoke" on a session sends DELETE /api/sessions/&#123;sessionId&#125; to the backend. The backend: (1) marks the session as revoked in the session store (Redis: SET session:&#123;id&#125;:revoked 1 EX 86400); (2) publishes a REVOKE event to the session's WebSocket channel. The revoked client's WebSocket receives the REVOKE event, clears the in-memory access token, deletes the session cookie (client-side document.cookie manipulation, or a redirect to /auth/logout that clears the HttpOnly cookie), and redirects to the login page with a "Your session has been signed out" message.</li>
        ...
        <ul className="space-y-2">
          ...
          <li><strong>Confirmation for destructive actions:</strong> "Revoke all sessions," "Remove MFA method," and "Delete account" require a re-authentication step (enter current password or complete MFA) before proceeding. This prevents an attacker with temporary access to an unlocked computer from locking out the legitimate user.</li>
        </ul>
      </section>

      <section>
        <h2>High-Level Architecture</h2>
        <p>The security dashboard is a read-heavy interface (mostly displaying state) with a few high-stakes write operations (revocation, MFA enrollment). The dashboard loads session data on mount (GET /api/sessions, GET /api/login-history, GET /api/security-settings — batched via BFF), renders the state, and subscribes to a WebSocket channel for real-time security events (new login, session revoked). Destructive operations follow a confirm-then-execute pattern with re-authentication gates. The current session's WebSocket connection listens for its own revocation event (in case it is revoked by another session) — on receiving REVOKE for its own sessionId, it executes logout immediately.</p>
      </section>
      ...
      <section>
        ...
        <p>Each session record (in Redis or a sessions table) stores: sessionId, userId, createdAt, lastActiveAt, ipAddress, userAgent, deviceFingerprint (a hash of: OS, browser, screen resolution, timezone, language — stable identifiers that distinguish devices), geoLocation &#123;city, country, countryCode&#125; (derived server-side from IP on session creation using MaxMind GeoLite2), isTrusted (boolean — set when user clicks "Trust this device"), and revokedAt (null if active). The frontend renders device type by parsing the user-agent string client-side using ua-parser-js — showing a laptop icon for desktop, phone icon for mobile, tablet icon for tablets.</p>
        <p>The "This device" identification: the current session's ID is included in the auth context (returned as part of the /auth/me response). The session list compares each session's ID to the current session ID and adds the "This device" badge. The current session's "Revoke" button is replaced with "Sign out" (which performs a normal logout of the current session, not an admin revocation).</p>

        <h3 className="mt-6 mb-3 text-lg font-semibold">Login History Anomaly Detection UI</h3>
        <p>Anomaly detection is performed server-side and stored with each login event — the frontend does not run anomaly logic, it renders server-generated anomaly flags. Each login event has an anomalyFlags array: ["NEW_COUNTRY", "UNUSUAL_HOUR", "CREDENTIAL_STUFFING_PATTERN"]. The UI renders these as colored badges: amber for individual anomalies, red for "CREDENTIAL_STUFFING_PATTERN" (high severity). The anomaly flag definitions: NEW_COUNTRY — first login from this country in the user's history; UNUSUAL_HOUR — login between midnight and 5am in the user's timezone; CREDENTIAL_STUFFING_PATTERN — 5+ failed attempts from different IPs followed by a successful login within 30 minutes.</p>
        <p>The "Wasn't me" action flow: the user clicks "Wasn't me" on a suspicious login event. A modal confirms: "We'll sign out all other sessions and send you a password reset email. Continue?" On confirm: (1) POST /api/security/account-lockdown — revokes all sessions except the current one, invalidates all active MFA challenges, and sends a password reset email; (2) the modal closes, a success toast shows "All other sessions have been signed out. Check your email to reset your password."</p>

        <h3 className="mt-6 mb-3 text-lg font-semibold">TOTP Enrollment Flow</h3>
        <p>The TOTP enrollment flow requires care to avoid showing the TOTP secret in an unsafe context. Flow: (1) User clicks "Add authenticator app"; (2) POST /api/mfa/totp/enroll — the server generates a random 20-byte TOTP secret (base32-encoded), stores it temporarily (not committed to the user's account yet — only committed after verification), and returns the QR code URL (otpauth://totp/AppName:user@email?secret=BASE32SECRET&issuer=AppName); (3) The client renders the QR code using qrcode.js on a canvas element — the secret is in the QR code URL displayed in the client but never stored anywhere beyond the temporary server record; (4) User scans with authenticator app; (5) User enters the current 6-digit TOTP code to verify; (6) POST /api/mfa/totp/verify &#123;code&#125; — server verifies and commits the TOTP secret to the user's MFA record; (7) Server generates 10 backup codes (8-character alphanumeric), stores hashed versions, returns plaintext versions once; (8) UI shows backup codes in a modal with "Copy all" (uses navigator.clipboard.writeText) and "Download as .txt" (creates a Blob and a temporary &lt;a&gt; download link).</p>

        <h3 className="mt-6 mb-3 text-lg font-semibold">Device Trust and Remembered Devices</h3>
        <p>The "Trust this device for 30 days" option during MFA: when checked, the server sets a long-lived, device-specific cookie (trust-token) in addition to the session cookie. The trust token is a signed JWT containing: userId, deviceFingerprint, trustedUntil. On future logins from the same device (same fingerprint), the server checks the trust token — if valid and not expired, MFA is skipped. The trust token is stored as a separate HttpOnly cookie so it survives session logout (the session cookie is cleared on logout, but the trust token persists until its expiry). The security dashboard shows trusted devices and allows revoking trust tokens individually (DELETE /api/trusted-devices/&#123;tokenId&#125; — invalides the token without affecting the session).</p>
      </section>

      <section>
        <h2>Trade-offs and Considerations</h2>
        <p>Real-time revocation vs. polling: the WebSocket approach for revocation propagation adds complexity (maintaining WebSocket connections for every active session). An alternative: check the revoked flag in Redis on every API request (token introspection). This trades real-time propagation for simplicity — a revoked session may make up to one more API request before being blocked. For most use cases (the user saw a suspicious session and revoked it), the one-request delay is acceptable. For high-security scenarios (suspected account compromise), real-time WebSocket revocation is justified.</p>
        <p>Backup code UX tradeoffs: showing backup codes only once (at enrollment time) is the standard pattern — it forces the user to save them immediately. However, many users lose their backup codes. Alternatives: allow re-generating backup codes (which invalidates the old ones); provide an account recovery email (the user receives a recovery link to their verified email if they lose all MFA methods); or allow "recovery via trusted contact" (another user vouches for identity — complex but zero-server-knowledge). The "show once" policy with a forced confirmation ("I have saved these codes") before closing is the right balance for most applications.</p>
      </section>

      <section>
        <h2>Summary</h2>
        <p>An account security dashboard covers: (1) session listing (device type from UA parser, IP geolocation, lastActive, current session badge, revoke others); (2) real-time revocation (DELETE → Redis revoked flag → WS REVOKE event → target clears cookie + redirects login); (3) login history with server-side anomaly flags (NEW_COUNTRY, UNUSUAL_HOUR, CREDENTIAL_STUFFING — rendered as colored badges, "Wasn't me" triggers account lockdown + password reset email); (4) TOTP enrollment (server temp secret → QR code URL → canvas render → verify 6-digit → commit + generate backup codes shown once, copy/download); (5) HIBP k-anonymity breach check (SHA-1 first 5 chars to range API, full hash check client-side, full hash never sent externally); and (6) re-auth gate for destructive actions (confirm + password/MFA challenge before revoke-all or delete). The core security principle: every destructive action on the security dashboard requires fresh authentication — temporary access to a logged-in browser should not allow an attacker to lock out the legitimate user.</p>
      </section>
    </ArticleLayout>
  );
}
