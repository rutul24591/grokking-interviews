...
export const metadata: ArticleMetadata = {
  id: "article-hld-real-time-multiplayer-game-lobby-and-matchmaking-ui",
...
const definition = [
  "Design a Real-time Multiplayer Game Lobby and Matchmaking UI is a realtime distributed product system where multiple clients observe, edit, or coordinate around shared state with low perceived latency. A principal-ready design treats a real-time game lobby and matchmaking UI as shared-state replication with product semantics, not just a websocket channel.",
...
const concepts = [
  "The first concept is state classification. party state, matchmaker, and region selector should be classified as durable, derived, or ephemeral. Durable state needs replay and audit; ephemeral state needs freshness and expiry; derived state should be rebuildable.",
...
const architecture = [
  "The architecture contains party state, matchmaker, region selector, ready checks, lobby websocket. Clients keep local state and pending operations. Gateways authenticate connections and route room traffic. Sequencers or collaboration services assign order or merge operations. Snapshot stores compact history. Projections serve read-optimized views and replay.",
...
const pitfalls = [
  "stuck lobby usually means the system lacks clear operation identity, sequencing, or replay semantics. The fix is not more retries; it is a defined operation model.",
  "skill mismatch is often caused by treating ephemeral state as durable truth. Presence, cursor, and QoE hints need expiry and freshness rules.",
  "region latency shows that conflict policy must be product-specific. A game lobby, text editor, whiteboard, and video call do not share one merge strategy.",
  "party split appears during reconnect and offline replay. The client should not blindly resend operations without idempotency and base-version context.",
...
const useCases = [
  "game lobby needs low-latency local feedback while preserving convergence, authorization, replay, and operational recovery.",
  "tournament queue needs low-latency local feedback while preserving convergence, authorization, replay, and operational recovery.",
  "co-op party formation needs low-latency local feedback while preserving convergence, authorization, replay, and operational recovery.",
...
const questions = [
  {
    "question": "How would you design a real-time game lobby and matchmaking UI end to end?",
  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/realtime-collaboration-systems/real-time-multiplayer-game-lobby-and-matchmaking-ui-architecture.svg" alt="Design a Real-time Multiplayer Game Lobby and Matchmaking UI architecture" caption="Architecture view: clients, gateways, operation log, merge/sequencing, snapshots, authorization, and replay." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/realtime-collaboration-systems/real-time-multiplayer-game-lobby-and-matchmaking-ui-workflow.svg" alt="Design a Real-time Multiplayer Game Lobby and Matchmaking UI flow" caption="Flow view: local intent, acknowledgement, fanout, replay, conflict handling, and recovery." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/realtime-collaboration-systems/real-time-multiplayer-game-lobby-and-matchmaking-ui-state-machine.svg" alt="Design a Real-time Multiplayer Game Lobby and Matchmaking UI operations" caption="Operations view: fanout pressure, conflict rate, reconnects, stale clients, abuse controls, and rollback." />
      </section>
