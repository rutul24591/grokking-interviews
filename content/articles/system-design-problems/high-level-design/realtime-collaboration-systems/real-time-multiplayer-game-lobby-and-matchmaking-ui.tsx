  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">The lobby is a WebSocket room with exactly 10 players. The Lobby Service maintains the authoritative lobby state: (matchId, players: [&#123;userId, teamId, mmr, ready: bool, pingMs: number&#125;], startCountdown: null | number). When any player's state changes (toggles ready, disconnects), the Lobby Service broadcasts the full lobby state to all connected players. Broadcasting the full state (not a delta) is correct for a 10-player room: the state is small (~2KB), and full-state broadcasts avoid the ordering complexity of delta-based sync.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">Disconnect handling in the lobby: if a player disconnects from the WebSocket during the lobby phase (network drop, browser close), the Lobby Service marks them as disconnected and broadcasts the update. The player has 60 seconds to reconnect and rejoin the lobby. During this window, the match is held (the countdown does not start). If they do not reconnect within 60 seconds, they are removed from the match (treated as a decline, subject to dodge penalty), the match is cancelled, and the remaining 9 players are returned to the queue. This reconnection grace period handles the common case of accidental disconnection without penalizing players.</HighlightBlock>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Wait Time Estimation</h3>
        <HighlightBlock as="p" tier="important">Accurate wait time estimation is important for user retention: players who see "2 minutes" and wait 10 minutes feel deceived and abandon the queue. Estimation is based on a rolling average of recent successful match times for players with similar MMR and party composition in the same game mode. The Queue Service maintains a time-series of recent match durations (last 1000 matches per mode/MMR bracket combination, stored in Redis). The estimate is the 75th percentile of this distribution (not the average—the distribution is right-skewed by occasional long waits, and the 75th percentile more closely matches the user's expectation: "you will wait this long or less, 75% of the time").</HighlightBlock>
