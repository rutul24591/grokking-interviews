...
const definition = [
  "Design a Live Commenting and Annotation System is a realtime distributed product system where multiple clients observe, edit, or coordinate around shared state with low perceived latency. A principal-ready design treats a live commenting and annotation system as shared-state replication with product semantics, not just a websocket channel.",
...
const concepts = [
  "The first concept is state classification. anchor model, comment thread, and realtime updates should be classified as durable, derived, or ephemeral. Durable state needs replay and audit; ephemeral state needs freshness and expiry; derived state should be rebuildable.",
...
const architecture = [
  "The architecture contains anchor model, comment thread, realtime updates, permission service, moderation workflow. Clients keep local state and pending operations. Gateways authenticate connections and route room traffic. Sequencers or collaboration services assign order or merge operations. Snapshot stores compact history. Projections serve read-optimized views and replay.",
...
const pitfalls = [
  "orphan anchors usually means the system lacks clear operation identity, sequencing, or replay semantics. The fix is not more retries; it is a defined operation model.",
  "stale comments is often caused by treating ephemeral state as durable truth. Presence, cursor, and QoE hints need expiry and freshness rules.",
  "permission drift shows that conflict policy must be product-specific. A game lobby, text editor, whiteboard, and video call do not share one merge strategy.",
  "abuse threads appears during reconnect and offline replay. The client should not blindly resend operations without idempotency and base-version context.",
...
const useCases = [
  "PDF comments needs low-latency local feedback while preserving convergence, authorization, replay, and operational recovery.",
  "video timestamp comments needs low-latency local feedback while preserving convergence, authorization, replay, and operational recovery.",
  "design review annotations needs low-latency local feedback while preserving convergence, authorization, replay, and operational recovery.",
...
const questions = [
  {
    "question": "How would you design a live commenting and annotation system end to end?",
  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/realtime-collaboration-systems/live-commenting-annotation-system-architecture.svg" alt="Design a Live Commenting and Annotation System architecture" caption="Architecture view: clients, gateways, operation log, merge/sequencing, snapshots, authorization, and replay." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/realtime-collaboration-systems/live-commenting-annotation-system-workflow.svg" alt="Design a Live Commenting and Annotation System flow" caption="Flow view: local intent, acknowledgement, fanout, replay, conflict handling, and recovery." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/realtime-collaboration-systems/live-commenting-annotation-system-anchoring.svg" alt="Design a Live Commenting and Annotation System operations" caption="Operations view: fanout pressure, conflict rate, reconnects, stale clients, abuse controls, and rollback." />
      </section>
