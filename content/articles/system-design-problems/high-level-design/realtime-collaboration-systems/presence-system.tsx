...
const definition = [
  "Design a Presence System is a realtime distributed product system where multiple clients observe, edit, or coordinate around shared state with low perceived latency. A principal-ready design treats a presence system as shared-state replication with product semantics, not just a websocket channel.",
...
const concepts = [
  "The first concept is state classification. heartbeat service, session registry, and presence projection should be classified as durable, derived, or ephemeral. Durable state needs replay and audit; ephemeral state needs freshness and expiry; derived state should be rebuildable.",
...
const architecture = [
  "The architecture contains heartbeat service, session registry, presence projection, fanout gateway, privacy policy. Clients keep local state and pending operations. Gateways authenticate connections and route room traffic. Sequencers or collaboration services assign order or merge operations. Snapshot stores compact history. Projections serve read-optimized views and replay.",
...
const pitfalls = [
  "ghost online usually means the system lacks clear operation identity, sequencing, or replay semantics. The fix is not more retries; it is a defined operation model.",
  "flapping is often caused by treating ephemeral state as durable truth. Presence, cursor, and QoE hints need expiry and freshness rules.",
  "fanout cost shows that conflict policy must be product-specific. A game lobby, text editor, whiteboard, and video call do not share one merge strategy.",
  "privacy exposure appears during reconnect and offline replay. The client should not blindly resend operations without idempotency and base-version context.",
...
const useCases = [
  "chat online status needs low-latency local feedback while preserving convergence, authorization, replay, and operational recovery.",
  "document active users needs low-latency local feedback while preserving convergence, authorization, replay, and operational recovery.",
  "workspace member presence needs low-latency local feedback while preserving convergence, authorization, replay, and operational recovery.",
...
const questions = [
  {
    "question": "How would you design a presence system end to end?",
  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/realtime-collaboration-systems/presence-system-architecture.svg" alt="Design a Presence System architecture" caption="Architecture view: clients, gateways, operation log, merge/sequencing, snapshots, authorization, and replay." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/realtime-collaboration-systems/presence-system-semantics.svg" alt="Design a Presence System flow" caption="Flow view: local intent, acknowledgement, fanout, replay, conflict handling, and recovery." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/realtime-collaboration-systems/presence-system-scaling.svg" alt="Design a Presence System operations" caption="Operations view: fanout pressure, conflict rate, reconnects, stale clients, abuse controls, and rollback." />
      </section>
