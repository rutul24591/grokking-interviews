  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <HighlightBlock as="p" tier="important">A presence system answers the question "is this user available right now?" The answer informs dozens of downstream features: whether to show a typing indicator, whether a push notification is necessary (the user is active in the app), whether to show a "last seen" timestamp, whether to route a customer support request to an available agent. The deceptively simple online/offline binary state hides significant engineering complexity: determining when a user has transitioned from online to offline requires detecting the absence of activity (you cannot receive an "I have gone offline" event from a user who lost network connectivity).</HighlightBlock>
        <HighlightBlock as="p" tier="crucial">Presence systems are among the highest-fan-out problems in real-time systems. When a user with 1,000 followers comes online, 1,000 WebSocket connections must receive the update. At 100,000 daily active users each with an average of 200 followers, a presence change wave can generate 20,000,000 fan-out events per second during peak activity (morning login surge). The delivery mechanism must handle this fan-out without becoming a bottleneck.</HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">The Presence Service stores two Redis keys per device: presence:&#123;userId&#125;:&#123;deviceId&#125; (value: &#123;state, lastActivityMs&#125;, TTL: 60 seconds) and lastSeen:&#123;userId&#125; (value: timestamp, no TTL—persistent). The 60-second TTL means that if no heartbeat is received for 60 seconds (two missed 30-second heartbeats), the Redis key expires. A keyspace notification (Redis can emit pub/sub events when keys expire) fires the expired event to the Presence Service, which triggers the offline state transition for that device. If this was the user's only active device, the user transitions to offline; the lastSeen:&#123;userId&#125; key is updated to the current timestamp.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Detection latency: the gap between the user closing the app and the system detecting the offline transition is at most the heartbeat interval (30 seconds) plus the TTL buffer (60 seconds) = 90 seconds worst case. The average detection latency is approximately 45–75 seconds. For presence systems where faster detection is required (customer support agent availability), heartbeat intervals can be reduced (15 seconds) at the cost of higher server load (3.3× more heartbeat requests per user).</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">The idle state is important for presence display (idle users may show a yellow dot rather than green) and for notification routing (an idle user might receive a push notification rather than relying on in-app delivery, since they are not actively watching the screen). The idle threshold (5 minutes) and the offline threshold (20 minutes of no heartbeat) create a three-state presence model: ONLINE (active, in-app), IDLE (app open, not interacting), OFFLINE (app closed or device inactive).</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">For pull-based presence (high-follower-count users), privacy controls are enforced at query time: when a follower queries a user's presence, the Presence Service checks if the querying user is allowed to see the presence state. If not (privacy setting says "hide from this user"), the service returns an "offline" presence state regardless of the actual state. The user being queried is never notified that someone attempted to see their presence—the query is silently redirected to return offline.</HighlightBlock>
      </section>
