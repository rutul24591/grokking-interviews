...
const definition = [
  "Design an Embedded Chat System is a realtime distributed product system where multiple clients observe, edit, or coordinate around shared state with low perceived latency. A principal-ready design treats an embedded chat system as shared-state replication with product semantics, not just a websocket channel.",
...
const concepts = [
  "The first concept is state classification. message log, realtime gateway, and tenant policy should be classified as durable, derived, or ephemeral. Durable state needs replay and audit; ephemeral state needs freshness and expiry; derived state should be rebuildable.",
...
const architecture = [
  "The architecture contains message log, realtime gateway, tenant policy, moderation queue, notification bridge. Clients keep local state and pending operations. Gateways authenticate connections and route room traffic. Sequencers or collaboration services assign order or merge operations. Snapshot stores compact history. Projections serve read-optimized views and replay.",
...
const pitfalls = [
  "message loss usually means the system lacks clear operation identity, sequencing, or replay semantics. The fix is not more retries; it is a defined operation model.",
  "tenant leakage is often caused by treating ephemeral state as durable truth. Presence, cursor, and QoE hints need expiry and freshness rules.",
  "spam flood shows that conflict policy must be product-specific. A game lobby, text editor, whiteboard, and video call do not share one merge strategy.",
  "reconnect storms appears during reconnect and offline replay. The client should not blindly resend operations without idempotency and base-version context.",
...
const useCases = [
  "support chat widget needs low-latency local feedback while preserving convergence, authorization, replay, and operational recovery.",
  "in-product team chat needs low-latency local feedback while preserving convergence, authorization, replay, and operational recovery.",
  "community event chat needs low-latency local feedback while preserving convergence, authorization, replay, and operational recovery.",
...
const questions = [
  {
    "question": "How would you design an embedded chat system end to end?",
  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/realtime-collaboration-systems/embedded-chat-system-architecture.svg" alt="Design an Embedded Chat System architecture" caption="Architecture view: clients, gateways, operation log, merge/sequencing, snapshots, authorization, and replay." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/realtime-collaboration-systems/embedded-chat-system-workflow.svg" alt="Design an Embedded Chat System flow" caption="Flow view: local intent, acknowledgement, fanout, replay, conflict handling, and recovery." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/realtime-collaboration-systems/embedded-chat-system-reliability.svg" alt="Design an Embedded Chat System operations" caption="Operations view: fanout pressure, conflict rate, reconnects, stale clients, abuse controls, and rollback." />
      </section>
