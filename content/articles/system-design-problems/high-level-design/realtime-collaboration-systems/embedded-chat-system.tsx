  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <HighlightBlock as="p" tier="crucial">An embedded chat system is distinct from a standalone messaging app (Slack, WhatsApp) in that it is tightly integrated with the host application's context: a chat panel in a project management tool, a customer support widget in an e-commerce site, an in-game chat in a gaming platform. The context integration creates requirements that standalone chat apps do not have: messages may reference host application entities (a task, a product, a player), participants are limited to users with access to the host context (team members, support agents, teammates), and the chat UI must fit within the host application's layout constraints (a sidebar, a floating widget, an overlay panel).</HighlightBlock>
        <HighlightBlock as="p" tier="crucial">The core messaging challenge is reliable delivery: every message must be delivered to every participant, even if they are offline at the time of sending. Unlike real-time event streaming (where missed events are acceptable), chat messages are durable—a message sent must eventually be seen by the recipient, even if they are offline for days. This reliability requirement differentiates chat from presence systems (where missed events are acceptable) and requires a persistent message store, not just a real-time pub/sub system.</HighlightBlock>
