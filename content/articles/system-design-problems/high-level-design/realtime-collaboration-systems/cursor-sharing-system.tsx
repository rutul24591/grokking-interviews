  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">The server receives cursor messages from one client and relays them to all other clients in the canvas session. For a session with 200 collaborators, each cursor message is relayed to 199 clients. At 30Hz per collaborator, 200 × 30 = 6,000 messages per second per session flow through the relay server. The relay is implemented as a pure in-memory broadcast: incoming cursor WebSocket frames are forwarded to all other WebSocket connections in the session without parsing (the server processes the raw binary frame at the WebSocket level, not at the application level). This avoids JSON parsing and serialization overhead, reducing relay latency to sub-millisecond.</HighlightBlock>
        <HighlightBlock as="p" tier="important">For sessions distributed across multiple relay servers (when a session's connections span multiple WebSocket servers), the relay uses Redis pub/sub: each session has a Redis channel; incoming cursor messages are published to the channel, and all relay servers subscribed to the channel relay to their connected clients. For cursor data (high-frequency, loss-tolerant), Redis pub/sub is appropriate—dropped messages are handled by the client's interpolation. The fire-and-forget semantics of Redis pub/sub match the loss-tolerance requirement perfectly.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">Collaborator disconnect is handled by the server: when a WebSocket connection closes (collaborator disconnects), the server broadcasts a cursor_removed event to all other clients with the disconnected userId. Clients receiving this event immediately remove the cursor and edge indicator for that userId. This ensures disconnected collaborators do not leave ghost cursors on the canvas.</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">Transmission frequency: 30Hz is a practical balance between smoothness and bandwidth. At 60Hz, bandwidth doubles (190KB/s per client for 200 cursors) but smoothness improvement is imperceptible when combined with interpolation (the interpolation already fills the gap between 30Hz updates). At 10Hz, bandwidth is low but interpolation must predict over 100ms windows, making cursor extrapolation visibly wrong for fast-moving cursors. 30Hz is the industry standard for cursor sharing in collaborative tools.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Canvas coordinates versus relative coordinates: transmitting absolute canvas coordinates (as described) requires each receiver to know the sender's canvas origin—but since all collaborators share the same canvas, the canvas origin is the same for all. An alternative is transmitting relative viewport position (x as a fraction of the screen width, y as a fraction of the screen height)—this avoids the viewport transformation calculation but requires the receiver to know the sender's screen resolution to position the cursor on the shared canvas. Canvas coordinates are more meaningful for a shared infinite canvas and avoid the screen resolution assumption.</HighlightBlock>
