  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <p>The notification system has four layers. The Generation Layer produces NotificationEvent records when user actions occur (Kafka: like.created, comment.created, follow.created). The Aggregation Layer groups related events within a time window (5 minutes for social events, immediate for mentions) and writes aggregated Notification records to the notifications database. The Delivery Layer routes notifications to appropriate channels (in-app, push, email) based on user preferences and event priority; it also publishes badge update events to Redis Pub/Sub for SSE delivery. The Frontend Layer has three components: the SSE badge counter (subscribes to user-specific Redis channel for count updates), the notification inbox panel (cursor-paginated, cached with stale-while-revalidate), and the service worker (handles Web Push subscription registration and received push event rendering).</p>
      </section>
      ...
      <section>
        ...
        <p>Social notifications are aggregated server-side using a sliding window approach. When a LikeCreated event arrives for post X by user Y at time T, the aggregation worker queries: is there an open notification for the same (owner, object_type=post, object_id=X, notification_type=like) within the last 5 minutes? If yes, it adds user Y to the actors list of that notification and updates the notification body. If no, it creates a new notification. The actors list is capped at storing the 3 most recent distinct actors (for display purposes) plus a total count. The UI renders this as: [avatar1][avatar2][avatar3] "Alice, Bob, and 498 others liked your photo." Clicking "498 others" expands a scrollable list of all actor names. For follow notifications (where the object is the user themselves, not a post), each follow from a distinct user typically creates a separate notification, since users want to know each individual who followed them. The grouping policy is configurable per notification type.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibold">SSE for Real-Time Badge Updates</h3>
        <p>When the user is active in the app, a persistent SSE connection is maintained to GET /api/notifications/stream. The SSE server subscribes to the Redis Pub/Sub channel notif:{"{userId}"} for this connection. When a new notification is generated, the aggregation worker publishes to notif:{"{userId}"} with the updated unread count: PUBLISH notif:{"{userId}"} {"{ unreadCount: 7 }"}. The SSE server receives this and streams a data event to the browser: data: {"{ unreadCount: 7 }"}. The React component reading this stream updates the badge count in the Zustand store, which is reflected in the bell icon badge. The SSE connection uses the browser&apos;s native EventSource API with automatic reconnection. On reconnect, the client fetches the current unread count via GET /api/notifications/unread-count to catch any updates missed during the disconnection window.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibold">Web Push Registration and VAPID Authentication</h3>
        <p>Web Push requires VAPID (Voluntary Application Server Identification) for authentication. The flow: (1) The app generates a VAPID keypair (public/private). The public key is embedded in the frontend. (2) After the user has been active for 30 seconds (to avoid prompting immediately on page load, which has a high rejection rate), the app calls Notification.requestPermission(). (3) If the user grants permission, navigator.serviceWorker.register(&apos;/sw.js&apos;) registers the service worker. (4) registration.pushManager.subscribe({"{ userVisibleOnly: true, applicationServerKey: VAPID_PUBLIC_KEY }"}) creates a push subscription object containing the push endpoint URL and encryption keys. (5) The subscription is POSTed to /api/push/subscribe and stored in the push_subscriptions table (userId, endpoint, keys). (6) When a high-priority notification is generated for this user, the Delivery Service fetches the user&apos;s push subscriptions, constructs a Web Push payload (title, body, icon, badge, data.url), signs it with the VAPID private key, and POSTs it to the push endpoint.</p>
        <p>
