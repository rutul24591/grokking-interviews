...
const definition = [
  "Design an Infinite Scrolling Feed is a social-scale product system where the visible UI is only the last projection of graph state, ranking policy, media delivery, privacy rules, abuse controls, and user intent. A principal-ready design starts by separating durable social facts from derived surfaces.",
  ...
  "The goal is to design an infinite scrolling feed so users see relevant, fresh, and safe social content without exposing private actors, losing user actions, or amplifying abuse. The design must address graph scale, hot users, projection lag, mobile constraints, ranking changes, and trust-and-safety intervention.",
...
const concepts = [
  ...
  "The second concept is projection architecture. cursor service, ranked window, and dedupe set are optimized for different read patterns. The design should not force every UI to join raw social tables at request time.",
...
const architecture = [
  "The architecture has five cooperating planes: cursor service, ranked window, dedupe set, ad injector, prefetch buffer. The write plane records durable social facts and emits events. Projection workers build feed, profile, notification, counter, search, and analytics views. The read API serves surface-optimized projections. Integrity and moderation systems can remove or demote content quickly. The frontend renders stable cursors, pending actions, and recovery states.",
...
const pitfalls = [
  "scroll jump is a scale failure that appears suddenly. The design should define hot-key handling, fanout backpressure, cache windows, and degraded behavior before traffic arrives.",
  "duplicate cards is usually caused by inconsistent policy enforcement across derived surfaces. Fixing the main UI is not enough if notifications, search, emails, or previews still expose restricted information.",
  "unbounded memory undermines user trust because social products feel personal. Users notice missing posts, duplicate cards, incorrect counters, and unexplained ranking shifts quickly.",
  "stale ranking requires abuse-aware product design. Rate limits and classifiers help, but the system also needs support tooling, appeals, audit trails, and emergency controls.",
...
const useCases = [
  "news feed requires graph visibility, ranking or grouping policy, projection freshness, and abuse controls to work together rather than as separate features.",
  "commerce discovery feed requires graph visibility, ranking or grouping policy, projection freshness, and abuse controls to work together rather than as separate features.",
  "video feed requires graph visibility, ranking or grouping policy, projection freshness, and abuse controls to work together rather than as separate features.",
...
const questions = [
  {
    "question": "How would you design an infinite scrolling feed end to end?",
  ...
  },
  {
    ...
    "answer": "The main failures are scroll jump, duplicate cards, unbounded memory, stale ranking, plus hot users, viral content, counter drift, notification storms, moderation backlog, and cache stampedes. Prevention requires hybrid fanout, ranked windows, idempotent actions, integrity scoring, projection-lag monitoring, and emergency throttles."
  },
  {
  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/social-engagement/infinite-scrolling-feed.svg" alt="Design an Infinite Scrolling Feed architecture" caption="Architecture view: graph writes, projections, ranking, privacy, integrity, and surface-specific reads." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/social-engagement/infinite-scrolling-feed-flow.svg" alt="Design an Infinite Scrolling Feed flow" caption="Flow view: user action, fanout or projection, ranking, notification, moderation, and recovery." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/social-engagement/infinite-scrolling-feed-operations.svg" alt="Design an Infinite Scrolling Feed operations" caption="Operations view: fanout backlog, projection lag, privacy invalidation, abuse signals, and moderation controls." />
      </section>
