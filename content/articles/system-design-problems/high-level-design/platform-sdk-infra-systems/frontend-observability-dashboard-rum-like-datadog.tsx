...
const definition = [
  "Design a Frontend Observability Dashboard like Datadog RUM is a platform system, which means the product is not only the UI that users see but also the contract other teams, tenants, developers, plugins, SDKs, or automation depend on. A principal-ready design treats a frontend observability dashboard as a versioned control plane with operational guarantees.",
...
const concepts = [
  "The first concept is contract-first design. RUM ingest, session sampler, and metric store should expose stable schemas, versioned APIs, documented lifecycle states, and compatibility guarantees. Hidden contracts are what make platform migrations painful.",
...
const architecture = [
  "The architecture contains RUM ingest, session sampler, metric store, trace correlation, alert workflow. The control plane stores configuration, versions, ownership, approvals, and audit. The execution plane applies changes through SDKs, plugins, build systems, rollout engines, or tenant jobs. The observability plane measures impact and supports rollback.",
...
const pitfalls = [
  "high cardinality usually means the platform lacks compatibility gates, staged rollout, or blast-radius controls. Platform failures multiply because many teams depend on one surface.",
  "PII in telemetry often comes from treating permissions or integration boundaries as documentation instead of enforceable runtime policy.",
  "sampling bias is a state-distribution problem. SDK caches, build artifacts, edge nodes, tenants, and clients may observe different versions unless the design tracks propagation and freshness.",
  "alert fatigue requires rollback and audit. Platform users should know whether they can disable, revert, compensate, or escalate the issue.",
...
const useCases = [
  "web vitals dashboard requires versioned contracts, safe rollout, scoped permissions, observability, and a clear rollback path.",
  "session replay triage requires versioned contracts, safe rollout, scoped permissions, observability, and a clear rollback path.",
  "frontend incident console requires versioned contracts, safe rollout, scoped permissions, observability, and a clear rollback path.",
...
const questions = [
  {
    "question": "How would you design a frontend observability dashboard end to end?",
  ...
  },
  {
    ...
    "answer": "The main failures are high cardinality, PII in telemetry, sampling bias, alert fatigue, plus stale clients, incompatible versions, high-cardinality telemetry, queue backlogs, cross-tenant leakage, support overload, and uncontrolled blast radius. Prevention requires versioning, validation, staged rollout, idempotent execution, scoped permissions, and observability."
  },
  {
  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/platform-sdk-infra-systems/frontend-observability-dashboard-rum-like-datadog-architecture.svg" alt="Design a Frontend Observability Dashboard like Datadog RUM architecture" caption="Architecture view: control plane, execution plane, permissions, versioning, and observability boundaries." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/platform-sdk-infra-systems/frontend-observability-dashboard-rum-like-datadog-metrics-to-drilldown.svg" alt="Design a Frontend Observability Dashboard like Datadog RUM flow" caption="Flow view: authoring, validation, rollout, execution, health checks, and rollback." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/platform-sdk-infra-systems/frontend-observability-dashboard-rum-like-datadog-privacy-sampling.svg" alt="Design a Frontend Observability Dashboard like Datadog RUM operations" caption="Operations view: blast radius, stale versions, abuse controls, cost, privacy, and support reconstruction." />
      </section>
