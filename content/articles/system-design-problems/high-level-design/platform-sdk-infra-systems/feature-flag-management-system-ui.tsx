...
const definition = [
  "Design a Feature Flag Management System UI is a platform system, which means the product is not only the UI that users see but also the contract other teams, tenants, developers, plugins, SDKs, or automation depend on. A principal-ready design treats a feature flag management system UI as a versioned control plane with operational guarantees.",
...
const concepts = [
  "The first concept is contract-first design. flag registry, targeting engine, and approval workflow should expose stable schemas, versioned APIs, documented lifecycle states, and compatibility guarantees. Hidden contracts are what make platform migrations painful.",
...
const architecture = [
  "The architecture contains flag registry, targeting engine, approval workflow, SDK distribution, audit trail. The control plane stores configuration, versions, ownership, approvals, and audit. The execution plane applies changes through SDKs, plugins, build systems, rollout engines, or tenant jobs. The observability plane measures impact and supports rollback.",
...
const pitfalls = [
  "bad rollout usually means the platform lacks compatibility gates, staged rollout, or blast-radius controls. Platform failures multiply because many teams depend on one surface.",
  "targeting drift often comes from treating permissions or integration boundaries as documentation instead of enforceable runtime policy.",
  "stale SDK cache is a state-distribution problem. SDK caches, build artifacts, edge nodes, tenants, and clients may observe different versions unless the design tracks propagation and freshness.",
  "permission bypass requires rollback and audit. Platform users should know whether they can disable, revert, compensate, or escalate the issue.",
...
const useCases = [
  "kill switch console requires versioned contracts, safe rollout, scoped permissions, observability, and a clear rollback path.",
  "experiment rollout UI requires versioned contracts, safe rollout, scoped permissions, observability, and a clear rollback path.",
  "tenant feature management requires versioned contracts, safe rollout, scoped permissions, observability, and a clear rollback path.",
...
const questions = [
  {
    "question": "How would you design a feature flag management system UI end to end?",
  ...
  },
  {
    ...
    "answer": "The main failures are bad rollout, targeting drift, stale SDK cache, permission bypass, plus stale clients, incompatible versions, high-cardinality telemetry, queue backlogs, cross-tenant leakage, support overload, and uncontrolled blast radius. Prevention requires versioning, validation, staged rollout, idempotent execution, scoped permissions, and observability."
  },
  {
  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/platform-sdk-infra-systems/feature-flag-management-system-ui-architecture.svg" alt="Design a Feature Flag Management System UI architecture" caption="Architecture view: control plane, execution plane, permissions, versioning, and observability boundaries." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/platform-sdk-infra-systems/feature-flag-management-system-ui-rollout.svg" alt="Design a Feature Flag Management System UI flow" caption="Flow view: authoring, validation, rollout, execution, health checks, and rollback." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/platform-sdk-infra-systems/feature-flag-management-system-ui-targeting.svg" alt="Design a Feature Flag Management System UI operations" caption="Operations view: blast radius, stale versions, abuse controls, cost, privacy, and support reconstruction." />
      </section>
