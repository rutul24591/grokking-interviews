  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="crucial">The system has two planes. The control plane (the management UI and API): where flags are created and configured, targeting rules are defined, changes are audit-logged, and the flag ruleset is stored (PostgreSQL). The evaluation plane: the SDK downloads the flag ruleset for the customer's environment on initialization and evaluates flags locally in memory. SSE connections from the SDK to the Streaming Service receive real-time ruleset updates when any flag changes. The evaluation plane never contacts the control plane for individual flag evaluations—all evaluation is local, ensuring sub-millisecond latency and availability independence from the control plane.</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">The client SDK initializes by downloading the full flag ruleset for the environment (a JSON document containing all flag configurations) from a CDN-cached endpoint. The CDN caches the ruleset with a 60-second TTL; the SDK receives the latest ruleset within 60 seconds of initialization even without SSE. After initialization, the SDK opens an SSE connection to the Streaming Service; rule updates pushed via SSE update the in-memory ruleset immediately (bypassing the 60-second CDN cache).</HighlightBlock>
        <HighlightBlock as="p" tier="important">The SDK exposes two evaluation APIs: flagIsEnabled(flagKey, userContext) (returns boolean) and getVariant(flagKey, userContext) (returns the variant string or null). Both APIs evaluate the local in-memory ruleset synchronously; there is no async network call during evaluation. The userContext is a plain object containing the user's attributes (userId, email, plan, country, etc.). The SDK evaluates targeting rules against the userContext using the same algorithm as the management UI's preview mode—the client and server use identical evaluation logic, ensuring consistency.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Offline resilience: the SDK persists the downloaded ruleset to localStorage (in browser environments) or to an in-memory fallback (in server environments where localStorage is unavailable). On next initialization, if the CDN endpoint is unreachable, the SDK falls back to the persisted ruleset. The fallback ruleset may be stale (up to the age of the last successful download), but it is better than returning defaultVariant for all flags. The SDK indicates the fallback state via a isStale flag on the evaluation context, allowing the application to handle stale evaluations differently if needed (e.g., disabling A/B test tracking when the ruleset is stale).</HighlightBlock>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Real-Time Update Propagation</h3>
        <HighlightBlock as="p" tier="important">When a flag configuration is saved in the management UI, the control plane writes the change to PostgreSQL, appends an audit log entry, and publishes a ruleset_updated event to a Redis pub/sub channel keyed by the customer's environment ID. The Streaming Service is subscribed to all environment channels; on receiving the event, it re-fetches the full flag ruleset for that environment from PostgreSQL (or from a Redis cache warmed from PostgreSQL), serializes it as JSON, and pushes it to all SSE connections associated with that environment. Each SSE connection handles one SDK instance; the Streaming Service maintains a map of environment ID to a set of SSE connections.</HighlightBlock>
        <HighlightBlock as="p" tier="important">The full-ruleset push (rather than a delta of only the changed flag) simplifies the SDK's update handling: the SDK replaces its entire in-memory ruleset on each SSE event, rather than applying incremental patches that could produce inconsistent states if events arrive out of order. The full ruleset is typically 50–200KB JSON (for environments with 500–2,000 flags); this is a reasonable payload for a push that occurs at most a few times per minute per environment. For environments with very high change rates (continuous experimentation with hundreds of simultaneous experiments), the Streaming Service can apply rate limiting (coalescing multiple changes within a 500ms window into a single SSE event) to avoid overwhelming SDK instances with frequent full-ruleset pushes.</HighlightBlock>
