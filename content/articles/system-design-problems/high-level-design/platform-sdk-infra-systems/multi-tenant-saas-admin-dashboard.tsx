  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>A multi-tenant SaaS admin dashboard serves two distinct audiences with fundamentally different needs: platform operators (the SaaS company's own staff—support, engineering, sales) who need cross-tenant visibility and powerful management tools, and tenant administrators (customers who administer their own organization's account) who need scoped management tools for their own tenant only. The hardest engineering challenge is tenant isolation: a tenant administrator must never see or affect another tenant's data, while a platform operator needs to reach across tenant boundaries. Both user types interact with the same dashboard surface, requiring the authorization layer to be airtight.</p>
        <p>The second challenge is scale: a SaaS platform with 10,000+ tenants generates cross-tenant analytics that cannot be computed per-request. A query like "show me all tenants on the Pro plan who had zero active users this month" requires scanning data across all tenants—a query that cannot be satisfied from a single tenant's database shard. The admin dashboard must be backed by a cross-tenant analytics store (a data warehouse or aggregation service) that is separate from the transactional per-tenant databases.</p>
      ...
      <section>
        ...
        <p>RBAC within tenants: tenant administrators can have sub-roles within their organization (owner, admin, member, viewer). The roles determine which management actions are available: only owners can delete the organization or transfer billing; admins can invite and remove members; members have read-only management access. The RBAC is enforced by a permissions middleware layer that checks the user's role within the tenant against the required permission for each action. Permissions are defined as a flat list of permission strings (e.g., members:invite, billing:view) rather than a role hierarchy, which makes permission checks explicit and auditable.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Cross-Tenant Analytics</h3>
        <p>Cross-tenant queries (tenant list with usage metrics, cohort analysis, plan distribution) cannot be served from the transactional database without full-table scans that would degrade production performance. These queries are served by a ClickHouse analytics database that receives data from the primary database via a CDC (Change Data Capture) pipeline (Debezium → Kafka → ClickHouse). ClickHouse's columnar storage and vectorized query execution make cross-tenant aggregations over millions of rows respond in under 3 seconds.</p>
        <p>Pre-aggregated metrics: for the most common admin queries (active users per tenant in the last 30 days, storage usage per tenant, API calls per tenant per hour), the Analytics Service pre-computes and caches aggregates on a 1-hour schedule. Pre-aggregated results are stored in Redis keyed by the query type and time range. The admin dashboard UI reads from Redis for these common queries (sub-100ms response), falling through to ClickHouse for custom date ranges or filters that are not pre-aggregated. This two-tier approach (Redis for common queries, ClickHouse for custom) satisfies the 3-second latency requirement for the common case while preserving flexibility for ad-hoc analysis.</p>
