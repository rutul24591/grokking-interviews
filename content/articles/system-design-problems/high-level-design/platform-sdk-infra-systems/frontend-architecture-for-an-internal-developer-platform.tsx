...
const definition = [
  "Design Frontend Architecture for an Internal Developer Platform is a platform system, which means the product is not only the UI that users see but also the contract other teams, tenants, developers, plugins, SDKs, or automation depend on. A principal-ready design treats an internal developer platform frontend as a versioned control plane with operational guarantees.",
...
const concepts = [
  "The first concept is contract-first design. service catalog, workflow runner, and plugin shell should expose stable schemas, versioned APIs, documented lifecycle states, and compatibility guarantees. Hidden contracts are what make platform migrations painful.",
...
const architecture = [
  "The architecture contains service catalog, workflow runner, plugin shell, permission model, observability panel. The control plane stores configuration, versions, ownership, approvals, and audit. The execution plane applies changes through SDKs, plugins, build systems, rollout engines, or tenant jobs. The observability plane measures impact and supports rollback.",
...
const pitfalls = [
  "plugin sprawl usually means the platform lacks compatibility gates, staged rollout, or blast-radius controls. Platform failures multiply because many teams depend on one surface.",
  "stale ownership often comes from treating permissions or integration boundaries as documentation instead of enforceable runtime policy.",
  "workflow failure is a state-distribution problem. SDK caches, build artifacts, edge nodes, tenants, and clients may observe different versions unless the design tracks propagation and freshness.",
  "permission leakage requires rollback and audit. Platform users should know whether they can disable, revert, compensate, or escalate the issue.",
...
const useCases = [
  "service portal requires versioned contracts, safe rollout, scoped permissions, observability, and a clear rollback path.",
  "deployment console requires versioned contracts, safe rollout, scoped permissions, observability, and a clear rollback path.",
  "developer self-service platform requires versioned contracts, safe rollout, scoped permissions, observability, and a clear rollback path.",
...
const questions = [
  {
    "question": "How would you design an internal developer platform frontend end to end?",
  ...
  },
  {
    ...
    "answer": "The main failures are plugin sprawl, stale ownership, workflow failure, permission leakage, plus stale clients, incompatible versions, high-cardinality telemetry, queue backlogs, cross-tenant leakage, support overload, and uncontrolled blast radius. Prevention requires versioning, validation, staged rollout, idempotent execution, scoped permissions, and observability."
  },
  {
  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/platform-sdk-infra-systems/frontend-architecture-for-an-internal-developer-platform-architecture.svg" alt="Design Frontend Architecture for an Internal Developer Platform architecture" caption="Architecture view: control plane, execution plane, permissions, versioning, and observability boundaries." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/platform-sdk-infra-systems/frontend-architecture-for-an-internal-developer-platform-plugins.svg" alt="Design Frontend Architecture for an Internal Developer Platform flow" caption="Flow view: authoring, validation, rollout, execution, health checks, and rollback." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/platform-sdk-infra-systems/frontend-architecture-for-an-internal-developer-platform-workflows.svg" alt="Design Frontend Architecture for an Internal Developer Platform operations" caption="Operations view: blast radius, stale versions, abuse controls, cost, privacy, and support reconstruction." />
      </section>
