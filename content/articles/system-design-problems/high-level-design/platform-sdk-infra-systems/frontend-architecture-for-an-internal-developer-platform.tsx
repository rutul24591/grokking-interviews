  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <p>Data aggregation: the IDP backend aggregates data from multiple sources to populate this view. Deployment status comes from the CD system (ArgoCD or Spinnaker). Kubernetes pod status comes from the Kubernetes API (proxied through the IDP backend—engineers do not have direct kubectl access; they interact with the cluster only through the IDP). Pipeline run history comes from the CI system (GitHub Actions, CircleCI, or Jenkins). The BFF aggregates these three sources into a unified response per service, caching each source independently (Kubernetes pod status cached for 10 seconds, pipeline history cached for 60 seconds, deployment status cached for 30 seconds) so that a slow GitHub Actions API does not block the rendering of Kubernetes status.</p>
