  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <p>The builder has two co-running UIs: the builder canvas (the editing environment) and the preview iframe (the runtime environment). The app definition—a JSON document describing all components, their properties, layout, and data source bindings—is the shared state between them. The builder canvas edits the app definition; the preview iframe renders it. Changes to the app definition are transmitted to the preview iframe via postMessage; the runtime re-renders the changed components. The separation of builder and runtime into different JavaScript contexts (the preview runs in a sandboxed iframe) prevents the runtime's components from interfering with the builder's UI and vice versa.</p>
      </section>
      ...
      <section>
        ...
        <p>Collaborative editing: when multiple builders edit the same app simultaneously, changes from one builder must be merged with the other's in-progress edits. This is the same problem as the collaborative editor, and the solution is similar: operations are OT-transformed (for simple property changes, last-write-wins is sufficient; for structural changes like add/delete component, OT is needed to avoid conflicts). For most no-code builders, collaborative editing is a premium feature with limited concurrent editor counts (2–5 simultaneous editors), making simpler conflict resolution (optimistic last-write-wins with a "someone else made a change" notification and a refresh option) sufficient for the initial implementation.</p>
      </section>
