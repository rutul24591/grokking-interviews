  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>A customer support platform (Zendesk-like) is a multi-channel ticketing system that unifies customer conversations from email, live chat, web forms, social media, and API integrations into a single agent workspace. The core value proposition is that a support agent should never have to switch between tabs to handle a customer inquiry—all context (conversation history, customer profile, order history, prior tickets) is available in one view, and the ticket lifecycle (creation → assignment → resolution → closing) is managed through a single interface.</p>
        <p>The engineering challenge has several dimensions. Ticket ingestion must be real-time for chat channels (a chat message must appear in the agent workspace within 1 second) and near-real-time for email (within 30 seconds of receipt). Routing must be intelligent—assigning tickets to agents with the right skills, within their capacity, and respecting SLA time-to-first-response commitments. SLA tracking requires a precise timer system that pauses when awaiting customer reply and resumes when the customer responds. Collaboration is complex: multiple agents may view a ticket simultaneously; a supervisor may add an internal note; an agent may be reassigned mid-conversation. All of these concurrent mutations must be handled without data races.</p>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Real-time latency:</strong> Chat messages must appear in the agent workspace within 1 second. Status changes and new ticket assignments must be pushed to agents within 2 seconds.</li>
