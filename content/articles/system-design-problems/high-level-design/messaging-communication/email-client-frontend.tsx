    ...
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Inbox synchronization:</strong> The inbox reflects the server state within 30 seconds without user action. New emails arrive via server-sent events (SSE) or WebSocket push notifications with email metadata (subject, sender, snippet, timestamp, unread flag). Full message bodies are fetched on-demand when the user opens a thread. The JMAP protocol provides delta sync — the client sends its current state token and receives only changes since the last sync, not the full inbox.</li>
      ...
      <section>
        ...
        <p>JMAP (JSON Meta Application Protocol, RFC 8620) is an email protocol designed for efficient client-server sync. Unlike IMAP (which requires the client to poll for every change and download full message metadata), JMAP provides a state token per mailbox. The client sends its current state token with each sync request: POST /api/jmap with body &#123;"using": ["urn:ietf:params:jmap:mail"], "methodCalls": [["Email/changes", &#123;"sinceState": "current_token"&#125;, "0"]]&#125;. The server returns only the IDs of emails that have been created, updated, or destroyed since that state token, plus a new state token. The client fetches the full metadata for changed emails in a follow-up call. This means a sync round trip typically transfers a few hundred bytes (a list of changed IDs) rather than the full inbox.</p>
        <p>The state token is stored in IndexedDB alongside the thread headers. On page load, the client reads the cached state token, performs a delta sync to catch up from the last session, and then establishes an SSE connection for real-time push notifications of new emails. New email notifications via SSE contain only the thread ID — the client fetches the thread header via JMAP in a background call and inserts it into the inbox list.</p>
