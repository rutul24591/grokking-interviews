  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <p>The system has three layers. The management UI (browser): the prompt editor, version timeline, evaluation playground, A/B dashboard, and analytics charts. The prompt service (backend): stores prompt versions, resolves the active version for a given prompt ID and request context, manages A/B assignment, and exposes evaluation APIs. The observability pipeline: collects per-call metrics (latency, token count, cost, user feedback) tagged by prompt ID and version, aggregated into a time-series store for the analytics dashboard.</p>
      </section>
      ...
      <section>
        ...
        <p>A prompt template is a string with named variable slots: &#123;&#123;company&#125;&#125;, &#123;&#123;tone&#125;&#125;, &#123;&#123;context&#125;&#125;. Each variable is declared in a schema attached to the template: name (string), type (string / enum / list / number), description (for UI display and LLM documentation), default value (optional), and required (boolean). The schema serves three purposes: input validation (the playground and production call path validate that all required variables are provided before rendering the prompt), documentation (the schema is displayed in the template browser so other users know how to use the template), and token estimation (variable defaults are used for token count estimation in the editor's cost display).</p>
        <p>Prompt rendering: at call time (in the LLM proxy), the prompt template is fetched from the in-memory cache, variables are substituted, and the rendered prompt string is sent to the model. The rendering is pure string substitution—no server-side template engine is invoked. If a required variable is missing, the proxy returns a 400 error before making the LLM call, preventing unrendered &#123;&#123;variable&#125;&#125; literals from reaching the model.</p>
