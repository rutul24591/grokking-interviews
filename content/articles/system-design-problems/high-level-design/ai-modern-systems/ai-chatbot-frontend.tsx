  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <p>The chatbot frontend has three layers: the UI layer (message list renderer, input composer, streaming indicator, tool call cards), the state layer (conversation store managing message history, streaming state, and context window usage), and the transport layer (the streaming fetch connection and its token buffering pipeline). The backend is a thin proxy: an API route that receives the user's message and conversation history, constructs the prompt, calls the LLM's streaming API, and relays the response stream back to the browser. The browser never calls the LLM API directly (to keep API keys server-side and to allow server-side prompt augmentation).</p>
      </section>
      ...
      <section>
        ...
        <p>Conversation history is stored in IndexedDB (using a library like Dexie.js for a clean API). Each conversation has a conversationId, a list of messages (role: user/assistant/tool, content, timestamp, tokenCount), and metadata (title, model, created/updated timestamps). On page load, the active conversation is loaded from IndexedDB into memory. New messages are appended to IndexedDB synchronously with each turn (before the response is complete, to survive page reloads during generation).</p>
        <p>The virtual message list: conversations with 500+ messages cannot efficiently render all message components simultaneously. A virtualized list (using react-virtual or a custom implementation) renders only the messages within or near the viewport (typically 10–20 messages). As the user scrolls up (to read history), older messages are rendered on demand. The virtualized list maintains scroll position correctly during streaming (new tokens appended to the last message do not cause scroll jitter because the last message is always at the bottom of the viewport).</p>
        <p>Context window management: the backend tracks the token count of the conversation history being sent to the model. As the conversation grows, older messages are summarized or dropped from the context (the oldest messages are pruned first, preserving the most recent context). The UI shows a context usage bar (e.g., "Used 45K / 200K tokens"). When the context reaches 80% capacity, the UI shows a warning: "This conversation is getting long. Consider starting a new thread or summarizing." The user can trigger a "summarize conversation" action that sends the current history to the model with a summarization prompt and replaces the history with the summary.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Multimodal Input Handling</h3>
        <p>The message composer handles three non-text input types. Image input: the user pastes an image (paste event) or selects via file picker. The UI shows an inline thumbnail preview in the composer. Images are resized client-side (using a Canvas element) to the model's maximum image dimension (typically 1568px on the longest side for Claude's vision capability) and encoded as base64. For large images (&gt; 5MB), the client uploads to a presigned S3 URL and includes the URL in the message payload instead of base64 (to avoid sending a large base64 string in the JSON body). The backend fetches the image from S3 before forwarding to the model.</p>
