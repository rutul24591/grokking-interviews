    ...
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ul className="space-y-2">
          ...
          <HighlightBlock as="li" tier="important"><strong>Pagination:</strong> Results paginate at 24 items per page. Applying a filter resets to page 1. The current page is in the URL (?page=2). Sort order is also in the URL (?sort=price_asc) and is preserved when filters change.</HighlightBlock>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="important"><strong>Filter application latency:</strong> Results and facet counts update within 300ms of applying a filter. The result list shows a loading state (skeleton cards) during the update, not a blank page.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Facet count accuracy:</strong> Facet counts are computed server-side (not estimated) using Elasticsearch aggregations for datasets up to 10 million items. For larger datasets, approximate counts (using Elasticsearch cardinality aggregations with a max_doc_count of 40,000) are acceptable with a ±5% error displayed as "~234".</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Zero-result prevention:</strong> Facet values that would produce zero results (given the current filter context) are greyed out and non-selectable, not hidden — users can see that the option exists but understand it has no results in their current context.</HighlightBlock>
        </ul>
      </section>

      <section>
        <h2>High-Level Architecture</h2>
        <HighlightBlock as="p" tier="important">The faceted search system uses Elasticsearch as the search and aggregation backend. A single Elasticsearch query returns both the result set (paginated) and the facet counts (aggregations) in one request. The frontend sends the current filter state (parsed from the URL) to the Search API, which constructs an Elasticsearch query with: a bool/filter for the applied filters, a multi-bucket aggregation for each facet dimension, and a top-hits aggregation for the result items. The response includes both the result items and the facet counts. The URL is the single source of truth for all filter state — the React component reads filter state from the URL (using a useSearchParams hook), not from a Zustand store. This means direct URL navigation always shows the correct state without any state synchronization.</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">The URL is the single source of truth for all filter state, enabling shareability, bookmarkability, and correct browser Back/Forward navigation. The URL encoding: checkbox filters use repeated parameters (?brand=nike&brand=adidas), range filters use min/max parameters (?price_min=50&price_max=200), sort uses a single parameter (?sort=price_asc), and page uses a single parameter (?page=2). A useSearchParams hook (from Next.js or React Router) reads the URL on every navigation and derives the current filter state. When the user changes a filter (checks a checkbox, adjusts a slider), the filter change is encoded into the URL using window.history.pushState — this updates the URL without a page reload, triggers a re-render with the new URL state, and adds a history entry so the browser Back button restores the previous filter state.</HighlightBlock>
        <HighlightBlock as="p" tier="crucial">The filter state object is derived from URL params on every render — there is no separate filter state in React state or Zustand. This eliminates an entire class of state synchronization bugs (where URL state and React state diverge). The trade-off: every URL change triggers a re-render and a new Elasticsearch query. To prevent excessive queries during slider drag (the user drags a price slider, firing many intermediate values), range slider changes are debounced (300ms) before updating the URL.</HighlightBlock>

        <h3 className="mt-6 mb-3 text-lg font-semibold">Elasticsearch Query and Aggregation Design</h3>
        <HighlightBlock as="p" tier="important">A single Elasticsearch query serves both the result items and the facet counts. The query structure: a bool/filter clause applies all current filters to the result set. Each facet aggregation (terms aggregation for checkbox facets, histogram aggregation for range facets) computes counts across the filtered result set. The critical nuance is that selected facet values should not filter their own aggregation — if the user has selected "Brand: Nike," the brand aggregation should still show all brands (so the user can see how many Adidas items exist and can switch to Adidas if desired). This is implemented using Elasticsearch post-filter: the filter is applied after aggregations are computed, so aggregations always run on the full result set, and only the returned hits are filtered. This is sometimes called the "exclude selected" pattern.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Facet count accuracy: for datasets up to 10 million items, Elasticsearch terms aggregations are exact. For larger datasets, the size parameter (how many buckets to return) must be balanced against accuracy — requesting size=1000 for a brand aggregation is accurate but slow. Elasticsearch's shard_size parameter (how many buckets to collect per shard before global merging) can be tuned for the accuracy/performance trade-off. For very high-cardinality facets (e.g., seller name in a marketplace with 100K sellers), a filter-only approach (show only the top 20 sellers, let the user search within the facet) is preferable over returning all 100K with counts.</HighlightBlock>

        <h3 className="mt-6 mb-3 text-lg font-semibold">Facet Panel Rendering</h3>
        <HighlightBlock as="p" tier="important">The facet panel renders different control types based on the facet dimension type. Checkbox facets (brand, color, size) render a scrollable list of checkboxes, each showing the option label and its result count. The list is collapsed to the top 5 items by default with a "Show N more" button that expands the full list. Items are sorted by count (descending) by default, but selected items are pinned to the top so the user can see what they've applied. Zero-count items are greyed out and non-interactive (not hidden — hiding them would confuse users who remember seeing an option).</HighlightBlock>
        <HighlightBlock as="p" tier="important">Range slider facets (price, rating) render a dual-thumb slider with a histogram of item distribution overlaid on the track. The histogram is derived from the histogram aggregation in the Elasticsearch response (e.g., price values grouped in $50 buckets). The visual histogram helps users understand where items are concentrated in the price range. The slider thumbs snap to histogram bucket boundaries when dragging for better UX (smaller steps feel meaningless when there are no items in that price band). Date range facets render from/to date pickers. Hierarchical facets (category tree) render as an expandable tree: clicking a top-level category (Electronics) expands it to show subcategories (Phones, Laptops, Tablets) with their counts. Selecting a subcategory narrows the filter to that level.</HighlightBlock>

        <h3 className="mt-6 mb-3 text-lg font-semibold">Result Caching Strategy</h3>
        <HighlightBlock as="p" tier="important">Faceted search results are highly cacheable because filter combinations repeat across users (many users filter for Nike shoes in size 10, price under $150). The cache key is SHA256(filter_state_JSON + query + sort + page), normalized so that equivalent filter states produce the same key regardless of parameter order. Results are cached in Redis with a 2-minute TTL. Cache invalidation: when new items are indexed or existing items are updated (e.g., an item goes out of stock), a Kafka event triggers cache invalidation for affected keys. For high-traffic facet combinations (top 100 most common filter sets, computed from analytics), a background job pre-warms the cache every minute, ensuring that common filters are always served from cache. The cache pre-warming job runs the Elasticsearch queries and stores results ahead of user requests.</HighlightBlock>
      </section>

      <section>
        <h2>Trade-offs and Considerations</h2>
        <HighlightBlock as="p" tier="important">Client-side versus server-side facet application: client-side faceting (download all items once, filter in the browser using JavaScript) is only feasible for small datasets (under 10,000 items). For larger datasets, server-side faceting is required — the browser cannot download 10 million items to filter client-side. The client-side approach has one advantage: filter application is instant (no network latency), making the UI feel highly responsive. The hybrid approach (client-side filter for the currently loaded page of results, server-side re-fetch for subsequent pages) is used by some implementations but creates a confusing experience where filtering within the current page is instant but getting more results requires a server round-trip.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Facet ordering (by count versus alphabetical versus business priority): by default, facets are ordered by count (most results first), which helps users navigate to the most populated filters quickly. However, business logic may override this: a seller might want their own brand listed first in the brand facet, or an e-commerce platform might want to promote premium brands. The facet ordering algorithm should be configurable per facet dimension, supporting count-based, alphabetical, explicit order lists (pinned items at top), and promoted items (business-priority items that appear above the count-sorted list).</HighlightBlock>
      </section>

      <section>
        <h2>Summary</h2>
        <HighlightBlock as="p" tier="important">Faceted search for large datasets uses URL as the single source of truth for all filter state (useSearchParams → filter state object → Elasticsearch query → pushState on change). A single Elasticsearch query returns both results and facet counts using post-filter (aggregations run pre-filter so selected facet values don't suppress their own counts). The facet panel renders checkbox lists (with top-5 collapse + "Show more"), range sliders with histogram overlays (from histogram aggregations), hierarchical category trees, and date range pickers. Zero-count facet options are greyed out, not hidden. Applied filters are surfaced as removable chips derived from URL params. Redis caches results at SHA256(filter_state) with 2-minute TTL, invalidated on index updates via Kafka; top-100 filter combinations are pre-warmed. The core design constraint: every filter interaction must update counts, results, chips, and the URL atomically in one operation — the URL is the state, not React state, ensuring Back navigation always works correctly.</HighlightBlock>
      </section>
    </ArticleLayout>
  );
}
