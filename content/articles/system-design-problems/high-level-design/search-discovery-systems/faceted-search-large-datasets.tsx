...
const concepts = [
  ...
  "The second concept is retrieval strategy. facet engine, filter state, and result index may use lexical retrieval, vector retrieval, structured filters, popularity priors, freshness boosts, or hybrid retrieval depending on the product.",
...
const architecture = [
  "The architecture contains query parser, facet engine, filter state, result index, aggregation cache. The ingestion path normalizes documents or items and builds indexes. The query path parses user intent, retrieves candidates, ranks them, applies policy and permissions, and renders results. The feedback path records behavior for analytics, quality review, and controlled ranking updates.",
...
const pitfalls = [
  "facet count drift is usually caused by optimizing one path while ignoring the full discovery loop. Suggestions, results, facets, and recommendations must share policy and freshness assumptions.",
  "high-cardinality filters becomes a trust issue when ranking rewards behavior without integrity checks. Search systems need spam, abuse, and business-rule guardrails.",
  "slow aggregation happens when indexes and derived projections are treated as secondary. Users experience stale search as product incorrectness, not infrastructure lag.",
  "empty result traps is dangerous because search can expose private or unsafe content through snippets, suggestions, facets, caches, and analytics dashboards.",
...
const useCases = [
  "commerce catalog filters requires query understanding, retrieval, ranking, filtering, presentation, and feedback to work as one system rather than separate widgets.",
  "log analytics search requires query understanding, retrieval, ranking, filtering, presentation, and feedback to work as one system rather than separate widgets.",
  "BI data exploration requires query understanding, retrieval, ranking, filtering, presentation, and feedback to work as one system rather than separate widgets.",
...
const questions = [
  {
    "question": "How would you design faceted search over large datasets end to end?",
  ...
  },
  {
    ...
    "answer": "The main failures are facet count drift, high-cardinality filters, slow aggregation, empty result traps, plus cache stampedes, index lag, high-cardinality facets, hot queries, ranking drift, spam manipulation, privacy leaks in logs, and experiment regressions. Prevention requires versioned indexes, stable cursors, guardrail metrics, query sampling, cache strategy, and operational rollback."
  },
  {
...
export default function FacetedSearchLargeDatasetsArticle() {
  return (
    <ArticleLayout metadata={metadata}>
      <section><h2>Definition &amp; Context</h2><HighlightBlock as="p" tier="crucial" className="mb-4">System-design interview lens: frame Design Faceted Search for Large Datasets around query intent, ranking, filtering, latency, explainability, and feedback-driven relevance. This is the difference between describing a feature and designing a production system.</HighlightBlock><HighlightBlock as="p" tier="important" className="mb-4">Clarify the product promise, the non-negotiable correctness boundary, the main actor, and the failure mode users would actually notice first.</HighlightBlock><HighlightBlock as="p" tier="important">{definition[0]}</HighlightBlock>{definition.slice(1).map((item) => <p key={item}>{item}</p>)}</section>
      <section><h2>Core Concepts</h2><HighlightBlock as="p" tier="crucial" className="mb-4">Core interview invariant: query, filters, ranking experiment, pagination cursor, and rendered results must refer to the same search snapshot.</HighlightBlock><HighlightBlock as="p" tier="important" className="mb-4">Name the source of truth, derived state, speculative state, cache state, and audit or telemetry state separately; collapsing them hides most real design bugs.</HighlightBlock><HighlightBlock as="p" tier="important" className="mb-4">For Design Faceted Search for Large Datasets, the interviewer is checking whether you can defend why each subsystem exists, not just list components in a diagram.</HighlightBlock>{concepts.map((item, index) => index === 3 ? <HighlightBlock as="p" tier="crucial" key={item}>{item}</HighlightBlock> : <p key={item}>{item}</p>)}</section>
      <section>
        <h2>Architecture &amp; Flow</h2><HighlightBlock as="p" tier="crucial" className="mb-4">Architecture decisions to make explicit: index freshness, autocomplete path, facet model, ranking features, experiment bucketing, URL state, and empty-state recovery.</HighlightBlock><HighlightBlock as="p" tier="important" className="mb-4">Walk the hard path end to end: permission check, input validation, async work, timeout or partial failure, user-visible fallback, telemetry, and rollback.</HighlightBlock><HighlightBlock as="p" tier="important" className="mb-4">Call out which path is synchronous, which path is asynchronous, which artifacts are immutable, and which updates may arrive out of order.</HighlightBlock>
        {architecture.map((item, index) => index === 0 ? <HighlightBlock as="p" tier="important" key={item}>{item}</HighlightBlock> : <p key={item}>{item}</p>)}
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/search-discovery-systems/faceted-search-large-datasets.svg" alt="Design Faceted Search for Large Datasets architecture" caption="Architecture view: ingestion, query understanding, retrieval, ranking, policy, rendering, and feedback loops." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/search-discovery-systems/faceted-search-large-datasets-flow.svg" alt="Design Faceted Search for Large Datasets flow" caption="Flow view: query lifecycle, candidate retrieval, ranking, filtering, result presentation, and quality feedback." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/search-discovery-systems/faceted-search-large-datasets-operations.svg" alt="Design Faceted Search for Large Datasets operations" caption="Operations view: index freshness, ranking rollback, spam controls, privacy-safe logs, and search quality observability." />
      </section>
