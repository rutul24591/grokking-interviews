...
const concepts = [
  ...
  "The second concept is retrieval strategy. video index, recommendation ranker, and watch history may use lexical retrieval, vector retrieval, structured filters, popularity priors, freshness boosts, or hybrid retrieval depending on the product.",
...
const architecture = [
  "The architecture contains query intent, video index, recommendation ranker, watch history, safety filters. The ingestion path normalizes documents or items and builds indexes. The query path parses user intent, retrieves candidates, ranks them, applies policy and permissions, and renders results. The feedback path records behavior for analytics, quality review, and controlled ranking updates.",
...
const pitfalls = [
  "clickbait amplification is usually caused by optimizing one path while ignoring the full discovery loop. Suggestions, results, facets, and recommendations must share policy and freshness assumptions.",
  "cold start becomes a trust issue when ranking rewards behavior without integrity checks. Search systems need spam, abuse, and business-rule guardrails.",
  "unsafe recommendations happens when indexes and derived projections are treated as secondary. Users experience stale search as product incorrectness, not infrastructure lag.",
  "freshness bias is dangerous because search can expose private or unsafe content through snippets, suggestions, facets, caches, and analytics dashboards.",
...
const useCases = [
  "video search requires query understanding, retrieval, ranking, filtering, presentation, and feedback to work as one system rather than separate widgets.",
  "home recommendations requires query understanding, retrieval, ranking, filtering, presentation, and feedback to work as one system rather than separate widgets.",
  "watch-next rail requires query understanding, retrieval, ranking, filtering, presentation, and feedback to work as one system rather than separate widgets.",
...
const questions = [
  {
    "question": "How would you design a video search and recommendation UI end to end?",
  ...
  },
  {
    ...
    "answer": "The main failures are clickbait amplification, cold start, unsafe recommendations, freshness bias, plus cache stampedes, index lag, high-cardinality facets, hot queries, ranking drift, spam manipulation, privacy leaks in logs, and experiment regressions. Prevention requires versioned indexes, stable cursors, guardrail metrics, query sampling, cache strategy, and operational rollback."
  },
  {
  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/search-discovery-systems/youtube-search-recommendation-ui.svg" alt="Design a YouTube Search and Recommendation UI architecture" caption="Architecture view: ingestion, query understanding, retrieval, ranking, policy, rendering, and feedback loops." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/search-discovery-systems/youtube-search-recommendation-ui-flow.svg" alt="Design a YouTube Search and Recommendation UI flow" caption="Flow view: query lifecycle, candidate retrieval, ranking, filtering, result presentation, and quality feedback." />
        <ArticleImage src="/diagrams/system-design-problems/high-level-design/search-discovery-systems/youtube-search-recommendation-ui-operations.svg" alt="Design a YouTube Search and Recommendation UI operations" caption="Operations view: index freshness, ranking rollback, spam controls, privacy-safe logs, and search quality observability." />
      </section>
