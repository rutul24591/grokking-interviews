    ...
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Page load:</strong> Homepage loads within 1.5 seconds globally (Core Web Vitals: LCP &lt;2.5s, FID &lt;100ms, CLS &lt;0.1). Cache hit ratio &gt;95% for all static assets. HTML shell served from CDN edge within 50ms of request.</li>
          <li><strong>Authentication:</strong> Login/session management scales to 1M concurrent sessions. Session tokens stored in Redis cluster (sharded, 3-replica per shard). API requests authenticated via JWT (stateless, verifiable at the edge without hitting Redis for every request).</li>
          <li><strong>Real-time updates:</strong> Push updates to 1M connected clients via WebSocket or SSE. Updates are fan-out events (e.g., global announcement, trending content) delivered to all connected clients. Per-user personalized pushes are delivered to the user's specific connection.</li>
          <li><strong>API responses:</strong> All read APIs are cacheable with stale-while-revalidate. Write APIs (POST/PUT/DELETE) bypass cache. Read/write ratio is 95:5 for typical content platforms.</li>
        </ul>

        <h3 className="mt-6 mb-3 text-lg font-semibold">Non-Functional Requirements</h3>
        <ul className="space-y-2">
          <li><strong>Availability:</strong> 99.99% uptime (52 minutes downtime/year). Achieved via multi-region deployment with automatic failover, not active-active everywhere.</li>
          <li><strong>Throughput:</strong> CDN tier handles 1M+ concurrent users. API gateway tier handles 50K–100K requests/second (5–10% of users making API calls at any given moment). WebSocket tier handles 1M persistent connections across a cluster of connection servers.</li>
          <li><strong>Load shedding:</strong> Under extreme load, non-critical API endpoints are shed first (recommendations, trending lists) while critical paths (login, read primary content) are preserved.</li>
        </ul>
      </section>

      <section>
        <h2>High-Level Architecture</h2>
        <p>The architecture has four distinct tiers. The CDN Tier (Cloudflare, Fastly, or Akamai) serves all static assets and the HTML shell with immutable cache headers (Cache-Control: public, max-age=31536000, immutable for hashed JS/CSS bundles). The API Gateway Tier (Kong, AWS API Gateway, or a custom Nginx cluster) handles authenticated API requests, enforces rate limits (per-IP and per-user), and routes to backend services. The Application Tier consists of stateless Next.js SSR servers that generate personalized HTML when CDN cache is absent (cache miss path) and stateless API servers. The Real-Time Tier is a cluster of WebSocket servers (each handling 50K connections) behind a load balancer, connected to a Redis pub/sub cluster for message fan-out. The five tiers are designed to fail independently — CDN outage degrades to direct origin access; WebSocket server loss causes graceful degradation to polling.</p>
      </section>
