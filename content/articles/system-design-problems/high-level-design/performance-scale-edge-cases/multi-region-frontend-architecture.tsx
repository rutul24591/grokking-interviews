  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>A multi-region frontend architecture serves users in multiple geographic regions with the lowest possible latency, while maintaining data residency compliance and surviving regional failures. The fundamental challenge is that a single-origin server in us-east-1 has an unavoidable physical latency of 150–250ms for users in Mumbai (speed-of-light distance) and 200–300ms for Tokyo, before any application processing. For every 100ms of latency, e-commerce conversion rates drop approximately 1%. Multi-region deployment addresses this by placing compute and data close to users.</p>
        <p>But multi-region introduces new challenges: data consistency across regions (which region's database is authoritative for a write?), data residency (EU GDPR requires that EU user data stays in EU infrastructure; India's data localization laws require certain categories of data to be stored in India), failover (how do users in eu-west-1 continue to be served when the EU region goes down?), and routing (how does a user's browser know which region to talk to?). These challenges require careful coordination between DNS, CDN, API gateways, and the backend data layer.</p>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Geographic routing:</strong> Users are automatically routed to the nearest healthy region. Routing is transparent to the user — the same domain (api.example.com) resolves to different IP addresses depending on the user's location. Routing latency overhead is under 5ms (DNS TTL optimization).</li>
          <li><strong>Static assets:</strong> JS bundles, CSS, fonts, and images are served from CDN edge nodes globally (Cloudflare or Fastly anycast network with 300+ PoPs). Static asset latency is under 50ms for 95% of global users, regardless of origin region.</li>
          <li><strong>API routing:</strong> Read API requests are served by the nearest region's read replica. Write API requests are routed to the primary region for the user's data residency zone (EU writes go to eu-west-1, US writes go to us-east-1, India writes go to ap-south-1). Session data follows the user's assigned region.</li>
          <li><strong>Failover:</strong> If a region becomes unhealthy (&gt;5% error rate for 30 seconds), GeoDNS automatically routes new connections to the next-nearest healthy region within 60 seconds (DNS TTL of 30 seconds + 30-second detection window).</li>
        </ul>

        <h3 className="mt-6 mb-3 text-lg font-semibold">Non-Functional Requirements</h3>
        <ul className="space-y-2">
          <li><strong>Latency targets:</strong> API p95 latency under 200ms for users within their assigned region. Static asset p95 latency under 50ms globally. DNS resolution under 10ms (GeoDNS with anycast).</li>
          <li><strong>Data residency:</strong> EU user PII (email, address, payment data) stored exclusively in eu-west-1 and eu-central-1. India user financial data stored exclusively in ap-south-1. Non-PII data (analytics, product catalog) is replicated globally for read performance. Data residency is enforced at the API gateway layer, not just policy — requests routing EU data to non-EU infrastructure are rejected with 451 Unavailable For Legal Reasons.</li>
          <li><strong>Availability:</strong> 99.99% global uptime. Single-region failure does not cause a global outage. Planned maintenance can be performed per-region with zero downtime using canary routing.</li>
        </ul>
      </section>

      <section>
        <h2>High-Level Architecture</h2>
        <p>The architecture has five layers. The DNS Layer uses GeoDNS (AWS Route 53 with latency-based routing, or Cloudflare Load Balancing) to resolve api.example.com to the IP of the nearest healthy region. DNS TTL is 30 seconds to enable fast failover. The CDN Layer (Cloudflare anycast) serves all static assets globally with immutable caching — the CDN does not distinguish between regions for static content (a JS bundle is identical worldwide). The Edge Layer (Cloudflare Workers, deployed globally) handles JWT verification, geo-tagging of requests (adding X-User-Region header based on IP geolocation), and data-residency enforcement before any request reaches the origin. The Regional API Layer consists of three independent API clusters (us-east-1, eu-west-1, ap-south-1), each with its own stateless application servers, a read replica of the global product catalog, and a primary database for region-specific user data. The Global Data Layer consists of a globally replicated Aurora PostgreSQL cluster for non-PII data (async replication, eventual consistency) and region-isolated RDS instances for PII data (no cross-region replication).</p>
      </section>
