...
export const metadata: ArticleMetadata = {
  ...
  tags: ["hld","retry","recovery","failure-ux","reliability"],
...
export default function RetryFailureRecoveryUxArticle() {
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        <h2>Definition &amp; Context</h2><HighlightBlock as="p" tier="crucial" className="mb-4">System-design interview lens: frame Design Retry, Failure &amp; Recovery UX around system boundary, state ownership, failure handling, scalability, security, and observable recovery. This is the difference between describing a feature and designing a production system.</HighlightBlock><HighlightBlock as="p" tier="important" className="mb-4">Clarify the product promise, the non-negotiable correctness boundary, the main actor, and the failure mode users would actually notice first.</HighlightBlock>
        <HighlightBlock as="p" tier="important">
          A user-facing failure recovery system is the reliability layer that helps end users, support teams, API owners, platform engineers, and product teams designing critical workflows survive partial failures without losing trust or evidence. It is not just a modal, toast, or dashboard. It is a system for classifying failure, containing blast radius, guiding recovery, and creating enough signal for engineering teams to fix the cause.
        </HighlightBlock>
        <p>
          For staff and principal interviews, retry and recovery UX should be discussed as part of the product architecture. Users do not care whether the failure came from a frontend render crash, an API timeout, a stale projection, or a dependency outage. They care whether the system is truthful, recoverable, and safe.
        </p>
        <p>
      ...
      <section>
        <h2>Core Concepts</h2><HighlightBlock as="p" tier="crucial" className="mb-4">Core interview invariant: the design must preserve correctness under latency, concurrency, partial failure, and changing permissions.</HighlightBlock><HighlightBlock as="p" tier="important" className="mb-4">Name the source of truth, derived state, speculative state, cache state, and audit or telemetry state separately; collapsing them hides most real design bugs.</HighlightBlock><HighlightBlock as="p" tier="important" className="mb-4">For Design Retry, Failure &amp; Recovery UX, the interviewer is checking whether you can defend why each subsystem exists, not just list components in a diagram.</HighlightBlock>
        <p>
          The core entities are operations, retry policies, idempotency keys, failed mutations, offline queues, conflict states, recovery messages, and support traces. Each entity needs ownership, lifecycle state, correlation identifiers, severity, privacy classification, and a relationship to release or operation context. Without those fields, the system cannot answer whether a failure is isolated, repeated, customer-specific, or caused by a deployment.
        </p>
        <p>
        ...
        <p>
          Reliability design should define a trust contract for the retry operation. The contract says what the user is allowed to assume during failure: whether data is fresh, whether an action is pending, whether local input was preserved, whether a retry is safe, and whether support can reconstruct the event later. Without that contract, teams build isolated fallbacks that look polished but do not protect the user from a duplicate side effect, ambiguous outcome, or abandoned user task.
        </p>
        <p>
          The design also needs an ownership model. A retry operation can be caused by frontend code, backend dependencies, authorization policy, release configuration, browser compatibility, or customer-specific data. The workflow owner should not have to manually triage every incident from scratch. Error events and recovery states should route to service owners with enough release, dependency, tenant, and user journey context to make ownership clear.
        </p>
        <p>
      ...
      <section>
        ...
        <ArticleImage
          ...
          alt="Design Retry, Failure & Recovery UX architecture"
          caption="Architecture view: capture, classify, contain, report, correlate, and recover."
        />
        <ArticleImage
          src="/diagrams/system-design-problems/high-level-design/error-handling-reliability-systems/retry-failure-recovery-ux-flow.svg"
          alt="Design Retry, Failure & Recovery UX failure flow"
          caption="Failure flow showing the path from user-visible failure to evidence and mitigation."
        />
        <ArticleImage
          src="/diagrams/system-design-problems/high-level-design/error-handling-reliability-systems/retry-failure-recovery-ux-recovery.svg"
          alt="Design Retry, Failure & Recovery UX recovery model"
      ...
      <section>
        ...
        <p>
          Retry UX trades convenience against duplicate side effects. Automatic retry is useful for safe operations, but unsafe mutations need idempotency, clear state, and sometimes user confirmation.
        </p>
        <p>
      ...
      <section>
        ...
        <p>
          The common failure is adding a retry button without modeling operation state. Users need to know whether an action failed, is still pending, succeeded elsewhere, or requires manual recovery.
        </p>
        <p>
      ...
      <section>
        ...
        <p>
          user-facing failure recovery system is critical for critical user journeys such as checkout, admin changes, exports, collaboration, and workflow execution. The failure state often determines whether the user retries safely or creates duplicate side effects.
        </p>
        <p>
        ...
        <p>
          A good interview answer should name the invariant being protected. For retry operation, the invariant may be no lost user work, no duplicate side effects, no silent stale data, no unowned critical incident, or no sensitive telemetry leakage. Once the invariant is clear, architecture choices become easier to defend.
        </p>
      ...
      <section>
        ...
        <h3>How would you model user-facing failure recovery system at production scale?</h3>
        <p>
        ...
        <p>
          Retry UX trades convenience against duplicate side effects. Automatic retry is useful for safe operations, but unsafe mutations need idempotency, clear state, and sometimes user confirmation. I would also discuss consistency versus responsiveness, evidence depth versus privacy, automatic retry versus duplicate side effects, and generic fallback simplicity versus domain-specific recovery. The best answer ties each trade-off to user trust and operational proof.
        </p>
      </section>
      ...
      <section>
        <h2>References</h2>
        <ul>
          <li>AWS Builders Library on timeouts retries and backoff.</li>
          <li>Stripe idempotency documentation.</li>
          <li>Google SRE book on handling overload.</li>
          <li>MDN online and offline events.</li>
          <li>OWASP guidance on secure error handling.</li>
        </ul>
      </section>
    </ArticleLayout>
  );
