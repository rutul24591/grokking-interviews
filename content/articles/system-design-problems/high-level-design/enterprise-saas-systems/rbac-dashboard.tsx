  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>Role-Based Access Control (RBAC) is the authorization model used by virtually all enterprise SaaS products. Users are assigned roles (Admin, Manager, Contributor, Viewer), and roles are granted permissions (create:deal, read:report, delete:user). The dashboard challenge is twofold: the admin UI for managing roles and permissions (who can assign what permissions, how permission changes propagate to active sessions), and the end-user UI that adapts to the current user's permissions (hiding buttons they cannot use, blocking routes they cannot access, omitting fields they cannot see).</p>
        <p>The complexity grows significantly in multi-tenant SaaS: each organization has its own permission configuration that overrides the platform defaults. Org A may allow all Managers to delete records; Org B may restrict deletion to Admins only. Furthermore, some products offer resource-level permissions: a user can be a Manager of Project A and a Viewer of Project B simultaneously. This creates a permission evaluation challenge that is more complex than a simple role lookup — the system must evaluate the intersection of role permissions, resource-specific grants, and org-level overrides for each authorization check.</p>
      ...
      <section>
        ...
        <p>The permission system has two layers. The Permission Store is the source of truth: a PostgreSQL table (user_roles: userId, roleId, resourceType, resourceId, grantedBy, grantedAt) and a computed effective permissions cache in Redis (perms:{"{userId}"} → JSON set of {"{action:resource}"} pairs, TTL 5 minutes). The Authorization Service evaluates permissions for each API request: it reads from the Redis cache (fast path) or computes from PostgreSQL (cache miss). The Frontend Layer receives the current user&apos;s permission set as part of the session token (permissions embedded in JWT claims for common permissions) and uses a usePermission(action, resource) React hook to gate UI components. For permission changes, the admin action invalidates the Redis cache and publishes a PermissionChanged event to a Redis Pub/Sub channel; all SSE connections for the affected user receive the new permission set within seconds.</p>
      </section>
