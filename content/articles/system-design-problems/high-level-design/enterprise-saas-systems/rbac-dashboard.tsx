...
export const metadata: ArticleMetadata = {
  ...
  tags: ["hld","rbac","authorization","enterprise-saas","security"],
  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <p>
          Authorization is not a small middleware detail. RBAC dashboard often includes field-level visibility, scoped administration, external sharing, delegated ownership, support access, and break-glass operations. The UI should reflect effective access and the backend must enforce the same policy for reads, writes, exports, and background jobs.
        </p>
        <p>
      ...
      <section>
        ...
        <ArticleImage
          ...
          alt="Design an RBAC Dashboard architecture"
          caption="Architecture view for role and permission administration console: domain API, policy, source of truth, event projections, governance, and admin UI."
        />
        <ArticleImage
          src="/diagrams/system-design-problems/high-level-design/enterprise-saas-systems/rbac-dashboard-governance.svg"
          alt="Design an RBAC Dashboard governance flow"
          caption="Governance view showing versioning, policy checks, audit evidence, approval, and retention controls."
        />
        <ArticleImage
          src="/diagrams/system-design-problems/high-level-design/enterprise-saas-systems/rbac-dashboard-scaling.svg"
          alt="Design an RBAC Dashboard scaling and reliability flow"
        ...
        <p>
          The system should include a customer-safe diagnostics layer. Tenant admins and support engineers may need evidence about access review, but they should not need raw database access. Diagnostics should expose policy decisions, event ids, version numbers, job state, integration status, and redacted payload summaries through governed tools.
        </p>
        <p>
          Backpressure should be explicit across queues and integrations. Large tenants can generate bursts of break-glass grant activity that overwhelm projections, notifications, exports, or connector calls. Queue isolation, tenant quotas, retry budgets, and dead-letter review keep one customer&apos;s workload from degrading the whole platform.
        </p>
        <p>
      ...
      <section>
        ...
        <p>
          Create impact previews for high-blast-radius actions. Before publishing a resource scope, changing a policy, launching an automation, or exporting sensitive data, the UI should show affected users, records, workflows, integrations, and scheduled jobs. This turns dangerous admin power into an informed decision.
        </p>
        <p>
      ...
      <section>
        ...
        <p>
          Quarterly access review needs fast operational views for large tenants without leaking data across teams, regions, or roles. This depends on tenant-aware caching and policy-aware read models.
        </p>
        <p>
          Support break-glass access pushes the system into incident or compliance mode, where correctness and audit evidence matter more than visual polish.
        </p>
        <p>
          Permission migration shows why enterprise SaaS features need lifecycle, migration, and support tooling rather than only a happy-path workflow.
        </p>
      </section>
        <p>
      ...
      <section>
        ...
        <p>
          I would enforce tenant isolation in storage, cache keys, search indexes, queues, exports, and observability. I would add quotas for expensive operations, background job isolation, policy-aware caches, and admin audit trails. For RBAC dashboard, I would also expose operational signals such as projection lag, failed jobs, permission denials, and export volume so tenant-specific problems do not become global outages.
        </p>
        ...
        <p>
          The main trade-offs are flexibility versus governance, freshness versus cost, strong consistency versus scalability, and admin power versus blast radius. For role and permission administration console, I would make high-risk actions slower and auditable, keep everyday reads fast through projections, and make configuration versioned so customization does not destroy supportability.
        </p>
      </section>
      ...
      <section>
        <h2>References</h2>
        <ul>
          <li>NIST RBAC model.</li>
          <li>AWS IAM policy evaluation logic.</li>
          <li>Google Zanzibar paper.</li>
          <li>OWASP Authorization Cheat Sheet.</li>
          <li>SOC 2 access control criteria.</li>
        </ul>
      </section>
    </ArticleLayout>
  );
