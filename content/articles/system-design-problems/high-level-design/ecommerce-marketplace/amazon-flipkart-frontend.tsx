  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <HighlightBlock as="p" tier="crucial">Designing the frontend for a large-scale marketplace like Amazon or Flipkart means reconciling two fundamentally conflicting requirements: content freshness and delivery performance. Product prices, stock levels, and promotional banners change by the minute. At the same time, product detail pages must achieve sub-2-second LCP globally to prevent cart abandonment (studies show each 100ms of additional page load time reduces conversion rates by ~1%). Pure server-side rendering (every request hits origin) gives freshness but no performance at global scale. Pure static generation gives performance but stale prices and inventory. The real answer is a carefully layered rendering strategy per page type, combined with selective cache invalidation and client-side hydration only where truly needed.</HighlightBlock>
        <HighlightBlock as="p" tier="crucial">A second challenge is organizational scale. Amazon's frontend is built and deployed by hundreds of teams. The homepage is a composed page where the hero banner, sponsored products, browsing history widget, and deal of the day are each owned by different teams deploying on different cadences. This mandates a micro-frontend or server-side composition architecture where teams can deploy independently without a central release gate. The tradeoff: composed pages are harder to optimize for performance (each component may load its own JavaScript bundle) and harder to test end-to-end.</HighlightBlock>
