  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <p>The trading UI architecture separates the market data processing path from the UI rendering path. Market data arrives at high frequency via WebSocket and is processed off the main thread in a Web Worker. The Web Worker maintains the current order book state and produces rendered snapshots for the UI at the display rate (targeting 60fps or a configurable update rate). The main thread consumes these snapshots and updates the DOM, never doing expensive computation. The order entry path is entirely separate from the market data path: order submission goes directly to the OMS via a dedicated HTTP endpoint (not through the market data WebSocket).</p>
        <p>This architecture ensures that a burst of 1000 order book updates per second cannot cause input latency on the order entry form, because the heavy processing (order book state management, snapshot generation) runs in the Web Worker, and the main thread only receives pre-computed render payloads. The main thread's work is bounded to DOM updates, which is fast and deterministic.</p>
      </section>
      ...
      <section>
        ...
        <p>JSON is convenient but expensive for high-frequency market data. Parsing a JSON object for every order book update at 1000Hz introduces measurable CPU cost on the receiving end. Binary WebSocket messages (ArrayBuffer payloads) eliminate the string parsing cost: a 64-byte binary message encoding 8 price levels can be decoded in a single DataView pass with essentially zero garbage collection overhead. The protocol is defined by a fixed schema: message type (1 byte), instrument ID (4 bytes), sequence number (8 bytes), update type (full snapshot vs incremental), and then N price-level entries, each being price (8 bytes float64) and quantity (8 bytes float64).</p>
        <p>The sequence number is critical for correctness. Order book updates must be applied in sequence; an out-of-order update (sequence number gap) indicates a missed message. The Web Worker tracks the expected sequence number per instrument; if a gap is detected, it sends a snapshot request back to the server (via the same WebSocket, or a separate HTTP endpoint), receiving a full order book snapshot that resets the state to a known-good baseline. Without sequence tracking, missed messages would result in an incorrect order book that could cause trades at wrong prices.</p>
        ...
        <p>Every 16ms (aligned to the display refresh rate via a message from the main thread, or via the Worker's own setInterval), the Web Worker produces a render snapshot: a simple JavaScript object containing the current top-N bid and ask levels. This snapshot is posted to the main thread via postMessage. The main thread receives the snapshot and updates the DOM. The key efficiency is that the main thread never processes the raw 1000Hz message stream; it only sees pre-computed 60Hz snapshots. During periods of high volatility (1000Hz updates), the Worker processes all updates but the main thread still only renders 60 times per second, throwing away intermediate states that are overwritten before the next render.</p>
        <p>For the absolute minimum latency path (used by institutional traders who need every microsecond), SharedArrayBuffer can replace postMessage for snapshot delivery. The Worker writes the current snapshot directly into a shared memory buffer; the main thread reads from the same buffer on each animation frame without the serialization and deserialization overhead of postMessage. SharedArrayBuffer requires Cross-Origin Isolation headers (COOP + COEP) but eliminates the postMessage copy cost, reducing per-frame overhead by approximately 0.1–0.5ms.</p>
