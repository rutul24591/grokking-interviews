  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>The "multi-step" structure serves conversion optimization: research consistently shows that a single-page checkout with all fields visible simultaneously increases abandonment compared to a stepped flow that reveals complexity progressively. However, multi-step introduces its own challenges: state must persist across steps (so the user can go back and modify their address without losing their payment information), each step must validate independently (can't validate payment until address is entered for tax calculation), and the user may close the tab at any step and expect to resume seamlessly.</p>
      ...
      <section>
        ...
        <p>The payment submission flow: (1) user clicks "Place Order" on the review step; (2) the submit button is immediately disabled (prevents double-click); (3) the client confirms payment with Stripe using the PaymentIntent client secret and the card element collected by Stripe Elements; (4) Stripe processes and returns either succeeded, requires_action, or error; (5) on succeeded, the client calls a server endpoint like POST /checkout/sessions/:sessionId/complete, which triggers order creation and inventory deduction from reserved to sold; (6) the server returns the orderId and redirects the client to an order confirmation page.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">3D Secure / SCA Handling</h3>
        <p>European cards under Strong Customer Authentication (SCA) regulations often require a second factor: the user is redirected to their bank's authentication page to approve the payment. Stripe surfaces this as a requires_action status after confirmCardPayment. The Stripe SDK handles the 3DS redirect automatically when using stripe.confirmCardPayment()—it opens a popup or redirect to the bank's authentication URL. The client awaits the stripe.confirmCardPayment() promise, which does not resolve until the 3DS flow is complete (either authenticated or failed). The checkout UI must show a "Completing authentication..." loading state during this time, which can take 10–60 seconds.</p>
        <p>On 3DS completion, the promise resolves with succeeded or error. The frontend proceeds with the normal success or failure path. If the user closes the 3DS popup without completing authentication, Stripe returns requires_action again or an authentication_required error. The checkout UI returns to the payment step with an error message and the user can try again. The PaymentIntent is reusable: the same client_secret can be passed to stripe.confirmCardPayment() again without creating a new PaymentIntent or charge.</p>
