  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <p>The architecture has three primary subsystems. The location ingestion pipeline receives GPS updates from driver apps, validates and persists them, and publishes them to a Kafka topic. The location service consumes the Kafka topic, maintains an in-memory cache of current driver positions (using Redis Geo for geospatial queries), computes ETA updates using a routing API, and detects geofence events. The real-time delivery layer pushes location and ETA updates to customers via WebSocket (for the web app) and via push notifications for mobile customers who have the tracking page open in a background tab.</p>
        <p>The customer tracking page connects to a WebSocket endpoint authenticated with an order token (a time-limited signed token linked to the specific delivery). The WebSocket delivers only the updates relevant to this customer's order, not a broadcast of all location data. This scoping ensures customers cannot see other customers' delivery details by manipulating the WebSocket connection.</p>
      </section>
      ...
      <section>
        ...
        <p>Driver apps send GPS updates every 3–5 seconds as HTTP POST requests to the Location Ingestion API. At 100,000 concurrent deliveries with one driver each, that is 20,000–33,000 HTTP requests per second. Each request contains: driverId, orderId, latitude, longitude, accuracy (GPS accuracy in meters), heading (degrees, for map marker rotation), speed (for route interpolation), and timestamp. The Ingestion API is stateless—it validates the update (driverId matches an active delivery, timestamp is not stale), enriches it (reverse geocode to street-level address for display purposes), and publishes to a Kafka topic partitioned by driverId. Partitioning by driverId ensures that location updates for the same driver are processed in order by the Location Service consumer.</p>
        ...
        <p>The customer's tracking page must never receive the driver's precise GPS coordinates in the API response. The WebSocket message delivered to the customer contains only: marker_position (an encoded representation the Mapbox SDK uses to render the driver's position on the map) and eta_seconds (the ETA). The customer cannot extract latitude/longitude values from these fields. On the server side, the coordinate → Mapbox token translation happens in the WebSocket relay layer before the message reaches the customer.</p>
        <p>Additional privacy layer: the driver's home address and personal information are never accessible to customers. The driver's name and photo (for the delivery confirmation screen) are stored separately from the location data and accessed only by the customer's own order. A customer cannot look up a driver's location for an order that is not theirs. The order token (used to authenticate the WebSocket connection) is scoped to the specific orderId and expires when the delivery is completed or 24 hours after order placement, whichever comes first.</p>
      </section>
      ...
      <section>
        ...
        <p>WebSocket versus Server-Sent Events for location delivery: WebSocket is bidirectional (the server can push; the client can also send) but requires stateful server connections. SSE is one-directional (server to client only) but lighter-weight and HTTP/2-compatible. For delivery tracking, the customer only needs to receive updates (no client-to-server messages needed), making SSE technically sufficient. However, WebSocket is better supported across browser environments and mobile WebViews, and the bidirectional capability enables future features (customer messaging the driver). WebSocket is the pragmatic choice.</p>
        <p>GPS update frequency trade-offs: 3–5 second GPS updates balance location freshness against battery consumption on the driver's device and server load. Sub-second updates would provide smoother tracking but drain the driver's battery significantly faster (continuous GPS polling is the largest battery consumer on a smartphone) and multiply server load by 3–5×. The 3–5 second interval, combined with client-side marker animation interpolation, produces visually smooth tracking with acceptable battery impact. For high-speed deliveries (courier on a motorcycle at 80km/h), 5 seconds of position change is 110 meters—significant for urban navigation. For slow deliveries (food delivery in congested city traffic at 10km/h), 5 seconds is 14 meters, which is barely visible at typical map zoom levels.</p>
        <p>Internal routing versus third-party routing API: Google Maps and Mapbox Directions APIs provide accurate traffic-aware ETAs but cost per request. At 4,000 routing API calls per second for 100,000 concurrent deliveries, cost becomes significant. OSRM (Open Source Routing Machine) or Valhalla are open-source routing engines that can be self-hosted with OSM data, eliminating per-call costs. The trade-off is accuracy: third-party routing APIs incorporate real-time traffic data from their own data collection systems (GPS data from millions of users), while self-hosted OSRM without real-time traffic will produce less accurate ETAs in dynamic traffic conditions. Hybrid approach: use a self-hosted OSRM for route geometry (the actual path), query a traffic API for real-time congestion data on the route segments, and compute ETA as (route distance) / (average speed adjusted for traffic congestion).</p>
      </section>

      <section>
        <h2>Summary</h2>
        <p>A maps-based delivery tracking application is a real-time location streaming system with privacy constraints. GPS events from driver apps are ingested via HTTP to a Kafka topic, consumed by a Location Service that maintains current positions in Redis Geo, computes ETAs using a routing API (triggered by 50m movement or 60s elapsed, not every update), and detects geofence entry events for "arriving soon" push notifications. Location and ETA updates are delivered to customers via order-scoped WebSockets with coordinate masking (the customer never receives raw GPS coordinates). The Mapbox GL JS map animates driver marker positions smoothly between 3–5 second GPS updates using requestAnimationFrame interpolation. Route lines are trimmed to the remaining portion from the driver's current position. Geofence state is maintained in Redis to debounce entry/exit events from GPS noise at the boundary.</p>
      </section>
    </ArticleLayout>
  );
}
