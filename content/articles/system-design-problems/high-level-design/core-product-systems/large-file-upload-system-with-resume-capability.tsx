  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">For video files, the transcoding pipeline runs after the virus scan: the backend enqueues a transcoding job (via AWS Elastic Transcoder, MediaConvert, or FFmpeg on a dedicated worker). The job produces multiple output renditions (480p, 720p, 1080p as MP4 and HLS segments) and thumbnails. These outputs are stored in the distribution bucket and the database record is updated with the available renditions and their S3 keys. The frontend video player uses the highest-quality rendition that fits the available bandwidth (adaptive bitrate streaming).</HighlightBlock>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Upload Cancellation and Cleanup</h3>
        <HighlightBlock as="p" tier="important">When the user cancels an upload, the client sends a cancel request to the backend, which calls S3's AbortMultipartUpload API. This releases all parts that were uploaded to S3, freeing storage immediately. The local session state is cleared from localStorage. Without explicit cancellation, incomplete multipart uploads accumulate in S3 at cost (S3 charges for parts even when the multipart upload is not completed). An S3 lifecycle rule provides a safety net: incomplete multipart uploads older than 24 hours are automatically aborted by S3, preventing orphaned storage accumulation from abandoned uploads (browser crash, tab close without cancellation).</HighlightBlock>
      </section>
