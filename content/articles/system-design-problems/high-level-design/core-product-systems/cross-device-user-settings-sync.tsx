  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">The settings sync system uses a client-local cache as the primary read path (for offline support and read-your-writes) with a server-side authoritative store as the sync target. Each settings change is written to the local cache immediately (for instant UI response) and queued for upload. The upload is a delta of changes since the last successful sync, identified by a vector clock or a per-device logical timestamp. The server merges incoming deltas from all devices, resolves conflicts using last-write-wins semantics, and notifies other devices of changes via Server-Sent Events or push notifications. Other devices pull the changes on next foreground activation or via the push notification trigger.</HighlightBlock>
        <HighlightBlock as="p" tier="important">The data model treats each setting as an independent key-value pair with an associated logical timestamp and deviceId. This granularity (per-key, not per-settings-document) means a conflict on notification_email does not block syncing of dark_mode_enabled. Two devices can independently change different settings while offline and sync both changes successfully without conflict.</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">For mobile devices where maintaining a persistent SSE connection is battery-inefficient, the server sends a push notification (APNs/FCM) when the settings store is updated. The push notification triggers a background app fetch, which performs a delta sync. The push notification payload is minimal (it just signals "sync needed," not the actual changes) to stay within push payload size limits and for privacy (settings changes should not be transmitted in plaintext in push notification payloads, which transit third-party infrastructure).</HighlightBlock>
