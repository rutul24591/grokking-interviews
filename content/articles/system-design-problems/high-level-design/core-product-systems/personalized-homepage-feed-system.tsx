  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <p>The candidate sets from all sources are merged and deduplicated by itemId. Items the user has already seen and interacted with (viewed in the past 30 days, explicitly dismissed) are filtered out. The result is the full candidate set (2,000–5,000 items) passed to the pre-ranking stage. This entire candidate generation step must complete in under 50ms to stay within the overall latency budget, requiring all source lookups to run in parallel (not sequentially).</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Feature Store and Real-Time Features</h3>
        <p>The ranking model uses features from three domains. User features: the user's historical engagement rates by category, creator, time-of-day pattern, session length, and device. These are pre-computed daily and stored in a feature store (Redis for online serving, S3 for offline training). Item features: the item's historical engagement rates, age, quality signals (edit count for articles, production quality score for videos), creator follower count, and category. These are pre-computed and updated hourly. Context features: current time of day, day of week, device type, geographic region, and in-session features (items viewed in the current session). In-session features are not pre-computed—they are derived from the current request's session data (passed by the client) and represent the most valuable real-time signal.</p>
        <p>In-session features are the key to real-time personalization. If a user has viewed 5 articles about climate change in the current session, the next recommendation should reflect this current interest, even if their long-term history shows broad interests. The client passes the current session's interaction list (last 10 itemIds viewed) as part of the feed request. The ranking service looks up the categories and tags of these items, computes session-level interest vectors, and passes them to the model as real-time features. This session context lookup must complete in under 10ms; it queries a local cache (warmed by the item metadata service) rather than a database.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Two-Tower Model Architecture</h3>
        <p>The two-tower architecture is the standard approach for large-scale recommendation ranking. Two separate neural networks (towers) encode the user (left tower) and item (right tower) into a shared embedding space. The predicted engagement probability is the dot product (or cosine similarity) of the user and item embeddings. The key advantage: item embeddings can be pre-computed offline and stored in the item embedding store; at serving time, only the user embedding must be computed (a fast forward pass through the user tower with the user's current features). Item embedding computation is not on the critical path.</p>
        ...
        <p>Computing a fresh personalized feed for every homepage request at 100,000 requests per second is computationally impractical (even if each individual computation is fast, the aggregate GPU load would be enormous). The solution: pre-compute the top-50 feed for every active user once every 5–30 minutes and cache the result. When the user requests the homepage, the cached feed is served immediately. This reduces latency to a single Redis lookup (&lt;5ms) and eliminates per-request model inference entirely for the common case.</p>
