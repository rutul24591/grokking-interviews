  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="crucial">The audio streaming architecture centers on the browser's Media Source Extensions API, which allows JavaScript to programmatically feed audio data to an HTMLAudioElement. MSE enables adaptive bitrate switching (the application controls which segment quality is fetched next) and gapless playback (the application can append segments from the next track to the same buffer before the current track ends, eliminating the gap). The audio pipeline: the application fetches DASH manifest (an XML file listing all available segments and quality levels), selects the appropriate quality based on bandwidth estimation, fetches audio segments, decrypts them via the Encrypted Media Extensions API (EME), and appends them to the MSE SourceBuffer.</HighlightBlock>
        <HighlightBlock as="p" tier="important">The playback controller manages the playback queue, segment fetching, bandwidth adaptation, and cross-device state synchronization. It is separate from the UI layer: the playback controller is a background service (or a Web Worker) that continues operating even when the user navigates between pages of the application. The UI registers listeners on the controller to update progress bars, track information displays, and queue views.</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">Each track's DASH manifest lists the available Representations (quality levels) with their bitrates, codec information, and URLs. Each Representation consists of an initialization segment (containing codec configuration data, required before any audio segment can be decoded) and a series of media segments (typically 2–5 seconds each). The application selects a Representation based on the estimated available bandwidth, with hysteresis to prevent rapid quality oscillation: switch up only when measured bandwidth exceeds the next quality's bitrate by 1.5×, switch down when bandwidth drops below the current quality's bitrate by 0.8×.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Bandwidth estimation uses the download speed of recent segment fetches: segment_size (bytes) / download_time (seconds). An exponential weighted moving average smooths the estimate (new_estimate = 0.3 × measured_speed + 0.7 × previous_estimate). The first segment fetch uses a conservative initial quality (128kbps) to minimize time-to-first-sound; subsequent fetches can upgrade quality if bandwidth allows. This matches the Spotify approach: start fast (lower quality), upgrade transparently during playback.</HighlightBlock>
