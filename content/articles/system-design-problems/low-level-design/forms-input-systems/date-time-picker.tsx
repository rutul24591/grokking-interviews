    ...
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <p>
          ...
          and range-mode machine state. External controlled
      ...
      <section>
        ...
        <p>
          ...
          <code> presets</code>. Output: ISO string with
      ...
      <section>
        ...
        <p>
          The month grid memoizes by (year, month, locale,
          timezone, disabledDate ref); recomputation only
          happens when those change. <code>Intl.DateTimeFormat</code>
          instances are cached per (locale, options) tuple
          because constructing one is surprisingly expensive
          (sub-ms but adds up). The Calendar component is
      ...
      <section>
        ...
        <p>
          Highlight today distinctly. Mark weekends and
          ...
          end date than the start. Type-to-jump for month
      ...
      <section>
        ...
        <p>
          The calendar uses
          <code> role=&quot;grid&quot;</code> with rows
          (<code>role=&quot;row&quot;</code>) and cells
          (<code>role=&quot;gridcell&quot;</code>). Selected
          cells carry
          <code> aria-selected=&quot;true&quot;</code>;
          disabled cells carry
          <code> aria-disabled=&quot;true&quot;</code>.
          Single tabstop: only the focused cell is
          tabbable, others are
          <code> tabIndex=&quot;-1&quot;</code>. The field
          carries an
          <code> aria-label</code> describing the value
          (&ldquo;March 15, 2026&rdquo;). Live region
          announces month changes
          (&ldquo;March 2026&rdquo;) on Page Up/Down. We do
          not rely on color alone for state — disabled
          cells have a distinct text style; selected cells
          have both color and a visible border. Sufficient
          contrast (WCAG AA at minimum) on all states.
        </p>
      </section>
      ...
      <section>
        ...
        <p>
          ...
          change between picker open and submit.
        </p>
      </section>
      ...
      <section>
        ...
        <p>
          Unit tests cover date math (DST transitions, year
          boundaries, leap years), parser correctness across
          locale formats, range flip logic, week-start
          variations across locales. Integration tests
          exercise keyboard flows: open popover, navigate
          via arrows, select via Enter, escape to close.
          Visual snapshot tests for boundary months
          (February of leap years, months that span DST
          transitions). Cross-locale tests verify
          formatting and parsing in en-US, en-GB, ar-SA,
          ja-JP, hi-IN. Cross-time-zone tests verify
          canonical-storage round-trips.
        </p>
      </section>
      ...
      <section>
        ...
        <p>
          DST spring-forward: 2:30 AM doesn&rsquo;t exist on
          some days; the picker rounds forward to the next
          valid minute. DST fall-back: wall time happens
          twice; the picker uses Temporal&rsquo;s default
          disambiguation and reflects the offset in the
          display. Time zone change while popover open:
          re-derive selection display without changing
          canonical value. Locale week start: read from
          <code> Intl.Locale.weekInfo</code>; fall back to
          a small data table. Range with same start and end
          on a disabled day: treat as invalid; surface the
          reason. Two-month view when selection spans
          months: keep both visible. Pasted invalid string:
          surface error inline; do not corrupt state.
          Calendar opened on a disabled-day-only week:
          focus the next valid day, not a disabled one. A
          <code> min</code> date that&rsquo;s after
          <code> max</code>: treat as a misconfiguration; log
          and clamp.
        </p>
      </section>
      ...
      <section>
        ...
        <p>
          The DateAdapter is the primary extension point —
          ...
          rendering let consumers customize without forking.
          Pluggable presets and custom highlights add via
      ...
      <section>
        ...
        <p>
          ...
          conventions. Numbering systems (Arabic-Indic,
          Devanagari) via
          ...
          <code> Intl.Locale</code> when configured.
        </p>
      </section>
      ...
      <section>
        ...
        <p>
          NLP parsing of free-text dates (&ldquo;next Tuesday
          at 3pm&rdquo;) with a confirmation step would
          accelerate input for power users. Server-driven
          availability calendars (busy/free streamed in)
          would close the gap with native scheduling apps.
          Calendar integration (Google, Outlook) for
          recurring events. Wider Temporal adoption as
          ...
          insufficient (e.g. very granular time selection).
        </p>
      </section>
      ...
      <section>
        <h2>📌 Summary</h2>
        <p>
          A solid Date/Time Picker is a{" "}
          <strong>headless, locale- and zone-aware control</strong>{" "}
          wrapped in an accessible WAI-ARIA grid. Store
          canonical ISO-with-offset values, format with
          <code> Intl</code>, pick the right adapter
          (Temporal where available), and use the native
          input on mobile while offering a rich custom UI
          on desktop. The non-negotiable invariants are
