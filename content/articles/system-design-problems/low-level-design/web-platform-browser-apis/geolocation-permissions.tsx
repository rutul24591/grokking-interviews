...
export default function GeolocationPermissionsArticle() {
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        <h1>Design Geolocation Permissions UX</h1>
        <h2>Definition &amp; Context</h2><HighlightBlock as="p" tier="crucial" className="mb-4">Design Geolocation Permissions UX should be framed as an implementation-level design problem with a clear runtime boundary, not as a visual mock or helper function.</HighlightBlock><HighlightBlock as="p" tier="important" className="mb-4">Interview signal: identify the browser capability detection, permission UX, worker boundaries, observers, scheduling, and progressive enhancement before discussing APIs or code structure.</HighlightBlock><HighlightBlock as="p" tier="important" className="mb-4">The answer should connect user-visible behavior to engineering constraints: correctness, accessibility, latency, failure recovery, testability, and operational ownership.</HighlightBlock>
        <p>
          Design Geolocation Permissions UX is a low-level design problem about wrapping a powerful but inconsistent browser capability in a production-safe privacy-aware permission and location runtime. Browser APIs are not normal libraries: availability differs by browser, permissions can change at runtime, callbacks may fire on the main thread, and user activation, privacy, storage, and lifecycle rules can invalidate a happy-path implementation.
        </p>
        <p>
          The implementation contract starts with explainNeed(context), requestLocation(options), watchPosition(scope), revokeOrDegrade(reason). The runtime should model these states explicitly: unknown, explaining, prompted, granted, denied, approximate, watching, expired. The invariant is: Location access must be purpose-bound, revocable, and degraded safely when precision or permission is unavailable. The hard case is when a user grants location once, moves to background, and later expects the app not to keep tracking silently. A principal-ready answer should explain the API facade, capability detection, fallback behavior, lifecycle cleanup, privacy and security constraints, and the telemetry that proves the abstraction works in the field.
        </p>
        <ArticleImage
          src="/diagrams/system-design-problems/low-level-design/web-platform-browser-apis/geolocation-permissions-runtime.svg"
          alt="Design Geolocation Permissions UX runtime model"
      ...
      <section>
        ...
        <p>
          The third concept is lifecycle ownership. Browser API handles often outlive a render: observers must disconnect, workers must terminate, file references must be released, permission watches must stop, callbacks must be batched, and hidden tabs may throttle timers. The durable structures are permission state, accuracy policy, watcher registry, timeout budget, privacy scope, stale location cache. These structures give the implementation enough evidence to clean up safely and debug incidents.
        </p>
      ...
      <section>
        ...
        <ArticleImage
          src="/diagrams/system-design-problems/low-level-design/web-platform-browser-apis/geolocation-permissions-failure.svg"
          alt="Design Geolocation Permissions UX failure and fallback model"
      ...
      <section>
        ...
        <p>
          Direct calls duplicate edge handling across components and make behavior inconsistent. A runtime centralizes invariants: Location access must be purpose-bound, revocable, and degraded safely when precision or permission is unavailable. It also makes permissions, fallbacks, cleanup, and metrics testable. The cost is an abstraction layer, but the benefit is predictable behavior across browsers and product surfaces.
        </p>
      ...
      <section>
        <h2>References</h2>
        <ul>
          <li><a href="https://developer.mozilla.org/en-US/docs/Web/API/Geolocation_API" target="_blank" rel="noreferrer">MDN reference for the primary browser API</a></li>
