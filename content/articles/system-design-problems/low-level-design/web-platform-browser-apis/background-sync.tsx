    ...
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="important"><strong>Action queuing:</strong> When a network request fails or the browser detects offline status, queue the action (endpoint, method, payload) for later retry.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Automatic retry on recovery:</strong> When the browser detects network connectivity recovery, automatically retry all queued actions in order.</HighlightBlock>
          ...
          <HighlightBlock as="li" tier="crucial"><strong>Backoff strategy:</strong> Retry with exponential backoff (1s, 2s, 4s, 8s, etc.) to avoid overwhelming the server if it's recovering slowly.</HighlightBlock>
          ...
          <HighlightBlock as="li" tier="important"><strong>Idempotency:</strong> Retrying the same action multiple times produces the same result as executing once. Server detects duplicate requests via idempotency keys.</HighlightBlock>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="important"><strong>Latency:</strong> Queuing should be instant (under 50ms). Recovery detection within 5 seconds of network restoration. Retry attempt within 500ms of recovery detection.</HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">The system consists of two primary components: client-side queue management and Service Worker-based retry logic. When a request fails (network error, server 5xx), the client captures the request details and queues them in IndexedDB via the Service Worker. The client notifies the UI of the queued item so the user knows to expect eventual delivery.</HighlightBlock>
        <HighlightBlock as="p" tier="crucial">The Service Worker listens for online/offline events. When the browser comes back online, the Service Worker queries IndexedDB for queued items, sorts by priority and timestamp, and attempts retry. It uses exponential backoff with jitter to avoid thundering herd scenarios. If a retry succeeds (2xx response), the item is removed from the queue and marked complete in the UI. If it fails with a non-retriable error (4xx, rate limit), it's moved to a failed bucket and the user is notified. On retriable errors (5xx, timeout), the item is re-queued with incremented retry count.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Idempotency is ensured by including an idempotency key (UUID) with each queued request. The server uses this key to deduplicate requests. If the same request is retried, the server recognizes the idempotency key and returns the previous result rather than re-processing.</HighlightBlock>
...
<ArticleImage
	        ...
	        <HighlightBlock as="p" tier="crucial">
	          Interview signal: the diagram captures the end-to-end flow for <strong>Background Sync System</strong>. You should be able to explain the happy path and the failure paths (retries, cancellation, backpressure), not just the API surface.
	        </HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">IndexedDB schema example: Store "sync-queue" with indexes on status, priority, and createdAt. Each item includes a requestId (idempotency key) to prevent server-side duplicates. Metadata includes the full HTTP request details so retries can be executed identically to the original attempt.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">On detection of recovery, the Service Worker retrieves all pending and retrying items from the queue, sorted by priority and creation time. For each item, it attempts a request. If successful, the item is marked succeeded and removed from active queue. If the response status is 4xx (client error), the item is moved to failed and the user notified (no further retries; the request is fundamentally broken). If 5xx or timeout, the item is re-queued with incremented retry count.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Backoff strategy: retry delay = min(300 * (2 ^ retryCount) + jitter, 3600) seconds. Retries at 300ms, 600ms, 1.2s, 2.4s, 4.8s, and then cap at 1 hour. Jitter (random ±10% variance) prevents multiple queued items from retrying simultaneously. After 5-10 retries over several hours, an item is marked failed if still unsuccessful. The user is notified and given the option to retry manually or discard.</HighlightBlock>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Idempotency and Deduplication</h3>
        <HighlightBlock as="p" tier="important">Each queued request includes an idempotency key (generated client-side as UUID). The server endpoint must support the Idempotency-Key header. On first request with key X, the server processes and returns success. On retry with the same key X, the server recognizes the key, skips processing, and returns the previous result.</HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="crucial">Guaranteed delivery vs simplicity: fully guaranteed delivery (multiple retries, persistent queue, idempotency keys) adds complexity. Simple best-effort (retry once or twice, forget) is simpler but data loss possible. Most applications balance with exponential backoff and 5-10 retries over a day, catching 99%+ of transient failures.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Server support: background sync requires server support for idempotency keys. If the server doesn't support idempotency, retries may cause duplicates. New implementations should always assume client-side retries and design idempotency in from the start.</HighlightBlock>
        <HighlightBlock as="p" tier="important">User experience: showing "queued" items persistently in the UI provides transparency but can feel slow if recovery takes seconds. Some apps hide the queue internally and only show it if items fail after 30+ seconds. Trade-off: transparency vs perceived speed.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Offline simulation: in development, test thoroughly with network throttling and explicit offline mode. The complexity of background sync isn't apparent in normal online testing; offline scenarios reveal edge cases.</HighlightBlock>
      </section>

      <section>
        <h2>Implementation Patterns</h2>
        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 1: Simple Service Worker Queue</h3>
        <HighlightBlock as="p" tier="important">Service Worker listens for fetch events that fail. On failure, queue request metadata in IndexedDB. On online/offline event transition, trigger sync from queued items. Suitable for small, non-critical queues.</HighlightBlock>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 2: Priority-Based Queue with Backoff</h3>
        <HighlightBlock as="p" tier="important">Queue items with priority and exponential backoff. Periodic task (every 5s when offline) retries highest-priority items first. Failed items accumulate and are visibly tracked in the UI for user awareness.</HighlightBlock>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 3: File Upload Queue with Resumable Chunks</h3>
        <HighlightBlock as="p" tier="crucial">For large file uploads, split into chunks. Queue each chunk separately with offset metadata. On retry, resume from last successful chunk rather than re-uploading entire file. Reduces bandwidth waste on transient failures.</HighlightBlock>
      </section>

      <section>
        <h2>Summary</h2>
        <HighlightBlock as="p" tier="important"><Highlight tier="crucial">Real-world systems (PWAs, email clients, note-taking apps) rely on background sync to hide network unreliability. For best results, ensure server-side</Highlight></HighlightBlock>
<HighlightBlock as="p" tier="important">idempotency support, implement 5-10 retries with exponential backoff over 24 hours, persist queues indefinitely until success or user deletion, and provide clear UI feedback about queued items and retry status. Monitoring queue depth and retry patterns reveals network issues before users report them.</HighlightBlock>
      </section>
    </ArticleLayout>
  );
}
