  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>JavaScript executes on a single main thread. When heavy computation runs on the main thread (parsing large JSON files, cryptographic hashing, image processing), that thread is blocked. The UI can't respond to user input (clicks, scrolling, typing) until the computation completes. The user perceives the app as frozen: buttons don't react, scrolling stutters, input lags. This is a terrible user experience.</p>
        <p>Example: a user uploads a 100MB JSON file. The app parses it to validate structure. Parsing takes 2 seconds on modern devices. During those 2 seconds, the main thread is busy; the UI is completely frozen. User can't click buttons, scroll, or interact.</p>
        <p>Web Workers solve this by providing true background threads. Heavy computation runs in a separate thread (Worker). The main thread remains free to respond to user input. The Worker and main thread communicate via message passing (asynchronous).</p>
        <p>Key challenges: managing worker lifecycle (creation, termination), communicating data (structured clone for copying, or Transferable objects for zero-copy transfer), debugging in separate context, handling errors in workers, preventing memory leaks from orphaned workers, and coordinating multiple workers for parallelization.</p>
        <p><strong>Explicit assumptions:</strong> Heavy CPU-bound tasks exist that block the main thread for longer than about 100 ms. Workers can run independently without DOM access. Data can be serialized and sent between threads. Multiple workers can be created for parallelization. Message passing overhead is acceptable (typically under about 1 ms for most payloads).</p>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Worker creation and lifecycle:</strong> Spawn workers from separate files, manage their lifecycle from creation through termination, and handle graceful shutdown.</li>
          <li><strong>Message passing:</strong> Send data to workers and receive results via asynchronous message passing without blocking the main thread.</li>
          ...
          <li><strong>Timeout management:</strong> Detect hung or slow workers and timeout long-running tasks to prevent stalls.</li>
        </ul>
        ...
        <ul className="space-y-2">
          ...
          <li><strong>Latency:</strong> Message passing overhead under 10ms. Worker startup under 50ms.</li>
          <li><strong>Memory:</strong> Each worker uses approximately 1-5MB overhead (depends on shared code size). Pool of 4-8 workers under 50MB total.</li>
      ...
      <section>
        ...
        <p>The system creates Worker instances from JavaScript files. The main thread sends a task to a worker via postMessage, passing data (which is structured-cloned to the worker). The worker processes the task in the background and sends the result back via postMessage. The main thread registers a message handler to receive the result and continues executing other code without waiting.</p>
        <p>For efficiency, rather than creating a new worker for each task, the system maintains a worker pool: a set of reusable workers managed by a task queue. When a task arrives, the scheduler checks if a worker is available. If yes, assigns the task immediately. If no, queues the task. As workers complete tasks, they're returned to the available pool. The next queued task is assigned to the now-available worker.</p>
        <p>The pool size is tuned to the hardware: navigator.hardwareConcurrency gives CPU core count. A pool of 4-8 workers is typical; creating more workers than cores doesn't improve performance due to context switching overhead. Error handling is critical: if a worker throws an error, the error is propagated to the main thread, and the worker is restarted or returned to the pool (depending on error severity).</p>
      </section>
      ...
      <section>
        ...
        <p>Data passed between the main thread and worker is structured-cloned by default. Structured clone creates a deep copy: the worker receives an independent copy of the data, not a reference. This is safe (no shared memory concurrency issues) but has overhead for large payloads (100MB+ data is slow to clone).</p>
        ...
        <p>Worker code runs in a separate scope with limited access. The global object is WorkerGlobalScope (not window). Unavailable: DOM (no document, no element access), window object, parent/opener references. Available: setTimeout, setInterval, fetch, indexedDB, cache API, and importScripts for loading helper modules.</p>
        ...
        <p><strong>Task Queuing and Backpressure Handling:</strong> Implement a bounded queue to prevent memory issues. If the queue grows beyond a threshold (e.g., 10k pending tasks), apply backpressure: reject new tasks with a "queue full" error until the queue drains. Additionally, implement task priorities: critical tasks (user-initiated) go to the front of the queue; background tasks (prefetching, cleanup) go to the back. This ensures user-facing operations complete faster even if many background tasks are queued. Additionally, monitor queue depth and alert if it grows uncontrollably (workers are slower than task arrival rate).</p>
        ...
        <p>Timeout handling is critical for long-running tasks. Set a timeout for each task (e.g., 30 seconds). If the worker doesn't respond within the timeout, reject the task promise and possibly restart the worker (it might be hung). On timeout, move the task to a failed state and notify the caller.</p>
        ...
        <p>Recovery strategy: on worker error or timeout, terminate the worker and create a new one. This is safer than trying to reuse a potentially corrupted worker. For critical tasks, implement retry logic: after a worker error, retry the task with a fresh worker (up to N retries). Idempotency is essential: tasks must be safe to retry without duplicate side effects.</p>
      ...
      <section>
        ...
        <p>Browser support for Web Workers is excellent in modern browsers (Chrome 4+, Firefox 3.5+, IE 10+). Check typeof(Worker) !== 'undefined' before using workers; provide fallback to main thread execution in older browsers.</p>
        <p>Debugging workers is straightforward in Chrome DevTools: workers appear as separate contexts in the Sources tab. Set breakpoints in worker code just like main thread code. Console.log in workers outputs to the main DevTools console. Firefox DevTools also supports worker debugging.</p>
        <p>Testing workers: unit tests should mock the worker to avoid spawning real workers (slow, unreliable in test environments). Use a mock that simulates postMessage and onmessage. Integration tests should use real workers to test actual message passing and error scenarios. Keep worker tests isolated; avoid testing both worker and main thread logic in the same test.</p>
      </section>

      <section>
        <h2>Advanced Production Patterns</h2>
        <p>Several libraries simplify worker management. Comlink provides RPC-like communication: call worker functions as if they were local async functions, hiding message passing complexity. Piscina is a sophisticated worker pool library with built-in timeout handling, queue management, and error recovery. For critical production systems, these libraries eliminate boilerplate.</p>
        <p>Blob workers allow creating workers from inline code (no separate file needed). This is useful for bundled applications where worker code is embedded in the main bundle as a string/blob. Avoid in development (harder to debug); prefer separate files for development clarity.</p>
        <p>Module workers use ES6 import/export syntax (type: 'module'). This is cleaner than importScripts and allows proper module loading within workers. Browser support is modern (Chrome 91+, Firefox 78+). Prefer module workers for new code.</p>
        <p>Common pitfall: forgetting to terminate workers → memory leak. Solution: maintain a registry of all created workers and terminate on shutdown or idle timeout. Another pitfall: sending non-cloneable data (functions, DOM nodes). Solution: validate data before sending or explicitly convert to transferable format.</p>
        <p>Incident response: if the UI is still janky despite using workers, check if the CPU-bound code actually moved to workers (not still on main thread). Profile with DevTools to verify. If workers crash frequently, check error logs and restart the pool with fresh workers.</p>
      </section>

      <section>
        <h2>Trade-offs and Considerations</h2>
        <p>Complexity vs responsiveness: workers add complexity (separate files, message passing protocol, debugging in separate context). However, they're essential for apps with heavy computation that blocks the UI. Rule of thumb: use workers for tasks expected to take longer than about 100 ms on the main thread. Very quick tasks do not justify the overhead.</p>
        <p>Memory vs parallelization: each worker uses 1-5MB memory overhead (varies by complexity of shared code). A pool of 8 workers uses ~40MB. For memory-constrained environments (mobile, embedded), this may be significant. However, parallelization on multi-core systems can provide 2-4x speedup, often justifying the memory cost.</p>
        <p>Structured clone vs transferable objects: structured clone is safe (no concurrency issues) but slow for large data. Transferable objects are fast (zero-copy) but require careful ownership tracking (transferred data can't be reused). Use structured clone for safety unless performance profiling identifies it as a bottleneck.</p>
        <p>Message passing vs shared memory: message passing is safe and simple. SharedArrayBuffer is fast but complex (race conditions, atomic operations required). For 99% of applications, message passing is the right choice. Reserve SharedArrayBuffer for extreme performance requirements (audio processing, video codecs) with expert concurrency programming.</p>
      </section>

      <section>
        <h2>Summary</h2>
        <p>Web Workers are essential for maintaining UI responsiveness during CPU-intensive computation. By offloading heavy tasks to background threads, the main thread remains free to handle user interactions, ensuring perceived performance even during long operations. For staff and principal engineers designing production systems, the critical patterns are: worker lifecycle management (creation, communication via postMessage, graceful termination), message passing with structured clone for safety or Transferable objects for performance, worker pool pattern for amortizing creation overhead and enabling parallelization, robust error handling with timeouts and recovery, and task queuing to handle more tasks than available workers.</p>
        <p>Advanced considerations include SharedArrayBuffer for extremely performance-sensitive scenarios (audio/video processing) with careful synchronization using Atomics. Monitoring is essential: track task execution times, queue depth, error rates, and worker memory usage to detect saturation or crashes. Testing should include both unit tests with mocked workers and integration tests with real workers.</p>
        <p>Common production use cases: cryptographic operations (hashing, encryption) on large files, data processing (JSON parsing, image manipulation), background sync, and real-time rendering. Real-world systems (Google Maps, Figma, video editors) use worker pools (Piscina, Comlink libraries) with careful error handling to prevent memory leaks. For best results, measure main thread latency before and after worker adoption (should improve 30-50%), use pool size matching CPU cores, set task timeouts at 30-60 seconds, and always terminate workers on page unload to prevent orphaned processes.</p>
      </section>
    </ArticleLayout>
  );
}
