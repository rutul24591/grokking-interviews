  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <HighlightBlock as="p" tier="important">A gesture system abstracts these low-level details. Instead of writing raw touchstart handlers, developers use gesture recognizers: "on swipe left, advance carousel", "on pinch, zoom map". The system handles velocity calculation, distance measurement, state management, and multi-touch disambiguation. It also handles edge cases: what if two gestures start simultaneously (pinch + rotate)? Which one wins? How long do we wait before deciding?</HighlightBlock>
        <HighlightBlock as="p" tier="important"><strong>Explicit assumptions:</strong> Multi-touch is supported (minimum 2, typically 10 simultaneous touches). Gesture recognition must be accurate (minimize false positives). Gestures can be cancelled (user decides mid-gesture they don't want the action). Desktop mouse events must be handled (tap maps to click, pan maps to drag, long-press maps to right-click). Frameworks like React Native, Flutter, and web libraries like Hammer.js exist but custom implementation is common for specific needs.</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          ...
          <HighlightBlock as="li" tier="important"><strong>Pan Detection:</strong> Recognize slow continuous multi-directional movement. No velocity requirement, just track movement. Useful for scrolling, dragging. Report pan offset and velocity to handler.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Pinch Detection:</strong> Recognize two-finger pinch gesture (fingers moving apart or together). Report scale (new distance / initial distance) to handler. Support both zoom-in (expanding) and zoom-out (contracting).</HighlightBlock>
          ...
          <HighlightBlock as="li" tier="important"><strong>Long-Press Detection:</strong> Recognize stationary finger held for duration (e.g., 500ms). Fire event after threshold met. Allow gesture to be cancelled if finger moves before threshold. Useful for opening context menus, initiating drag-and-drop.</HighlightBlock>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="crucial"><strong>Latency:</strong> Gesture recognition completes within 100ms of gesture completion (end of swipe, pinch, etc.). Real-time updates (swipe moving) should fire with under 50ms latency for smooth visual feedback.</HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">Layer 3 (Handling): Register gesture handlers ("when user swipes left, advance carousel"). When a gesture event fires, call the registered handler. Handle gesture state transitions: on recognize (gesture began), on update (gesture data changed, e.g., zoom scale increased), on end (gesture completed), on cancel (gesture rejected).</HighlightBlock>
        <HighlightBlock as="p" tier="important">Key design pattern: gestures are stateful. A pinch gesture begins (two touches, distance stable), moves (distance increases), and ends (fingers lifted). Handlers receive state updates ("pinch moved, scale 1.2x") so they can provide realtime feedback. Handlers can reject a gesture mid-stream (e.g., "pinch cancelled because third finger touched"), triggering cancellation state.</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">Long-press recognition: on touchstart, set a timeout (500ms). On touchmove, check if movement exceeds 10px. If yes, cancel the long-press (user is moving, not holding). If timeout fires without cancel, emit longPress event. If touchend before timeout, emit tap event instead.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">Desktop users interact with mouse, not touch. Map mouse events to touch equivalents: mousedown → touchstart, mousemove → touchmove, mouseup → touchend. Use a synthetic "touch" object with mouse coordinates. Pinch is simulated with keyboard modifier: Ctrl+scroll wheel becomes pinch.</HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important"><strong>Complex Gestures vs Recognition Load:</strong> Supporting many gesture types (swipe, pinch, rotate, long-press, double-tap) increases computational load and false positive risk. Prioritize gestures by user value. Long-press and swipe are common; rotate is less common (only for photo apps).</HighlightBlock>
        <HighlightBlock as="p" tier="important"><strong>Desktop Parity vs Mobile-First:</strong> Full mouse support (pinch via Ctrl+wheel, double-tap via click) adds complexity. Mobile-first approach: optimize for touch, provide basic mouse fallback (click instead of tap). Accept that desktop UX may be slightly different.</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">Track two-finger pinch. On pinch event with scale above 1.0, increase image size proportionally to the scale delta. Use a scale transform. Clamp scale to a bounded range such as 0.5 to 3.0 to prevent over-zooming. Include momentum: on pinch end, continue scaling with deceleration based on velocity.</HighlightBlock>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 3: Long-Press Context Menu</h3>
        <HighlightBlock as="p" tier="important">On longPress event, show context menu (fixed position or floating popup). Menu appears at touch location. Provide visual feedback during long-press (highlight item). If user moves finger before long-press fires, cancel (reset highlight).</HighlightBlock>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 4: Multi-Gesture Responder Chain</h3>
        <HighlightBlock as="p" tier="important">Implement responder protocol: view can claim gesture or pass to parent. ListItem implements swipe to delete. ScrollableList implements swipe to advance page. On swipe: ListItem checks if swipe is horizontal/far enough to delete. If yes, claim and delete. If no, pass to parent (ListItem returns false). ScrollableList receives swipe, advances page.</HighlightBlock>
      </section>
