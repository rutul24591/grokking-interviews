  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <HighlightBlock as="p" tier="important">Auto-save is the feature that eliminates the anxiety of document editing: the user types, the system silently persists, and no content is ever lost to a browser crash or network blip. Implementing it incorrectly introduces its own problems: saving on every keystroke hammers the server (100 requests per second from an active typist), saving on an interval fires regardless of whether anything changed (wasted requests), and saving too infrequently creates windows where content loss is still possible.</HighlightBlock>
        <HighlightBlock as="p" tier="crucial">Beyond the basic debouncing problem, a production auto-save system must handle: concurrent editing from multiple devices (user switches from phone to desktop mid-document; both have unsaved changes), save failures with retry and user notification, a "saving" / "saved" / "error" status indicator that accurately reflects the persistence state, and an emergency flush on page unload (save immediately when the user closes the tab, without waiting for the debounce timer).</HighlightBlock>
        <HighlightBlock as="p" tier="important"><strong>Explicit assumptions:</strong> The document is a single structured entity (a JSON document, rich text content, or a form). Edits are captured as full document snapshots (not incremental diffs—those require a more complex CRDT or OT system). Auto-save targets under 2 seconds of data loss risk; the debounce timer is 2 seconds of idle. Conflict detection uses optimistic concurrency control (OCC) with the document's server-side version as the conflict key.</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="important"><strong>Debounced save:</strong> Trigger a save 2 seconds after the user stops typing. Do not save on every keystroke.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Maximum save interval:</strong> Even if the user never pauses, force a save every 30 seconds to bound data loss risk.</HighlightBlock>
          ...
          <HighlightBlock as="li" tier="important"><strong>Status indicator:</strong> Show "Unsaved changes," "Saving...," "All changes saved," or "Save failed. Retrying..." in real-time.</HighlightBlock>
          ...
          <HighlightBlock as="li" tier="crucial"><strong>Retry on failure:</strong> On save failure (network error, server error), retry with exponential backoff. Notify the user if retries are exhausted.</HighlightBlock>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="important"><strong>Data loss risk:</strong> Maximum 2 seconds of edits can be lost (bounded by debounce + max interval strategy).</HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="crucial">The auto-save system is a state machine layered on top of the document editor. The states are: idle (no unsaved changes), unsaved (changes pending, debounce timer running), saving (server request in flight), saved (server confirmed), and error (save failed, retry scheduled). Transitions are driven by user edits (idle → unsaved), debounce timer fire (unsaved → saving), server success (saving → saved), and server failure (saving → error).</HighlightBlock>
        <HighlightBlock as="p" tier="important">The debounce timer uses a trailing approach: every edit resets the timer to T+2s. If edits are continuous, the timer keeps resetting. After 2 seconds of idle, the timer fires and the save begins.</HighlightBlock>
<HighlightBlock as="p" tier="important">A parallel maximum-interval timer fires every 30 seconds unconditionally, flushing any pending changes regardless of typing state. This combination provides responsive saves (within 2 seconds of the last edit in a pause) and bounded worst-case loss (30 seconds of continuous typing).</HighlightBlock>
      </section>
...
<ArticleImage
        ...
        <HighlightBlock as="p" tier="crucial">
          Interview signal: the diagram captures the end-to-end flow for <strong>Design Debounced Auto-Save System</strong>. You should be able to explain the happy path and the failure paths (retries, cancellation, backpressure), not just the API surface.
        </HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">The status indicator is driven directly by the auto-save state machine. Transitions: user types → state = unsaved → show "Unsaved changes" with a dot indicator. Debounce fires → state = saving → show "Saving..." with a spinner. Server returns 200 → state = saved → show "All changes saved" with a checkmark (fade out after 3 seconds). Server returns error → state = error → show "Save failed. Retrying in 5s..." with a warning icon and retry countdown.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">On save failure (network error, server 5xx), the auto-save system enters a retry loop with exponential backoff: retry after 5 seconds, then 10, then 20, capping at 60 seconds. The status indicator shows the next retry time. The user can click "Retry now" to skip the backoff and retry immediately. After 5 consecutive failures over 10+ minutes, the system stops automatic retries and shows a persistent "Unable to save. Your changes are stored locally and will sync when connectivity is restored." This is the offline-tolerant posture: accept that saves will fail during outages, preserve the draft in localStorage, and resume syncing automatically when the server is reachable again.</HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="crucial">Debounce delay versus data loss risk: a shorter debounce (500ms) saves more frequently but generates more server load. A longer debounce (5 seconds) reduces load but increases data loss risk. The 2-second default is calibrated to the typical writing pause between sentences—fast enough to feel safe, slow enough not to spam the server. For code editors or spreadsheets where changes are more discrete, a shorter debounce may be appropriate.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Full document save versus patch-based: saving the entire document on every auto-save is simple but wasteful for large documents (sending 100KB JSON when only one sentence changed). Patch-based saves (compute a diff and send only the delta) reduce payload size but require the server to apply patches correctly and handle out-of-order patch delivery. For most documents under 50KB, full saves are the right choice. For larger documents (multi-thousand-word articles, complex data), the engineering cost of patch-based saves is justified.</HighlightBlock>
        <HighlightBlock as="p" tier="important">OCC versus last-write-wins for conflicts: OCC (return 412 on conflict, require user resolution) is correct for documents where precision matters (legal, configuration). Last-write-wins is simpler and appropriate for lower-stakes content (personal notes, drafts) where the user is the only editor and conflicts are rare. The choice should be explicit—defaulting to last-write-wins silently drops changes when concurrent editing occurs.</HighlightBlock>
      </section>

      <section>
        <h2>Summary</h2>
        <HighlightBlock as="p" tier="important"><Highlight tier="crucial">Conflict detection uses If-Unmodified-Since headers with server-side version tracking; conflicts surface a resolution UI rather than silently overwriting. The status indicator</Highlight></HighlightBlock>
<HighlightBlock as="p" tier="important">is an ARIA live region driven by state machine transitions. Retry logic with exponential backoff handles transient server failures without losing the user's work. The system's defining constraint is that the editing experience must never be blocked—all save operations are asynchronous, and all failure states degrade gracefully to local draft preservation.</HighlightBlock>
      </section>
    </ArticleLayout>
  );
}
