  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <HighlightBlock as="p" tier="crucial">Audit logs are a compliance and security requirement for enterprise applications: every significant action (data modification, permission change, login event, configuration change) must be recorded with who performed it, what they changed, when, and from where. The audit log viewer is the interface through which administrators, security teams, and compliance auditors review this history. It must handle potentially billions of records while providing sub-second search and filtering, and the records themselves must be tamper-evident (an attacker who compromises the application should not be able to delete or modify audit records).</HighlightBlock>
        <HighlightBlock as="p" tier="important">The UI design challenge is presenting dense, structured event data in a form that allows investigators to quickly find what they're looking for. A security analyst investigating a breach might need to find all actions taken by a specific user in a 2-hour window, or all changes to a specific record across all users and all time. A compliance auditor might need all privileged access events from the past 90 days exported to a CSV. Each query pattern requires different indexing, filtering, and export capabilities.</HighlightBlock>
        <HighlightBlock as="p" tier="important"><strong>Explicit assumptions:</strong> Audit logs are append-only; no modification or deletion is permitted through the UI (admin users can query but not mutate). The backend stores logs in Elasticsearch for fast faceted search alongside a cold storage archive (S3 + Parquet) for compliance retention. The UI is accessible only to users with the "auditor" or "admin" role. Large result sets are exported asynchronously (the export system design applies).</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="important"><strong>Faceted search and filtering:</strong> Filter by actor (userId, email), event type (login, data_change, permission_change), target resource (resourceType, resourceId), date range, IP address, and outcome (success, failure).</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Full-text search:</strong> Search across actor names, resource names, and change descriptions.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Event detail view:</strong> Click an event to see full details: before/after JSON diff for data changes, request metadata (IP, user agent), session information.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Cursor pagination:</strong> Navigate through large result sets without page-flip (load more on scroll or page navigation).</HighlightBlock>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="crucial"><strong>Search latency:</strong> Filtered queries return within 2 seconds for up to 90-day lookback windows.</HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">The audit log viewer is a read-only search interface backed by Elasticsearch. The application writes audit events to a Kafka topic, which is consumed by two sinks: Elasticsearch (for fast interactive search, 90-day hot window) and S3 (for long-term archival in Parquet format, queried via Athena for compliance exports beyond 90 days).</HighlightBlock>
<HighlightBlock as="p" tier="important">The viewer UI sends search queries to an API layer that translates filter criteria into Elasticsearch queries and returns paginated results. The API enforces RBAC by injecting must_match clauses that restrict results to records the requester is authorized to see.</HighlightBlock>
        <HighlightBlock as="p" tier="crucial">Tamper evidence uses a hash chain: each audit record includes a hash of its own content concatenated with the hash of the previous record. If any record is modified or deleted, the hash chain breaks. The viewer periodically verifies hash chain integrity for the displayed window and shows a visual integrity indicator. A broken chain surfaces a red warning: "Log integrity verification failed between events X and Y."</HighlightBlock>
      </section>
...
<ArticleImage
        ...
        <HighlightBlock as="p" tier="crucial">
          Interview signal: the diagram captures the end-to-end flow for <strong>Design Audit Log Viewer UI</strong>. You should be able to explain the happy path and the failure paths (retries, cancellation, backpressure), not just the API surface.
        </HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">The filter panel on the left (desktop) or expandable bottom sheet (mobile) contains: a date range picker (quick selects: last hour, last 24h, last 7 days, custom range), actor search (autocomplete backed by the users API), event type multi-select (checkboxes for each event type category), resource type and ID search, outcome filter (success/failure/all), and IP address filter (CIDR range supported for network investigations).</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="crucial">Audit logs are naturally time-series: queries filter by date range and sort by timestamp descending (most recent first). Cursor pagination for time-series uses the (timestamp, eventId) tuple as the cursor, similar to the activity feed pattern. The query condition: events WHERE timestamp &lt; cursor.timestamp OR (timestamp = cursor.timestamp AND eventId &lt; cursor.eventId), sorted by timestamp DESC, eventId DESC, LIMIT N.</HighlightBlock>
	        <HighlightBlock as="p" tier="important">For Elasticsearch, cursor pagination uses a continuation token derived from the last item in the current page, based on the same stable sort keys used in the query (typically timestamp and eventId). Subsequent queries pass this continuation token so Elasticsearch can resume from the correct position in the sorted index without scanning and discarding all earlier pages. This remains efficient even for very deep pagination.</HighlightBlock>
        ...
        <h3 className="mt-6 mb-3 text-lg font-semibuild">RBAC Enforcement</h3>
	        <HighlightBlock as="p" tier="important">RBAC is enforced at the API query layer by injecting mandatory scope filters that cannot be overridden by client-supplied filter parameters. For example, a manager whose scope is limited to their team will always have an organization scope filter applied, and an actor scope filter limited to their team members. Client-provided filters are applied on top of these mandatory scope constraints, which means the client can only narrow the result set further, never broaden it beyond what they are authorized to see.</HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="crucial">Elasticsearch versus database for audit search: a relational database with appropriate indexes can support audit log queries, but Elasticsearch provides better full-text search, native faceted aggregations (counts per event type, per actor), and CIDR range queries (for IP filtering) without custom implementation. The operational overhead of running Elasticsearch is the trade-off. For applications with strict data residency requirements, a managed Elasticsearch service (Elastic Cloud, OpenSearch Serverless) may be required to avoid self-hosting.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Hot versus cold storage separation: storing all audit events in Elasticsearch indefinitely is expensive. A 90-day Elasticsearch window covers the vast majority of interactive investigations; events older than 90 days are rare to query interactively and are better served from a data warehouse (Athena, BigQuery). The trade-off is query complexity: the UI must either query both systems (hot + cold) for queries spanning the boundary, or accept that the interactive UI only covers 90 days and compliance exports beyond that use a separate workflow. The latter is simpler and sufficient for most compliance auditing patterns.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Hash chain integrity versus performance: computing and verifying hash chains adds CPU overhead at write time (hash of previous record required before inserting a new one, which serializes inserts) and at read time (verification requires fetching records in order). For very high-volume audit logging (millions of events per day), hash chains may create write bottlenecks. Alternatives: block-level signing (hash a batch of 1000 events together and sign the block) reduces the chain computation cost while preserving tamper evidence at block granularity.</HighlightBlock>
      </section>

      <section>
        <h2>Summary</h2>
        <HighlightBlock as="p" tier="important"><Highlight tier="crucial">The UI provides faceted filters encoded in URL parameters (for shareability), event detail views with before/after diffs, related</Highlight></HighlightBlock>
<HighlightBlock as="p" tier="important">event traversal for incident investigation, and async export with email delivery for compliance reporting. The defining constraint is immutability: no UI action can modify or delete audit records, and even administrator access to another user's records is itself audited.</HighlightBlock>
      </section>
    </ArticleLayout>
  );
}
