  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>Bulk editing allows users to select multiple items from a list and apply a single operation to all of them: archive 50 emails, change the status of 200 orders to "shipped," delete 30 outdated records, or reassign 100 tasks to a different team member. Without bulk operations, users must repeat the same action N times, which is both tedious and error-prone (they may miss items or accidentally apply the action to the wrong records).</p>
        <p>The UI challenge is selection state management at scale. A list with 10,000 items cannot have 10,000 checkboxes in memory simultaneously (virtual scrolling means many rows are unmounted). "Select all" on a paginated list must select all 10,000 records, not just the 50 currently visible. Range selection (click a checkbox, shift-click another) must work correctly across page boundaries. Partial selection state (some but not all items selected) must be reflected in the header checkbox ("indeterminate" state). After the operation, items may disappear from the list (archived items hidden), requiring list re-render without losing the user's context.</p>
        <p><strong>Explicit assumptions:</strong> Items are displayed in a virtualized list with cursor pagination. Selection is managed client-side as a Set of IDs. "Select all" uses a virtual select-all flag plus an excluded IDs set, not by loading all IDs into memory. Bulk operations are async (server processes in a background job for large batches). Undo is supported for a 5-second window after the operation completes.</p>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Individual selection:</strong> Click a checkbox on any row to add/remove it from the selection set.</li>
          ...
          <li><strong>Partial success handling:</strong> When some items succeed and others fail, show a summary and allow retry of failed items.</li>
          <li><strong>Undo:</strong> Show a toast with an "Undo" action for 5 seconds after a destructive bulk operation. Undo reverses the operation for all affected items.</li>
        </ul>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Non-Functional Requirements</h3>
        <ul className="space-y-2">
          <li><strong>Selection performance:</strong> Selecting/deselecting individual items must be O(1). Toggling "select all" must not require iterating all items client-side.</li>
          <li><strong>Memory efficiency:</strong> Selection state for 100,000 items must not require 100,000 objects in memory; use the virtual select-all pattern.</li>
      ...
      <section>
        ...
        <p>Selection state uses a two-mode model: "explicit selection" (a Set of selected IDs, used when the user picks individual items) and "virtual select-all" (a flag indicating all items are selected plus a Set of explicitly excluded IDs, used when the user clicks "Select all N items"). This two-mode approach allows "select all 100,000 items" to be represented as a single boolean plus a small exclusion set—rather than 100,000 IDs in memory.</p>
        <p>The context action bar appears (slides up from the bottom or replaces the header) when the selection count is greater than zero. It shows the count of selected items and the available bulk actions for the current content type. On action confirmation, the application sends the action to the server with either the explicit list of IDs or the "select all with exclusions" criteria. The server processes items asynchronously and returns a jobId. The client polls the jobId endpoint for progress until the job completes.</p>
      </section>
      ...
      <section>
        ...
        <p>After 5 seconds without clicking Undo, the toast fades and the server may garbage-collect the before-state snapshot (though it's good practice to keep it for 24 hours for recovery purposes). If the user navigates away during the 5-second window, the batchId should be stored in sessionStorage so the undo action is still available on the next page (or on return to the same page). The undo action should be idempotent—calling it twice for the same batchId should have no effect on the second call.</p>
