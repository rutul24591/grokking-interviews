  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <HighlightBlock as="p" tier="crucial">Comments are a fundamental engagement mechanism: they turn static content into conversations. The design space spans from simple flat lists (YouTube-style) to deeply threaded trees (Reddit-style) to inline document comments (Google Docs-style). Each topology has different data model requirements, rendering strategies, and real-time update patterns. The wrong choice for the content type creates a poor experience: flat comments on developer documentation make it hard to follow discussion threads; deeply nested comments on a news site create confusing indentation hierarchies that are hard to read.</HighlightBlock>
        <HighlightBlock as="p" tier="important">The frontend challenges are: rendering potentially large comment trees efficiently (a popular post may have thousands of comments), supporting real-time append (new comments from other users appear without refresh), enabling optimistic posting (the user's comment appears immediately while awaiting server confirmation), handling nested replies at arbitrary depth without infinite recursion, and providing moderation controls (flag, hide, delete) without disrupting the reading experience.</HighlightBlock>
        <HighlightBlock as="p" tier="important"><strong>Explicit assumptions:</strong> Comments are stored as an adjacency list (each comment has a parentId). Maximum display depth is 3 levels (top-level → reply → reply-to-reply); deeper nesting shows as "view more replies" links. The first page loads the top N top-level comments sorted by recency (or popularity). Replies are lazy-loaded per thread on user interaction. Real-time new comments are delivered via WebSocket. Moderation (flagging, hiding) is a server-side operation reflected optimistically in the UI.</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="important"><strong>Post comment:</strong> Authenticated user submits a comment on a content item. Optimistic posting with server confirmation.</HighlightBlock>
          ...
          <HighlightBlock as="li" tier="important"><strong>Pagination:</strong> Load first N top-level comments; load more on demand. Load replies per thread on demand.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Editing and deletion:</strong> Author can edit a comment (with "edited" indicator) or delete (soft delete with "comment removed" placeholder to preserve thread structure).</HighlightBlock>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="crucial"><strong>Render performance:</strong> Initial comment section renders in under 500ms. Comment trees with 1000+ items use virtual scrolling or pagination to avoid DOM bloat.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Posting latency:</strong> Optimistic post makes the comment visible in under 100ms. Server confirmation within 2 seconds on normal network.</HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">The comment section loads the first page of top-level comments on mount. Each top-level comment shows its direct reply count but not its replies—replies are loaded lazily when the user clicks "View N replies." Real-time new comments arrive via WebSocket and are prepended to the list (if sorting by newest) or appended (if sorting by oldest).</HighlightBlock>
<HighlightBlock as="p" tier="important">Optimistic posting adds the new comment immediately with a temporary ID; on server confirmation, the temporary ID is replaced with the permanent one.</HighlightBlock>
        <HighlightBlock as="p" tier="crucial">The data model is an adjacency list: each comment has a parentId (null for top-level). Fetching a thread loads all comments with parentId equal to the thread root, sorted by createdAt. Rendering is recursive but depth-limited (no more than 3 visual levels). Beyond 3 levels, additional replies are shown as flat replies to the closest ancestor within the depth limit.</HighlightBlock>
      </section>
...
<ArticleImage
        ...
        <HighlightBlock as="p" tier="crucial">
          Interview signal: the diagram captures the end-to-end flow for <strong>Design Comments System</strong>. You should be able to explain the happy path and the failure paths (retries, cancellation, backpressure), not just the API surface.
        </HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="crucial">When the user submits a comment, the UI should not make them wait for the server. The optimistic flow: (1) generate a temporary commentId (prefixed "temp-" to distinguish from permanent UUIDs); (2) insert the comment into local state with the temp ID, the user's own author information, and createdAt: new Date(); (3) render the comment immediately with a subtle "sending" indicator (spinning icon, slightly dimmed); (4) send POST /comments to the server; (5) on success, replace the temp ID with the server-assigned permanent ID in local state and remove the "sending" indicator; (6) on failure, remove the comment from local state and show an error with a "retry" option that re-submits the same content.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">For comments with many replies (50+), the initial reply fetch loads the first 20 and shows a "Load 30 more replies" link. Subsequent loads use cursor pagination. Each nested level independently tracks its own pagination cursor, so loading more replies for thread B doesn't affect thread A's pagination state. The reply fetch includes grandchild reply counts but not the grandchildren themselves—each nested level is independently lazy-loaded on interaction.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">Running spam and policy filters server-side adds latency to the post flow (the server can't respond before checking). Running filters client-side before submission catches obvious violations without server latency but can be circumvented by technical users (who can inspect and modify the filter logic). The pragmatic solution is to run lightweight client-side filters in a Web Worker (regex patterns for known spam URLs, profanity word lists) as a fast first pass, then run authoritative server-side filters after submission.</HighlightBlock>
        <HighlightBlock as="p" tier="important">The client-side filter runs in a Web Worker to avoid blocking the main thread during the typing experience. It checks the comment text as the user types (debounced at 500ms) and shows an inline warning for obvious violations: "Your comment appears to contain spam links." This gives the user a chance to correct the content before submitting. If the user submits anyway, the server filter makes the final call and can reject the post with a specific error message that the client displays.</HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">Adjacency list versus nested set versus closure table for comment storage: adjacency list (parentId on each row) is simple to write and easy to understand but requires multiple queries to fetch a full thread (one per depth level, or a recursive CTE). Closure table (a separate table storing all ancestor-descendant pairs) enables fetching an entire thread in one query but doubles write complexity. Nested sets allow range queries for subtrees but are expensive to update (any insert requires renumbering). For most comment systems (max depth 3-5), the adjacency list with recursive CTEs is the correct balance of simplicity and performance.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Real-time delivery to comment sections: WebSocket per-page (subscribe to all comments on the current content item) is straightforward but creates many subscriptions for popular content pages. Server-Sent Events with content-item-specific channels are simpler (unidirectional, reconnects automatically) and sufficient for read-heavy comment sections. WebSocket is preferred if the comment UI also sends real-time events (e.g., typing indicators in a collaborative document comment).</HighlightBlock>
        <HighlightBlock as="p" tier="crucial">Comment loading strategy: load all top-level comments versus paginate: for content with fewer than 50 top-level comments, loading all at once simplifies the implementation and allows full client-side sort/filter. Above 50, pagination is necessary to avoid large payloads and slow renders. The threshold should be configured per content type—a developer documentation page with 200 comments might prefer pagination; a product page with 12 comments might prefer loading all.</HighlightBlock>
      </section>

      <section>
        <h2>Summary</h2>
        <HighlightBlock as="p" tier="important"><Highlight tier="crucial">Depth limiting (max 3 visual levels) keeps the UI readable. Sort preference (newest/oldest/top) stored in localStorage</Highlight></HighlightBlock>
<HighlightBlock as="p" tier="important">persists the user's choice. The deduplication logic for optimistic posts and real-time WebSocket events is the highest-complexity piece of the implementation and must handle the race condition where the WebSocket event arrives before the POST response.</HighlightBlock>
      </section>
    </ArticleLayout>
  );
}
