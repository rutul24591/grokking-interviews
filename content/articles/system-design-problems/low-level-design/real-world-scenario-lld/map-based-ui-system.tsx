    ...
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Viewport-based data loading:</strong> Only load markers for the currently visible map area. Reload when the viewport changes (pan or zoom) with debounce.</li>
          <li><strong>Marker clustering:</strong> Group nearby markers into cluster markers at lower zoom levels. Show count badge on cluster. Zoom to cluster on click.</li>
          <li><strong>Marker interaction:</strong> Click individual marker to show a popup with details. Hover to show a tooltip. Selected marker highlighted.</li>
          <li><strong>Real-time updates:</strong> Live-tracking markers (delivery drivers, vehicles) update their position in real-time without reloading all markers.</li>
        ...
        <ul className="space-y-2">
          ...
          <li><strong>Viewport query latency:</strong> Bounding box query returns within 500ms for viewports up to city scale.</li>
      ...
      <section>
        ...
        <p>The map renders using Mapbox GL JS, which uses WebGL for the base tiles and a GeoJSON source layer for custom markers. All markers are in a single GeoJSON FeatureCollection; Mapbox renders them efficiently in a single WebGL draw call rather than creating one DOM element per marker. The clustering layer wraps this GeoJSON source with Mapbox's built-in clustering (which uses Supercluster under the hood) to automatically merge nearby points into clusters at lower zoom levels.</p>
        <p>When the map viewport changes (pan or zoom ends), the application reads the new bounding box and fires a debounced query to the server. The server returns GeoJSON features for all data points within the bounding box. The client updates the GeoJSON source; Mapbox re-renders the layer. The debounce prevents a flood of queries during continuous pan/zoom interactions—the query fires 300ms after the last viewport change event.</p>
        <p>Live-tracking markers (delivery drivers, for example) are managed separately from the static marker layer. They are stored in a separate GeoJSON source updated via WebSocket events. Position updates modify only the relevant feature's coordinates in the GeoJSON source, not the entire source. Mapbox re-renders only the affected layer section.</p>
      </section>
      ...
      <section>
        ...
        <p>Efficient position update handling: calling setData() with the full FeatureCollection on every position update is O(n) data transfer per update (where n is the number of features). For large numbers of live markers (100+ drivers), this is expensive. Mapbox GL JS supports partial GeoJSON source updates via updateData() (in newer versions), which allows updating individual features without replacing the entire source. If updateData() is not available, the alternative is to maintain the live markers as a separate GeoJSON source from the static markers—the live source has fewer features and can be fully replaced more cheaply.</p>
        <p>Smooth position animation: receiving a position update and instantly teleporting the marker to the new position looks jarring. Interpolating the marker's position between the old coordinates and the new coordinates over the update interval (e.g., interpolate over 500ms if updates arrive every second) creates the appearance of smooth motion. This is implemented using requestAnimationFrame to lerp the coordinates at 60fps and calling setData() on each animation frame. The interpolation must be stopped if a new update arrives mid-interpolation (replace the target coordinates and continue interpolating to the new target).</p>
        ...
        <p>For the list view to scroll to a list item without layout shifts, the list must be virtualized (only rendering visible items). Finding and scrolling to a specific item in a virtualized list requires the virtualization library's scrollToIndex() API (react-virtual or react-window provide this). The list must maintain a stable mapping from markerId to list index; this mapping is computed when the list data loads and updated on viewport query refresh.</p>
      ...
      <section>
        ...
        <p>Client-side clustering versus server-side clustering: client-side clustering (Supercluster on the data returned from the server) is simpler—the client applies clustering to whatever the server returns. Server-side clustering (the server returns pre-clustered data based on the zoom level) reduces the data volume sent to the client (especially useful when the viewport contains millions of points that cannot all be sent client-side). For most applications (up to ~100,000 points in a viewport), client-side clustering is sufficient. For truly massive datasets, server-side clustering (using PostGIS's ST_Collect or Elasticsearch's geo_tile aggregation) is necessary.</p>
        <p>DOM markers versus WebGL markers: Mapbox GL JS supports two marker types: HTML markers (Mapbox Marker API, each marker is a DOM element) and GeoJSON layer markers (WebGL). HTML markers support richer interactivity (full HTML/CSS, React components) but do not scale past ~1000 markers without significant performance impact. WebGL markers scale to millions of points but support only what Mapbox's data-driven styling can express. The decision rule: use WebGL markers for any application that might show hundreds or thousands of markers simultaneously; use HTML markers only for applications that will always have fewer than 50 markers or need very custom interactivity (embedded React component in a marker popup).</p>
        <p>Viewport query versus static full dataset: if the total dataset is small (under 5000 points), loading the entire dataset once and filtering client-side is simpler than viewport queries and eliminates loading gaps during pan. This works for property search with a small inventory or a delivery dashboard with fewer than 1000 active drivers. For larger datasets, viewport queries are necessary.</p>
      </section>

      <section>
        <h2>Summary</h2>
        <p>A production map-based UI uses WebGL-based marker rendering (Mapbox GL JS GeoJSON layers) to handle thousands of markers without DOM overhead. Viewport-based data loading (debounced bounding box queries, 300ms after pan/zoom) prevents loading the full dataset. Client-side clustering via Mapbox's built-in Supercluster integration groups dense areas into cluster markers at lower zoom levels, expanding to individual markers as the user zooms in. Real-time marker position updates via WebSocket use efficient GeoJSON source updates with coordinate interpolation for smooth animation. List-map synchronization uses shared selectedMarkerId state, with map layers applying data-driven highlight styles and the list using virtualized scrollToIndex for large result sets. URL encoding of center, zoom, and selected marker makes map views bookmarkable and shareable. The defining performance constraint is the WebGL render path: keeping all markers in GeoJSON sources (not DOM elements) and using Mapbox's data-driven styling for appearance variations enables scaling to tens of thousands of visible markers at interactive frame rates.</p>
      </section>
    </ArticleLayout>
  );
}
