  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <HighlightBlock as="p" tier="important">Version history gives users confidence to make bold changes: if a document, configuration, or data record can be reverted to any previous state, the cost of mistakes is low. Without version history, users work conservatively (afraid to break something that cannot be undone) or maintain their own versioning externally (appending "_v2", "_backup", "_final_FINAL" to filenames). With good version history, users work faster and experiment more freely.</HighlightBlock>
        <HighlightBlock as="p" tier="crucial">The design problem is multidimensional. Storage efficiency: storing complete copies of every version is wasteful for large documents with small incremental changes. Storage using only deltas (diffs) is efficient but requires reconstructing versions by applying diffs in sequence, which is slow for old versions. The correct design for most applications stores complete snapshots (not deltas) with efficient content-addressed deduplication—unchanged sections don't consume additional storage. Diff display: showing what changed between two versions requires a text or structured diff algorithm. The display must be readable for non-technical users. Rollback: creating a "new version" that restores old content (rather than overwriting the current version in place) preserves the history of the rollback itself.</HighlightBlock>
        <HighlightBlock as="p" tier="important"><strong>Explicit assumptions:</strong> The versioned entity is a structured document (JSON or rich text). Versions are created on explicit save events (not auto-saved every second—that would create thousands of versions). Rollback creates a new version (does not overwrite history). Named versions ("Published Draft", "Before Review") allow flagging significant milestones. The version history UI shows a timeline of versions with author, timestamp, and change summary.</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="important"><strong>Version creation:</strong> A new version is created on explicit save. Auto-save creates or updates a draft, not a version.</HighlightBlock>
          <HighlightBlock as="li" tier="crucial"><strong>Version timeline UI:</strong> Show a chronological list of versions with: version number, author, timestamp, optional name, and a one-line change summary.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Diff view:</strong> Select any two versions to compare. Show word-level diff (added/removed/unchanged text) for rich text; field-level diff for structured JSON.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Rollback:</strong> Create a new version with the content of an older version. The rollback itself is a version in the timeline ("Rolled back to v12").</HighlightBlock>
        ...
        <ul className="space-y-2">
          ...
          <HighlightBlock as="li" tier="important"><strong>Diff performance:</strong> Diff between any two versions computes within 500ms for documents up to 100,000 words.</HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="crucial">Each version is an immutable snapshot stored as a complete copy of the document content at that point in time. Deduplication is handled at the block level (the document is split into content blocks; unchanged blocks are referenced by hash rather than stored again). The version record stores: versionId, documentId, authorId, content (or a reference to the content in block storage), contentHash (SHA-256 of the full content, used to detect duplicate saves), parentVersionId (the version this was based on, forming a DAG for branching support), optional versionName, and timestamps.</HighlightBlock>
        <HighlightBlock as="p" tier="important">The diff between two versions is computed server-side on demand (not precomputed for every version pair) using a word-level diff algorithm (diff-match-patch for text, RFC 6902 JSON Patch for structured data). The result is returned as an annotated document with each word or field tagged as added, removed, or unchanged. The client renders this annotated diff in a two-panel or inline display.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Rollback creates a new version record with the content of the target historical version, plus a rollback annotation in the version metadata (rolledBackFromVersionId). The current version advances—history is always append-only, never mutated. This means the timeline shows the rollback as a new version event, preserving the full record of what happened: "v14: Rolled back to v12 (by Alice at 3:45pm)."</HighlightBlock>
      </section>
...
<ArticleImage
        ...
        <HighlightBlock as="p" tier="crucial">
          Interview signal: the diagram captures the end-to-end flow for <strong>Design Version History System</strong>. You should be able to explain the happy path and the failure paths (retries, cancellation, backpressure), not just the API surface.
        </HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">The version timeline is a right-side panel in the document editor. It loads the version list on open: GET /documents/:docId/versions returns a paginated list of version metadata (versionId, versionNumber, authorDisplayName, authorAvatarUrl, createdAt, versionName, changeSummary, isNamed). Content is not included in the list response—it's fetched on demand when the user previews or diffs a version.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Each version row shows: the version number, author avatar, relative timestamp (e.g., "2 hours ago"), optional name badge (for named versions), and the change summary. Named versions are visually distinguished (bold, pinned to the top of named-version group, or shown with a flag icon). Hovering a version row shows action buttons: Preview, Compare to current, Restore. Clicking a row expands it inline (no navigation away from the editor) to show the full change summary and a mini-preview of the diff.</HighlightBlock>
        <HighlightBlock as="p" tier="crucial">The timeline uses cursor pagination (createdAt + versionId as cursor) and loads the most recent 20 versions initially. Scrolling to the bottom loads older versions. Named versions are always included (they are never pruned by the automatic limit) but may appear at any position in the timeline (they are interleaved with regular versions by timestamp).</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">This append-only approach is critical for accountability: in a collaborative environment, users need to know not just what the current content is, but the full history of who changed it and when—including the decision to roll back to an older version. Overwriting history (replacing v14 with v12's content without creating a new version) would hide this information and break the audit trail.</HighlightBlock>
