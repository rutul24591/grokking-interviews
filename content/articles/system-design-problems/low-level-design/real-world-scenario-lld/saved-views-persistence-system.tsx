  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>Power users of data-dense applications (project management tools, CRMs, analytics dashboards, issue trackers) repeatedly apply the same sets of filters, sort orders, and column configurations to navigate to their specific slice of data. Without saved views, every session starts from a default view and the user must re-apply their usual filters manually—tedious and error-prone. With saved views, the user configures their view once ("My Open Tasks, sorted by due date, showing priority and assignee columns") and recalls it with one click thereafter.</p>
        <p>The design challenge involves: representing view state completely enough to reproduce the exact view on recall (all active filters, sort direction, visible columns, grouping), synchronizing view state to the URL for shareability (a saved view can be a shareable link), providing CRUD operations for named views, handling view state that becomes invalid (a filter references a status that has been deleted), and deciding the visibility scope of views (private to the user, shared with the team, default for everyone).</p>
        <p><strong>Explicit assumptions:</strong> Saved views persist server-side for cross-device consistency. View state is also reflected in the URL for shareability. The URL is the single source of truth for the current view state; saved views are named snapshots of URL state. Views can be private (user-only), shared (visible to team members), or default (the view shown to all team members who haven't created their own). CRUD operations for views are provided in the view selector UI.</p>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>URL state sync:</strong> All view state (filters, sort, columns, grouping) is encoded in URL query parameters. Sharing the URL shares the exact view.</li>
          <li><strong>Save current view:</strong> User names and saves the current URL state as a view. View is persisted server-side.</li>
          <li><strong>Load saved view:</strong> Selecting a saved view applies its stored state to the URL, changing the current view without a page reload.</li>
          ...
          <li><strong>Visibility scopes:</strong> Private (only the creating user sees it), Shared (team members can see and load it), Default (replaces the system default for all team members who haven't customized).</li>
          ...
          <li><strong>Invalid state handling:</strong> If a saved view references deleted entities (a filter for status "In Review" which was deleted), the view loads with the invalid filter highlighted and an error message prompting the user to update the view.</li>
        </ul>
      ...
      <section>
        ...
        <p>View state lives in the URL as query parameters: ?status=open&assignee=alice,bob&sort=dueDate:asc&columns=title,assignee,dueDate,priority&groupBy=status. This is the single source of truth. The URL is parsed on mount and whenever the URL changes; the data table renders based on the parsed view state. Filters, sort, and column changes update the URL (via history.pushState or Next.js router.replace) which triggers a re-render.</p>
        <p>Saving a view captures the current URL's query string and sends it to the server with a name: POST /views {"{"}name: "My Open Tasks", state: "status=open&assignee=alice...", scope: "private"{"}"}. Loading a view applies the stored state string to the URL: router.replace(currentPath + "?" + view.state). The URL update triggers the view state to update, which triggers data refetch. This architecture means saving and loading views are simple URL manipulation operations.</p>
      </section>
      ...
      <section>
        ...
        <p>The URL encodes view state as structured query parameters. The schema must be stable (changing the parameter names breaks existing saved views and shared links) and legible (the URL should be readable by a technical user for debugging). Common parameters: filters (multiple key:value pairs, one per active filter), sort (field:direction, e.g., sort=dueDate:asc), columns (comma-separated list of visible column IDs), groupBy (field ID for grouping), page (current page for pagination), and viewId (the ID of the currently active saved view, if any—helps the selector UI highlight the active view).</p>
        <p>Multi-value filters (multiple selected values for one filter, e.g., status=open&status=in-progress) use repeated parameters. Some URL parsers handle repeated parameters as arrays natively; others require the application to parse them explicitly. Using a library like URLSearchParams consistently across the application ensures correct multi-value handling. The parser and serializer must be implemented as a pure pair: serialize(parse(urlString)) === urlString for all valid states.</p>
      ...
      <section>
        ...
        <p>URL state versus localStorage for view state: URL state is shareable, bookmarkable, and doesn't require additional persistence code—it's free from using the router. localStorage is more opaque (the user can't see or share the state easily) and requires explicit serialization and deserialization. URL state is the clear winner for any view state the user might want to share or bookmark. The trade-off is URL length: deeply complex view states with many filters can produce long URLs (over 2000 characters starts to cause issues with some servers and tools). Compression (base64-encoded gzip) of the URL state mitigates length while preserving shareability, at the cost of legibility.</p>
        <p>Server-side versus client-side saved views: server-side (the approach described here) provides cross-device consistency and allows team-shared views. Client-side (localStorage) is simpler and works offline but is device-specific and lost when localStorage is cleared. For a feature used by power users who work across devices, server-side is necessary. For a quick-win implementation, localStorage is acceptable as a starting point that can be migrated to server-side without breaking the UX.</p>
        <p>View state completeness: should the URL encode all possible view state (pagination, current sort, all filters, column widths) or only the "meaningful" state (filters, sort, column visibility)? Column widths and scroll position are ephemeral preferences that clutter the URL without adding shareability value. The principle: encode state that is meaningful to reproduce the view (filters, sort, columns, grouping), not purely UI state (scroll position, column widths). The latter belongs in localStorage or React state.</p>
      </section>

      <section>
        <h2>Summary</h2>
        <p>A saved views persistence system treats the URL as the single source of truth for view state (filters, sort, columns, grouping encoded as query parameters). Saving a view captures the current URL's query string with a name and scope (private/shared/default) via a server-side API. Loading a view applies the stored query string to the URL, triggering a data refetch. The view selector UI highlights the active view (when URL state matches a saved view) or shows "(modified)" when the state has diverged. Invalid views (filters referencing deleted entities) load with valid filters and surface invalid ones with inline errors and removal options. Team default views are set by admins and apply to users without personal defaults. The architecture is simple because URL manipulation is the core operation: both save and load are transformations on the URL's query string, using the same parser and serializer the application already uses for live filter changes.</p>
      </section>
    </ArticleLayout>
  );
}
