  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>E-commerce platforms need reliable shopping carts: users add items, adjust quantities, and apply discounts across potentially multiple sessions and devices. The surface-level problem—store a list of items—hides substantial complexity. Stock levels change while users browse (an item popular enough to sell out between "add to cart" and "checkout"). Users abandon carts and return days later expecting their items. Users shop on a phone and complete checkout on a desktop. Two browser tabs open simultaneously produce conflicting cart state. A coupon applied yesterday may have expired today.</p>
        <p>The naive implementation—a simple array in component state—fails immediately on refresh. Local storage helps but cannot share state across devices. A server-only cart adds latency to every interaction. The production pattern is a hybrid: instant optimistic updates against local state, asynchronous server sync, and server-authoritative validation at checkout.</p>
        <p><strong>Explicit assumptions:</strong> Users may be anonymous (guest cart) or authenticated (server-synced cart). Stock is managed by an inventory service that is the authoritative source. Price snapshots are taken at add-to-cart time but re-validated at checkout. Coupons are server-validated, not trusted from client. Cart persistence is required across sessions (30-day default expiry for idle carts). Concurrent modifications from multiple devices should merge, not silently overwrite.</p>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Add/Remove/Update Items:</strong> Add an item with a specific quantity, update quantity (increment, decrement, direct input), remove item when quantity reaches zero.</li>
          ...
          <li><strong>Cross-Device Sync:</strong> Cart items added on mobile appear on desktop on next load for authenticated users.</li>
          ...
          <li><strong>Guest-to-User Migration:</strong> When a guest logs in, merge guest cart with any existing server cart.</li>
        </ul>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Non-Functional Requirements</h3>
        <ul className="space-y-2">
          <li><strong>Latency:</strong> Add-to-cart must feel instant—under 100ms UI response. Server sync can happen asynchronously in the background.</li>
          <li><strong>Consistency:</strong> Cart must never oversell. Server is the authoritative source for stock and price at checkout time.</li>
      ...
      <section>
        ...
        <p>The cart architecture has three layers: an in-memory reactive store (Zustand or Redux) for immediate UI response, a local persistence layer (localStorage or IndexedDB) for session survival and offline support, and a server-side cart store (database-backed API) for cross-device sync and authoritative checkout validation.</p>
        <p>Every cart mutation follows an optimistic pattern: update the local store immediately (so the user sees instant feedback), persist locally, then fire an async request to sync with the server. If the server rejects the mutation (item out of stock, price changed), the local store is reverted with an appropriate error message. This gives users the perceived performance of a local app with the consistency guarantees of a server-backed system.</p>
        <p>For authenticated users, a WebSocket subscription pushes server-side cart changes (stock updates, price changes, modifications from other devices) to the active session. Guest carts are entirely client-side until login, at which point a merge algorithm combines the guest cart with any existing server cart for that account.</p>
      </section>
      ...
      <section>
        ...
        <p>The key invariant is that the local store is never considered final until the server acknowledges the mutation. For quantity updates specifically, rapid tapping (increment, increment, increment quickly) is handled by debouncing the server call: collect all quantity mutations within a 300ms window and send a single "set quantity to N" request instead of three "increment by 1" requests. The UI reflects the final desired value immediately, while the network call is coalesced to avoid race conditions from overlapping requests.</p>
        <p>Error states require careful handling. A 409 Conflict from the server (version mismatch) triggers a cart refetch, not a simple retry. The system fetches the current server cart, merges it with the pending local changes (using a per-item merge strategy: take the higher quantity, keep items that exist on either side), and re-presents the merged state to the user with a notification. A 422 Unprocessable Entity (item out of stock) reverts only the specific line item, not the entire cart, and shows a targeted message. Network errors (timeout, 503) queue the mutation for retry with exponential backoff.</p>
      ...
      <section>
        ...
        <p>Optimistic UI versus pessimistic UI: optimistic gives instant perceived performance but requires careful error reversal and can show users items they cannot actually purchase (between optimistic add and server confirmation). Pessimistic UI (wait for server confirmation before showing the item) avoids inconsistency but feels sluggish, especially on mobile networks. Most production carts use optimistic UI because add-to-cart failures are rare relative to the volume of successful adds. The key is graceful reversal—the user should understand why an item disappeared, not just see it vanish.</p>
        <p>Version-based conflict detection versus timestamp-based: version counters are more reliable because they are monotonically increasing integers that cannot be affected by clock skew between client and server. Timestamps fail when the client's clock is wrong (common on mobile devices) or when two mutations happen within the same millisecond. Version counters should always be preferred for OCC in distributed systems.</p>
        <p>localStorage versus IndexedDB for persistence: localStorage is synchronous, which means a 50-item cart serialized to JSON (perhaps 10-20 KB) blocks the main thread briefly on every write. For typical cart sizes this is imperceptible, but it matters at scale. IndexedDB is async and non-blocking but significantly more complex. The correct choice: use localStorage for the cart (typical use case is well within its limits) and document the threshold at which a migration to IndexedDB would be warranted (large wishlists, complex cart metadata).</p>
        <p>Client-side price calculation versus server-only: displaying calculated totals client-side requires the client to have correct discount rules, tax calculation logic, and shipping logic—all of which are business-sensitive and change frequently. A cleaner architecture fetches the authoritative total from the server after significant state changes (coupon application, address entry) and displays a "calculating..." state in between. Client-side estimation (for instant feedback) should be clearly marked as an estimate. The checkout page must always show server-authoritative totals.</p>
      </section>

      <section>
        <h2>Summary</h2>
        <p>A production shopping cart is a distributed state management problem. The three-layer architecture—in-memory reactive store, local persistence, server cart—provides instant UI response while maintaining consistency. Optimistic updates with server confirmation handle the 99% happy path; explicit conflict resolution (merge algorithm, version-based OCC) handles the edge cases. Stock validation at two points (add time and checkout entry) prevents overselling without degrading the browse experience. Guest-to-user cart migration preserves items through the login flow. Cart abandonment tracking drives recovery campaigns server-side using reliable status transitions rather than brittle client events. The recurring theme: client-side is for UX; server-side is for truth.</p>
      </section>
    </ArticleLayout>
  );
}
