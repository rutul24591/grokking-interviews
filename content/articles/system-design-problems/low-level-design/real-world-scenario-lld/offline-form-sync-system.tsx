  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>Field workers, healthcare professionals, and users in areas with intermittent connectivity frequently need to fill out and submit forms without a reliable network connection. A construction site inspector fills out a safety checklist offline; a field sales rep fills out a customer visit report in a dead zone; a healthcare worker records patient observations in a hospital basement with poor signal. The form data must be preserved, the user must be able to continue working, and the data must sync to the server reliably when connectivity is restored—without data loss and without duplicates.</p>
        <p>The design problem extends beyond simple offline storage. The form may reference data that itself changes while the user is offline (a product price updates, a patient record is modified by another care provider). When the user submits, the server may need to detect that the form references stale data and present a conflict resolution UI rather than silently overwriting. The system must also handle multiple forms queued for submission, submitting them in dependency order, and surfacing partial failures without losing the submitted ones.</p>
        <p><strong>Explicit assumptions:</strong> The form references external data (records, IDs) that may change while offline. Conflicts are detected via optimistic concurrency control (If-Unmodified-Since or ETag). The offline queue is stored in IndexedDB (not localStorage, because forms may be large and IndexedDB has a higher storage quota). The Service Worker intercepts form submissions and queues them when offline. Network reconnection is detected via the online event and a probe request.</p>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Offline form filling:</strong> All form fields work without a network connection. Validation runs client-side. The form auto-saves to IndexedDB as the user types.</li>
          ...
          <li><strong>Conflict detection and resolution:</strong> If the referenced record changed while offline, the server returns a conflict response. The UI presents the before/after state and lets the user decide how to proceed.</li>
          ...
          <li><strong>Queue management:</strong> User can see the pending submission queue, reorder, remove individual items, or retry failed items manually.</li>
        </ul>
        ...
        <ul className="space-y-2">
          ...
          <li><strong>Sync reliability:</strong> Queued submissions retry with exponential backoff; permanent failures are surfaced to the user without losing data.</li>
          <li><strong>Idempotency:</strong> Each queued submission has a unique idempotency key; retries cannot create duplicate records on the server.</li>
      ...
      <section>
        ...
        <p>The offline form system has three layers: the draft layer (IndexedDB auto-save of in-progress form data), the submission queue (IndexedDB records of complete forms awaiting server submission), and the sync engine (Service Worker or application-level code that processes the queue on network restoration).</p>
        <p>Draft layer: as the user types, the form state is written to IndexedDB under a draft key (draftId keyed by formType + entityId + userId). On re-opening the app, drafts are restored into the form UI with a notification "Unsaved draft from [timestamp] restored." The user can discard the draft (clear it) or continue editing.</p>
        <p>Queue layer: when the user submits a complete form offline, the draft is promoted to a queued submission: a queue record with the full form data, the idempotency key (UUID generated at submission time), the form type, the target entity ID and its last-seen version (for conflict detection), and a status (pending, retrying, failed). The draft is cleared once the queue record is created.</p>
        <p>Sync engine: on detecting connectivity (online event + probe request), the sync engine dequeues records in FIFO order and submits them. Each submission includes the idempotency key and the If-Unmodified-Since header set to the entity's last-seen version. On 200 success, the queue record is deleted. On 412 Conflict, the queue record is updated with conflictDetails and status = conflict_resolution_required. On 5xx or network error, the queue record increments retryCount and a backoff timer is set for the next attempt.</p>
      </section>
      ...
      <section>
        ...
        <p>The sync engine runs as a Web Worker or Service Worker background process. On startup (or on the online event), it queries the queue store for items with status = pending or (status = retrying AND nextRetryAt &lt;= now()). Items are processed in FIFO order (sorted by createdAt). For each item, it assembles the HTTP request (POST or PATCH to the appropriate API endpoint, with the idempotency key as Idempotency-Key header and the entityVersion as If-Unmodified-Since), submits it, and handles the response.</p>
        <p>Retry backoff schedule: first retry immediately on reconnect, second retry 30 seconds later, third 2 minutes, fourth 10 minutes, fifth 30 minutes, cap at 1 hour. After 10 retries over 24+ hours, the item is moved to status = failed and the user is notified: "A form submission from [timestamp] could not be sent after multiple attempts. Please review and resubmit manually." The form data is preserved in IndexedDB so the user can inspect it and resubmit with a fresh submission if needed.</p>
        <p>Idempotency is enforced by the server: if the same idempotency key is submitted twice (due to a retry after a network error where the first submission actually succeeded), the server returns the result of the original submission without creating a duplicate record. The client uses the idempotency key generated at queue-time, not at retry-time. This key must be stored in IndexedDB alongside the queue record to survive app restarts between retries.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Network Detection</h3>
        <p>The browser's online/offline events (window.ononline, window.onoffline) indicate whether the browser has any network connection, but not whether the application server is reachable. A device may be connected to a WiFi network with no internet access (offline in terms of the application, but online in terms of the browser). The sync engine uses a two-stage check: (1) listen for the online event as a trigger; (2) on online event, send a lightweight HEAD request to a known-good application endpoint (GET /health); (3) if the probe returns 200, the server is reachable and sync begins; if the probe fails, schedule a retry in 10 seconds. This ensures syncs only begin when the server is confirmed reachable, not just when the device is connected to any network.</p>
      </section>

      <section>
        <h2>Trade-offs and Considerations</h2>
        <p>IndexedDB versus localStorage for the queue: localStorage is simpler but limited to ~5MB and synchronous reads/writes (blocking the main thread). IndexedDB is async, supports larger storage, and is queryable with indexes—essential for a queue with many records. For offline form systems, IndexedDB is the correct choice. The only trade-off is API complexity (mitigated by libraries like idb or Dexie.js).</p>
        <p>Service Worker versus application-level sync: Service Worker-based sync can run even when the app is not open (the browser can wake the Service Worker on network reconnection using the Background Sync API). Application-level sync only runs when the user has the app open. For field workers who may close the app and return hours later, Service Worker sync is significantly better—the submission queue drains in the background without requiring the user to reopen the app. The trade-off is Service Worker complexity and limited browser support for Background Sync (primarily Chrome).</p>
        <p>Conflict resolution at field level versus full document: field-level conflict resolution (which field has the conflict?) is more user-friendly but requires the server to return a per-field diff on 412. Full document conflict resolution (server version versus user version, user resolves all differences) is simpler to implement. For most form types, field-level resolution is worth the extra implementation effort because it narrows the user's attention to the specific fields that differ.</p>
      </section>

      <section>
        <h2>Summary</h2>
        <p>An offline form sync system requires three coordinated layers: draft auto-save to IndexedDB (per-form persistence with 500ms debounce), a submission queue (IndexedDB records with idempotency keys and entity versions), and a sync engine (triggered by confirmed network reconnection, processing queue items in FIFO order with exponential backoff retry). Conflict detection uses If-Unmodified-Since headers; 412 responses surface a field-level conflict resolution UI. Idempotency keys prevent duplicate submissions on retry. Binary file attachments use Cache Storage (higher quota) with IndexedDB references. The Service Worker Background Sync API enables queue draining when the app is closed. The defining principle: user data is never lost—even failed submissions are preserved in IndexedDB for manual review and resubmission, and the system surfaces failure clearly rather than silently discarding submissions.</p>
      </section>
    </ArticleLayout>
  );
}
