  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <HighlightBlock as="p" tier="crucial">Checkout is the highest-stakes UI flow in e-commerce: it converts browsing intent into revenue, and every friction point is a potential abandonment. Users must provide shipping address, select delivery speed, enter payment details, and confirm an order—all while the underlying cart state is being validated against live inventory and prices. The failure modes are severe: a double charge from a retried network request, an order confirmed for an out-of-stock item, form state lost on back navigation, or a confusing error message after a declined card that causes the user to give up rather than retry.</HighlightBlock>
        <HighlightBlock as="p" tier="important">The checkout flow is not just a multi-step form. It is a transactional workflow where partial completion is unacceptable: either the order is placed correctly in a single atomic operation, or it is not placed at all. This requires careful state management (preserving form data across steps and navigation), idempotency (ensuring retried payment requests cannot double-charge), PCI compliance (card data must never touch the application server), and recovery UX (users must understand what went wrong and how to proceed when errors occur).</HighlightBlock>
        <HighlightBlock as="p" tier="important"><strong>Explicit assumptions:</strong> The platform uses Stripe for payment processing (hosted fields + payment intents pattern). Users may be authenticated or guests. Address validation is advisory, not blocking (users can override suggestions). Inventory is re-validated at order creation time on the server. Checkout may span multiple minutes (user steps away); session state must survive.</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          ...
          <HighlightBlock as="li" tier="important"><strong>Address management:</strong> Autocomplete via Google Places API. Saved addresses for authenticated users. Address normalization and advisory validation (USPS API for US addresses).</HighlightBlock>
          ...
          <HighlightBlock as="li" tier="important"><strong>Payment:</strong> Credit/debit cards (Stripe Elements, no raw card data on app server). Digital wallets (Apple Pay, Google Pay). Saved payment methods for authenticated users. 3DS authentication when required.</HighlightBlock>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="important"><strong>Security:</strong> PCI-DSS compliance; card data handled entirely within Stripe's iframe; no card numbers, CVVs, or raw PANs on application servers.</HighlightBlock>
          <HighlightBlock as="li" tier="crucial"><strong>Idempotency:</strong> Payment submission with a unique idempotency key; retrying after a network failure cannot produce a duplicate charge.</HighlightBlock>
          ...
          <HighlightBlock as="li" tier="important"><strong>Performance:</strong> Each step must render in under 500ms; shipping rate calculation under 2 seconds; payment confirmation under 5 seconds (Stripe's typical processing time).</HighlightBlock>
        </ul>
      </section>

      <section>
        <h2>High-Level Approach</h2>
        <HighlightBlock as="p" tier="crucial">The checkout flow is modeled as a linear state machine with steps: cart validation → shipping address → shipping method → payment → review → order creation → confirmation. Each step transition validates the current step's data and only advances if validation passes. The entire checkout state (all entered values, current step, shipping rates, selected method) is persisted to sessionStorage after each mutation, so browser refresh or navigation returns the user to their exact position.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Payment is the most complex step. The application renders Stripe Elements (an iframe hosted by Stripe) for card input. The user's card data enters Stripe's servers directly—it never touches the application. On submit, Stripe returns a PaymentMethod token or directly confirms the PaymentIntent. The application server receives only the token and the idempotency key, calls Stripe server-to-server to complete the charge, and only creates the order record after receiving Stripe's success confirmation.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Order creation is a two-phase operation: reserve inventory and finalize price server-side (atomic database transaction), then charge the card. If inventory reservation fails (item went out of stock between cart and checkout), the user is returned to the cart with an error before any payment attempt occurs. This prevents the worst user experience: a successful charge for an item that cannot be shipped.</HighlightBlock>
      </section>
...
<ArticleImage
        ...
        <HighlightBlock as="p" tier="crucial">
          Interview signal: the diagram captures the end-to-end flow for <strong>Design a Checkout Flow</strong>. You should be able to explain the happy path and the failure paths (retries, cancellation, backpressure), not just the API surface.
        </HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">Digital wallets (Apple Pay, Google Pay) are implemented via Stripe's Payment Request Button, which presents the native OS wallet interface. These methods do not require the user to enter card details at all—they authenticate with their device biometrics (Face ID, fingerprint) and Stripe receives a device-generated payment token. The integration point is identical from the application's perspective: a PaymentMethod ID is returned on success.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">The confirmation page must render correctly even if the user refreshes it or shares the URL. The page is driven by orderId, which is passed as a URL parameter. The application fetches the order details by ID (authenticated endpoint for account users, public endpoint with order token for guest orders). The confirmation page is deliberately simple and server-rendered to ensure it loads quickly even on slow connections—the user should see their order number immediately, not spin on a loading state after a tense payment experience.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">After a declined card, the Stripe Elements fields are cleared (never pre-populated with the declined card's details) and the user can enter a new card or select a saved method. The idempotency key is refreshed so that retrying with the new card does not hit the existing idempotency record from the declined attempt.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Network errors during payment submission show a recoverable error state: "Something went wrong. Your card has not been charged. Please try again." The "try again" button re-submits with the same idempotency key. If the original attempt actually succeeded server-side, the retry hits the idempotency guard and returns the existing order—the user is taken to the confirmation page. If the original attempt genuinely failed, the retry creates a fresh attempt. In both cases, the user is not double-charged. This makes network error recovery safe to expose to the user as a simple retry button.</HighlightBlock>
      </section>

      <section>
        <h2>Trade-offs and Considerations</h2>
        <HighlightBlock as="p" tier="important">Multi-step versus single-page checkout: multi-step (one section at a time) reduces cognitive load and allows step-specific validation, but increases navigation steps. Single-page checkout (all fields on one page, collapsible sections) requires only one scroll and one submit button but can feel overwhelming and makes partial validation harder. A/B tests at major retailers have produced contradictory results—the optimal design depends heavily on device type (multi-step performs better on mobile, single-page on desktop with large screens) and on the amount of information required.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Address validation strictness: requiring a valid USPS-confirmed address prevents failed deliveries but rejects valid international addresses (USPS only validates US addresses), rural addresses that USPS doesn't recognize, and new addresses not yet in the USPS database. Making validation advisory (show suggestion, allow override) is correct for most cases. Only block on clearly malformed input (missing required fields, invalid zip code format).</HighlightBlock>
        <HighlightBlock as="p" tier="crucial">Storing payment methods: users expect one-click checkout for repeat purchases, which requires storing a Stripe Customer ID and PaymentMethod ID server-side. This creates a security consideration (payment method records are high-value targets) and a compliance one (GDPR/CCPA require disclosure and the right to delete). The architecture should store Stripe's opaque token identifiers (not any card data) and provide a UI for users to view and delete their saved payment methods. Never store CVVs—Stripe's tokens are intentionally not usable without re-verification for high-value transactions.</HighlightBlock>
      </section>

      <section>
        <h2>Summary</h2>
        <HighlightBlock as="p" tier="important"><Highlight tier="crucial">Error recovery must be user-comprehensible (specific declined card messages, safe retry on network</Highlight></HighlightBlock>
<HighlightBlock as="p" tier="important">errors) and technically safe (idempotency prevents duplicate charges on retry). Guest checkout, saved addresses, and digital wallets reduce friction at the highest-abandonment stage of the purchase funnel.</HighlightBlock>
      </section>
    </ArticleLayout>
  );
}
