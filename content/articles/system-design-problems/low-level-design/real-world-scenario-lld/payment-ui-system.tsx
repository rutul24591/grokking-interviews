    ...
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ul className="space-y-2">
          ...
          <li><strong>Payment method selection:</strong> For authenticated users with saved cards, show a list of saved payment methods. Allow adding a new card.</li>
          <li><strong>Digital wallet support:</strong> Apple Pay and Google Pay via Stripe Payment Request Button. No card input needed; device authenticates payment.</li>
          ...
          <li><strong>Double-submit prevention:</strong> Submit button disabled and shows a spinner during payment processing. Idempotency key prevents server-side double charge on retry.</li>
        </ul>
        ...
        <ul className="space-y-2">
          ...
          <li><strong>Idempotency:</strong> Retrying payment after a network failure cannot produce a duplicate charge.</li>
          ...
          <li><strong>Accessibility:</strong> Payment form is keyboard-navigable and screen-reader-compatible. Stripe Elements provides ARIA labels for its hosted fields.</li>
        </ul>
      </section>

      <section>
        <h2>High-Level Approach</h2>
        <p>The payment flow uses Stripe's PaymentIntents API with a client-server handshake. On form mount, the application server creates a PaymentIntent (specifying amount, currency, and metadata) and returns its client_secret to the frontend. The frontend initializes Stripe Elements with this client_secret. When the user submits, the frontend calls stripe.confirmCardPayment(client_secret, paymentMethodOptions), which sends the card token directly to Stripe's servers. Stripe processes the payment and returns a result. If additional authentication is required (3DS), Stripe presents its authentication UI. On success, the application server is notified via webhook to fulfill the order.</p>
        <p>The client_secret is the key security primitive: it authorizes the frontend to confirm the specific PaymentIntent but cannot be used to create new charges or access other data. It can be safely included in the frontend response—it is scoped to the single PaymentIntent and expires when the intent is confirmed or cancelled.</p>
      </section>
      ...
      <section>
        ...
        <p>The payment UI is driven by a state machine with states: idle (form ready for input), validating (submit clicked, Stripe Elements validation running), processing (confirmCardPayment in flight), requires_action (3DS authentication needed), succeeded (payment confirmed), and failed (payment rejected with error). Transitions are deterministic: the user can only retry from the failed state; the processing state disables all inputs and the submit button; the succeeded state transitions immediately to the confirmation page.</p>
        ...
        <p>From the failed state, the user can retry with the same card (for transient errors like processing_error) or switch to a different payment method (for card-specific errors like card_declined). Each retry generates a fresh Stripe Elements instance (to clear the previous card state) and, critically, uses the same idempotency key if retrying after a network error, or a new idempotency key if the user is intentionally retrying with the same card (a new payment attempt, not a retry of the same attempt).</p>
        ...
        <p>Apple Pay and Google Pay use Stripe's PaymentRequestButton component. This component checks whether the user's browser and device support a digital wallet. If supported, it renders a branded button ("Pay with Apple Pay" or the Google Pay button). Clicking the button opens the native OS payment sheet—the device's secure enclave handles authentication (Face ID, Touch ID, fingerprint, or PIN). The user does not enter card details; the device generates a payment token authenticated by the user's biometrics.</p>
      ...
      <section>
        ...
        <p>PaymentElement versus CardElement: Stripe's newer PaymentElement (a single component for all payment methods) is simpler to integrate and automatically shows the right inputs for the user's context (card, SEPA, iDEAL, etc.). CardElement (card-only input) is simpler and predictable but doesn't support non-card payment methods. For applications that only need card payments, CardElement is appropriate; for applications that may expand to other payment methods (SEPA, Klarna, buy-now-pay-later), starting with PaymentElement is more future-proof.</p>
        <p>Client-side validation versus Stripe's built-in validation: Stripe Elements provides real-time validation for card numbers (Luhn algorithm), expiry dates (not in the past), and CVV length (varies by card brand). The application does not need to duplicate this validation logic. The only application-level validation needed is ensuring the card fields are complete (Stripe's complete event fires when all fields are filled without errors) before enabling the submit button.</p>
        <p>Webhook versus synchronous confirmation for order fulfillment: after stripe.confirmCardPayment() succeeds on the client, the application could fulfill the order immediately based on the client-side success. However, the authoritative confirmation should come from Stripe's webhook (payment_intent.succeeded event delivered to the server), because the client-side result can be spoofed or interrupted. The application server should: fulfill the order on webhook receipt, and the client should poll or wait for the server's confirmation of fulfillment (separate from Stripe's payment confirmation) before showing the success page. This prevents showing a "success" page for an order that wasn't actually created.</p>
      </section>

      <section>
        <h2>Summary</h2>
        <p>A PCI-compliant payment UI uses Stripe Elements (hosted iframes) to keep card data entirely out of the application's JavaScript and servers. The integration flow is: server creates PaymentIntent → returns client_secret → frontend initializes Elements with client_secret → user enters card → stripe.confirmCardPayment() sends card token directly to Stripe → 3DS authentication handled by Stripe's handleNextAction() → webhook confirms payment → application fulfills order. The payment state machine (idle → validating → processing → requires_action → succeeded/failed) drives the UI through every state with appropriate loading indicators, clear error messages, and disabled submit to prevent double submission. Idempotency is handled by retrievePaymentIntent on network error rather than creating a new PaymentIntent. Digital wallets (Apple Pay, Google Pay) require domain verification and Stripe account configuration but provide the highest-converting checkout experience for mobile users.</p>
      </section>
    </ArticleLayout>
  );
}
