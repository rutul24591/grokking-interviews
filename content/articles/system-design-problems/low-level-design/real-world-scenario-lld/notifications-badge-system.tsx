    ...
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Badge count:</strong> Show the number of unread notifications. Update in real-time when new notifications arrive or existing ones are read.</li>
          <li><strong>Notification panel:</strong> On badge click, show a list of recent notifications with metadata (actor, action, target, timestamp). Paginated for large counts.</li>
          <li><strong>Mark as read:</strong> Reading a notification (clicking it to navigate) marks it read and decrements the badge. "Mark all read" zeros the badge.</li>
        ...
        <ul className="space-y-2">
          ...
          <li><strong>Latency:</strong> New notification appears in badge within 1 second of creation. Mark-as-read reflects immediately (optimistic update).</li>
          <li><strong>Cross-device propagation:</strong> Read state sync across devices within 3 seconds.</li>
      ...
      <section>
        ...
        <p>The system maintains the unread count as a server-side integer per user (not computed by counting unread records on each request). This counter is the source of truth for the badge display. On session start, the client fetches the current count in the initial data load. A WebSocket subscription receives increments (new notifications) and decrements (read events) as delta messages. The client applies deltas to the local count state rather than refetching the full count on every change.</p>
        <p>When the user opens the notification panel, the first page of notifications is fetched from the server (most recent N, with read/unread status). Scrolling loads older pages. Clicking a notification marks it read optimistically in the client (mark UI as read, decrement local count) and sends a PATCH to the server. "Mark all read" sends a single server request with a timestamp cutoff; the server marks everything read and returns the new count (should be 0).</p>
        <p>Cross-device sync works via the same WebSocket channel. When the user reads a notification on Device A, the server publishes a read event to all of the user's active WebSocket connections. Device B receives this event and decrements its local count. This is the same mechanism as real-time delivery—a single pub/sub channel handles both new notifications and read state changes.</p>
      </section>
      ...
      <section>
        ...
        <p>When the user clicks a notification, the UI should immediately reflect the read state (notification loses its unread styling, badge decrements) without waiting for the server acknowledgment. The optimistic update is: (1) decrement local badge count; (2) set the notification's isRead: true in local state; (3) fire an async PATCH /notifications/:id/read to the server; (4) on server success, do nothing (already updated); (5) on server error, revert—increment badge count and set isRead: false. Reverts are rare but must be handled to maintain badge accuracy.</p>
        ...
        <p>The message format for BroadcastChannel should match the WebSocket delta format for consistency: {"{"}type: "notification_read", notificationIds: ["uuid-1"], badgeDelta: -1{"}"}. The receiving tab applies the same handler as for WebSocket events. This allows the notification state management code to be written once and handle both transport channels.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Notification Panel Pagination</h3>
        <p>The notification panel displays the most recent notifications first. On open, the first page (20 items) is fetched. Scrolling to the bottom of the panel triggers the next page using cursor-based pagination (same as the activity feed). The panel tracks cursor state separately from the badge count state.</p>
      ...
      <section>
        ...
        <p>Counter-based versus query-based badge count: the counter approach (Redis INCR/DECR) provides O(1) badge reads but introduces a consistency gap—the counter can diverge from the true count. The query approach (COUNT(*) on every load) is always accurate but adds database load and latency. For applications where badge accuracy is critical (a wrong count erodes user trust quickly), use the counter with periodic reconciliation. For low-traffic applications, the query approach is simpler and avoids the reconciliation complexity.</p>
        <p>WebSocket versus polling for real-time delivery: WebSocket provides true real-time updates (under 1 second) but requires persistent connections that consume server resources. Polling (every 30 seconds) is simpler and more compatible with serverless or CDN-cached architectures but introduces up to 30-second delays in badge updates. For notification badges, real-time is a UX expectation in most applications; polling delays are noticeable and frustrating. WebSocket is the correct choice for notification systems.</p>
        <p>Notification retention and storage: how long should notifications be retained? Indefinitely (full history) is expensive for high-volume users. A rolling 90-day window is typical. Archiving old notifications (move to cold storage, exclude from default pagination) keeps the active table small while preserving history for users who need it. The badge count should only reflect notifications within the retention window—it makes no sense to show unread counts for notifications too old to be actionable.</p>
      </section>

      <section>
        <h2>Summary</h2>
        <p>A production notification badge system requires: a server-side counter (not a per-request COUNT query) for O(1) badge reads, WebSocket delta messages for real-time increment and decrement, optimistic mark-as-read with revert on server error, BroadcastChannel for same-browser multi-tab sync, and cross-device sync via the same WebSocket channel. The badge count is display state driven by the server counter; the notification list is separate data fetched on panel open with cursor pagination. The defining challenge is maintaining count accuracy across distributed read events: the counter approach with periodic reconciliation balances performance and accuracy. Multi-device sync requires server-side pub/sub (the server notifies all of a user's active connections when read events occur on any one of them).</p>
      </section>
    </ArticleLayout>
  );
}
