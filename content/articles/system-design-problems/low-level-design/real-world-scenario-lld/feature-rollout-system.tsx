  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>A feature rollout system manages the controlled introduction of new features into production, turning what was previously a binary "deploy to all users at once" event into a staged, observable, and reversible process. The distinction from feature flags (which this system builds upon) is the rollout lifecycle management: flags are the mechanism; rollouts are the process—with automated stage progression, monitoring integration, and automatic rollback triggers when anomalies are detected.</p>
        <p>The stakes are high: a feature with a subtle bug that crashes the application for 1% of users is bad; the same bug affecting 100% of users is a service outage. Gradual rollout gives the engineering team the ability to detect the problem at 1% exposure and roll back before the impact reaches 100%. But manual monitoring and manual rollback decisions require engineers to be actively watching during every rollout—impractical for a team shipping dozens of features per week. The rollout system must automate the monitoring and rollback decisions where possible.</p>
        <p><strong>Explicit assumptions:</strong> The rollout system is built on top of the feature flag infrastructure (feature-flag-system article). Percentage allocation uses consistent hashing (same user always gets the same variant at the same percentage). Monitoring is integrated with an error rate metric (number of JavaScript errors per session or per page load). Automatic rollback triggers fire when the monitored metric exceeds a configured threshold relative to the control group. The rollout can be paused, resumed, accelerated, or rolled back manually at any time.</p>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Staged rollout:</strong> Progress through predefined stages (0% → 1% → 5% → 25% → 50% → 100%) with configurable dwell time at each stage.</li>
          <li><strong>Automatic stage advancement:</strong> After the configured dwell time at a stage, advance to the next stage if monitored metrics are within acceptable thresholds.</li>
          ...
          <li><strong>Kill switch:</strong> An emergency disable that sets the feature to 0% and applies immediately, bypassing the normal rollback flow. Separate from rollback—this is for when immediate action is required before metrics even confirm a problem.</li>
        ...
        <ul className="space-y-2">
          ...
          <li><strong>Metric evaluation frequency:</strong> Metrics are evaluated every 60 seconds during active rollout stages. Evaluation must complete before the next evaluation cycle starts.</li>
          <li><strong>Consistency:</strong> Users in the rollout group must consistently see the feature enabled across sessions and page loads (consistent hashing, not random assignment).</li>
      ...
      <section>
        ...
        <p>A feature rollout is a state machine layered on top of the feature flag system. The rollout has stages (each with a target percentage and a dwell time), a current stage, and a monitoring configuration (which metric to watch, what threshold triggers automatic rollback). A background service (rollout manager) runs on the server and is responsible for: advancing stages when dwell time expires and metrics are healthy, triggering automatic rollback when metrics are unhealthy, and publishing state changes to the flag delivery system (so SDK instances receive the updated percentage in real-time).</p>
        <p>The frontend SDK is unaware that it is participating in a rollout—it sees only the feature flag with a given percentage and evaluates it the same way as any other flag. The rollout system is entirely a server-side orchestration concern. The frontend's contribution is emitting the telemetry (error rates, latency metrics) that the rollout manager uses for its metric evaluations.</p>
      </section>
      ...
      <section>
        ...
        <p>Before beginning percentage rollout, the rollout optionally passes through a "targeting" phase: enable the feature for a specific cohort (internal employees, defined by email domain; beta users, defined by a beta flag; a specific customer organization). This provides controlled testing with known, engaged users who can provide feedback before the feature reaches random users. The targeting phase is manual—it advances to percentage rollout only when the team explicitly opts in.</p>
	        ...
	        <p>Monotonic expansion must not be broken by stage rollback. If the rollout rolls back from 25% to 0%, the feature is disabled for all users including those who were in the 1% and 5% stages. On re-enabling, those users re-enter the rollout from the beginning—they are not given special treatment. This inconsistency (the user saw the feature, it disappeared, it reappeared) is a known limitation of rollback scenarios. For features with significant UX impact, the rollout plan should include communication to affected users when a rollback occurs.</p>
		        ...
		        <p>The rollout manager evaluates metrics every 60 seconds during active rollout stages. The evaluation queries the observability system for the feature-specific error rate by comparing two cohorts: users who are currently exposed to the feature at this rollout stage, and users who are not exposed (the control cohort). This A/B comparison isolates the feature's impact from external factors. For example, an infrastructure incident that affects all users equally should move both cohorts in the same direction and therefore should not trigger a feature rollback.</p>
        ...
        <p>The rollout history log is an immutable append-only record of all state changes: each entry has a timestamp, the action (stage_advanced, stage_rolled_back, kill_switch_triggered, manually_paused), the actor (engineer username or "automatic"), the metric values at the time of the action, and an optional comment. This history is the post-incident data source: when investigating "why did the rollout roll back at 25%?", the log shows the exact metric values that triggered the rollback and who (or what) made the decision.</p>
      </section>

      <section>
        <h2>Trade-offs and Considerations</h2>
        <p>Automatic rollback versus requiring human decision: automatic rollback (metric-triggered) is faster (seconds vs the time it takes an on-call engineer to notice and act) but can produce false positives (rolling back a healthy feature because of a temporary metric spike from an unrelated cause). Human decision (alerts but requires manual rollback) prevents false positives but adds response time. The solution is a graduated response: automatic rollback at a 3× threshold (likely a real problem), alert without rollback at a 2× threshold (human evaluates). This reduces false positives while still providing automation for severe cases.</p>
        <p>Rollout speed: aggressive stage schedules (1% for 5 minutes, then 100%) move fast but leave little time to detect slow-developing issues (memory leaks that manifest after 30 minutes of use, rate-limiting errors that only appear at moderate usage levels). Conservative schedules (1 week at each stage) are safe but slow product iteration significantly. The right schedule depends on the feature's risk profile: high-risk features (payment flows, authentication changes) warrant slower schedules; low-risk features (UI tweaks, new optional features) can move faster.</p>
        <p>Metric selection for rollback: error rate is the most common rollback metric because it's universal and easy to measure. Latency metrics (API response time for the feature group), conversion metrics (checkout completion rate), and user engagement metrics (feature usage rate) provide richer signals but require more sophisticated analysis (controlling for confounders, statistical significance testing). Most rollout systems start with error rate and add additional metrics as the monitoring infrastructure matures.</p>
      </section>

      <section>
        <h2>Summary</h2>
        <p>A feature rollout system is a state machine layered on top of the feature flag infrastructure, managing staged percentage expansion (1% → 5% → 25% → 50% → 100%) with configurable dwell times, integrated metric monitoring, and automated rollback. Consistent hashing ensures monotonic rollout expansion (users who see the feature at 1% continue to see it as the percentage grows). Metric evaluation every 60 seconds compares feature-group versus control-group error rates; a 2-3× relative threshold triggers automatic rollback. The kill switch provides immediate emergency disable that propagates to all SDK instances in seconds via SSE. The rollout dashboard provides stage controls (advance, pause, roll back) and an immutable history log. Targeting phases (enable for internal/beta users first) precede percentage rollout. The defining operational principle: every stage transition (manual or automatic) is logged with actor, timestamp, and metric values—providing the audit trail needed for post-incident analysis and building the team's confidence in the rollout system over time.</p>
      </section>
    </ArticleLayout>
  );
}
