    ...
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="important"><strong>Async loading:</strong> SDK scripts load asynchronously without blocking HTML parsing or the application's JavaScript execution.</HighlightBlock>
          ...
          <HighlightBlock as="li" tier="important"><strong>Timeout handling:</strong> If an SDK fails to load within a configurable timeout (5 seconds), the application proceeds without it. No blocking wait for SDK readiness.</HighlightBlock>
          ...
          <HighlightBlock as="li" tier="important"><strong>Graceful degradation:</strong> Features that depend on a failed SDK display appropriate fallbacks rather than broken states or blank UI sections.</HighlightBlock>
        </ul>
        ...
        <ul className="space-y-2">
          ...
          <HighlightBlock as="li" tier="important"><strong>Security:</strong> SDK scripts verified via SRI hashes. No CDN-injected code can execute without matching the expected hash.</HighlightBlock>
          <HighlightBlock as="li" tier="crucial"><strong>Observability:</strong> SDK initialization times, error rates, and API call failures are tracked in the application's error monitoring.</HighlightBlock>
        </ul>
      </section>

      <section>
        <h2>High-Level Approach</h2>
        <HighlightBlock as="p" tier="important">Each SDK integration follows a common pattern: a wrapper module that encapsulates the SDK, providing a stable API to the rest of the application while abstracting the SDK's specific loading and initialization details.</HighlightBlock>
<HighlightBlock as="p" tier="important">The wrapper handles: injecting the SDK script into the DOM asynchronously, implementing the stub/queue pattern for pre-load API calls, catching and logging SDK errors, implementing timeouts, and providing a degraded fallback when the SDK is unavailable.</HighlightBlock>
        <HighlightBlock as="p" tier="crucial">The rest of the application uses only the wrapper's API—never calling the SDK directly. This creates a boundary: if the SDK's API changes in a new version, only the wrapper needs to be updated, not every call site in the application. If the SDK needs to be swapped out (replacing Intercom with Zendesk), only the wrapper changes. This is the Adapter pattern applied to third-party SDKs.</HighlightBlock>
      </section>
...
<ArticleImage
        ...
        <HighlightBlock as="p" tier="crucial">
          Interview signal: the diagram captures the end-to-end flow for <strong>Design Third-Party SDK Integration</strong>. You should be able to explain the happy path and the failure paths (retries, cancellation, backpressure), not just the API surface.
        </HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">Programmatic injection (creating a script element via JavaScript and appending it to the document) is equivalent to async in terms of non-blocking behavior. It's useful when the decision to load an SDK is conditional (only load Intercom if the user is authenticated, only load the analytics SDK if consent is granted). The script element is created with src set to the version-pinned SDK URL and integrity set to the SRI hash, then appended to document.head.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="crucial">For calls that need return values (e.g., fetching the user's A/B test variant), the stub returns a Promise that resolves when the SDK loads and the real call completes. The calling code awaits this Promise. The wrapper's timeout mechanism rejects the Promise after 5 seconds if the SDK has not loaded—the calling code can then use a default value rather than waiting indefinitely.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">SDK initialization time should be measured and tracked as a performance metric. Using the Performance API: mark("sdk_load_start") before injecting the script, mark("sdk_load_end") in the SDK's onLoad callback, then measure("sdk_load", "sdk_load_start", "sdk_load_end") to compute the delta. Send this measurement to the application's performance monitoring (or the analytics SDK itself, once it loads). Tracking initialization time over time allows detecting regressions (an SDK update that slows initialization by 500ms) before users report it.</HighlightBlock>
