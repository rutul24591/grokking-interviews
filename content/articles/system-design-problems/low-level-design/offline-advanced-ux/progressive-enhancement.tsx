  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>A user visits a single-page application (SPA). JavaScript bundle is 500KB. Slow network (3G) takes 8 seconds to download and parse. For those 8 seconds, the page shows a blank screen. The user sees nothing, thinks the page is broken, and leaves. Even if they wait, perceived latency is high.</p>
        <p>Progressive enhancement flips the paradigm: the server renders core content as HTML (no JavaScript required). The page displays immediately (under about 1 second). Core functionality works: users can view content, submit forms, navigate. JavaScript loads asynchronously and enhances the experience: smooth animations, instant form validation, rich interactions. If JavaScript fails to load, the app still works at baseline level.</p>
        <p>Key insight: don't make users wait for JavaScript to render basic content. Deliver HTML immediately, enhance with JavaScript. This improves perceived performance, resilience (app works if JavaScript fails), accessibility (semantic HTML works with screen readers), and SEO (server renders content for crawlers).</p>
        <p>Challenges: building apps that work with and without JavaScript requires discipline. Forms must post to server (fallback) and also POST via fetch (enhanced). Navigation must use links (fallback) and also use client-side routing (enhanced). State management must sync between server and client.</p>
        <p><strong>Explicit assumptions:</strong> Server can render core content. HTML forms are acceptable baseline UX (not ideal, but functional). JavaScript failures won't break core workflows. Progressive loading (prioritize critical JS) is feasible. User tolerance for baseline HTML UX exists.</p>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Server rendering:</strong> Server renders core HTML content without requiring JavaScript on client.</li>
          <li><strong>Form submission:</strong> Forms work via traditional POST (server-side processing). JavaScript enhances with client-side validation and instant feedback (no page reload).</li>
          ...
          <li><strong>Accessibility:</strong> Semantic HTML ensures screen readers and assistive technology work without JavaScript.</li>
        </ul>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Non-Functional Requirements</h3>
        <ul className="space-y-2">
          <li><strong>Time to Interactive (TTI):</strong> Core content accessible within 1-2 seconds (HTML only). Full interactivity within 3-5 seconds (with JavaScript).</li>
          ...
          <li><strong>Accessibility:</strong> Works with screen readers and keyboard navigation without JavaScript.</li>
      ...
      <section>
        ...
        <p>The architecture uses server-side rendering (SSR) to deliver initial HTML with core content. The server handles form submissions and navigation via traditional HTTP requests. JavaScript loads asynchronously and replaces traditional interactions with client-side equivalents (fetch-based form submission, client-side routing). If JavaScript fails, the traditional flow still works.</p>
        <p>Layer 1 (HTML): semantic content and forms. Layer 2 (CSS): styling and layout. Layer 3 (JavaScript): interactivity and polish. Each layer builds on previous; functionality degrades gracefully if later layers fail.</p>
        <p>JavaScript strategy: load critical JS (routing, form enhancement) inline or as small bundle. Defer non-critical JS (analytics, tracking, heavy libraries). Prioritize loading by user importance: JavaScript for visible content loads first; off-screen content deferred.</p>
      </section>
      ...
      <section>
        ...
        <p>JavaScript can enhance accessibility: add aria-live regions for dynamic updates, manage focus, provide keyboard shortcuts. But baseline accessibility comes from HTML. An app built with semantic HTML remains usable for screen reader users even without JavaScript.</p>
        <p><strong>Semantic HTML and ARIA Usage:</strong> Use semantic elements (button elements rather than divs with button roles), native form controls (select elements rather than custom dropdowns), and anchor tags for navigation. These have built-in keyboard navigation and screen reader announcements. Avoid over-using ARIA: ARIA is a bridge for inaccessible patterns, not a replacement for semantic HTML. When JavaScript is unavailable, ARIA attributes are often ignored by assistive tech. Instead, use server-side rendering to show/hide content. Additionally, ensure form labels are properly associated, help text is descriptive, and error messages are clearly linked to fields.</p>
        <p><strong>Testing Accessibility Without JavaScript:</strong> Disable JavaScript in your browser DevTools and test the app. Can you navigate with keyboard only? Can you read the form with a screen reader? Automated accessibility testing tools can catch missing labels and semantic issues. Additionally, test with actual assistive tech: screen readers (NVDA on Windows, VoiceOver on Mac), keyboard-only navigation. Real user testing with disabled users reveals issues automated tools miss. For progressive enhancement, this testing is critical: the fallback UX must be fully accessible.</p>
        ...
        <p><strong>JavaScript Failure Tracking and Fallback Monitoring:</strong> Monitor if JavaScript fails to load: set a global timer, if no JS runs within 10s, increment a counter (`window.__jsLoadFailed = true`). Send to analytics. If JS fails, the fallback should still work (all forms still POST, links still navigate), but without enhancements. Track this: measure task completion rates for users with JS failures. If completion rate is low, fallback UX needs improvement. Additionally, monitor hydration errors: if React detects mismatches during hydration, log the error count. High error counts indicate a problem with server/client sync. Set alerts: if hydration errors exceed 5% of page loads, or if JS load failure exceeds 2%, investigate.</p>
      </section>

      <section>
        <h2>Trade-offs and Considerations</h2>
        <p>Development complexity: building progressive enhancement requires discipline. Every form needs server-side handler and client-side enhancement. Every link is dual-mode (navigate traditional or client-side). This adds complexity. Pure SPA (JavaScript-only) is simpler to code but more fragile.</p>
        <p>Server load: server-rendering increases server CPU. Caching (render once, cache HTML) or streaming (render while sending) mitigates. SSG (static site generation) pre-renders HTML at build time, eliminating server CPU.</p>
        <p>JavaScript-first assumptions: JavaScript libraries (React, Vue) assume JavaScript is available. Progressive enhancement requires rethinking: JavaScript enhances existing HTML, not the primary interface. Many frameworks are moving toward this (Remix, Fresh) but older frameworks (Vue, CRA) require extra work.</p>
        <p>User base: if your users have fast networks and modern browsers, progressive enhancement overhead may not be worth it. If your users include slow networks or old browsers, it's essential. Consider your user demographics.</p>
      </section>

      <section>
        <h2>Implementation Patterns</h2>
        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 1: Dual-Mode Forms</h3>
        <p>Form submits to server endpoint (/api/comment). Server validates, processes, returns response. JavaScript intercepts submit, POST via fetch, reads response, updates page without reload. Form still works without JavaScript (traditional POST).</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 2: Link Hijacking with History API</h3>
        <p>JavaScript intercepts all link clicks, fetches the page, updates content, calls history.pushState to update URL. Browser back/forward still works. Links still work without JavaScript (traditional GET).</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 3: Gradual JavaScript Loading</h3>
        <p>Critical JS (routing, form handlers) loads inline or as small module. Non-critical (animations, tracking) deferred or lazy-loaded. Monitor for JavaScript failures; if detected, fall back gracefully.</p>
      </section>

      <section>
        <h2>Summary</h2>
        <p>Progressive enhancement builds resilient applications by delivering core functionality as server-rendered HTML, enhanced with JavaScript when available. Essential patterns include server-side rendering for initial content, dual-mode forms (POST + fetch), navigation with fallback (traditional links + client-side routing), and asynchronous JavaScript loading. JavaScript failures don't break the app; users fall back to baseline HTML UX (slower, but functional). Trade-offs include development complexity (dual modes for everything) versus improved resilience, and server load (mitigated by caching/SSG) versus perceived performance gains. Real-world systems (GitHub, Basecamp, modern frameworks like Remix) use progressive enhancement. For best results, prioritize server-rendering for above-fold content, load critical JS inline, defer non-critical JS, test thoroughly without JavaScript, and monitor JavaScript failures in production. Progressive enhancement improves FCP, TTI, resilience, and SEO simultaneously.</p>
      </section>
    </ArticleLayout>
  );
}
