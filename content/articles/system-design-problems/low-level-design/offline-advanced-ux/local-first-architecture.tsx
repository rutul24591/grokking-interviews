...
export default function LocalFirstArchitectureArticle() {
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        <h1>Design Local-First Architecture</h1>
        ...
        <p>
          Design Local-First Architecture is a low-level design problem about building a local authoritative client runtime that keeps a user journey coherent when the network, browser capability, storage, or server version cannot be trusted. A principal-ready answer should not stop at saying &quot;cache it&quot; or &quot;retry later&quot;. It should define the public API, local durability model, conflict semantics, privacy boundaries, and the exact user-visible states when the system cannot safely continue.
        </p>
        <p>
          The implementation contract starts with writeLocal(intent), readSnapshot(query), syncPeer(delta), reconcileServerAck(ack). The runtime should make these states explicit: bootstrapping, localReady, syncing, divergent, converged, degraded. The central invariant is: Local reads and writes remain available while the system eventually converges with trusted peers or server state. The hard case to defend in an interview is when a user edits data on a low-connectivity device for hours and then reconnects after the schema has migrated. That case forces the design to explain durability, ordering, rollback, and how much ambiguity the UI is allowed to hide.
        </p>
        <ArticleImage
          src="/diagrams/system-design-problems/low-level-design/offline-advanced-ux/local-first-architecture-runtime.svg"
          alt="Design Local-First Architecture runtime architecture"
      ...
      <section>
        ...
        <p>
          The third concept is convergence with evidence. The system should know which local operations are pending, which server acknowledgements have been received, which conflicts were auto-merged, and which conflicts were shown to the user. The durable structures are embedded store, mutation log, sync cursor, peer vector, schema migration registry, compaction checkpoint. These structures are the difference between a demo and a production design that can survive reloads, retries, and support investigations.
        </p>
      ...
      <section>
        ...
        <ArticleImage
          src="/diagrams/system-design-problems/low-level-design/offline-advanced-ux/local-first-architecture-reconciliation.svg"
          alt="Design Local-First Architecture reconciliation and failure model"
