    ...
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Queue management:</strong> Add operations to queue, persist, process in order on reconnect.</li>
          <li><strong>Dependency tracking:</strong> Specify operation dependencies; don't process dependent operations before dependencies complete.</li>
          <li><strong>Coalescing:</strong> Combine redundant operations (two edits to same field → single operation).</li>
          <li><strong>Retry logic:</strong> Automatically retry failed operations with backoff.</li>
        ...
        <ul className="space-y-2">
          <li><strong>Latency:</strong> Sync starts within 5 seconds of network restoration. Operations process at 1 per 100-500ms depending on server latency.</li>
          ...
          <li><strong>Memory:</strong> Queue in IndexedDB (efficient); in-memory working set under about 10 MB.</li>
      ...
      <section>
        ...
        <p>When a user action occurs offline, instead of immediately updating local state and hoping to sync later, the app enqueues an operation describing the action. The operation includes: action type (create, update, delete), resource ID, payload, and optional dependencies.</p>
        <p>The queue stores operations in IndexedDB, persisting across app restarts. The app processes the queue: when online, dequeue operation, execute on server, mark as synced. If server rejects (conflict, validation error), handle gracefully (move to failed, notify user). If server accepts, remove from queue.</p>
        <p>Dependencies ensure ordering: if "create note" and "add comment to note" are queued, the create must complete first. The queue respects this: blocks dependent operations until their dependencies sync.</p>
        <p>Coalescing optimizes: if the same operation is queued twice in quick succession, or if operations cancel each other (add then delete), the queue combines them into a single server call.</p>
      </section>
      ...
      <section>
        ...
        <p>Each operation in the queue is a record: unique ID, action type, resource ID, payload, dependencies (array of operation IDs that must complete first), status (pending, syncing, synced, failed, conflict), retry count, last retry timestamp, created timestamp.</p>
        ...
        <p>Exponential backoff for retries: first retry immediately, then 1s, 2s, 4s, 8s, capping at 30s. After 5-10 retries with no success, move operation to permanent failed state (user manual intervention required).</p>
        ...
        <p>Memory: queue operations are stored in IndexedDB (disk), not memory. Working set (in-memory) contains only actively processing operations (typically under about 10 MB). After an operation syncs and is marked completed, remove it from IndexedDB after N hours (archive old synced operations to keep the database lean).</p>
      ...
      <section>
        ...
        <p>Strict ordering vs throughput: serial processing respects all dependencies but is slower. Parallel processing (independent operations in parallel) is faster but more complex. For most apps, serial is fine (a few hundred ms delay is acceptable).</p>
        <p>Client-side merging vs server-side: client attempts merge/conflict resolution locally (UX: no server round-trip needed). Server attempts merge (UX: more likely to succeed, but slower). Hybrid: client optimistic merge, server verifies (if divergence detected, server re-syncs).</p>
        <p>Permanent failure handling: after max retries, operation is stuck. User must manually delete (discard changes) or resolve conflict. Alternative: auto-resolve (pick client or server version), but risks data loss.</p>
      </section>

      <section>
        <h2>Implementation Patterns</h2>
        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 1: Simple FIFO Queue</h3>
        <p>Enqueue operations as they occur. Process in FIFO order on sync. No coalescing, no dependencies. Works for simple apps without complex relationships.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 2: Dependency Graph with Coalescing</h3>
        <p>Track operation dependencies. Coalesce redundant operations. Process respecting dependencies. Works for complex offline scenarios with related edits.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 3: CRDT-Based Sync</h3>
        <p>Use CRDT data structure for operations (e.g., Yjs, Automerge). Operations are commutative; no strict ordering required. Merge naturally. Most robust for collaborative editing.</p>
      </section>

      <section>
        <h2>Summary</h2>
        <p>Background sync queues coordinate offline operations for syncing when connectivity restores. Essential patterns include operation persistence (IndexedDB), dependency tracking (ensure create before comment), coalescing (combine redundant operations), retry logic with exponential backoff, and conflict resolution on divergence. UI shows queue status, retry counts, and failed operations. Trade-offs include strict ordering (simpler, slower) versus parallel processing (faster, complex), and client-side merge (fast) versus server-side (more reliable but slower). Real-world systems (Notion, Obsidian, Figma) use queue-based sync with CRDTs for robust handling of offline edits. For best results, implement dependency tracking and coalescing to minimize server load, use exponential backoff for retries (max 30s, up to 10 retries), provide clear UI status, and test thoroughly with simulated offline scenarios. CRDT-based approach is most robust for collaborative documents.</p>
      </section>
    </ArticleLayout>
  );
}
