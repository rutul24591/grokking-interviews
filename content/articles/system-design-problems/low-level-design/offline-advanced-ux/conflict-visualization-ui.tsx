  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>In offline-first or local-first systems, multiple devices edit the same document independently. Device A edits a note's title to "Meeting Notes". Device B edits the same title to "Q4 Review". When both devices sync, the server detects a conflict: which version is correct? Without visualization, the user is unaware of the conflict; they see one version (maybe the server overwrites with device B), losing device A's changes.</p>
        <p>A conflict visualization UI shows the user both versions and lets them decide. "You edited this to X, another device edited it to Y. Which do you want to keep?" The user can see the differences, understand the context, and make an informed choice.</p>
        <p>Key challenges: detecting conflicts (comparing versions), visualizing differences clearly (highlighting what changed), resolving without data loss (preserving both versions until user chooses), and handling cascading conflicts (if the note has 5 edited fields, show all 5 conflicts).</p>
        <p><strong>Explicit assumptions:</strong> Version tracking exists (each document has versions). Conflict detection is accurate (can identify when divergence occurred). UI can show side-by-side diffs. User can make decisions quickly. Conflicts are relatively rare (not overwhelming the user with options on every sync).</p>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Conflict detection:</strong> Identify when local version conflicts with server/other device version.</li>
          <li><strong>Conflict display:</strong> Show both conflicting versions side-by-side with visual diff highlighting.</li>
          <li><strong>User choice:</strong> Allow user to select which version to keep (local, remote, merged, or manual edit).</li>
          <li><strong>Cascading conflicts:</strong> Handle documents with multiple conflicting fields; visualize all conflicts.</li>
          <li><strong>Context preservation:</strong> Show enough context (surrounding text, timestamps, author) for informed decision.</li>
        ...
        <ul className="space-y-2">
          <li><strong>Latency:</strong> Detect conflicts within 1-2 seconds of sync. Display conflict UI within 500ms.</li>
      ...
      <section>
        ...
        <p>When sync detects a version divergence, the system displays a modal or panel showing the conflicting versions. For simple conflicts (single field), a side-by-side diff suffices. For complex conflicts (multiple fields), a list of conflicts with the ability to view/resolve each individually.</p>
        <p>User actions: select which version to keep, or manually edit to create a merged version. Once resolved, the chosen version becomes the new local copy, and the next sync uploads the resolved version to the server.</p>
        <p>Conflict prevention: use CRDTs or operation-based merging to automatically resolve some conflicts without user intervention. Display UI only for truly unresolvable conflicts.</p>
      </section>
      ...
      <section>
        ...
        <p>Progressive disclosure: show conflicts one at a time (wizard style) or all at once (list). Wizard is simpler but slower; list is faster but overwhelming. Choose based on expected conflict frequency.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Persistence and Undo</h3>
        <p>After user resolves a conflict and syncs, store the resolution decision (which version was chosen, when, by whom). If the user later realizes they chose wrong, provide an "undo" button that re-opens the conflict for re-resolution.</p>
        ...
        <p>Diff computation is O(n) where n is document size. For large documents (100KB+), compute diff in a web worker to avoid blocking the UI. Cache diffs to avoid recomputing if user dismisses and reopens.</p>
      </section>

      <section>
        <h2>Trade-offs and Considerations</h2>
        <p>Auto-merge vs user decision: CRDTs auto-merge more cases but are complex. Simple last-write-wins is fast but loses data. Hybrid: auto-merge safe cases (different fields), ask user for truly conflicting edits (same field).</p>
        <p>UI complexity: simple side-by-side diff is clear but works only for small data. Complex UI (tabs, lists) needed for documents with many conflicts. Choose based on expected conflict patterns.</p>
        <p>Conflict frequency: if conflicts are rare (most devices edit different parts), auto-merge handles 99% of cases; conflict UI needed only 1%. If conflicts are frequent, user gets fatigued from constant decisions; better to improve merge logic.</p>
      </section>

      <section>
        <h2>Implementation Patterns</h2>
        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 1: Side-by-Side Diff for Single Conflicts</h3>
        <p>Simple modal showing local vs remote with highlighted differences. User picks one or manually edits.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 2: Multi-Field Conflict List</h3>
        <p>Tabbed interface or list showing all conflicted fields. User resolves each field independently, then syncs.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 3: CRDT-Based Auto-Merge with Fallback</h3>
        <p>Automatically merge CRDT-compatible edits. Show conflict UI only for unresolvable conflicts.</p>
      </section>

      <section>
        <h2>Summary</h2>
        <p>Conflict visualization UI enables offline-first and multi-device apps to handle data divergence gracefully by showing users both versions and letting them decide. Essential patterns include version-based conflict detection, side-by-side diff display for clarity, resolution options (keep local/remote/merge), and cascading conflict handling for documents with multiple conflicts. Three-way diff (comparing local, remote, common ancestor) enables smart auto-merging for non-conflicting changes. CRDT-based automatic merge reduces conflicts needing user intervention. Trade-offs include auto-merge complexity (handles more cases) versus simplicity (user decides everything), and UI complexity (handles many conflicts) versus clarity (simple diffs). Real-world systems (Notion, Obsidian, Google Docs) use conflict visualization for multi-device sync. For best results, implement three-way diff to auto-resolve non-conflicting changes, use CRDT for text/list operations, reserve conflict UI for truly unresolvable conflicts, and provide clear diffs with context (timestamps, author). This minimizes user fatigue while ensuring no data loss.</p>
      </section>
    </ArticleLayout>
  );
}
