  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>Push notifications engage users when the app is not running: "You have a new message", "Your order shipped". Without push, users only see notifications when they open the app. With push, they're notified proactively, improving engagement and retention.</p>
        <p>However, excessive push notifications drive users away: spamming notifications causes opt-out, uninstall, and poor app ratings. A user who receives 10 notifications daily will mute notifications. A user who receives 1 relevant notification weekly will engage. The art is balance: notify when there's genuinely important information, but not excessively.</p>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Permission request:</strong> Request notification permission from the user with clear explanation of notification types.</li>
          <li><strong>User preferences:</strong> Allow users to customize frequency (instant, daily digest, weekly), categories (messages only, orders, promotions), and quiet hours (9pm-8am no notifications).</li>
          <li><strong>Push reception:</strong> Receive push events from server even when app is closed (via Service Worker push event).</li>
        ...
        <ul className="space-y-2">
          <li><strong>Latency:</strong> Notification delivery within 1-5 seconds of server sending (depends on push service).</li>
          <li><strong>Reliability:</strong> 99%+ delivery rate for critical notifications (order shipment, security alerts).</li>
      ...
      <section>
        ...
        <p>The system comprises three parts: client (web app), server (notification service), and push infrastructure (Firebase Cloud Messaging or Web Push standard).</p>
        <p>Client: registers a Service Worker push event listener. When the user grants notification permission, the client requests a push subscription from the browser (via serviceWorkerRegistration.pushManager.subscribe). This returns an endpoint (unique per device/user) that the client sends to the server. The Service Worker listens for push events; when a push arrives, it displays a notification to the user.</p>
        ...
        <p>Engagement tracking: when the user interacts with a notification (clicks it, dismisses it), the Service Worker notifies the server (via background sync or beacon API), logging the interaction. This drives analytics and machine learning on notification timing/content optimization.</p>
      </section>
      ...
      <section>
        ...
        <p><strong>Opt-Out Monitoring and Churn Analysis:</strong> Track opt-out rate as a key metric. An increasing opt-out rate signals a problem: either too many notifications, irrelevant content, or poor timing. Investigate: correlate opt-out with notification history. Did the user receive 10 notifications in one day before opting out? They were likely overwhelmed by capping failure. Did they receive irrelevant notifications (promotions when they prefer messages)? Preference-based targeting failed. Use churn funnels: identify cohorts of heavy opt-outers and analyze their notification history. Additionally, offer in-app preference adjustment before opt-out: if user is about to disable notifications, show a modal: "You're about to disable notifications. Would you prefer to adjust frequency instead?" Many users prefer granular control over total opt-out.</p>
      ...
      <section>
        ...
        <p>Instant vs digest: instant notifications are timely but can overwhelm. Digest (hourly, daily) is less intrusive but less timely. Balance based on notification type: urgent (security alerts, high-priority messages) instant; informational (promotions, likes) digest.</p>
        <p>Server-side vs client-side logic: capping and coalescing can be done on server (consistent, easy to adjust) or client (less load on server, faster local logic). Server-side is preferred for consistency; all users get same behavior regardless of client implementation.</p>
        <p>Engagement optimization: aggressive personalization (send only high-predicted-engagement notifications) maximizes short-term engagement but may reduce long-term habit formation (users don't develop expectation to check app). Conservative approach (send on schedule, regardless of prediction) trains habit but may frustrate users with irrelevant notifications.</p>
      </section>

      <section>
        <h2>Implementation Patterns</h2>
        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 1: Permission Request with Value Proposition</h3>
        <p>Request notification permission only after user experiences value. Show permission prompt after user's first action (send message, create post), with clear explanation of notification frequency and types. Improves grant rate from 30% (on load) to 60%+ (after value demonstrated).</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 2: Frequency Capping with Priority Levels</h3>
        <p>Implement tiered notifications: urgent (security, high-priority messages) bypass caps, high (likes, followers) subject to daily cap, low (promotions, digests) only in digest or disabled by default. Ensures critical notifications reach users while maintaining low spam.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 3: Engagement-Driven Re-sending</h3>
        <p>If user didn't interact with notification in 24 hours, automatically re-send it once with slightly different messaging. Increases engagement without doubling spam (re-send only for ignored, not for clicked).</p>
      </section>

      <section>
        <h2>Summary</h2>
        <p>Push notifications drive engagement through timely, relevant notifications but risk user annoyance if poorly designed. Essential patterns include permission requests timed after value demonstration, user preference management (frequency, categories, quiet hours), server-side frequency capping and coalescing to prevent spam, Service Worker handling of push events with rich notifications and actions, and engagement analytics to measure effectiveness and guide optimization. Trade-offs include instant (timely, risky spam) versus digest (less intrusive, less timely), and server-side logic (consistent) versus client-side (performant). Real-world systems aim to keep opt-out rates low through careful targeting, relevant content, and conservative frequency. For best results, request permission after value demonstration, cap at a small number of high-quality notifications daily, analyze engagement metrics, and continuously test and optimize timing, frequency, and content. Monitor opt-out trends; sustained increases indicate over-notification.</p>
      </section>
    </ArticleLayout>
  );
}
