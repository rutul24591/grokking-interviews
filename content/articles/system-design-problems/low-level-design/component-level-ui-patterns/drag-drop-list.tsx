...
export default function DragDropListArticle() {
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        <h1>Design a Drag-and-Drop List</h1>
        <h2>Definition &amp; Context</h2>
        <p>Design a Drag-and-Drop List is a low-level design problem about implementing gesture activation, pointer capture, keyboard lifting, collision calculation, auto-scroll, optimistic ordering, persistence, and rollback. A principal-level interview answer must define ownership boundaries, browser and accessibility semantics, local data structures, lifecycle cleanup, server reconciliation, and explicit degraded behavior.</p>
        <p>Keep the committed item order separate from the transient drag projection. Persist only a validated drop decision and roll back from the mutation journal on conflict. The central structures are ordered ids, drag session, source index, projected index, item rectangles, scroll container, auto-scroll velocity, mutation id, version, and live-region announcement. The implementation is not complete until cancellation, stale work, SSR behavior, privacy, metrics, and rollback are deliberate rather than incidental.</p>
        <ArticleImage src="/diagrams/system-design-problems/low-level-design/component-level-ui-patterns/drag-drop-list-runtime.svg" alt="Design a Drag-and-Drop List runtime flow" caption="Runtime flow: input becomes a guarded state transition, a semantic projection, and a recoverable outcome." />
      </section>
      <section>
        <h2>Core Concepts</h2>
      ...
      <section>
        ...
        <ArticleImage
          ...
          alt="Drag and drop list architecture showing drag state, hit testing, FLIP reorder, and auto-scroll"
      ...
      <section>
        ...
        <p>For this topic, the critical state rule is: Keep the committed item order separate from the transient drag projection. Persist only a validated drop decision and roll back from the mutation journal on conflict. During interaction, record enough context to cancel safely. On commit, validate the latest intent, update the durable projection, and release temporary listeners, timers, observers, pointer capture, and abort controllers. On unmount, cleanup must be idempotent.</p>
        <ArticleImage src="/diagrams/system-design-problems/low-level-design/component-level-ui-patterns/drag-drop-list-edge-cases.svg" alt="Design a Drag-and-Drop List edge-case defense map" caption="Edge-case map: validate intent, contain scale pressure, recover from failure, reconcile committed state, and emit evidence." />
      </section>
      <section>
        <h2>Trade offs &amp; Comparison</h2>
        <p>native drag events are simple for desktop files; pointer and keyboard controllers are more work but provide touch support, virtualization control, and accessible reordering. The custom design should still lean on native semantics and browser primitives where they remain correct. Replacing them creates testing obligations for keyboard behavior, focus ownership, reduced motion, touch interaction, zoom, SSR hydration, and assistive technology.</p>
        <p>A local projection gives immediate feedback. Persist with a version or fractional position key; reject and reconcile when a concurrent reorder invalidates the base version. At scale, the failure pressure is large virtualized lists, nested scroll containers, touch delays, rapid reorders, concurrent editors, and expensive rectangle measurement. Defend the latency budget by batching measurement, aborting stale async work, bounding caches and prefetch, and emitting analytics only for committed outcomes.</p>
      ...
      <h3>Principal defense: consistency, abuse, and lifecycle rollback</h3><p>For a reusable component, consistency means one committed semantic snapshot drives DOM attributes, focus behavior, and callbacks. Pointer movement, hover previews, timers, measurements, and async settlements are transient projections. Guard every delayed effect with ownership identity so stale work cannot reopen, overwrite, or announce a component after blur, disposal, navigation, or replacement. Rollback restores the last committed semantic state and performs idempotent cleanup.</p><p>Bound work even for small widgets: cap queued notices, cached failures, measured items, portal layers, suggestion rows, and animation updates. Validate externally supplied labels, URLs, markup, dimensions, and item ids before rendering or measuring. Avoid leaking private labels or raw payloads through telemetry. Track rejected transitions, timer drift, focus-return failures, layout shifts, cleanup counts, and degraded fallbacks.</p><section>
        ...
        <p>For this component specifically, the failure policy is to cancel on escape or pointer loss, stop auto-scroll on unmount, restore committed order after rejection, and announce the rollback to keyboard users. Security and privacy require the implementation to authorize reorder operations, validate membership and destination server-side, limit mutation rate, and avoid trusting client-supplied order arrays blindly.</p>
      </section>
      <section>
        <h2>Real-world use cases</h2>
        <p>Representative deployments include a prioritized work queue, a playlist editor, and a kanban column with concurrent ordering updates. In each case, the same component shell may be reused, but the policy layer changes: latency budget, permissions, persistence, fallback, and telemetry should be injected explicitly instead of hidden in presentation code.</p>
      </section>
      <section>
        <h2>Common interview question with detailed answer</h2>
        <h3>How would you model component state?</h3><p>I would separate committed state, transient interaction state, derived presentation, and async request generations. For this component, Keep the committed item order separate from the transient drag projection. Persist only a validated drop decision and roll back from the mutation journal on conflict. That model makes cancellation and rollback explicit.</p>
        <h3>What breaks at scale?</h3><p>The dominant pressures are large virtualized lists, nested scroll containers, touch delays, rapid reorders, concurrent editors, and expensive rectangle measurement. I would bound work per interaction, virtualize or cache only where measured, and cancel work that is no longer relevant.</p>
        <h3>What consistency model applies?</h3><p>A local projection gives immediate feedback. Persist with a version or fractional position key; reject and reconcile when a concurrent reorder invalidates the base version. The interview answer must state which layer is authoritative and how stale completion is rejected.</p>
        <h3>How do you handle failure and rollback?</h3><p>I would cancel on escape or pointer loss, stop auto-scroll on unmount, restore committed order after rejection, and announce the rollback to keyboard users. I would also emit a reason code so product metrics distinguish expected cancellation from defects and provider failures.</p>
        <h3>How do you defend the architecture over alternatives?</h3><p>native drag events are simple for desktop files; pointer and keyboard controllers are more work but provide touch support, virtualization control, and accessible reordering. I would choose the smallest design that satisfies the required behavior and explicitly accept the testing and operability cost of custom interaction.</p>
      </section>
      <section>
        <h2>References</h2>
        <ul>
