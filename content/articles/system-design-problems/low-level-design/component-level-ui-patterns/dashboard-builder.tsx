    ...
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="important">Dashboard is a grid of configurable columns (12-column grid).</HighlightBlock>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="important"><strong>Widget Catalog:</strong> Users can browse available widget types (line chart, bar chart, table, metric card, etc.) and add them to the dashboard.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Drag Rearrange:</strong> Drag widgets to reposition within the grid. Grid auto-snaps to nearest valid position.</HighlightBlock>
          ...
          <HighlightBlock as="li" tier="important"><strong>Persistence:</strong> Layout (widget positions, sizes, configurations) saved to server, restored on load.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Lazy Loading:</strong> Widget data fetched only when widget enters viewport (IntersectionObserver).</HighlightBlock>
        ...
        <ul className="space-y-2">
          ...
          <HighlightBlock as="li" tier="crucial"><strong>Accessibility:</strong> Keyboard navigation between widgets, ARIA grid roles, screen reader announces widget content.</HighlightBlock>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="important"><strong>react-grid-layout:</strong> Popular library but adds bundle weight and limits customization of the snap/collision logic.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>CSS Grid only:</strong> CSS Grid handles layout but not drag/resize persistence. Requires JS layer for interactivity.</HighlightBlock>
        </ul>
        <HighlightBlock as="p" tier="crucial">
      ...
      <section>
        ...
        <div className="my-6 rounded-lg border border-theme bg-panel-soft p-6">
          ...
          <HighlightBlock as="p" tier="crucial"><code>Widget</code> (id, type, x, y, w, h, config), <code>Layout</code> (widgets array, grid columns), <code>WidgetCatalog</code> (available widget types with metadata).</HighlightBlock>
        </div>
        <div className="my-6 rounded-lg border border-theme bg-panel-soft p-6">
          <h4 className="mb-3 font-semibold">2. Dashboard Store</h4>
          <HighlightBlock as="p" tier="important">Zustand store: widgets array, drag state, resize state. Actions: addWidget, removeWidget, moveWidget, resizeWidget, saveLayout, loadLayout.</HighlightBlock>
        </div>
        <div className="my-6 rounded-lg border border-theme bg-panel-soft p-6">
          <h4 className="mb-3 font-semibold">3. Grid Layout Engine</h4>
          <HighlightBlock as="p" tier="important">Computes grid positions, resolves collisions (push-down strategy), snaps drag targets to grid units.</HighlightBlock>
        </div>
        <div className="my-6 rounded-lg border border-theme bg-panel-soft p-6">
          <h4 className="mb-3 font-semibold">4. Pointer Drag Handler</h4>
          <HighlightBlock as="p" tier="important">Pointer events for drag (move) and resize (edge/corner handles). Grid snapping during drag.</HighlightBlock>
        </div>
        <div className="my-6 rounded-lg border border-theme bg-panel-soft p-6">
          <h4 className="mb-3 font-semibold">5. Lazy Loader Hook</h4>
          <HighlightBlock as="p" tier="important">IntersectionObserver per widget. Triggers data fetch when widget enters viewport. Shows skeleton when not visible.</HighlightBlock>
        </div>
      ...
      <section>
        ...
        <ul className="space-y-3">
          <HighlightBlock as="li" tier="important"><strong>Collision resolution:</strong> When a widget is dragged onto another, the engine pushes overlapping widgets down (or right) using a push-down algorithm. If pushing would push a widget off the grid, the drag position is rejected.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Responsive layout:</strong> On resize, the grid column count changes. Widget widths are recalculated proportionally (widget with w=6 on 12-col grid becomes w=4 on 8-col grid).</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Deprecated widgets:</strong> When a widget type is no longer in the catalog, the renderer shows a placeholder with the widget&apos;s last known data and a &quot;This widget is no longer available&quot; message.</HighlightBlock>
        </ul>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="crucial"><strong>Collision check on every drag frame:</strong> O(n) per pointermove event. Mitigation: throttle collision check to 16ms (60fps), use spatial index (grid cell occupancy map) for O(1) lookup.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Widget data fetches:</strong> 50 widgets × 50 concurrent fetches. Mitigation: IntersectionObserver with rootMargin preload (fetch when 200px from viewport), fetch priority hints.</HighlightBlock>
        </ul>

        <h3 className="mt-6 mb-3 text-lg font-semibold">Optimization Strategies</h3>
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="important"><strong>Spatial index for collision:</strong> Maintain a 2D occupancy grid (columns × rows). Check cell occupancy in O(1) instead of scanning all widgets.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Widget memoization:</strong> Each widget component is <code>React.memo</code> with comparator on its config. Re-renders only when its own config changes.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Batched layout save:</strong> Debounce layout saves at 500ms during drag/resize to avoid excessive API calls.</HighlightBlock>
        </ul>
      </section>
      ...
      <section>
        ...
        <div className="my-6 rounded-lg border border-accent/30 bg-accent/10 p-6">
          ...
          <ul className="space-y-2">
            <HighlightBlock as="li" tier="important">Tab moves focus between widgets. Arrow keys move focused widget within grid (with collision resolution).</HighlightBlock>
        ...
        <div className="my-6 rounded-lg border border-accent/30 bg-accent/10 p-6">
          ...
          <ul className="space-y-2">
            <HighlightBlock as="li" tier="important">Dashboard has <code>role=&quot;application&quot;</code> with <code>aria-label=&quot;Dashboard builder&quot;</code>.</HighlightBlock>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="crucial"><strong>Grid engine:</strong> Test collision detection, push-down resolution, boundary enforcement.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Store:</strong> Test add/remove/move/resize widgets, layout serialization/deserialization.</HighlightBlock>
          <li><strong>Lazy loader:</strong> Test IntersectionObserver triggers data fetch when widget enters viewport.</li>
        </ul>
        <h3 className="mt-6 mb-3 text-lg font-semibold">Integration Tests</h3>
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="important"><strong>Drag and resize:</strong> Simulate pointer events, verify grid positions update, verify collision resolution.</HighlightBlock>
