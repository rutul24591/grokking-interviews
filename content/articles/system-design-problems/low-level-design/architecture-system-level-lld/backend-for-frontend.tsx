  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>
          Design a Backend for Frontend is an architecture-level low-level design problem about implementing a experience-specific aggregation boundary. The design must be concrete enough that platform and product teams can integrate with it safely: public APIs, state model, artifact formats, ownership rules, compatibility contracts, failure isolation, rollout, and observability all belong in the answer.
        </p>
        <p>
          The facade is requestView, authorize, fanOut, transform, cacheProjection, degradePartial, trace. Runtime or lifecycle states are idle, authorizing, fetching, partial, ready, degraded, failed. The governing invariant is: The frontend must receive a stable experience contract without inheriting downstream service complexity. The pressure-test case is when one dependency times out while the page can still render a safe partial experience. A principal-ready answer should explain how local implementation decisions become organization-wide reliability, velocity, and migration outcomes.
        </p>
        <ArticleImage src="/diagrams/system-design-problems/low-level-design/architecture-system-level-lld/backend-for-frontend-runtime.svg" alt="Design a Backend for Frontend runtime architecture" caption="Architecture runtime: consumer intent flows through contract validation, versioned artifacts, rollout controls, and observable delivery." />
      </section>

      <section>
        <h2>Core Concepts</h2>
        <p>
          The first concept is an explicit platform contract. A platform is not just shared code; it is a promise about interfaces, compatibility, support windows, ownership, and operational response. The core structures are view contract, request context, downstream budget, cache key, partial response model, fallback policy, trace span. These structures make changes reviewable and let consuming teams understand what is stable, what is experimental, and what requires migration.
        </p>
        <p>
      ...
      <section>
        ...
        <p>Shared invariants need a shared enforcement point. The platform makes The frontend must receive a stable experience contract without inheriting downstream service complexity. enforceable while retaining extension points for product-specific policy.</p>
        ...
        <p>Use immutable artifacts, validation, scoped caches, permission checks, redacted metrics, bounded retention, canary release, rollback pointers, owner metadata, and SLO dashboards. Then defend the edge case: one dependency times out while the page can still render a safe partial experience.</p>
