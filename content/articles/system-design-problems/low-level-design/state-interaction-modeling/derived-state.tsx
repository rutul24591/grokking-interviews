  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>
          Design Derived State is a low-level design problem about building a reusable dependency graph selector engine that product teams can depend on under real user behavior, not just the happy path. In a staff or principal interview, the answer should move past naming a pattern and describe the runtime contract: public API, internal state shape, transition rules, ownership boundaries, observability, and what the component refuses to do when correctness is uncertain.
        </p>
        <p>
          The design target is an implementation that can live inside a complex web application with concurrent user actions, remounts, retries, background work, and multiple teams integrating it. The core API is derive(key, deps, compute), invalidate(sourceKey), read(key), subscribe(key, listener). The core state model is clean, dirty, recomputing, stale, failed. The most important interview signal is explaining why those states exist, which transitions are legal, and how the design behaves when A derived selector reads stale base state while another update is batched and has not notified subscribers.
        </p>
        <ArticleImage
          src="/diagrams/system-design-problems/low-level-design/state-interaction-modeling/derived-state-state-runtime.svg"
          alt="Design Derived State runtime state model"
      ...
      <section>
        ...
        <ArticleImage
          src="/diagrams/system-design-problems/low-level-design/state-interaction-modeling/derived-state-failure-debugging.svg"
          alt="Design Derived State failure and debugging model"
      ...
      <section>
        ...
        <p>
          A simple component-local implementation is cheaper for one screen, but it pushes correctness into every caller. That approach usually fails when several components share the same resource, when work outlives a component, or when a late event arrives after the user has changed intent. A centralized runtime adds indirection, but it gives the organization one place to enforce transactional snapshot consistency inside one state batch and lazy eventual recomputation outside it.
        </p>
        <p>
      ...
      <section>
        ...
        <p>
          The third pitfall is ignoring dependency cycle, expensive recomputation storm, equality false negative, selector exception until after the implementation is shipped. These failures must be represented in state and telemetry from the beginning. If the runtime cannot explain what happened after a bad transition, it is not ready for production or a principal-level interview answer.
        </p>
        <p>
      ...
      <section>
        ...
        <p>
          I would state that the design optimizes for correctness, debuggability, and reuse across high-value flows. If the interviewer pushes on complexity, I would narrow the runtime to the invariants that must be shared and keep feature policy outside the core. If they push on latency, I would explain batching, selector subscriptions, and lazy recomputation. If they push on edge cases, I would walk through A derived selector reads stale base state while another update is batched and has not notified subscribers.
        </p>
      </section>
