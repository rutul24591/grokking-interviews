  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li>Tokens arrive faster than can render (backpressure).</li>
      ...
      <section>
        ...
        <p>
          Implement ring buffer (circular queue) for tokens. On each token arrival, append to buffer. Every 50-100ms (or N tokens), flush buffer: render buffered tokens as batch. On backpressure (render queue backed up), pause reading from network stream. Cancel stream: abort fetch, clear buffer, stop rendering. Monitor memory: if buffer grows too large, flush early.
        </p>
      </section>
      ...
      <section>
        <h2>Summary</h2>
        <p>
          Token streaming buffer systems critical for smooth AI response rendering. For staff/principal engineers, essential aspects include ring buffer for fixed-memory design, adaptive batching (time + token triggers), backpressure handling via stream.pause/resume, and frame-budget-aware rendering (requestAnimationFrame). Sentence detection enables pause points for audio synthesis. Monitoring FPS and batch latency detects jank. At scale, adaptive batching based on token arrival rate, priority rendering for multi-stream scenarios, and overflow to IndexedDB for very large responses. Testing must cover high-throughput scenarios (10K tokens/sec), backpressure conditions, cancellation, and jank measurements. Real-world systems use bounded ring buffers, adaptive batching, and careful frame timing to stream thousands of tokens smoothly without UI jank.
        </p>
      </section>
    </ArticleLayout>
  );
