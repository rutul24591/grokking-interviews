    ...
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ul className="space-y-2">
          ...
          <li>Template variables in a versioned prompt that are not provided at runtime — the system should surface missing variables before production deployment.</li>
      ...
      <section>
        ...
        <p>
          For production prompt management: a PromptVersion schema stores each version as an immutable record with full text content, author, timestamp, parent version ID, and evaluation metrics. A version is "current" by having the highest version number in its prompt lineage. Rollback creates a new version identical to the target version (not mutating the historical record). A/B testing is managed by a traffic router that assigns each request a consistent hash to a version bucket.
        </p>
        <p>
      ...
      <section>
        <h2>Summary</h2>
        <p>
          Prompt history and versioning provides the version control infrastructure that makes prompt engineering a professional engineering practice rather than an informal trial-and-error process. The design centers on immutable version records with parent chain for audit trail, word-level diff view for understanding changes, conversation tree branching for exploration without loss, template variable extraction for reuse, and A/B testing with statistical significance gates for safe production promotion. For staff-level engineers, the key architectural insights are: enforce immutability at the data layer (not just the UI); build evaluation pipeline integration from day one (version without evaluation data is just documentation, not engineering); use consistent hashing for A/B traffic routing to preserve within-session consistency; and integrate with prompt management platforms (PromptLayer, LangSmith) before building custom infrastructure unless there are specific requirements that existing tools don't address.
        </p>
      </section>
    </ArticleLayout>
  );
