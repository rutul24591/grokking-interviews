  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <p>
          The generation pipeline has five stages: (1) User describes the UI in natural language. (2) The LLM generates a structured JSON component tree using its knowledge of the component library (provided in the system prompt). (3) The JSON is validated against a schema — unknown components are flagged, invalid prop values are corrected or rejected. (4) The validated JSON is rendered in a sandboxed iframe using a whitelist renderer that maps component types to actual React components. (5) The user refines via conversation, and the LLM updates the JSON incrementally.
        </p>
        <p>
      ...
      <section>
        <h2>Summary</h2>
        <p>
          AI UI generators accelerate prototyping by translating natural language descriptions into validated, renderable component specifications. The architecture centers on JSON component tree output (not raw code), a prompt-engineered system prompt embedding the component library documentation, a multi-layer validation and sanitization pipeline, and sandboxed iframe rendering with strict CSP. Security is the non-negotiable constraint — treat LLM output as untrusted user input, enforce component whitelists in the renderer (not just the validator), and isolate rendering in a sandboxed context. The conversation-based refinement loop produces higher-quality output than single-shot generation, and version history enables safe experimentation. For staff-level engineers, the key insights are: the output format (JSON spec vs code) determines the entire security posture of the system; component library documentation quality directly determines generation quality; retrieval-augmented prompt construction scales to large design systems; and accessibility validation must be automated (not trusted to the LLM alone).
        </p>
      </section>
    </ArticleLayout>
  );
