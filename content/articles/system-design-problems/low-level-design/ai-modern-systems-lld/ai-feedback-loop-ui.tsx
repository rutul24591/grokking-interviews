  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li>Feedback submitted on a streaming-in-progress response — must be attributed to the partial response and flagged as "early" feedback, not confused with feedback on the completed response.</li>
      ...
      <section>
        ...
        <p>
          The feedback UI lives as a small control cluster on each assistant response: thumbs up (👍), thumbs down (👎), and a copy button. Clicking thumbs down expands an inline form with category chips (single-tap reason selection) and an optional text area for a correction. All feedback is submitted asynchronously via a fire-and-forget API call. The UI updates optimistically on click (button changes to filled/selected state) without waiting for the server.
        </p>
        <p>
      ...
      <section>
        <h2>Summary</h2>
        <p>
          AI feedback loop UI closes the quality improvement cycle by systematically collecting, analyzing, and acting on user signals about AI output quality. The design centers on progressive feedback disclosure (micro-feedback for volume, category chips for specificity, correction forms for high-value RLHF data), optimistic UI with offline-resilient fire-and-forget submission, implicit behavioral signal collection to cover the silent majority, and a multi-stage pipeline from raw feedback to curated training data. Analytics dashboards enable quality monitoring and experiment evaluation. Spam detection prevents feedback poisoning. For staff-level engineers, the critical insights are: design feedback collection primarily around the category chips tier (the best signal/friction tradeoff); build the training pipeline with human review as a mandatory gate on corrections (automated ingestion of corrections is dangerous); implement per-model-version and per-prompt-version attribution from day one (unattributed feedback is nearly useless for systematic improvement); and respect user privacy by storing the minimum necessary data with genuine opt-out.
        </p>
      </section>
    </ArticleLayout>
  );
