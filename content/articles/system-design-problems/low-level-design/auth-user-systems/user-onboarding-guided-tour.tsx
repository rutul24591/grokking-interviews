  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>A new user signs up for a project management app. They see a blank dashboard with confusing buttons. They don't know how to create a project, invite teammates, or set up workflows. Without guidance, they get lost, frustrated, and abandon the app (high churn). With a guided tour, they click through 5 steps: "Create your first project", "Invite teammates", "Set up workflow", "Configure notifications", "Start using". By the end, they're activated and confident.</p>
        <p>Onboarding is critical for user activation. But it's also tricky: users hate being patronized with obvious tutorials, they want to explore independently, and forcing them through long walkthroughs causes rage-quits. The challenge is balancing guidance (new users need help) with autonomy (experienced users want freedom).</p>
        <p>Key challenges: (1) engagement (tutorials are boring, make them interactive and in-context), (2) optionality (allow users to skip and explore), (3) progress tracking (user closes tab mid-tour, can they resume?), (4) role-based tours (onboarding differs for admin vs regular user), (5) mobile responsiveness (tour must work on phone), and (6) performance (tour overlay doesn't slow app).</p>
        <p>Naive approach: static help pages or long video tutorials (rarely watched). Better: interactive guided tours highlighting features in real app context, allowing skip, and tracking progress. Users see the actual UI, understand how it works, and can immediately try it.</p>
        <p><strong>Explicit assumptions:</strong> App has multiple features (5-15 steps to explain). Users can skip or pause tour. Progress is tracked in user profile. Multiple tours exist (onboarding, advanced features). Mobile and desktop both supported. Tours are role-based (different for admin, editor, viewer).</p>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Multi-Step Guided Tours:</strong> Display sequenced tour steps (5-15 steps). Each step highlights a target UI element and shows instructions. Steps can be navigated forward/backward with Next/Previous buttons. Include step counter (e.g., "Step 3 of 7").</li>
          <li><strong>Interactive Hotspots:</strong> Tour can include clickable hotspots. User clicks "Confirm" button in tooltip, or manually clicks the highlighted element, to advance. Optionally require user to perform action (create project) before allowing next step (enforced learning).</li>
          ...
          <li><strong>Tour Replay:</strong> Add "Replay Tour" button in settings/help menu. User can revisit tour anytime. Useful for refreshing memory or showing colleagues.</li>
        ...
        <ul className="space-y-2">
          <li><strong>Performance:</strong> Tour overlay and highlight rendering don't cause jank or frame drops. Tour doesn't slow page load. Lazy-load tour content only when needed.</li>
          <li><strong>Accessibility:</strong> Tour content readable with screen reader. Keyboard navigable (Tab through Next/Previous buttons, Escape to close). High contrast on overlays for visibility. Tooltips have sufficient padding and readability.</li>
        ...
        <section>
          ...
          <p>The tour system has three components: tour definition (structured steps), runtime (render highlights and tooltips), and persistence (track progress).</p>
        <p>Definition: Create a tour configuration (data, not markup). Each step specifies a target element selector, title, description, an action type (Next, Confirm, Action), and optional conditions for whether the step should appear. For example, a first step might target the create-project button, show a short title, and advance on Next.</p>
        <p>Runtime: On app load, check if user should see tour (first time, not completed). If yes, render overlay (semi-transparent dark layer), highlight target element (spotlight or bright background), position tooltip near element, show navigation buttons. On "Next", advance to step 2. On "Skip", mark tour as dismissed. On "Previous", go back.</p>
        <p>Persistence: Store tour progress in the user profile, including the tour identifier, the current step number, whether the tour is completed, and when it was dismissed. On the next visit, read this state to resume the tour or skip it entirely if already completed.</p>
        <p>Mobile handling: On small screens, reposition tooltip to fit. Use full-screen overlay if needed. Ensure highlight and buttons are visible and clickable on touch devices.</p>
      </section>
      ...
      <section>
        ...
        <p><strong>Tour Versioning and Updates:</strong> Tours evolve as the app changes. Use semantic versioning such as onboarding v1 and onboarding v2. When the UI changes materially, publish a new tour version rather than silently editing the old one. Track which tour version a user has completed in their profile. If a newer version is available, offer an opt-in prompt explaining that the experience has changed. During transitions, support selector aliasing so the runtime can map older step targets to newer DOM selectors without breaking in-flight rollouts.</p>
        <p><strong>Conditional Tour Display Logic:</strong> Tour visibility depends on multiple factors, such as whether the user is new, their role, whether the underlying feature is enabled, experimentation assignment, and whether the user dismissed the tour recently. Implement a single decision function in the product that evaluates these conditions consistently so the UI does not “randomly” show tours. Store dismissal state with an expiration window, so you can avoid re-showing a tour for a period such as 30 days while still allowing a re-prompt later if appropriate.</p>
        <p><strong>Role-Based Tour Customization:</strong> Different roles need different guidance. Admin tours might focus on permissions, billing, and team management. Editor tours might focus on creation workflows and collaboration. Viewer tours can be minimal. Model tours as role-scoped content bundles so onboarding selects the correct tour at signup. If a user’s role changes later, you can show a short “what changed” tour to teach new capabilities and reduce support load.</p>
      ...
      <section>
        ...
        <p><strong>Mandatory vs Optional Tours:</strong> Mandatory tours (can't skip, must complete) guarantee visibility and learning but frustrate power users (high abandonment). Optional tours (skip anytime) respect user autonomy but many skip and miss crucial info. Compromise: make tours optional, but show prominent "Help" link so users can access later. Alternatively, make only the critical onboarding mandatory (3-5 steps), optional advanced tours can be skipped.</p>
        <p><strong>Tour Length vs Completion Rate:</strong> Long comprehensive tours (15 steps) cover everything but have low completion (users drop off mid-tour). Short focused tours (5-7 steps) have high completion but might miss details. Solution: break into multiple shorter tours. First tour covers essentials (5 steps, high completion). Second tour covers advanced features (optional). Users complete the first, skip the second if not interested.</p>
        <p><strong>Pre-Built vs Contextual Tours:</strong> Pre-built tours explain features in a fixed order, independent of what user is doing. Contextual tours appear when user hovers over elements (more discoverable). Contextual tours have better UX but harder to implement. Compromise: use pre-built for onboarding, contextual tooltips for advanced features.</p>
        <p><strong>Tour Library vs Custom Implementation:</strong> Libraries (Joyride, Shepherd) are battle-tested, handle edge cases, mobile support. Custom is lightweight, fully customizable. For most apps, library is better (less development). Custom for complex scenarios (animated tours, embedded videos).</p>
      </section>

      <section>
        <h2>Implementation Patterns</h2>
        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 1: Multi-Step Sequential Tour</h3>
        <p>Define 5-7 critical steps. Show step 1 immediately after signup. User navigates forward with Next button. Steps have progress counter ("Step 1 of 5"). Include optional Skip button to exit anytime. Resume from last step on revisit.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 2: Role-Based Tour Variants</h3>
        <p>Create separate tour configs for each role (admin, editor, viewer). Load appropriate tour based on user role. Admin sees permissions step, editor doesn't. Allows targeted onboarding.</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 3: Contextual Help with Spotlight</h3>
        <p>Show interactive spotlight highlighting a UI element. Tooltip explains what it does. User can click element or Next button to advance. Spotlight moves with user as they scroll. Good for "learning by doing."</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 4: Feature Announcement Tours</h3>
        <p>When new feature launches, show optional tour announcing it. Different from onboarding (shorter, 2-3 steps). User can dismiss or complete. Track if user saw announcement (analytics).</p>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 5: Analytics-Driven Tour Optimization</h3>
        <p>Track completion rate per step. If step 4 has 50% drop-off, rewrite or check element exists. A/B test different wording. Monitor over time, iterate.</p>
      </section>

      <section>
        <h2>Summary</h2>
        <p>User onboarding tours guide new users through features interactively, improving activation and reducing support burden. Essential components include tour configuration with sequential steps, element highlighting via spotlight overlays, tooltip callouts with navigation buttons, progress persistence (resume from last step), role-based variants (different tours for different users), mobile responsiveness, and analytics tracking (completion rate, drop-off). Trade-offs include mandatory vs optional (coverage vs user frustration), tour length (depth vs completion rate), and library vs custom (simplicity vs flexibility). Real-world systems (Slack, Figma, Notion) use tour libraries, track completion metrics, and iterate based on user behavior. For best results, keep initial onboarding short (5-7 critical steps), allow skip (respect user autonomy), provide replay option, track analytics (identify where users drop off), A/B test content, and supplement with contextual help (tooltips on UI elements). Well-designed onboarding significantly improves user activation, reduces support tickets, and improves retention.</p>
      </section>
    </ArticleLayout>
  );
}
