  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <HighlightBlock as="p" tier="important">Real-world solution: HttpOnly cookies for storage (XSS-safe, automatic), combined with CSRF tokens (separate, non-HttpOnly token that must be included in request body/header). Silent refresh (automatically refresh token before expiry) keeps users logged in without forcing re-login. Error handling gracefully degrades to re-login if token is lost.</HighlightBlock>
        <HighlightBlock as="p" tier="important"><strong>Explicit assumptions:</strong> Server issues unique tokens on login. Tokens can be revoked (blacklist or expiry). App makes authenticated API requests (includes token). Attackers may inject JavaScript (XSS) or trick browsers into cross-site requests (CSRF). Users expect to stay logged in across reloads. HTTPS is available (secure transmission).</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          ...
          <HighlightBlock as="li" tier="important"><strong>Token Retrieval:</strong> Token is automatically sent with API requests (via HTTP headers or cookies). No manual JavaScript access needed (reduces XSS surface). Token is accessible to network layer (fetch, axios) but not to JavaScript directly.</HighlightBlock>
        ...
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="crucial"><strong>Security Against XSS:</strong> Token must be inaccessible to injected JavaScript code (XSS). HttpOnly cookies cannot be read by JavaScript, making them safe. localStorage is vulnerable (XSS can read it). Never store sensitive tokens in localStorage.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Security Against CSRF:</strong> Prevent attackers from tricking users into making unauthorized requests from other sites. HttpOnly cookies are sent automatically, making them vulnerable to CSRF. Mitigate with CSRF tokens (separate, must be explicitly included in request body/header).</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Privacy and Secrecy:</strong> Token values should never be logged, printed, or exposed in developer tools (at least not in a readable way). Avoid logging "Authorization: Bearer [token]" in console. Mask tokens in logs or use non-HttpOnly cookies for CSRF tokens (they're expected to be visible).</HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">Layer 3 (Refresh): Access tokens are short-lived (15-30 minutes). Before expiry, automatically refresh (client periodically polls token/refresh endpoint or server pushes expiry notification). On refresh, server issues new access token. User never experiences interrupted session due to token expiry. If tokens do expire (unlikely with proper refresh), automatically redirect to login with clear message.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Error handling: if API returns 401 (unauthorized), attempt refresh. If refresh succeeds, retry the original request. If refresh fails, logout user (token is truly invalid). If network error during refresh, retry with exponential backoff or queue request for later.</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">Recommended: HttpOnly cookies for authentication tokens. This is the industry standard (Google, Microsoft, AWS use it). XSS cannot steal the token. CSRF is addressed separately with CSRF tokens. Token is sent automatically with requests, requiring no JavaScript code. User stays logged in across reloads naturally.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">CSRF token storage: store in a non-HttpOnly cookie (accessible to JavaScript, needed for form submission). On page load, fetch CSRF token (either from a separate endpoint or embedded in HTML). When making a POST request, include CSRF token in header (X-CSRF-Token) or form body. Server verifies token matches the session.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">Clear client-side state: remove cached user data, tokens from memory. Some frameworks cache auth state in global store; flush it on logout.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">If frontend needs a third-party token (rare), treat it like a CSRF token: store in non-HttpOnly cookie or memory, include in requests. Accept the XSS risk if it's a read-only token without sensitive operations.</HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important"><strong>Refresh Token Storage:</strong> Where to store refresh tokens? HttpOnly cookie (most secure) but persistent across domains (risk). Separate HttpOnly cookie with different domain/path (better isolation) or server-side session storage (no client-side token at all, backend manages it). Each approach trades security, complexity, and portability.</HighlightBlock>
        <HighlightBlock as="p" tier="important"><strong>Performance Overhead:</strong> Every request includes the auth cookie (minimal overhead). Refresh calls add periodic requests (silent refresh every 5 minutes = 288 requests/day per user). Acceptable but noticeable. Optimize: refresh only if token will expire soon (proactive), not every request.</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">Access token in HttpOnly cookie (short-lived, 15 min). Refresh token in separate HttpOnly cookie (longer-lived, 7 days). On app load, check if access token expires soon. If yes, refresh (POST /auth/refresh). On 401, also refresh. This ensures seamless experience without constant re-login.</HighlightBlock>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 3: Secure Axios/Fetch Interceptor</h3>
        <HighlightBlock as="p" tier="important">Create HTTP interceptor that automatically handles token refresh. On 401 response, intercept, call refresh, retry request with new token. Transparent to components—they don't know about token expiry. Simplifies application code.</HighlightBlock>
