  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <HighlightBlock as="p" tier="important">Third, suspicious activity detection: how do we identify compromised accounts? A hacker might log in from an unexpected location (user in New York, attacker in Russia). A password breach might generate rapid logins from multiple countries. The UI must alert users while minimizing false positives (traveling user legitimately logging in from another country shouldn't trigger alarm).</HighlightBlock>
        <HighlightBlock as="p" tier="crucial">Fourth, security of the management UI itself: logout actions must be secure. If user clicks "logout on iPhone", the backend must verify the request is authentic (signed with current session token) before revoking the iPhone session. A CSRF attack could trick the user into logging out legitimate sessions.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important"><strong>Explicit assumptions:</strong> Unique session tokens are issued per login. Sessions have metadata (device, IP, user agent, timestamp). Device fingerprinting is available (no 100% reliable, but good enough). Backend maintains authoritative list of active sessions. Suspicious activity detection rules are configurable. Real-time updates available (WebSocket or polling). Logout operations are idempotent (logging out twice is safe).</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          ...
          <HighlightBlock as="li" tier="important"><strong>Unrecognized Device Challenge:</strong> When a new device logs in, prompt user: "Unknown device logged in from New York. Is this you?" Allow user to verify (approve) or revoke immediately (prevent unauthorized access).</HighlightBlock>
        </ul>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Non-Functional Requirements</h3>
        <ul className="space-y-2">
          <HighlightBlock as="li" tier="crucial"><strong>Privacy and Security:</strong> Do not expose full IP addresses (show masked: 192.168.x.x or omit entirely, show only location). Do not expose exact hardware identifiers or serial numbers. Transmit all session data over HTTPS. Logout actions require authentication (CSRF-safe: include user's current session token or CSRF token).</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Real-Time Updates:</strong> Session list updates immediately after logout action. If user logs out device A from device B, device A should detect logout within 1-2 seconds and redirect to login. Use WebSocket for real-time sync or polling (every 30 seconds) for simplicity.</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Performance:</strong> Load session list within 1 second, even with 50+ sessions. Cache session data client-side to avoid re-fetching on each visit (cache TTL: 30 seconds). Logout operations complete within 2 seconds (backend revocation + client update).</HighlightBlock>
          <HighlightBlock as="li" tier="important"><strong>Scalability:</strong> Support users with hundreds of sessions (e.g., IoT devices, automated integrations). Use pagination or lazy-load if session list exceeds 100 items. Session data storage must scale to billions of sessions (archived sessions are purged after 90 days).</HighlightBlock>
        </ul>
      </section>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="crucial">The UI fetches the session list and displays it with privacy: show device name, OS, browser, location (city/country), last activity time. Highlight the current session. Show logout button per session (grayed out for current session). Show logout-all button. On logout, send POST /sessions/{"{id}"}/logout with CSRF token, backend revokes the session token, frontend removes the session from the list. For logout-all, send POST /sessions/logout-all-others, backend revokes all sessions except current, frontend refreshes the list.</HighlightBlock>
        <HighlightBlock as="p" tier="important">Real-time updates: if user logs out device A while viewing on device B, device B should see the change immediately (WebSocket notification) or detect it on next poll (30 seconds). Device A should detect its logout within 1-2 seconds (token no longer valid on next API call, redirect to login).</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">Logout-single: POST /sessions/{"{id}"}/logout with authentication (include current session token or CSRF token to prove the request is from user, not attacker). Backend verifies authentication, marks target session as revoked, responds with success. Client removes the session from the UI.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="important">WebSocket: client subscribes to session changes. Backend broadcasts logout events to all connected sessions. Low latency (100-200ms), but requires persistent connection and server infrastructure (redis pub-sub).</HighlightBlock>
        <HighlightBlock as="p" tier="important">Polling: client fetches session list every 30 seconds. On logout, next poll shows updated list. Higher latency (up to 30 seconds), but simpler (no WebSocket, just GET endpoint). Hybrid: poll every 30 seconds, but also trigger manual refresh after logout action.</HighlightBlock>
        ...
        <HighlightBlock as="p" tier="crucial">Transmit session data over HTTPS only. Don't expose session IDs or tokens in URLs (only in headers or secure cookies). Use Secure cookie flag (only sent over HTTPS) and HttpOnly flag (inaccessible to JavaScript, preventing XSS theft).</HighlightBlock>
      </section>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="crucial"><strong>Real-Time Updates Complexity:</strong> WebSocket provides real-time updates (logout reflects instantly on all devices) but requires persistent connections and server infrastructure. Polling is simpler but has latency (up to 30 seconds before device B sees device A logout). Most apps use polling with optional WebSocket for real-time (hybrid approach).</HighlightBlock>
        <HighlightBlock as="p" tier="important"><strong>Session Storage and Cost:</strong> Storing session records for all logins in all users forever is expensive. Mitigate with session archival: keep active/recent sessions (30 days) in fast storage, archive older sessions to cold storage (audit-only). Purge after 1 year. This reduces cost while maintaining audit trail.</HighlightBlock>
      ...
      <section>
        ...
        <HighlightBlock as="p" tier="important">Detect new device or unusual location, send confirmation email: "Unusual login detected. Is this you?" [Approve] [Revoke]. If approve, whitelist device for future. If revoke, logout the session. If no response in 24 hours, auto-logout.</HighlightBlock>

        <h3 className="mt-6 mb-3 text-lg font-semibuild">Pattern 3: Device Fingerprinting with Persistent UUID</h3>
        <HighlightBlock as="p" tier="important">On first login, generate device fingerprint (screen size, timezone, language) and optional device UUID (from platform). Store fingerprint with session. On subsequent logins, compare fingerprints. If match, recognize device. If no match, it's new—trigger alert.</HighlightBlock>
