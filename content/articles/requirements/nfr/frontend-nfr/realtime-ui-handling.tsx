  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>
          <strong>Real-Time UI Handling</strong> encompasses patterns for displaying live, updating
          data without user refresh. This includes chat messages, notifications, collaborative editing,
          live feeds, stock prices, sports scores, and presence indicators (who&apos;s online).
        </p>
        <p>
          For staff engineers, real-time UI involves trade-offs between freshness, performance,
          battery life, and complexity. Push-based (WebSockets, SSE) is efficient for frequent
          updates. Poll-based is simpler but wasteful. The right choice depends on update frequency,
        ...
        <ul>
          <li><strong>Chat/Messaging:</strong> Instant message delivery, typing indicators</li>
          <li><strong>Notifications:</strong> Push notifications, in-app alerts</li>
          <li><strong>Collaborative editing:</strong> Google Docs, Figma, Miro</li>
          <li><strong>Live feeds:</strong> Social media feeds, news tickers</li>
          <li><strong>Presence:</strong> Online status, &quot;user is typing&quot;</li>
          <li><strong>Live data:</strong> Stock prices, sports scores, auctions</li>
          <li><strong>IoT dashboards:</strong> Sensor data, monitoring</li>
        </ul>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li><strong>Protocol:</strong> Full-duplex, bidirectional communication</li>
          <li><strong>Connection:</strong> Persistent TCP connection</li>
          <li><strong>Latency:</strong> Very low (no request overhead)</li>
          <li><strong>Efficiency:</strong> Efficient for frequent updates</li>
          <li><strong>Browser support:</strong> 98%+</li>
          <li><strong>Use when:</strong> Bidirectional, frequent updates, low latency needed</li>
          <li><strong>Libraries:</strong> Socket.IO, ws, SignalR</li>
        </ul>

        <h3 className="mt-8 mb-4 text-xl font-semibold">Server-Sent Events (SSE)</h3>
        <ul className="space-y-2">
          <li><strong>Protocol:</strong> Server-to-client only (unidirectional)</li>
          <li><strong>Connection:</strong> HTTP-based, persistent</li>
          <li><strong>Latency:</strong> Low</li>
          <li><strong>Efficiency:</strong> Efficient for server-push scenarios</li>
          <li><strong>Browser support:</strong> 95%+ (not IE, limited mobile)</li>
          <li><strong>Use when:</strong> Server pushes updates, client doesn&apos;t need to send</li>
          <li><strong>Native API:</strong> EventSource</li>
        </ul>

        <h3 className="mt-8 mb-4 text-xl font-semibold">Long Polling</h3>
        <ul className="space-y-2">
          <li><strong>Protocol:</strong> HTTP request held open until data available</li>
          <li><strong>Connection:</strong> Request/response, but long-lived</li>
          <li><strong>Latency:</strong> Medium (request overhead)</li>
          <li><strong>Efficiency:</strong> Better than short polling, worse than WebSockets</li>
          <li><strong>Browser support:</strong> Universal</li>
          <li><strong>Use when:</strong> Fallback for WebSockets, moderate update frequency</li>
        </ul>

        <h3 className="mt-8 mb-4 text-xl font-semibold">Short Polling</h3>
        <ul className="space-y-2">
          <li><strong>Protocol:</strong> Periodic HTTP requests</li>
          <li><strong>Connection:</strong> Standard request/response</li>
          <li><strong>Latency:</strong> High (up to poll interval)</li>
          <li><strong>Efficiency:</strong> Wasteful (many empty responses)</li>
          <li><strong>Browser support:</strong> Universal</li>
          <li><strong>Use when:</strong> Simple implementation, infrequent updates acceptable</li>
        </ul>

        <ArticleImage
          src="/diagrams/frontend-nfr/realtime-protocols-comparison.svg"
        ...
        <h3 className="mt-8 mb-4 text-xl font-semibold">Protocol Selection Matrix</h3>
      ...
      <section>
        ...
        <h3 className="mt-8 mb-4 text-xl font-semibold">Connection Management</h3>
        <ul className="space-y-2">
        ...
        <h3 className="mt-8 mb-4 text-xl font-semibold">State Management Integration</h3>
        <ul className="space-y-2">
        ...
        <h3 className="mt-8 mb-4 text-xl font-semibold">Scaling Considerations</h3>
        <ul className="space-y-2">
        ...
        <ArticleImage
          src="/diagrams/frontend-nfr/websocket-architecture.svg"
      ...
      <section>
        ...
        <ArticleImage
          src="/diagrams/frontend-nfr/realtime-ui-patterns.svg"
      ...
      <section>
        ...
        <ul className="space-y-2">
          ...
          <li>Use binary protocols (MessagePack, Protocol Buffers) for high-volume</li>
      ...
      <section>
        ...
        <h3 className="mt-8 mb-4 text-xl font-semibold">Reconnection Strategy</h3>
        <ul className="space-y-2">
      ...
      <section>
        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            <p className="font-semibold">Q: When would you use WebSockets vs SSE vs polling?</p>
            <p className="mt-2 text-sm">
              A: WebSockets for bidirectional, frequent updates (chat, collaborative editing). SSE
              for server-push only (notifications, live feeds) — simpler than WebSockets. Long
              polling as WebSocket fallback. Short polling for infrequent updates where simplicity
              matters more than efficiency. Consider battery impact, infrastructure complexity, and
              browser support.
            </p>
          </div>
          ...
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            ...
            <p className="mt-2 text-sm">
              A: Exponential backoff (1s, 2s, 4s, 8s, max 30s) with jitter. Queue messages while
              disconnected. Flush queue on reconnection. Reset backoff on success. After N failed
              attempts, show error and provide manual reconnect. Handle connection state in UI
              (connected, reconnecting, offline).
            </p>
          </div>

          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            <p className="font-semibold">Q: How do you optimize real-time UI performance?</p>
            <p className="mt-2 text-sm">
              A: Throttle rapid updates (typing indicators, stock prices). Batch messages. Use
              requestAnimationFrame for UI updates. Virtualize long lists. Send deltas not full
              state. Limit message history in memory. Disconnect when app backgrounded. Use binary
              protocols for high-volume data.
            </p>
          </div>

          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            <p className="font-semibold">Q: How do you handle message ordering and duplicates?</p>
            <p className="mt-2 text-sm">
              A: Include sequence numbers or timestamps in messages. Buffer and reorder on client.
              Acknowledge messages server-side. Detect duplicates by sequence number. Use idempotent
              operations where possible. For critical ordering, use single-threaded processing or
              consensus protocols.
            </p>
          </div>

          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            <p className="font-semibold">Q: What are the challenges of scaling WebSockets?</p>
            <p className="mt-2 text-sm">
              A: WebSockets are stateful — need sticky sessions or shared state. Use Redis Pub/Sub
              for multi-server message broadcasting. Monitor connection count (each connection uses
              resources). Implement connection limits. Consider managed services (Pusher, Ably) for
              scale. Handle server restarts gracefully (reconnection storm prevention).
            </p>
          </div>
        </div>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          ...
          <li>
            <a href="https://pusher.com/" className="text-accent hover:underline" target="_blank" rel="noopener noreferrer">
