  ...
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        ...
        <ArticleImage
          src="/diagrams/requirements/nfr/frontend-nfr/multi-tab-message-protocol.svg"
          alt="Multi-Tab Message Protocol Flow"
          caption="Multi-tab message protocol — state change in Tab A creates typed message with timestamp and source ID, broadcast via BroadcastChannel, filtered by receivers to prevent loops and duplicates, applied to local state"
        />
      </section>
      ...
      <section>
        ...
        <p>
          State reconciliation strategies address the challenge of bringing divergent tab states back into consistency after periods of disconnection or conflicting updates. When a tab loses its connection to the sync channel (network interruption, BroadcastChannel closed, SharedWorker terminated), it accumulates local changes that are not reflected in other tabs. When the connection is restored, the tab must reconcile its local state with the current shared state. The simplest reconciliation is full state replacement — the disconnected tab discards its local state and adopts the current shared state from another tab. This is appropriate for read-heavy state (notification counts, user preferences) where local changes are minimal and easily recoverable. For state with pending local modifications (unsaved form data, cart changes), full replacement discards user work and is therefore unacceptable. Selective reconciliation compares the local state with the shared state field by field, applying remote changes for fields that were not modified locally and preserving local changes for fields that were. This requires tracking which fields have been modified locally since the last sync, typically with a dirty flag per field or a version vector per state object. The reconciliation process must be idempotent — running it multiple times produces the same result — because the tab may receive the same reconciliation data from multiple sources or at multiple points in time.
        </p>
        <p>
          Security considerations for cross-tab communication center on ensuring that messages are authentic, intended, and not exploitable by malicious scripts. BroadcastChannel messages are same-origin only — tabs from different origins cannot communicate through the same channel — which provides a basic security boundary. However, any JavaScript running on the page (including third-party scripts) can create a BroadcastChannel with the same name and send or receive messages. This means that sensitive data (authentication tokens, PII) should never be transmitted via BroadcastChannel, because a compromised third-party script could intercept it. The message protocol should validate incoming messages — check that the message structure is correct, the message type is recognized, and the payload conforms to expected constraints — before applying the message to local state. This prevents malicious or malformed messages from corrupting the application state. For applications using SharedWorker, the worker process has access to all tabs&apos; data and must be implemented with the same security rigor as server-side code — input validation, output encoding, and principle of least privilege. Cross-tab communication should also be audited in the application&apos;s Content Security Policy — the CSP should restrict which scripts can execute, indirectly controlling which scripts can participate in cross-tab communication.
        </p>
        <p>
          Performance optimization of message throughput in multi-tab sync systems requires balancing message frequency with processing overhead. When a user interacts with the application in one tab (typing in a form, scrolling, clicking buttons), the tab generates state change events that are broadcast to other tabs. Broadcasting every individual event — every keystroke, every scroll position change — generates excessive message volume that can saturate the BroadcastChannel and cause processing delays in receiving tabs. The solution is event batching and debouncing — group rapid, related events into a single broadcast message. Form input changes are debounced at 200-500ms, sending only the final value after the user stops typing rather than every intermediate character. Scroll position updates are throttled to 1-2 broadcasts per second, sending the current position rather than every intermediate scroll event. Significant state changes (login, logout, cart modification, settings change) are broadcast immediately without batching because their timeliness is more important than message volume optimization. On the receiving side, tabs should process incoming messages asynchronously using requestIdleCallback or setTimeout(0) to avoid blocking the main thread during periods of high message volume. Message processing should be O(1) or O(log n) with respect to the state size — avoid iterating over the entire state object for each message. For applications with complex state synchronization needs, consider using a differential sync protocol that sends only the changes (diff) rather than the entire state, reducing message size and processing time.
        </p>
      </section>
