  ...
  return (
    <ArticleLayout metadata={metadata}>
      <section>
        ...
        <p>
          <strong>Client Persistence Strategy</strong> encompasses how
          applications store data locally to survive page refreshes, enable
          offline usage, and improve performance through caching. This includes
          choosing storage mechanisms, data serialization, hydration on app
          load, and synchronization with server state.
        </p>
        <p>
          For staff engineers, persistence decisions affect UX (fast loads,
          offline support), security (sensitive data handling), and architecture
          (sync patterns, conflict resolution). The right strategy depends on
          data characteristics: size, structure, sensitivity, and access
          patterns.
        </p>
        <p>
        ...
        <ul>
          <li>
            <strong>User preferences:</strong> Theme, language, layout settings
          </li>
          <li>
            <strong>Drafts:</strong> Unsaved form data, document drafts
          </li>
          <li>
            <strong>Offline data:</strong> Cached API responses, queued actions
          </li>
          <li>
            <strong>Session state:</strong> Shopping cart, multi-step form
            progress
          </li>
          <li>
            <strong>Performance cache:</strong> Previously fetched data,
            computed results
          </li>
        </ul>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li>
            <strong>Capacity:</strong> 5-10MB per origin
          </li>
          <li>
            <strong>API:</strong> Simple key-value (setItem, getItem,
            removeItem)
          </li>
          <li>
            <strong>Persistence:</strong> Until explicitly cleared
          </li>
          <li>
            <strong>Sync/Async:</strong> Synchronous (blocks main thread)
          </li>
          <li>
            <strong>Data type:</strong> Strings only (JSON.stringify for
            objects)
          </li>
          <li>
            <strong>Best for:</strong> Small settings, preferences, feature
            flags
          </li>
          <li>
            <strong>Avoid for:</strong> Large data, complex queries, sensitive
            information
          </li>
        </ul>
        ...
        <ul className="space-y-2">
          ...
          <li>
            <strong>API:</strong> Same as localStorage
          </li>
          <li>
            <strong>Persistence:</strong> Until tab/window closes
          </li>
          <li>
            <strong>Scope:</strong> Per-tab (not shared across tabs)
          </li>
          <li>
            <strong>Best for:</strong> Single-session data, temporary form state
          </li>
        </ul>
        ...
        <ul className="space-y-2">
          <li>
            <strong>Capacity:</strong> 50MB+ (varies by browser, can request
            more)
          </li>
          <li>
            <strong>API:</strong> Complex, transactional, async
          </li>
          <li>
            <strong>Persistence:</strong> Until explicitly deleted
          </li>
          <li>
            <strong>Sync/Async:</strong> Asynchronous (non-blocking)
          </li>
          <li>
            <strong>Data type:</strong> Structured data (objects, arrays, blobs)
          </li>
          <li>
            <strong>Features:</strong> Indexes, cursors, versioning,
            transactions
          </li>
          <li>
            <strong>Best for:</strong> Large datasets, offline apps, complex
            queries
          </li>
          <li>
            <strong>Libraries:</strong> idb, Dexie.js (simplify API)
          </li>
        </ul>
        ...
        <ul className="space-y-2">
          <li>
            <strong>Capacity:</strong> Varies by browser (generous)
          </li>
          <li>
            <strong>API:</strong> Request/Response caching
          </li>
          <li>
            <strong>Persistence:</strong> Until storage pressure or explicit
            deletion
          </li>
          <li>
            <strong>Best for:</strong> HTTP response caching (Service Workers)
          </li>
          <li>
            <strong>Use with:</strong> Service Workers for offline support
          </li>
        </ul>
        ...
        <ul className="space-y-2">
          <li>
            <strong>Capacity:</strong> 4KB per cookie, ~20 cookies per domain
          </li>
          <li>
            <strong>API:</strong> document.cookie or libraries
          </li>
          <li>
            <strong>Persistence:</strong> Session or with expiration
          </li>
          <li>
            <strong>Sync/Async:</strong> Synchronous access
          </li>
          <li>
            <strong>Best for:</strong> Small data that needs server access
            (session IDs)
          </li>
          <li>
            <strong>Avoid for:</strong> Large data (sent with every request)
          </li>
        </ul>
      ...
      <section>
        ...
        <p>
          Hydration is restoring application state from persisted storage on app
          load.
        </p>
        ...
        <ul className="space-y-2">
          ...
          <li>
            Handle schema migrations (old persisted data vs new app version)
          </li>
      ...
      <section>
        ...
        <p>Keeping client data in sync with server state.</p>
        ...
        <ul className="space-y-2">
          <li>
            <strong>Last-write-wins:</strong> Simple but can lose data
          </li>
          <li>
            <strong>Field-level merge:</strong> Merge changes per field
          </li>
          <li>
            <strong>Manual resolution:</strong> Show conflicts to user
          </li>
          <li>
            <strong>CRDTs:</strong> Automatic convergence for specific data
            types
          </li>
        </ul>
      </section>
      ...
      <section>
        ...
        <ul className="space-y-2">
          <li>
            <strong>LRU:</strong> Evict least recently used first
          </li>
          <li>
            <strong>TTL:</strong> Evict data older than threshold
          </li>
          <li>
            <strong>Priority-based:</strong> Keep high-priority data longer
          </li>
          <li>
            <strong>Size-based:</strong> Evict largest items first
          </li>
        </ul>
        ...
        <div className="space-y-4">
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            ...
            <p className="mt-2 text-sm">
              A: localStorage for small data (&lt;5MB), simple key-value,
              synchronous access OK. IndexedDB for large data (50MB+), complex
              queries, async needed, structured data. localStorage for
              preferences and settings. IndexedDB for offline apps, large
              caches, or when you need indexes and cursors.
            </p>
          </div>

          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            <p className="font-semibold">
              Q: How do you handle data persistence across page refreshes?
            </p>
            <p className="mt-2 text-sm">
              A: On app load, read from storage (localStorage for small data,
              IndexedDB for large). Validate data schema, merge with defaults,
              handle missing/corrupted data. On state changes, debounce writes
              to storage. Version your data structure for migrations. Clear
          ...
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            <p className="font-semibold">
              Q: How do you sync offline changes with the server?
            </p>
            <p className="mt-2 text-sm">
              A: Queue mutations locally when offline. When connectivity
              returns, process queue with retry logic. Use optimistic updates
              for better UX — update UI immediately, rollback on server
              rejection. Handle conflicts with version vectors or timestamps.
          ...
          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            <p className="font-semibold">
              Q: How do you handle storage quota limits?
            </p>
            <p className="mt-2 text-sm">
              A: Check available space with StorageManager API. Request
              persistent storage for critical data. Implement eviction policies
              (LRU, TTL) for cache data. Handle quota exceeded errors gracefully
              — inform user, offer to clear cache. Monitor usage over time and
              proactively clean up old data.
            </p>
          </div>

          <div className="rounded-lg border border-theme bg-panel-soft p-4">
            <p className="font-semibold">
              Q: What is hydration and why is it important?
            </p>
            <p className="mt-2 text-sm">
              A: Hydration is restoring application state from persisted storage
              on app load. Without hydration, users lose their data on refresh.
              Read from storage during initialization, validate schema, merge
              with defaults. Handle migrations for schema changes. Consider lazy
              hydration for non-critical data to improve initial load time.
            </p>
          </div>
        </div>
      </section>
