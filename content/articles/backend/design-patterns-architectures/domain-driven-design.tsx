...
export default function DomainDrivenDesignArticle() {
  return (
    <ArticleLayout metadata={metadata}>
      ...
      <section>
        <h2>Model Discovery: Finding Concepts and Boundaries</h2>
        <p>
          DDD becomes practical when you treat modeling as a discovery activity, not as an upfront design ceremony. The
          goal is to make <em>meaning</em> explicit: what are the core nouns and verbs of the domain, what rules constrain
          them, and where the meaning diverges across workflows.
        </p>
        <p>
          A common technique is to run structured conversations with domain experts using concrete examples. You start
          from real scenarios (happy path and edge cases) and capture the language people naturally use. When the same
          word means different things in different conversations, that is usually a boundary signal.
        </p>
        <ul className="mt-4 space-y-2">
          <li>
            <strong>Event storming and timelines:</strong> map key events and commands to reveal hidden dependencies and
            ownership.
          </li>
          <li>
            <strong>Example mapping:</strong> tie rules to examples and counter-examples so invariants are testable and
            unambiguous.
          </li>
          <li>
            <strong>Glossary ownership:</strong> treat the ubiquitous language as a living artifact, not a one-time doc.
          </li>
          <li>
            <strong>Boundary tests:</strong> validate that a bounded context can answer its questions without reaching
      ...
      <section>
        <h2>Tactical Modeling Choices That Keep DDD Practical</h2>
        <p>
          Tactical DDD can be over-applied. The goal is not to maximize patterns; it is to enforce invariants and keep
          change safe. A few modeling choices tend to decide whether the approach stays maintainable.
        </p>
        <ul className="mt-4 space-y-2">
          <li>
            <strong>Entities vs value objects:</strong> identities belong on things you track over time; value objects
            are immutable concepts like money, addresses, and time ranges that carry rules and validation.
          </li>
          <li>
            <strong>Aggregate sizing:</strong> keep aggregates small enough to change atomically, and accept eventual
            consistency across aggregates when the invariant allows it.
          </li>
          <li>
            <strong>Domain services:</strong> use them when a rule does not naturally belong to one entity, but keep them
            in domain language, not as generic utility functions.
          </li>
          <li>
            <strong>Persistence mapping:</strong> avoid letting table shape dictate domain shape; repositories and mappers
            exist to keep database concerns from leaking inward.
          </li>
        </ul>
        <p className="mt-4">
          The simplest health check is to ask: can a developer explain the invariant in business terms and point to a
          single place where it is enforced? If the answer is no, the model is drifting toward an anemic design.
        </p>
      </section>
