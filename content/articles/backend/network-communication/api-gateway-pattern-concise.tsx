...
<section>
  ...
  <p>
    The real complexity in API Gateway Pattern appears once traffic is uneven. Request routing and path-based service selection may look
    stable at low load, but under bursts it interacts with Authn/authz enforcement and policy evaluation in surprising ways.
    This is why scaling tests must include skewed traffic and partial failures, not
    just average throughput.
  </p>
  <p>
    A useful mental model is to treat Gateway saturation causing global latency spikes and Misconfigured routes leading to partial outages as design constraints. If you
...
<section>
  ...
  <p>
    Reduce the number of tuning knobs for API Gateway Pattern. If every performance issue requires
    manual tuning, the design is too fragile. Prefer a small set of well‑understood
    controls tied to Request routing and path-based service selection.
  </p>
  <p>
