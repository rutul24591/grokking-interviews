...
const DOMAINS = [
  {
    name: "problem-set-lld",
    path: path.join(ROOT, "content", "examples", "system-design-problems", "low-level-design"),
    manifestPrefix: "low-level-design",
  },
  {
    name: "problem-set-hld",
    path: path.join(ROOT, "content", "examples", "system-design-problems", "high-level-design"),
    manifestPrefix: "high-level-design",
  },
];
...
function buildExampleGroup(exampleDir, exampleId) {
  ...
  return { id: exampleId, label, files: sortFilesByPriority(files) };
}
...
function buildEntries() {
  ...
  for (const domain of DOMAINS) {
    if (!isDirectory(domain.path)) continue;

    for (const subcategory of listDirs(domain.path)) {
      const subDir = path.join(domain.path, subcategory);
      for (const slug of listDirs(subDir)) {
        ...
        if (examples.length > 0) {
          const manifestKey = `${domain.manifestPrefix}/${subcategory}/${slug}`;
...
function main() {
  ...
  const updatedKeys = Object.keys(entries).sort();
  for (const key of updatedKeys) existing[key] = entries[key];

  fs.writeFileSync(MANIFEST_PATH, JSON.stringify(existing, null, 2) + "\n", "utf-8");

  console.log(`Updated examples-manifest.json with ${updatedKeys.length} problem-set key(s).`);
