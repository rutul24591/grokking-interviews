#!/usr/bin/env node
/**
 * Clean up duplicate diagrams in articles
 * Ensure exactly 3 diagrams per article
 */
...
// Map each article to its EXACT 3 diagrams
const articleDiagrams: Record<string, string[]> = {
  "access-control-policies.tsx": ["access-control-policies.svg", "policy-evaluation.svg", "policy-management.svg"],
  "account-lockout.tsx": ["account-lockout-flow.svg", "account-lockout-unlock.svg", "account-lockout-security.svg"],
  "account-recovery-ui.tsx": ["account-recovery-flow.svg", "account-recovery-options.svg", "account-recovery-security.svg"],
  "account-settings-ui.tsx": ["account-settings-flow.svg", "account-settings-gdpr.svg", "account-settings-security.svg"],
  "account-verification.tsx": ["account-verification-flow.svg", "verification-token-flow.svg", "account-verification-security.svg"],
  "authentication-audit-logs.tsx": ["auth-audit-logs.svg", "auth-audit-schema.svg", "auth-audit-analysis.svg"],
  "authentication-service.tsx": ["authentication-service-architecture.svg", "token-management.svg", "auth-scalability.svg"],
  "credential-rotation.tsx": ["credential-rotation-flow.svg", "token-rotation.svg", "credential-rotation-security.svg"],
  "device-session-management-ui.tsx": ["device-session-management.svg", "device-session-security.svg", "device-session-tracking.svg"],
  "device-session-tracking.tsx": ["device-session-tracking.svg", "session-metadata.svg", "device-tracking-security.svg"],
  "email-verification.tsx": ["email-verification-flow.svg", "email-verification-security.svg", "email-verification-ux.svg"],
  "identity-providers.tsx": ["identity-providers.svg", "idp-integration.svg", "idp-enterprise.svg"],
  "login-attempt-tracking.tsx": ["login-attempt-tracking.svg", "login-threat-detection.svg", "login-tracking-implementation.svg"],
  "login-interface.tsx": ["login-interface-flow.svg", "login-security.svg", "login-ux-patterns.svg"],
  "logout.tsx": ["logout-flow.svg", "logout-patterns.svg", "logout-security.svg"],
  "mfa-setup.tsx": ["mfa-setup-flow.svg", "mfa-methods.svg", "mfa-recovery.svg"],
  "oauth-providers.tsx": ["oauth-flow.svg", "oauth-providers.svg", "oauth-security.svg"],
  "password-hashing-and-validation.tsx": ["password-hashing-flow.svg", "password-validation.svg", "password-algorithms.svg"],
  "password-reset.tsx": ["password-reset-flow.svg", "password-reset-security.svg", "password-reset-ux.svg"],
  "permission-validation.tsx": ["permission-validation-flow.svg", "resource-permissions.svg", "permission-caching.svg"],
  "phone-verification.tsx": ["phone-verification-flow.svg", "phone-verification-methods.svg", "phone-verification-security.svg"],
  "profile-settings-ui.tsx": ["profile-settings-flow.svg", "profile-avatar-management.svg", "profile-privacy.svg"],
  "rbac.tsx": ["rbac-model.svg", "rbac-schema.svg", "rbac-vs-abac.svg"],
  "security-audit-logging.tsx": ["security-audit-logging.svg", "audit-event-schema.svg", "audit-compliance.svg"],
  "security-settings-ui.tsx": ["security-settings-dashboard.svg", "security-mfa-management.svg", "security-recommendations.svg"],
  "session-management.tsx": ["session-management-flow.svg", "session-lifecycle.svg", "session-scalability.svg"],
  "session-persistence.tsx": ["session-persistence-flow.svg", "session-token-refresh.svg", "session-security.svg"],
  "session-revocation.tsx": ["session-revocation-flow.svg", "session-revocation-patterns.svg", "session-revocation-distributed.svg"],
  "signup-interface.tsx": ["signup-interface-flow.svg", "signup-patterns.svg", "signup-security.svg"],
  "social-login-options.tsx": ["social-login-flow.svg", "social-account-linking.svg", "social-login-conversion.svg"],
  "sso-integrations.tsx": ["sso-flow.svg", "saml-flow.svg", "sso-enterprise.svg"],
  "token-generation.tsx": ["token-generation-flow.svg", "jwt-structure.svg", "token-security.svg"],
  "user-registration-service.tsx": ["user-registration-flow.svg", "registration-validation.svg", "registration-scalability.svg"],
};
...
function cleanArticle(articleFile: string, diagrams: string[]) {
  const filePath = path.join(ARTICLES_DIR, articleFile);
  let content = fs.readFileSync(filePath, "utf-8");
  
  // Find the section tag and insert diagrams after the first paragraph
  const sectionMatch = content.match(/(<section>\s*<h2>Definition[^<]*<\/h2>[\s\S]*?<\/p>)/);
  if (!sectionMatch) {
    console.log(`⚠️  No Definition section found in ${articleFile}`);
    return false;
  }
  
  // Generate exactly 3 ArticleImage components
  const imageComponents = diagrams.map((diagram) => {
    const caption = diagramCaptions[diagram] || `Diagram for ${articleFile}`;
  ...
  // Remove all existing ArticleImage components
  content = content.replace(/<ArticleImage[\s\S]*?\/>/g, "");
  
  // Insert the 3 new diagrams after the first paragraph of Definition section
  const insertPosition = sectionMatch.index! + sectionMatch[1].length;
...
console.log("CLEANING UP DUPLICATE DIAGRAMS - ENSURING EXACTLY 3 PER ARTICLE");
console.log("=".repeat(80));
console.log("");
...
for (const [article, diagrams] of Object.entries(articleDiagrams)) {
  const success = cleanArticle(article, diagrams);
  if (success) {
...
console.log("SUMMARY");
console.log("=".repeat(80));
console.log(`Articles cleaned: ${cleaned}`);
console.log(`Errors: ${errors}`);
console.log(`Total diagrams: ${cleaned * 3}`);
console.log("");
