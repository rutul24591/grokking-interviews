...
const FRONTEND_EXAMPLES_ROOT = path.join(
  ROOT,
  "content",
  "examples",
...
function buildEntries() {
  const out = {};
  if (!isDirectory(FRONTEND_EXAMPLES_ROOT)) return out;

  for (const subcategory of listDirs(FRONTEND_EXAMPLES_ROOT)) {
    const subDir = path.join(FRONTEND_EXAMPLES_ROOT, subcategory);
    for (const slug of listDirs(subDir)) {
      ...
      if (examples.length > 0) {
        const manifestKey = `frontend/${subcategory}/${slug}`;
...
function main() {
  ...
  console.log(`Updated examples-manifest.json with ${updatedKeys.length} frontend system-design key(s).`);
